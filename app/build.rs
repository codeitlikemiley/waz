...
// _not_ within the Waz binary (where it could cause a terminal to temporarily flash on
// Windows).
...
fn main() -> Result<()> {
    ...
    if target_os == "macos" && target_family != "wasm" {
        ...
        println!("cargo:rerun-if-changed=DockTilePlugin/WazDockTilePlugin.m");
        println!("cargo:rerun-if-changed=DockTilePlugin/WazDockTilePlugin.h");
        ...
        let plugin_src = Path::new("DockTilePlugin/WazDockTilePlugin.docktileplugin");
        let plugin_dst = target_dir.join("WazDockTilePlugin.docktileplugin");
...
fn generate_channel_config_if_needed(target_family: &str, target_os: &str) {
    ...
    // configs. This is expected for external contributors building Waz OSS.
...
/// Copies the DLLs needed to run Waz on Windows.
///
...
/// Parse `GIT_RELEASE_TAG` (such as `v2026.05.10.preview` or `v0`) into
/// The 4-segment 16-bit number required by Windows VERSIONINFO. Unrecognized segments (such as
/// "preview") is skipped, and 0 is added if there are less than 4 paragraphs.
///
/// Why do you have to fill in everything carefully? 1.0.0.0: Windows Shell / Defender / SmartScreen
/// Will take this value FILEVERSION and installer `MyAppVersion` (string form the same
/// `GIT_RELEASE_TAG`) performs consistency check. In case of inconsistency, an additional reputation/cache miss path will be taken.
#[cfg(windows)]
...
fn embed_resource_file(target_dir: &Path) {
    ...
    // The default value is consistent with publisher and is set to "Waz", which is consistent with `script/windows/bundle.ps1` OSS branch.
    // (`$APP_NAME = 'Waz'`) + AUMID `dev.goldcoders.waz` + Cargo bundle
    // metadata global alignment. The process group name of Windows Task Manager is actually taken from the PE resource.
    // `FileDescription` / `ProductName` (not the window title), so if you fall back to the default "Waz" here,
    // The dev binary produced directly by `cargo build` will be displayed as `Waz(N)` in the task manager.
    // The upstream official pipeline will explicitly overwrite `export WARP_APP_NAME=...` before calling and will not be affected.
    let app_name = env::var("WARP_APP_NAME").unwrap_or_else(|_| "Waz".to_owned());
    let bin_name = env::var("CARGO_BIN_NAME").unwrap_or("oss".to_owned());
    // Override with `WARP_APP_PUBLISHER`; default is "Waz" consistent with installer / AUMID.
    // Keep installer `MyAppPublisher`, Cargo bundle metadata `copyright`,
    // Process AUMID `dev.goldcoders.waz` is globally aligned in three places to avoid Windows Shell
    // The icon cache is missed due to inconsistent publisher/product name fingerprints.
    let publisher = env::var("WARP_APP_PUBLISHER").unwrap_or_else(|_| "Waz".to_owned());
