...
agent-management-source-local-agent = Waz (ローカルエージェント)
agent-management-source-cloud-agent = Waz (ローカルエージェント)
...
workspace-menu-update-warp-manually = Waz を手動で更新
...
workspace-menu-view-warp-logs = Waz ログを表示
...
workspace-update-and-relaunch-warp = Waz を更新して再起動
workspace-updating-to-version = ({ $version }) に更新中
workspace-update-warp-manually = Waz を手動で更新
...
terminal-banner-shell-config-incompatible = シェルの設定が Waz と互換性がありません...{"  "}
...
ai-document-save-and-sync-tooltip = この計画を Waz Drive に保存して自動同期
ai-document-show-in-warp-drive = Waz Drive で表示
...
ai-footer-enable-notifications-tooltip = Warp プラグインをインストールして Waz 内のリッチなエージェント通知を有効化
...
waz-launch-visit-repo = リポジトリを見る
waz-launch-title = Waz がオープンソースになりました
waz-launch-description = コミュニティの皆さんがエージェントファーストのワークフローで Waz の構築に参加できます。
waz-launch-contribute-title = コントリビュート
waz-launch-contribute-description = Waz のクライアントコードがオープンソースになりました。/feedback スキルで Issue を作成し、こちらのコントリビューションガイドラインに従ってください。
waz-launch-contribute-link-text = こちら
waz-launch-oad-title = Open Automated Development
waz-launch-oad-description = Waz リポジトリは、Oz のローカルエージェント体験を活用したエージェントファーストワークフローで管理されています。
waz-launch-auto-model-title = 'auto (open-weights)' 登場
waz-launch-auto-model-description = タスクに最適なオープンウェイトモデル (Kimi、MiniMax など) を選ぶ新しい auto モデルを追加しました。
...
terminal-warpify-subshell-tooltip = このセッションで Waz シェル統合を有効化
terminal-use-agent = エージェントを使う
terminal-use-agent-tooltip = Waz エージェントに支援を依頼
terminal-give-control-back-to-agent = 制御をエージェントに戻す
terminal-resume-agent-tooltip = Waz エージェントに再開を依頼
...
terminal-open-in-warp = Waz で開く
terminal-open-conversation-in-warp-tooltip = この会話を Waz デスクトップアプリで開く
...
terminal-input-agent-hint-deploy-react-vercel = 何でも Waz で。例: React アプリを Vercel にデプロイし環境変数を設定
terminal-input-agent-hint-debug-python-ci = 何でも Waz で。例: CI で Python テストが失敗する原因をデバッグ
terminal-input-agent-hint-setup-microservice = 何でも Waz で。例: Docker で新しいマイクロサービスをセットアップしデプロイパイプラインを作成
terminal-input-agent-hint-fix-node-memory-leak = 何でも Waz で。例: Node.js アプリのメモリリークを発見して修正
terminal-input-agent-hint-backup-postgres = 何でも Waz で。例: PostgreSQL DB のバックアップスクリプトを作成しスケジュール
terminal-input-agent-hint-migrate-mysql-postgres = 何でも Waz で。例: MySQL から PostgreSQL へのデータ移行
terminal-input-agent-hint-monitor-aws = 何でも Waz で。例: AWS インフラの監視とアラートを設定
terminal-input-agent-hint-build-fastapi = 何でも Waz で。例: FastAPI でモバイルアプリ向け REST API を構築
terminal-input-agent-hint-optimize-sql = 何でも Waz で。例: 遅い SQL クエリを最適化
terminal-input-agent-hint-github-actions = 何でも Waz で。例: マージ時に自動デプロイする GitHub Actions ワークフローを作成
terminal-input-agent-hint-redis-cache = 何でも Waz で。例: Web アプリに Redis キャッシュを設定
terminal-input-agent-hint-kubernetes-pods = 何でも Waz で。例: Kubernetes Pod がクラッシュし続ける原因をトラブルシュート
terminal-input-agent-hint-bigquery-pipeline = 何でも Waz で。例: CSV を処理して BigQuery に投入するデータパイプラインを構築
terminal-input-agent-hint-ssl-https = 何でも Waz で。例: SSL 証明書を設定してドメインを HTTPS 化
terminal-input-agent-hint-refactor-legacy-code = 何でも Waz で。例: レガシーコードをモダンな設計パターンにリファクタ
terminal-input-agent-hint-unit-tests = 何でも Waz で。例: 認証サービスのユニットテストを作成
terminal-input-agent-hint-elk-logs = 何でも Waz で。例: 分散システム向けに ELK スタックでログ集約を構築
terminal-input-agent-hint-oauth-express = 何でも Waz で。例: Express.js アプリに OAuth2 認証を実装
terminal-input-agent-hint-optimize-docker = 何でも Waz で。例: ビルド時間とサイズを削減するため Docker イメージを最適化
terminal-input-agent-hint-ab-testing = 何でも Waz で。例: Web アプリ向けに A/B テスト基盤を構築
...
terminal-model-specs-description = Waz のハーネスでのモデル性能、クレジット消費レート、タスク速度のベンチマーク。
...
onboarding-callout-meet-input-title = Waz 入力欄のご紹介
...
onboarding-callout-nl-support-text-prefix = 自然言語入力は既定で無効です。有効にすると、平易な英語でリクエストを入力でき、Waz がエージェント向けクエリを自動判定します。次の方法でいつでも上書きできます:
onboarding-callout-enable-nl-detection = 自然言語判定を有効にする
onboarding-callout-new-agent-title = Waz の新しいエージェント体験のご紹介
...
language-widget-secondary = 変更を完全に反映するには Waz を再起動してください。
language-restart-required-title = 言語が変更されました
language-restart-required-body = Waz の UI 言語が更新されました。一部のテキストは即座に切り替わりますが、すべての箇所に反映するには再起動が必要です。
# =============================================================================
...
# Waz: settings-section-account は Account 設定ページとともに削除。
...
settings-section-warp-agent = Waz エージェント
...
settings-about-automatic-updates-description = 有効にすると、Waz はバックグラウンドで新バージョンを確認しダウンロードします。無効でも手動で更新を確認できます。
...
# Waz: main_page.rs 関連の文字列は Account 設定ページとともに削除された。
...
# ---- list_page.rs ----
settings-mcp-list-description = MCP サーバーを追加して Waz エージェントの機能を拡張します。MCP サーバーは標準化されたインターフェースを通じてデータソースやツールをエージェントに公開し、プラグインのように動作します。カスタムサーバーを追加するか、プリセットを使用して人気のサーバーから始められます。
...
settings-mcp-list-section-shared-by-warp-and-team = Waz と { $name } から利用可能
settings-mcp-list-section-shared-by-warp-and-other-devices = Waz および別のデバイスから共有
settings-mcp-list-section-shared-from-warp = Waz から共有
...
settings-platform-description = API キーを作成・管理して、ローカルエージェントから Waz アカウントへのアクセスを許可します。
    詳細は次を参照
...
settings-platform-zero-state-description = キーを作成して Waz への外部アクセスを管理します
settings-platform-create-api-key-description-personal = この API キーはユーザーに紐付き、Waz アカウントに対するリクエストを実行できます。
settings-platform-create-api-key-description-team = この API キーはチームに紐付き、チームを代理してリクエストを実行できます。
settings-platform-create-api-key-name-placeholder = Waz API キー
...
settings-referrals-page-title = 友人を Waz に招待
settings-referrals-anonymous-header = ローカル Waz ビルドでは紹介プログラムを利用できません
...
settings-referrals-reward-intro = 紹介で Waz 限定グッズを獲得*
...
settings-warpify-description-prefix = Waz が特定のシェルを「Warpify」(ブロック・入力モード等のサポートを追加) するかを設定します。
...
settings-warpify-install-ssh-extension-description = リモートホストに Waz の SSH 拡張がインストールされていない場合のインストール挙動を制御します。
...
# Section / sub-headers
settings-ai-warp-agent-header = Waz エージェント
...
settings-ai-command-denylist-description = Waz エージェントが実行する前に必ず権限を確認するコマンドにマッチする正規表現。
settings-ai-command-allowlist = コマンド許可リスト
settings-ai-command-allowlist-description = Waz エージェントが自動的に実行できるコマンドにマッチする正規表現。
...
settings-ai-mcp-allowlist-description = Waz エージェントによるこれらの MCP サーバーの呼び出しを許可します。
settings-ai-mcp-denylist = MCP 拒否リスト
settings-ai-mcp-denylist-description = Waz エージェントはこのリストにある MCP サーバーを呼び出す前に必ず権限を確認します。
settings-ai-info-banner-managed-by-workspace = 一部の権限はワークスペースによって管理されています。

# Models / Profiles
settings-ai-base-model = ベースモデル
settings-ai-base-model-description = このモデルは Waz エージェントの中心エンジンとして機能します。ほとんどのやり取りを担い、必要に応じて計画やコード生成などのタスクで他のモデルを呼び出します。Waz はモデルの可用性や、会話要約などの補助タスクに応じて自動的に別のモデルへ切り替えることがあります。
settings-ai-show-model-picker-in-prompt = プロンプトにモデル選択を表示
settings-ai-codebase-context = コードベースのコンテキスト
settings-ai-codebase-context-description = Waz エージェントによるコードベースの概要生成を許可し、コンテキストとして利用します。コードがサーバーに保存されることはありません。
...
settings-ai-rules-description = ルールは、Waz エージェントがコードベースや特定のワークフローのお作法に沿うよう導きます。
...
settings-ai-cloud-agent-computer-use-description = Waz アプリから開始したエージェント会話でコンピュータ操作を有効化します。

# AWS Bedrock
settings-ai-aws-bedrock-toggle = AWS Bedrock の認証情報を使用
settings-ai-aws-bedrock-description = Waz はローカルの AWS CLI 認証情報を読み込み、Bedrock 対応モデルへ送信します。
settings-ai-aws-bedrock-description-managed = Waz はローカルの AWS CLI 認証情報を読み込み、Bedrock 対応モデルへ送信します。この設定は組織によって管理されています。
...
settings-features-ssh-wrapper = Waz SSH ラッパー
settings-features-receive-desktop-notifications = Waz からのデスクトップ通知を受信
...
settings-features-make-default-terminal = Waz を既定のターミナルにする
...
settings-features-default-terminal-current = Waz が既定のターミナルです
...
settings-features-restart-warp-to-apply = 変更を反映するには Waz を再起動してください。
...
settings-exec-profile-editor-base-model-desc = このモデルはエージェントの主要エンジンとして機能します。ほとんどのやり取りを駆動し、必要に応じて計画やコード生成などのために他モデルを呼び出します。Waz はモデルの可用性や、会話の要約などの補助タスクのため、自動的に代替モデルに切り替える場合があります。
...
settings-show-blocks-confirm-dialog-text = このブロックを共有解除してもよろしいですか?
    ...
    リンク経由でアクセスできなくなり、Waz サーバーから完全に削除されます。
...
settings-appearance-custom-icon-restart-warning = MacOS で希望のアイコンスタイルを適用するには Waz の再起動が必要な場合があります。
...
settings-appearance-input-mode-pinned-bottom = 下部に固定 (Waz モード)
...
settings-environments-page-description = 環境はアンビエントエージェントが実行される場所を定義します。GitHub (推奨)、Waz 支援セットアップ、または手動構成で数分で設定できます。
...
settings-environments-section-team-default = Waz とこのデバイスから提供
settings-environments-section-team-named = Waz と { $team } で共有
...
quit-warning-title-app = Waz を終了しますか?
...
keybinding-desc-workspace-left-panel-warp-drive = 左パネル: Waz Drive
...
keybinding-desc-workspace-toggle-warp-drive = Waz Drive を切り替え
...
keybinding-desc-workspace-search-drive = Waz Drive を検索
...
keybinding-desc-workspace-terminate-app = Waz を終了
...
keybinding-desc-workspace-export-all-warp-drive-objects = すべての Waz Drive オブジェクトをエクスポート
keybinding-desc-workspace-install-cli = Oz CLI コマンドをインストール
keybinding-desc-workspace-uninstall-cli = Oz CLI コマンドをアンインストール

# AI assistant / agents
keybinding-desc-workspace-toggle-ai-assistant = Waz AI を切り替え
...
# Waz: keybinding-desc-workspace-show-settings-account は Account 設定ページとともに削除。
...
keybinding-desc-workspace-show-settings-about-menu = Waz について
...
keybinding-desc-workspace-view-logs = Waz ログを表示
...
keybinding-desc-workspace-open-waz-launch-modal = [Debug] Waz Launch モーダルを開く
keybinding-desc-workspace-reset-waz-launch-modal-state = [Debug] Waz Launch モーダル状態をリセット
...
keybinding-desc-input-show-network-log = Waz ネットワークログを表示
...
keybinding-desc-ai-assistant-close = Waz AI を閉じる
keybinding-desc-ai-assistant-focus-terminal-input = Waz AI からターミナル入力にフォーカス
keybinding-desc-ai-assistant-restart = Waz AI を再起動
...
keybinding-desc-terminal-ask-ai-selection = 選択について Waz AI に質問
keybinding-desc-terminal-ask-ai-last-block = 直前のブロックについて Waz AI に質問
keybinding-desc-terminal-ask-ai = Waz AI に質問
...
toggle-suffix-ssh-wrapper = Waz SSH ラッパー
...
settings-external-editor-choose-layout = Waz でファイルを開くレイアウトを選択
...
settings-external-editor-prefer-markdown = Markdown ファイルを既定で Waz の Markdown ビューアで開く
...
menu-block-open-in-warp = Waz で開く
...
menu-block-ask-warp-ai = Waz AI に質問
...
menu-input-ask-warp-ai = Waz AI に質問
...
prompt-editor-warp-prompt-section = Waz ターミナルプロンプト
...
welcome-tips-command-palette-description = キーボードから手を離さずに Waz の機能をすべて簡単に発見できます。
...
welcome-tips-theme-picker-description = 組み込みテーマから選んで Waz を自分好みに。または自作も可能。
...
drive-local-storage-limit-description = ローカルストレージ上限はこのデバイス上で適用されます。不要な項目を削除して、新しい Waz Drive オブジェクト用の容量を空けてください。
...
workflow-ai-assist-tooltip = Waz AI でタイトル・説明・パラメータを生成
...
wasm-nux-open-desktop-title = Waz デスクトップで開きますか?
wasm-nux-open-desktop-detail = 今後のリンクは自動的にデスクトップで開きます。
wasm-nux-open-desktop-confirm = Waz で開く
wasm-nux-download-title = Waz デスクトップをダウンロードしますか?
wasm-nux-download-description = Waz は AI と開発チームのナレッジを内蔵したインテリジェントなターミナルです。
wasm-nux-learn-more = 詳しく見る
wasm-nux-download-confirm = ダウンロード
wasm-nux-object-kind-drive-objects = Waz Drive オブジェクト
wasm-nux-object-kind-warp-links = Waz リンク
...
auth-override-warning-confirm-title = 個人の Waz Drive オブジェクトと環境設定を削除しますか?
auth-override-warning-description = Web ブラウザから Waz アカウントにログインしたようです。続行すると、この匿名セッションの個人 Waz Drive オブジェクトおよび環境設定はすべて完全に削除されます。
...
auth-override-warning-accessibility-help = Waz が Web ブラウザからの新しいログインを検出しました。ログインせずに Waz の使用を続けるには Esc を押してキャンセルしてください。
...
auth-needs-sso-link-detail = 下のボタンをクリックして Waz アカウントを SSO プロバイダにリンクしてください。
...
auth-offline-first-use-description = 現在オフラインです。Waz を初めて使用するにはインターネット接続が必要です。
auth-offline-first-use-learn-more = 詳しく見る
auth-offline-overlay-title = Waz をオフラインで使用
auth-offline-overlay-paragraph-1 = Waz のローカル機能はオフラインで動作します。
auth-offline-overlay-paragraph-2 = BYOP AI 機能を使う場合のみ、選択したプロバイダーへの接続が必要です。
auth-offline-overlay-paragraph-3 = Waz は匿名のローカルユーザー ID で動作し、外部オブジェクトや利用量計測のためにインターネット接続を要求しません。
auth-offline-overlay-dismiss = 閉じる
auth-privacy-settings-title = プライバシー設定
auth-privacy-settings-done = 完了
auth-privacy-settings-help-improve = Waz の改善に協力する
auth-privacy-settings-help-improve-description = ハイレベルな機能利用データは Waz プロダクトチームのロードマップ優先順位付けに役立ちます。
auth-privacy-settings-learn-more = 詳しく見る
auth-privacy-settings-send-crash-reports = クラッシュレポートを送信
auth-privacy-settings-crash-reports-description = クラッシュレポートは Waz エンジニアリングチームが安定性を理解しパフォーマンスを改善するのに役立ちます。
...
auth-logout-unsynced-drive-objects-warning = 未同期の Waz Drive オブジェクトが { $count } { $count ->
        [one] 件
       *[other] 件
    }あります。ログアウトすると、{ $count ->
        [one] そのオブジェクト
...
cli-agent-plugin-codex-install-title = Codex の Waz 通知を有効化
cli-agent-plugin-codex-install-subtitle = Codex を最新版に更新し、フォーカス時通知を有効にすることで Waz が作業中に通知を表示できるようにします。
...
cli-agent-plugin-deepseek-install-title = DeepSeek の Waz 通知を有効化
...
terminal-bootstrapping-installing-warp-ssh-extension-progress = Waz SSH 拡張をインストール中... ({ $p }%)
terminal-bootstrapping-installing-warp-ssh-extension = Waz SSH 拡張をインストール中...
terminal-bootstrapping-updating-warp-ssh-extension = Waz SSH 拡張を更新中...
...
agent-tip-command-palette = <keybinding> でコマンドパレットを開き、Waz のアクションやショートカットにアクセスできます。
...
agent-tip-add-context = `@` でファイル、ブロック、Waz Drive オブジェクトからコンテキストをプロンプトに追加できます。
...
hoa-welcome-banner-title = ユニバーサルエージェントサポートのご紹介: Waz であらゆるコーディングエージェントをレベルアップ
...
hoa-feature-native-code-review-description = Waz のコードレビューからインラインコメントを Claude Code、Codex、OpenCode へ直接送信
resource-center-whats-new-section = 新着情報
resource-center-getting-started-section = はじめに
resource-center-maximize-warp-section = Waz を最大限に活用
...
resource-center-command-palette-description = キーボードから Waz のすべての機能にアクセス。
resource-center-set-theme-title = テーマを設定
resource-center-set-theme-description = テーマを選んで Waz を自分好みに。
resource-center-custom-prompt-title = カスタムプロンプトを使用
resource-center-custom-prompt-description = PS1 設定を尊重するよう Waz をセットアップ
resource-center-view-documentation = ドキュメントを表示
resource-center-integrate-ide-title = Waz を IDE と連携
resource-center-integrate-ide-description = よく使う開発ツールから Waz を起動できるよう設定
resource-center-how-warp-uses-warp-title = Waz チームの Waz 活用法
resource-center-how-warp-uses-warp-description = Waz のエンジニアリングチームがお気に入り機能をどう使っているか学びます
...
onboarding-intention-title = Waz へようこそ
...
onboarding-ai-feature-warp-agents = Waz エージェント
...
onboarding-agent-title = Waz エージェントをカスタマイズ
...
onboarding-agent-disable-warp-agent = Waz エージェントを無効化
...
onboarding-project-subtitle = Waz でのコーディング向けにプロジェクトを設定します。
...
onboarding-customize-title = Waz をカスタマイズ
...
onboarding-free-user-agent-title = Waz 内蔵エージェントによるエージェント駆動開発
onboarding-free-user-agent-description = Waz 内蔵エージェント Oz でローカルに反復・計画・構築。
...
onboarding-free-user-subscribe-title = サブスクライブして Waz のエージェント駆動開発を利用しましょう。
...
onboarding-free-user-subscribe-item-drive = 無制限の Waz Drive オブジェクトとコラボレーション
onboarding-free-user-subscribe-item-support = プライベートメールサポート
onboarding-free-user-subscribe-item-cloud-storage = ローカル会話保存

auth-opt-out-line-1 = Waz はオンボーディングの選択をこのデバイスに保存します。
...
auth-local-privacy-note = Waz はオンボーディングの選択をこのデバイスに保存します。
...
auth-login-slide-title-warp-drive = Waz Drive をはじめる
...
auth-disable-warp-drive = Waz Drive を無効化
auth-disable-ai-features = AI 機能を無効化
auth-enable-warp-drive = Waz Drive を有効化
...
auth-disable-warp-drive-confirm-title = 本当に Waz Drive を無効化しますか？
auth-disable-ai-features-confirm-title = 本当に AI 機能を無効化しますか？
auth-disable-warp-drive-confirm-body = Waz Drive はワークフローやナレッジをデバイス間で保存し、チームと共有できます。続行すると、以下の機能が利用できなくなります:
auth-disable-ai-features-confirm-body = Waz は AI でより便利になります。続行すると、以下の機能はいずれも利用できなくなります:
...
auth-require-login-ai-collaboration = ローカル AI 機能に Waz アカウントは必要ありません。
auth-require-login-drive-limit = Waz では Waz Drive オブジェクトをローカルに保存します。
auth-require-login-share = ローカル Waz ビルドでは共有機能を利用できません。
auth-welcome-title = Waz へようこそ！
auth-sign-up-for-warp = Waz を続行
...
get-started-welcome-title = Waz へようこそ
...
terminal-banner-p10k-supported = Powerlevel10k が Waz に対応しました！{"  "}
...
terminal-banner-pure-unsupported = Pure はまだ Waz で対応していません。代替として対応プロンプトの利用をご検討ください。{"  "}
...
settings-import-reset-to-warp-defaults = Waz の既定にリセット
...
workspace-notification-permission-denied-toast = Waz にはデスクトップ通知を送る権限がありません。
workspace-troubleshoot-notifications-link = 通知のトラブルシューティング
workspace-plan-synced-to-warp-drive-toast = プランを Waz Drive に同期しました
workspace-remote-control-link-copied-toast = リモートコントロールリンクをコピーしました。
workspace-update-now = 今すぐ更新
workspace-update-warp = Waz を更新
...
workspace-version-deprecation-without-permissions-banner = 直ちに更新しないと一部の Waz 機能が想定どおり動作しない可能性がありますが、Waz は更新を実行できません。
workspace-new-version-unable-to-update-banner = 新しいバージョンが利用可能ですが、Waz は更新を実行できません。
workspace-unable-to-launch-new-installed-version = Waz はインストール済みの新バージョンを起動できませんでした。
...
terminal-ssh-report-issue-prefix = Waz の SSH 安定性向上に取り組んでいます。次の対応をご検討ください:{" "}
...
terminal-agent-mode-setup-title = このコードベース向けに Waz を最適化しますか？
...
notifications-banner-permissions-denied-title = Waz は通知送信の権限を拒否されました。
...
tooltip-secrets-not-sent-to-warp-server = *シークレットは Waz のサーバーに送信されません。
