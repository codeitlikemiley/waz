# Waz Desktop — English (source-of-truth locale)
...
common-skip = Skip
common-get-warping = Get Wazping
...
agent-management-source-local-agent = Waz (local agent)
agent-management-source-cloud-agent = Waz agent
...
workspace-menu-update-warp-manually = Update Waz manually
...
workspace-menu-view-warp-logs = View Waz logs
...
app-menu-set-default-terminal = Set Waz as Default Terminal
app-menu-file = File
app-menu-edit = Edit
app-menu-use-warp-prompt = Use Waz's Prompt
...
app-menu-help = Help
app-menu-warp-documentation = Waz Documentation...
app-menu-github-issues = GitHub Issues...
app-menu-warp-slack-community = Waz Slack Community...
workspace-update-and-relaunch-warp = Update and relaunch Waz
workspace-updating-to-version = Updating to ({ $version })
workspace-update-warp-manually = Update Waz manually
...
terminal-banner-shell-config-incompatible = Your shell configuration is incompatible with Waz...{"  "}
...
ssh-remote-choice-install-extension = Install Waz's SSH extension
ssh-remote-choice-install-extension-desc = Install Waz's extension to enable agent features like file browsing, code review, and intelligent command completions in this session.
...
ai-document-save-and-sync-tooltip = Save and auto-sync this plan to your Waz Drive
ai-document-show-in-warp-drive = Show in Waz Drive
...
ai-footer-enable-notifications-tooltip = Install the Warp plugin to enable rich agent notifications within Waz
...
waz-launch-visit-repo = Visit the repo
waz-launch-title = Waz is now open-source
waz-launch-description = You, our community, can participate in building Waz using an agent-first workflow.
waz-launch-contribute-title = Contribute
waz-launch-contribute-description = Waz's client code is now open source. Get started by using the /feedback skill to open an issue, and follow the contribution guidelines here.
waz-launch-contribute-link-text = here
waz-launch-oad-title = Open Automated Development
waz-launch-oad-description = The Waz repo is managed by an agent-first local workflow powered by Oz.
waz-launch-auto-model-title = Introducing 'auto (open-weights)'
waz-launch-auto-model-description = We've added a new auto model that picks the best open weight model for a task, like Kimi or MiniMax.
hoa-see-whats-new = See what's new
hoa-finish = Finish
session-config-get-warping = Get Wazping
...
terminal-warpify-subshell-tooltip = Enable Waz shell integration in this session
terminal-use-agent = Use agent
terminal-use-agent-tooltip = Ask the Waz agent to assist
terminal-give-control-back-to-agent = Give control back to agent
terminal-resume-agent-tooltip = Ask the Waz agent to resume
...
terminal-open-in-warp = Open in Waz
terminal-open-conversation-in-warp-tooltip = Open this conversation in the Waz desktop app
...
terminal-input-agent-hint-deploy-react-vercel = Waz anything e.g. Deploy my React app to Vercel and set up environment variables
terminal-input-agent-hint-debug-python-ci = Waz anything e.g. Help me debug why my Python tests are failing in CI
terminal-input-agent-hint-setup-microservice = Waz anything e.g. Set up a new microservice with Docker and create the deployment pipeline
terminal-input-agent-hint-fix-node-memory-leak = Waz anything e.g. Find and fix the memory leak in my Node.js application
terminal-input-agent-hint-backup-postgres = Waz anything e.g. Create a backup script for my PostgreSQL database and schedule it
terminal-input-agent-hint-migrate-mysql-postgres = Waz anything e.g. Help me migrate my data from MySQL to PostgreSQL
terminal-input-agent-hint-monitor-aws = Waz anything e.g. Set up monitoring and alerts for my AWS infrastructure
terminal-input-agent-hint-build-fastapi = Waz anything e.g. Build a REST API for my mobile app using FastAPI
terminal-input-agent-hint-optimize-sql = Waz anything e.g. Help me optimize my SQL queries that are running slowly
terminal-input-agent-hint-github-actions = Waz anything e.g. Create a GitHub Actions workflow to automatically deploy on merge
terminal-input-agent-hint-redis-cache = Waz anything e.g. Set up Redis caching for my web application
terminal-input-agent-hint-kubernetes-pods = Waz anything e.g. Help me troubleshoot why my Kubernetes pods keep crashing
terminal-input-agent-hint-bigquery-pipeline = Waz anything e.g. Build a data pipeline to process CSV files and load them into BigQuery
terminal-input-agent-hint-ssl-https = Waz anything e.g. Set up SSL certificates and configure HTTPS for my domain
terminal-input-agent-hint-refactor-legacy-code = Waz anything e.g. Help me refactor this legacy code to use modern design patterns
terminal-input-agent-hint-unit-tests = Waz anything e.g. Create unit tests for my authentication service
terminal-input-agent-hint-elk-logs = Waz anything e.g. Set up log aggregation with ELK stack for my distributed system
terminal-input-agent-hint-oauth-express = Waz anything e.g. Help me implement OAuth2 authentication in my Express.js app
terminal-input-agent-hint-optimize-docker = Waz anything e.g. Optimize my Docker images to reduce build times and size
terminal-input-agent-hint-ab-testing = Waz anything e.g. Set up A/B testing infrastructure for my web application
...
terminal-model-specs-description = Waz's benchmarks for how well a model performs in our harness, the rate at which it consumes credits, and task speed.
...
onboarding-callout-meet-input-title = Meet the Waz input
...
onboarding-callout-nl-support-text-prefix = Natural language input is off by default. If enabled, you can type requests in plain English and Waz will autodetect queries for the agent. You can always override them using
onboarding-callout-enable-nl-detection = Enable Natural Language Detection
onboarding-callout-new-agent-title = Introducing Waz's new agent experience
...
language-widget-secondary = Restart Waz for the change to fully take effect.
language-restart-required-title = Language changed
language-restart-required-body = Waz's UI language has been updated. Some text will switch immediately, but a full restart is required for the change to take effect everywhere.
...
# Waz: settings-section-account removed alongside the Account settings page.
...
# about_page.rs
settings-about-copyright = Copyright 2026 Waz
settings-about-automatic-updates-label = Automatic updates
settings-about-automatic-updates-description = When enabled, Waz checks for new versions in the background and downloads the installer to a local cache. The currently running Waz is not touched until you click "Install now" to launch the installer yourself.
settings-about-update-checking = Checking for updates…
settings-about-update-up-to-date = Waz is up to date.
...
settings-about-update-install-hint-macos = The installer will open — drag Waz into your Applications folder to finish.
settings-about-update-install-hint-windows = The setup wizard will launch — follow the prompts to finish the upgrade.
settings-about-update-install-hint-linux = The AppImage will be replaced in place and Waz will restart.
...
# Waz: main_page.rs (Account / version / autoupdate) strings removed alongside
...
# ---- list_page.rs ----
settings-mcp-list-description = Add MCP servers to extend the Waz Agent's capabilities. MCP servers expose data sources or tools to agents through a standardized interface, essentially acting like plugins. Add a custom server, or use the presets to get started with popular servers.
...
settings-mcp-list-section-shared-by-warp-and-team = Available from Waz and { $name }
settings-mcp-list-section-shared-by-warp-and-other-devices = Shared by Waz and from other devices
settings-mcp-list-section-shared-from-warp = Shared from Waz
...
settings-platform-description = Create and manage API keys to allow local agents to access your Waz account.
...
settings-platform-zero-state-description = Create a key to manage external access to Waz
settings-platform-create-api-key-description-personal = This API key is tied to your user and can make requests against your Waz account.
...
settings-platform-create-api-key-name-placeholder = Waz API Key
...
settings-referrals-page-title = Invite a friend to Waz
settings-referrals-anonymous-header = Referral program is unavailable in local Waz builds
...
settings-referrals-reward-intro = Get exclusive Waz goodies when you refer someone*
...
settings-warpify-description-prefix = Configure whether Waz attempts to "Warpify" (add support for blocks, input modes, etc) certain shells.
...
settings-warpify-install-ssh-extension-description = Controls the installation behavior for Waz's SSH extension when a remote host doesn't have it installed.
...
# Section / sub-headers
settings-ai-warp-agent-header = Waz Agent
...
settings-ai-command-denylist-description = Regular expressions to match commands that the Waz Agent should always ask permission to execute.
settings-ai-command-allowlist = Command allowlist
settings-ai-command-allowlist-description = Regular expressions to match commands that can be automatically executed by the Waz Agent.
...
settings-ai-mcp-allowlist-description = Allow the Waz Agent to call these MCP servers.
settings-ai-mcp-denylist = MCP denylist
settings-ai-mcp-denylist-description = The Waz Agent will always ask for permission before calling any MCP servers on this list.
...
settings-ai-base-model-description = This model serves as the primary engine behind the Waz Agent. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. Waz may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.
settings-ai-show-model-picker-in-prompt = Show model picker in prompt
settings-ai-codebase-context = Codebase Context
settings-ai-codebase-context-description = Allow the Waz Agent to generate an outline of your codebase that can be used for context. No code is ever stored on our servers.
...
settings-ai-rules-description = Rules help the Waz Agent follow your conventions, whether for codebases or specific workflows.
...
settings-ai-cloud-agent-computer-use-description = Enable computer use in agent conversations started from the Waz app.

# AWS Bedrock
settings-ai-aws-bedrock-toggle = Use AWS Bedrock credentials
settings-ai-aws-bedrock-description = Waz loads and sends local AWS CLI credentials for Bedrock-supported models.
settings-ai-aws-bedrock-description-managed = Waz loads and sends local AWS CLI credentials for Bedrock-supported models. This setting is managed by your organization.
...
settings-features-ssh-wrapper = Waz SSH Wrapper
settings-features-receive-desktop-notifications = Receive desktop notifications from Waz
...
settings-features-make-default-terminal = Make Waz the default terminal
...
settings-features-default-terminal-current = Waz is the default terminal
...
settings-features-restart-warp-to-apply = Restart Waz for changes to take effect.
...
settings-exec-profile-editor-base-model-desc = This model serves as the primary engine behind the agent. It powers most interactions and invokes other models for tasks like planning or code generation when necessary. Waz may automatically switch to alternate models based on model availability or for auxiliary tasks such as conversation summarization.
...
settings-show-blocks-confirm-dialog-text = Are you sure you want to unshare this block?
    ...
    It will no longer be accessible by link and will be permanently deleted from Waz servers.
...
settings-appearance-custom-icon-restart-warning = You may need to restart Waz for MacOS to apply the preferred icon style.
...
settings-appearance-input-type-warp = Waz
settings-appearance-input-type-shell = Shell (PS1)
settings-appearance-input-position-label = Input position
settings-appearance-input-mode-pinned-bottom = Pin to the bottom (Waz mode)
...
settings-environments-page-description = Environments define where your ambient agents run. Set one up in minutes via GitHub (recommended), Waz-assisted setup, or manual configuration.
...
settings-environments-section-team-default = Provided by Waz and this device
settings-environments-section-team-named = Shared by Waz and { $team }
...
quit-warning-title-app = Quit Waz?
...
# Manage Rules 页面(Waz Drive 中的 AI Fact Collection)。
...
keybinding-desc-workspace-left-panel-warp-drive = Left Panel: Waz Drive
...
keybinding-desc-workspace-toggle-warp-drive = Toggle Waz Drive
keybinding-desc-workspace-toggle-warp-drive-menu = Waz Drive
...
keybinding-desc-workspace-search-drive = Search Waz Drive
...
keybinding-desc-workspace-terminate-app = Quit Waz
...
keybinding-desc-workspace-export-all-warp-drive-objects = Export all Waz Drive objects
...
# AI assistant / agents
keybinding-desc-workspace-toggle-ai-assistant = Toggle Waz AI
...
# Waz: keybinding-desc-workspace-show-settings-account removed alongside the
...
keybinding-desc-workspace-show-settings-about-menu = About Waz
...
keybinding-desc-workspace-view-logs = View Waz logs
...
keybinding-desc-workspace-open-waz-launch-modal = [Debug] Open Waz Launch Modal
keybinding-desc-workspace-reset-waz-launch-modal-state = [Debug] Reset Waz Launch Modal State
...
keybinding-desc-input-show-network-log = Show Waz network log
...
keybinding-desc-ai-assistant-close = Close Waz AI
keybinding-desc-ai-assistant-focus-terminal-input = Focus Terminal Input From Waz AI
keybinding-desc-ai-assistant-restart = Restart Waz AI
...
keybinding-desc-terminal-ask-ai-selection = Ask Waz AI about Selection
keybinding-desc-terminal-ask-ai-last-block = Ask Waz AI about last block
keybinding-desc-terminal-ask-ai = Ask Waz AI
...
toggle-suffix-ssh-wrapper = Waz SSH wrapper
...
settings-external-editor-choose-layout = Choose a layout to open files in Waz
...
settings-external-editor-prefer-markdown = Open Markdown files in Waz's Markdown Viewer by default
...
menu-block-open-in-warp = Open in Waz
...
menu-block-ask-warp-ai = Ask Waz AI
...
menu-input-ask-warp-ai = Ask Waz AI
...
slash-cmd-open-skill-desc = Open a skill's markdown file in Waz's built-in editor
...
slash-cmd-open-file-desc = Open a file in Waz's code editor
...
prompt-editor-warp-prompt-section = Waz terminal prompt
...
welcome-tips-command-palette-description = Easily discover everything you can do in Waz without your hands leaving the keyboard.
...
welcome-tips-theme-picker-description = Make Waz your own by choosing a built-in theme. Or create your own.
...
search-filter-display-drive = Waz Drive
...
drive-local-storage-limit-description = Local storage limits are enforced on this device. Remove unused items to create space for new Waz Drive objects.
...
workflow-ai-assist-tooltip = Generate a title, descriptions, or parameters with Waz AI
...
workspace-left-panel-warp-drive = Waz Drive
...
wasm-nux-open-desktop-title = Open in Waz Desktop?
wasm-nux-open-desktop-detail = Future links will automatically open on desktop.
wasm-nux-open-desktop-confirm = Open in Waz
wasm-nux-download-title = Download Waz Desktop?
wasm-nux-download-description = Waz is the intelligent terminal with AI and your dev team's knowledge built-in.
wasm-nux-learn-more = Learn more
wasm-nux-download-confirm = Download
wasm-nux-object-kind-drive-objects = Waz Drive objects
wasm-nux-object-kind-warp-links = Waz links
...
auth-override-warning-confirm-title = Delete personal Waz Drive objects and preferences?
auth-override-warning-description = It looks like you logged into a Waz account through a web browser. If you continue, any personal Waz Drive objects and preferences from this anonymous session will be permanently deleted.
...
auth-override-warning-accessibility-help = Waz has detected a new login from a web browser. Press escape to cancel and continue using Waz without login.
...
auth-needs-sso-link-detail = Click the button below to link your Waz account to your SSO provider.
...
auth-offline-first-use-description = You are currently offline. An internet connection is required to use Waz for the first time.
auth-offline-first-use-learn-more = Learn more
auth-offline-overlay-title = Using Waz Offline
auth-offline-overlay-paragraph-1 = Waz can be used offline for local terminal and agent workflows.
...
auth-privacy-settings-help-improve = Help improve Waz
auth-privacy-settings-help-improve-description = High-level feature usage data helps Waz's product team prioritize the roadmap.
auth-privacy-settings-learn-more = Learn more
auth-privacy-settings-send-crash-reports = Send crash reports
auth-privacy-settings-crash-reports-description = Crash reporting helps Waz's engineering team understand stability and improve performance.
...
auth-logout-unsynced-drive-objects-warning = You have { $count } unsynced Waz Drive { $count ->
        [one] object
       *[other] objects
...
cli-agent-plugin-codex-install-title = Enable Waz Notifications for Codex
cli-agent-plugin-codex-install-subtitle = Update Codex to the latest version, then enable in-focus notifications so Waz can display them while you work.
...
cli-agent-plugin-deepseek-install-title = Enable Waz Notifications for DeepSeek
...
cli-agent-plugin-claude-install-title = Install Waz Plugin for Claude Code
...
cli-agent-plugin-claude-update-title = Update Waz Plugin for Claude Code
...
cli-agent-plugin-gemini-install-title = Install Waz Plugin for Gemini CLI
...
cli-agent-plugin-gemini-update-title = Update Waz Plugin for Gemini CLI
...
cli-agent-plugin-opencode-install-title = Install Waz Plugin for OpenCode
...
cli-agent-plugin-opencode-update-title = Update Waz Plugin for OpenCode
...
terminal-bootstrapping-installing-warp-ssh-extension-progress = Installing Waz SSH Extension... ({ $p }%)
terminal-bootstrapping-installing-warp-ssh-extension = Installing Waz SSH Extension...
terminal-bootstrapping-updating-warp-ssh-extension = Updating Waz SSH Extension...
...
agent-tip-command-palette = <keybinding> to open the Command Palette and access Waz actions and shortcuts.
...
agent-tip-add-context = `@` to add context from files, blocks, or Waz Drive objects to your prompt.
...
agent-tip-action-warp-drive = Waz Drive.
...
hoa-welcome-banner-title = Introducing universal agent support: level up any coding agent with Waz
...
hoa-feature-native-code-review-description = Send inline comments from Waz's code review directly to Claude Code, Codex, or OpenCode
resource-center-whats-new-section = What's New?
resource-center-getting-started-section = Getting Started
resource-center-maximize-warp-section = Maximize Waz
...
resource-center-command-palette-description = Access all of Waz via the keyboard.
resource-center-set-theme-title = Set your theme
resource-center-set-theme-description = Make Waz your own by choosing a theme.
resource-center-custom-prompt-title = Use your custom prompt
resource-center-custom-prompt-description = Set up Waz to honor your PS1 setting
resource-center-view-documentation = View documentation
resource-center-integrate-ide-title = Integrate Waz with your IDE
resource-center-integrate-ide-description = Configure Waz to launch from your most used development tools
resource-center-how-warp-uses-warp-title = How Waz uses Waz
resource-center-how-warp-uses-warp-description = Learn how Waz's engineering team uses their favorite features
...
onboarding-intention-title = Welcome to Waz
...
onboarding-ai-feature-warp-agents = Waz agents
...
onboarding-agent-title = Customize your Waz Agent
...
onboarding-agent-disable-warp-agent = Disable Waz Agent
onboarding-project-title = Open a project
onboarding-project-subtitle = Set up a project to optimize it for coding in Waz.
...
onboarding-customize-title = Customize your Waz
...
onboarding-customize-warp-drive = Waz Drive
onboarding-customize-tools-panel = Tools panel
onboarding-customize-code-review = Code review

auth-opt-out-line-1 = Waz stores onboarding choices locally.
...
auth-local-privacy-note = Waz stores onboarding choices locally on this device.
...
auth-login-slide-title-warp-drive = Get started with Waz Drive
...
auth-disable-warp-drive = Disable Waz Drive
auth-disable-ai-features = Disable AI features
auth-enable-warp-drive = Enable Waz Drive
...
auth-disable-warp-drive-confirm-title = Are you sure you want to disable Waz Drive?
auth-disable-ai-features-confirm-title = Are you sure you want to disable AI features?
auth-disable-warp-drive-confirm-body = Waz Drive lets you save workflows and knowledge across devices and share them with your team. By continuing, you won't have access to the following features:
auth-disable-ai-features-confirm-body = Waz is better with AI. By continuing, you won't have access to any of the following features:
...
auth-require-login-ai-collaboration = Local AI features do not require a Waz account.
auth-require-login-drive-limit = Waz Drive objects are stored locally in Waz.
auth-require-login-share = Sharing is unavailable in local Waz builds.
auth-welcome-title = Welcome to Waz!
auth-sign-up-for-warp = Continue in Waz
...
get-started-welcome-title = Welcome to Waz
...
terminal-banner-p10k-supported = Powerlevel10k now supports Waz!{"  "}
...
terminal-banner-pure-unsupported = Pure is not yet supported in Waz. You might consider one of the supported prompts as an alternative.{"  "}
...
settings-import-reset-to-warp-defaults = Reset to Waz defaults
...
workspace-notification-permission-denied-toast = Waz doesn't have permission to send desktop notifications.
workspace-troubleshoot-notifications-link = Troubleshoot notifications
workspace-plan-synced-to-warp-drive-toast = Plan synced to your Waz Drive
workspace-remote-control-link-copied-toast = Remote control link copied.
workspace-update-now = Update now
workspace-update-warp = Update Waz
...
workspace-version-deprecation-without-permissions-banner = Some Waz features may not work as expected without updating immediately, but Waz is unable to perform the update.
workspace-new-version-unable-to-update-banner = A new version is available but Waz is unable to perform the update.
workspace-unable-to-launch-new-installed-version = Waz was unable to launch the new installed version.
...
terminal-ssh-report-issue-prefix = We are actively working on improving the stability of SSH in Waz. Please consider{" "}
...
terminal-agent-mode-setup-title = Optimize Waz for this codebase?
...
notifications-banner-permissions-denied-title = Waz was denied permissions to send you notifications.
...
tooltip-secrets-not-sent-to-warp-server = *Secrets are not sent to Waz's server.
