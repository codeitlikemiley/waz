...
agent-management-source-local-agent = Waz（本地智能体）
agent-management-source-cloud-agent = Waz（本地智能体）
...
workspace-menu-update-warp-manually = 手动更新 Waz
...
workspace-menu-view-warp-logs = 查看 Waz 日志
...
app-menu-debug = 调试
app-menu-set-default-terminal = 将 Waz 设为默认终端
app-menu-file = 文件
app-menu-edit = 编辑
app-menu-use-warp-prompt = 使用 Waz 提示符
...
app-menu-show-warpified-ssh-blocks = 显示 Waz 化 SSH 块
app-menu-hide-warpified-ssh-blocks = 隐藏 Waz 化 SSH 块
...
app-menu-help = 帮助
app-menu-warp-documentation = Waz 文档...
app-menu-github-issues = GitHub Issues...
app-menu-warp-slack-community = Waz Slack 社区...
workspace-update-and-relaunch-warp = 更新并重启 Waz
workspace-updating-to-version = 正在更新到（{ $version }）
workspace-update-warp-manually = 手动更新 Waz
...
terminal-banner-shell-config-incompatible = 你的 shell 配置与 Waz 不兼容...{"  "}
...
ssh-remote-choice-install-extension = 安装 Waz 的 SSH 扩展
...
ssh-remote-choice-continue-without-installing-desc = 你仍可获得 Waz 化体验，但不包含智能体功能。
...
ai-document-save-and-sync-tooltip = 保存并将此计划自动同步到你的 Waz Drive
ai-document-show-in-warp-drive = 在 Waz Drive 中查看
...
ai-footer-enable-notifications-tooltip = 安装 Warp 插件以在 Waz 中启用丰富的智能体通知
...
waz-launch-visit-repo = 访问仓库
waz-launch-title = Waz 现已开源
waz-launch-description = 你和我们的社区现在可以使用智能体优先的工作流参与构建 Waz。
waz-launch-contribute-title = 参与贡献
waz-launch-contribute-description = Waz 客户端代码现已开源。你可以先使用 /feedback skill 创建 issue，并阅读这里的贡献指南。
waz-launch-contribute-link-text = 这里
waz-launch-oad-title = 开放自动化开发
waz-launch-oad-description = Waz 仓库由智能体优先的本地工作流管理，并由 Oz 本地智能体体验提供支持。
waz-launch-auto-model-title = 介绍「auto（开放权重）」
waz-launch-auto-model-description = 我们新增了一个 auto 模型，可为任务选择最佳开放权重模型，例如 Kimi 或 MiniMax。
hoa-see-whats-new = 查看新变化
hoa-finish = 完成
session-config-get-warping = 开始使用 Waz
...
terminal-warpify-subshell-tooltip = 在此会话中启用 Waz shell 集成
terminal-use-agent = 使用智能体
terminal-use-agent-tooltip = 让 Waz 智能体协助
terminal-give-control-back-to-agent = 将控制权交还给智能体
terminal-resume-agent-tooltip = 让 Waz 智能体继续
...
terminal-open-in-warp = 在 Waz 中打开
terminal-open-conversation-in-warp-tooltip = 在 Waz 桌面端打开此对话
...
terminal-input-agent-hint-deploy-react-vercel = 让 Waz 处理任何任务，例如将 React 应用部署到 Vercel 并配置环境变量
terminal-input-agent-hint-debug-python-ci = 让 Waz 处理任何任务，例如帮我调试 Python 测试在 CI 中失败的原因
terminal-input-agent-hint-setup-microservice = 让 Waz 处理任何任务，例如用 Docker 搭建新的微服务并创建部署流水线
terminal-input-agent-hint-fix-node-memory-leak = 让 Waz 处理任何任务，例如查找并修复 Node.js 应用中的内存泄漏
terminal-input-agent-hint-backup-postgres = 让 Waz 处理任何任务，例如为 PostgreSQL 数据库创建备份脚本并设置定时任务
terminal-input-agent-hint-migrate-mysql-postgres = 让 Waz 处理任何任务，例如帮我把数据从 MySQL 迁移到 PostgreSQL
terminal-input-agent-hint-monitor-aws = 让 Waz 处理任何任务，例如为 AWS 基础设施设置监控和告警
terminal-input-agent-hint-build-fastapi = 让 Waz 处理任何任务，例如用 FastAPI 为移动应用构建 REST API
terminal-input-agent-hint-optimize-sql = 让 Waz 处理任何任务，例如帮我优化运行缓慢的 SQL 查询
terminal-input-agent-hint-github-actions = 让 Waz 处理任何任务，例如创建 GitHub Actions 工作流，在合并后自动部署
terminal-input-agent-hint-redis-cache = 让 Waz 处理任何任务，例如为 Web 应用设置 Redis 缓存
terminal-input-agent-hint-kubernetes-pods = 让 Waz 处理任何任务，例如帮我排查 Kubernetes Pod 持续崩溃的原因
terminal-input-agent-hint-bigquery-pipeline = 让 Waz 处理任何任务，例如构建数据流水线来处理 CSV 文件并加载到 BigQuery
terminal-input-agent-hint-ssl-https = 让 Waz 处理任何任务，例如设置 SSL 证书并为域名配置 HTTPS
terminal-input-agent-hint-refactor-legacy-code = 让 Waz 处理任何任务，例如帮我将这段遗留代码重构为现代设计模式
terminal-input-agent-hint-unit-tests = 让 Waz 处理任何任务，例如为认证服务创建单元测试
terminal-input-agent-hint-elk-logs = 让 Waz 处理任何任务，例如为分布式系统设置 ELK 日志聚合
terminal-input-agent-hint-oauth-express = 让 Waz 处理任何任务，例如帮我在 Express.js 应用中实现 OAuth2 认证
terminal-input-agent-hint-optimize-docker = 让 Waz 处理任何任务，例如优化 Docker 镜像以减少构建时间和体积
terminal-input-agent-hint-ab-testing = 让 Waz 处理任何任务，例如为 Web 应用搭建 A/B 测试基础设施
...
terminal-model-specs-description = Waz 对模型在执行器中的表现、额度消耗速率和任务速度的基准测试。
...
onboarding-callout-meet-input-title = 认识 Waz 输入框
...
onboarding-callout-nl-support-text-prefix = 默认情况下自然语言输入处于关闭状态。启用后，你可以直接用自然语言输入请求，Waz 会为智能体自动识别查询。你也始终可以使用
onboarding-callout-enable-nl-detection = 启用自然语言识别
onboarding-callout-new-agent-title = 认识 Waz 全新的智能体体验
...
language-widget-secondary = 重启 Waz 以让此更改完全生效。
language-restart-required-title = 语言已切换
language-restart-required-body = Waz 的界面语言已更新。部分文字会立即切换，完整生效需要重启 Waz。
...
# Waz: settings-section-account 随 Account 设置页一同删除。
...
settings-section-warp-agent = Waz 智能体
...
# about_page.rs
settings-about-copyright = 版权所有 2026 Waz
settings-about-automatic-updates-label = 自动检查更新
settings-about-automatic-updates-description = 开启后，Waz 会在后台定期检查是否有新版本;发现新版本后自动下载安装包到本地缓存,等您主动点击「立即安装」时才会启动安装程序,期间不会改动当前正在运行的 Waz。
...
settings-about-update-install-hint-macos = 安装包将打开,请把 Waz 拖到「应用程序」文件夹完成升级。
settings-about-update-install-hint-windows = 安装向导将启动,按提示完成升级。
settings-about-update-install-hint-linux = AppImage 将就地更新并重启 Waz。
...
# Waz：main_page.rs 相关文案随 Account 设置页一同删除。
...
# ---- list_page.rs ----
settings-mcp-list-description = 添加 MCP 服务器以扩展 Waz Agent 的能力。MCP 服务器通过标准化接口向 agent 暴露数据源或工具，本质上类似插件。你可以添加自定义服务器，或使用预设快速开始使用流行的服务器。
...
settings-mcp-list-section-shared-by-warp-and-team = 来自 Waz 和 { $name }
settings-mcp-list-section-shared-by-warp-and-other-devices = 由 Waz 和其他设备共享
settings-mcp-list-section-shared-from-warp = 来自 Waz 的共享
...
settings-platform-description = 创建并管理 API 密钥，允许本地 agent 访问你的 Waz 账户。
    了解更多请访问
...
settings-platform-zero-state-description = 创建密钥以管理对 Waz 的外部访问
settings-platform-create-api-key-description-personal = 此 API 密钥与你的用户绑定，可用于向你的 Waz 账户发起请求。
settings-platform-create-api-key-description-team = 此 API 密钥与你的团队绑定，可用于代表你的团队发起请求。
settings-platform-create-api-key-name-placeholder = Waz API 密钥
...
settings-referrals-page-title = 邀请朋友加入 Waz
settings-referrals-anonymous-header = 本地 Waz 构建不提供推荐计划
...
settings-referrals-reward-intro = 推荐朋友即可获得 Waz 专属周边*
...
settings-warpify-description-prefix = 配置 Waz 是否尝试对特定 Shell 执行 "Warpify"（为其添加命令块、输入模式等支持）。
...
settings-warpify-install-ssh-extension-description = 控制远程主机未安装 Waz 的 SSH 扩展时的安装行为。
...
# 章节 / 副标题
settings-ai-warp-agent-header = Waz 智能体
...
settings-ai-command-denylist-description = 匹配命令的正则表达式，Waz 智能体执行这些命令前必须征得许可。
settings-ai-command-allowlist = 命令允许列表
settings-ai-command-allowlist-description = 匹配命令的正则表达式，Waz 智能体可自动执行这些命令。
...
settings-ai-mcp-allowlist-description = 允许 Waz 智能体调用这些 MCP 服务器。
settings-ai-mcp-denylist = MCP 拒绝列表
settings-ai-mcp-denylist-description = Waz 智能体调用此列表中的任何 MCP 服务器前都必须征得许可。
settings-ai-info-banner-managed-by-workspace = 你的部分权限由工作区管理。

# 模型 / 配置
settings-ai-base-model = 基础模型
settings-ai-base-model-description = 此模型作为 Waz 智能体背后的主要引擎，驱动大部分交互，并在需要时调用其他模型完成规划或代码生成等任务。Waz 可能根据模型可用性自动切换备用模型，或将其用于会话摘要等辅助任务。
settings-ai-show-model-picker-in-prompt = 在提示中显示模型选择器
settings-ai-codebase-context = 代码库上下文
settings-ai-codebase-context-description = 允许 Waz 智能体生成代码库的概要作为上下文。代码从不存储到我们的服务器。
...
settings-ai-rules-description = 规则帮助 Waz 智能体遵循你的约定，无论是针对代码库还是特定工作流。
...
settings-ai-cloud-agent-computer-use-description = 在 Waz 应用中启动的智能体会话中启用计算机使用。

# AWS Bedrock
settings-ai-aws-bedrock-toggle = 使用 AWS Bedrock 凭证
settings-ai-aws-bedrock-description = Waz 加载并发送本地 AWS CLI 凭证以使用 Bedrock 支持的模型。
settings-ai-aws-bedrock-description-managed = Waz 加载并发送本地 AWS CLI 凭证以使用 Bedrock 支持的模型。此设置由你的组织管理。
...
settings-features-ssh-wrapper = Waz SSH 包装器
settings-features-receive-desktop-notifications = 接收来自 Waz 的桌面通知
...
settings-features-make-default-terminal = 将 Waz 设为默认终端
...
settings-features-default-terminal-current = Waz 已是默认终端
...
settings-features-restart-warp-to-apply = 重启 Waz 以使更改生效。
...
settings-exec-profile-editor-base-model-desc = 该模型作为智能体的主要引擎，驱动绝大多数交互，并在需要时调用其他模型完成规划或代码生成等任务。Waz 可能基于模型可用性或辅助任务（如对话摘要）自动切换到备选模型。
...
settings-appearance-custom-icon-restart-warning = 你可能需要重启 Waz 才能让 macOS 应用所选图标样式。
...
settings-appearance-input-mode-pinned-bottom = 固定在底部（Waz 模式）
...
settings-environments-page-description = 环境定义了你的 ambient agent 在哪里运行。可通过 GitHub（推荐）、Waz 辅助配置或手动配置在几分钟内创建一个。
...
settings-environments-section-team-default = 由 Waz 和本地设备提供
settings-environments-section-team-named = 由 Waz 和 { $team } 共享
...
keybinding-desc-workspace-left-panel-warp-drive = 左侧面板：Waz Drive
...
keybinding-desc-workspace-toggle-warp-drive = 切换 Waz Drive
...
keybinding-desc-workspace-search-drive = 搜索 Waz Drive
...
keybinding-desc-workspace-terminate-app = 退出 Waz
...
keybinding-desc-workspace-export-all-warp-drive-objects = 导出所有 Waz Drive 对象
keybinding-desc-workspace-install-cli = 安装 Oz CLI 命令
keybinding-desc-workspace-uninstall-cli = 卸载 Oz CLI 命令

# AI 助手 / agent
keybinding-desc-workspace-toggle-ai-assistant = 切换 Waz AI
...
# Waz：keybinding-desc-workspace-show-settings-account 随 Account 设置页一同删除。
...
keybinding-desc-workspace-show-settings-about-menu = 关于 Waz
...
keybinding-desc-workspace-view-logs = 查看 Waz 日志
...
keybinding-desc-workspace-init-project = 为 Waz 初始化项目
...
keybinding-desc-workspace-open-waz-launch-modal = [调试] 打开 Waz 启动弹窗
keybinding-desc-workspace-reset-waz-launch-modal-state = [调试] 重置 Waz 启动弹窗状态
...
# 终端输入相关 binding
keybinding-desc-input-show-network-log = 显示 Waz 网络日志
...
keybinding-desc-ai-assistant-close = 关闭 Waz AI
keybinding-desc-ai-assistant-focus-terminal-input = 从 Waz AI 切回终端输入
keybinding-desc-ai-assistant-restart = 重启 Waz AI
...
keybinding-desc-terminal-ask-ai-selection = 就所选内容询问 Waz AI
keybinding-desc-terminal-ask-ai-last-block = 就最近的命令块询问 Waz AI
keybinding-desc-terminal-ask-ai = 询问 Waz AI
...
# Waz 已移除远端 Agent 入口，此 key 实际不会被渲染;保留以匹配 en 兜底链。
...
toggle-suffix-ssh-wrapper = Waz SSH 包装器
...
settings-external-editor-choose-layout = 选择在 Waz 中打开文件的布局
settings-external-editor-tabbed-header = 多个文件合并到同一编辑器面板
settings-external-editor-tabbed-desc = 开启后，同一标签页中打开的文件会自动归并到单一编辑器面板。
settings-external-editor-prefer-markdown = 默认用 Waz Markdown 查看器打开 Markdown 文件
...
menu-block-open-in-warp = 在 Waz 中打开
...
menu-block-ask-warp-ai = 询问 Waz AI
...
menu-input-ask-warp-ai = 询问 Waz AI
...
prompt-editor-warp-prompt-section = Waz 终端提示词
...
welcome-tips-command-palette-description = 无需双手离开键盘，即可轻松发现 Waz 的全部功能。
...
welcome-tips-theme-picker-description = 选择内置主题，让 Waz 更符合你的风格。也可以创建自己的主题。
...
drive-local-storage-limit-description = 本地存储上限会在此设备上生效。请移除不再需要的项目，为新的 Waz Drive 对象腾出空间。
drive-sign-up = 本地管理
...
workflow-ai-assist-tooltip = 使用 Waz AI 生成标题、描述或参数
...
wasm-nux-open-desktop-title = 在 Waz 桌面版中打开？
wasm-nux-open-desktop-detail = 之后的链接会自动在桌面版中打开。
wasm-nux-open-desktop-confirm = 在 Waz 中打开
wasm-nux-download-title = 下载 Waz 桌面版？
wasm-nux-download-description = Waz 是一款内置 AI 和团队知识的智能终端。
wasm-nux-learn-more = 了解更多
wasm-nux-download-confirm = 下载
wasm-nux-object-kind-drive-objects = Waz Drive 对象
wasm-nux-object-kind-warp-links = Waz 链接
...
auth-override-warning-confirm-title = 要删除个人 Waz Drive 对象和偏好吗？
auth-override-warning-description = 看起来你刚刚通过网页浏览器登录了一个 Waz 账号。如果继续，当前匿名会话中的所有个人 Waz Drive 对象和偏好都将被永久删除。
...
auth-override-warning-accessibility-help = Waz 检测到你通过网页浏览器进行了新的登录。按 Esc 键取消，并继续在未登录状态下使用 Waz。
...
auth-needs-sso-link-detail = 点击下方按钮，将你的 Waz 账号关联到你的 SSO 提供商。
...
auth-offline-first-use-description = 你当前处于离线状态。首次使用 Waz 时需要联网。
auth-offline-first-use-learn-more = 了解更多
auth-offline-overlay-title = 离线使用 Waz
auth-offline-overlay-paragraph-1 = Waz 的本地功能可以离线使用。
auth-offline-overlay-paragraph-2 = 首次使用时可能需要联网完成依赖初始化，之后本地智能体和终端功能不依赖 Waz 云服务。
auth-offline-overlay-paragraph-3 = Waz 会把必要状态保存在本机，用于本地会话、设置和智能体功能。
auth-offline-overlay-dismiss = 关闭
auth-privacy-settings-title = 隐私设置
auth-privacy-settings-done = 完成
auth-privacy-settings-help-improve = 帮助改进 Waz
auth-privacy-settings-help-improve-description = 高层级的功能使用数据可以帮助 Waz 产品团队确定路线图优先级。
auth-privacy-settings-learn-more = 了解更多
auth-privacy-settings-send-crash-reports = 发送崩溃报告
auth-privacy-settings-crash-reports-description = 崩溃报告能帮助 Waz 工程团队了解稳定性情况并改进性能。
...
auth-logout-unsynced-drive-objects-warning = 你有 { $count } 个未同步的 Waz Drive 对象。退出登录后将失去这些对象。
...
cli-agent-plugin-codex-install-title = 为 Codex 启用 Waz 通知
cli-agent-plugin-codex-install-subtitle = 将 Codex 更新到最新版本，然后启用聚焦时通知，这样你工作时 Waz 就能显示通知。
...
cli-agent-plugin-deepseek-install-title = 为 DeepSeek 启用 Waz 通知
...
terminal-bootstrapping-installing-warp-ssh-extension-progress = 正在安装 Waz SSH 扩展...（{ $p }%）
terminal-bootstrapping-installing-warp-ssh-extension = 正在安装 Waz SSH 扩展...
terminal-bootstrapping-updating-warp-ssh-extension = 正在更新 Waz SSH 扩展...
...
agent-tip-command-palette = 按 <keybinding> 打开命令面板，访问 Waz 操作和快捷方式。
agent-tip-warp-drive = 将可复用的工作流、Notebook 和提示词存入你的
agent-tip-redirect-running-agent = 输入新的提示词即可在智能体运行时重定向它。
agent-tip-add-context = 输入 `@` 将文件、块或 Waz Drive 对象作为上下文添加到提示词。
...
hoa-welcome-banner-title = 引入通用智能体支持：用 Waz 增强任何编码智能体
...
hoa-feature-native-code-review-description = 将 Waz 代码审查中的行内评论直接发送到 Claude Code、Codex 或 OpenCode
resource-center-whats-new-section = 新变化
resource-center-getting-started-section = 入门
resource-center-maximize-warp-section = 充分使用 Waz
...
resource-center-command-palette-description = 通过键盘访问 Waz 的全部功能。
resource-center-set-theme-title = 设置主题
resource-center-set-theme-description = 选择主题，让 Waz 符合你的偏好。
resource-center-custom-prompt-title = 使用自定义提示符
resource-center-custom-prompt-description = 设置 Waz 以遵循你的 PS1 设置
resource-center-view-documentation = 查看文档
resource-center-integrate-ide-title = 将 Waz 与 IDE 集成
resource-center-integrate-ide-description = 配置 Waz，从你最常用的开发工具中启动
resource-center-how-warp-uses-warp-title = Waz 如何使用 Waz
resource-center-how-warp-uses-warp-description = 了解 Waz 工程团队如何使用他们最喜欢的功能
...
onboarding-intention-title = 欢迎使用 Waz
...
onboarding-ai-feature-warp-agents = Waz 智能体
...
onboarding-agent-title = 自定义你的 Waz 智能体
...
onboarding-agent-disable-warp-agent = 禁用 Waz 智能体
...
onboarding-project-subtitle = 设置项目，让 Waz 更适合编码。
...
onboarding-customize-title = 自定义你的 Waz
...
onboarding-free-user-agent-title = 使用 Waz 内置智能体进行开发
onboarding-free-user-agent-description = 使用 Oz（Waz 内置智能体）迭代、规划和构建。本地运行，不依赖 Waz 云服务。
...
onboarding-free-user-subscribe-title = 订阅后即可使用 Waz 的智能体开发能力。
...
auth-disable-warp-drive = 停用 Waz Drive
auth-disable-ai-features = 停用 AI 功能
auth-enable-warp-drive = 启用 Waz Drive
...
auth-disable-warp-drive-confirm-title = 确定要停用 Waz Drive 吗？
auth-disable-ai-features-confirm-title = 确定要停用 AI 功能吗？
auth-disable-warp-drive-confirm-body = Waz Drive 可让你跨设备保存工作流和知识，并与团队分享。继续后，你将无法使用以下功能：
auth-disable-ai-features-confirm-body = 开启 AI 后 Waz 会更强大。继续后，你将无法使用以下功能：
...
auth-require-login-ai-collaboration = 本地 AI 功能不需要 Waz 账户。
auth-require-login-drive-limit = Waz 会将 Waz Drive 对象保存在本机。
auth-require-login-share = 本地 Waz 构建不提供分享功能。
auth-welcome-title = 欢迎使用 Waz！
auth-sign-up-for-warp = 继续使用 Waz
...
get-started-welcome-title = 欢迎使用 Waz
...
terminal-banner-p10k-supported = Powerlevel10k 现在支持 Waz！{"  "}
...
terminal-banner-pure-unsupported = Waz 尚不支持 Pure。你可以考虑改用受支持的提示符。{"  "}
...
settings-import-reset-to-warp-defaults = 重置为 Waz 默认设置
...
workspace-notification-permission-denied-toast = Waz 没有发送桌面通知的权限。
workspace-troubleshoot-notifications-link = 排查通知问题
workspace-plan-synced-to-warp-drive-toast = 计划已同步到你的 Waz Drive
workspace-remote-control-link-copied-toast = 远程控制链接已复制。
workspace-update-now = 立即更新
workspace-update-warp = 更新 Waz
...
workspace-version-deprecation-without-permissions-banner = 若不立即更新，部分 Waz 功能可能无法正常工作，但 Waz 无法执行更新。
workspace-new-version-unable-to-update-banner = 有新版本可用，但 Waz 无法执行更新。
workspace-unable-to-launch-new-installed-version = Waz 无法启动新安装的版本。
...
terminal-ssh-report-issue-prefix = 我们正在积极改进 Waz 中 SSH 的稳定性。请考虑
...
terminal-agent-mode-setup-title = 为此代码库优化 Waz？
...
notifications-banner-permissions-denied-title = Waz 被拒绝发送通知的权限。
...
tooltip-secrets-not-sent-to-warp-server = *密钥不会发送到 Waz 服务器。
