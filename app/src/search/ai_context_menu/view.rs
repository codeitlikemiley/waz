...
impl AIContextMenu {
    ...
    pub(crate) fn get_categories_for_mode(
        ...
        if is_cli_agent_input {
            ...
            // Waz: Originally we would push the Code category depending on outline_codebase_symbols_for_at_context_menu.
            // Now outline is deprecated/retired, so the Code category no longer appears.
            return categories;
        }
        ...
        if is_ai_or_autodetect_mode {
            ...
            // Waz: The Code category is deprecated/retired along with outline, and is no longer pushed.
            ...
            // Waz: Originally we would also push the Code category here depending on outline_codebase_symbols_for_at_context_menu.
            // Now outline is deprecated/retired, so the Code category no longer appears.
    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        // Waz: Originally CodeSymbolCache (subscribing to RepoOutlines) would be created here to support
        // code symbol searching. This feature has been retired along with outline, so these subscriptions
        // and creations are deleted altogether.
    ...
    fn reset_mixer(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        match self.state.navigation_state {
            ...
            // Waz: Code category is retired along with outline. This category won't appear in categories,
            // but the enum variant is retained to avoid breaking a large number of matches; it won't be hit here.
            #[cfg(not(target_family = "wasm"))]
    ...
    fn setup_data_sources_for_all_categories(&mut self, query: &str, ctx: &mut ViewContext<Self>) {
        ...
        for category in categories.iter() {
            match category {
                ...
                // Waz: Code category is retired along with outline; it won't appear but the noop branch is retained
    ...
    // Waz: Originally `render_code_symbols_indexing` was responsible for rendering the "Code symbols indexing..."
    // hint when indexing codebase symbols. After outline is retired, this rendering path is never called,
    // so the function is deleted altogether.
    ...
    fn render_empty_state(
        ...
        // Waz: Originally we would check if code symbols were being indexed under the Code category, and if so,
        // display the `render_code_symbols_indexing` hint. After outline is retired, this category won't appear, so this is completely deleted.
        let _ = category;
