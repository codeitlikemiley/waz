...
pub enum CommandPaletteItemAction {
    ...
    NewConversation,
    /// Open SSH server (exclusive to openWarp) -> routes to WorkspaceAction::OpenSshTerminal
    /// goes through SecretInjector + new tab.
    OpenSshServer {
        node_id: String,
...
pub enum ItemSummary {
    ...
    ForkConversation,
    NewConversation,
    /// SSH server (exclusive to openWarp). `node_id` is the UUID of the ssh_nodes table.
    SshServer {
        node_id: String,
    },
    /// No-op action (used for non-interactable separator items that don't do anything on click).
