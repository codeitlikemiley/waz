    ...
    pub fn query_result_from_summary(
        ...
        match summary {
            ...
            ItemSummary::SshServer { node_id: _ } => {
                // SSH server option recents are not shown for now — opening from the SSH manager tree
                // is the primary path, and the palette option is supplementary. Returning None does not affect search matchability.
                None
            }
            ItemSummary::NoOp => {
