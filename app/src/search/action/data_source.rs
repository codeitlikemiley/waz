...
impl CommandBindingDataSource {
    ...
    pub fn new(binding_source: ModelHandle<BindingSource>, ctx: &mut ModelContext<Self>) -> Self {
        // Waz: Command palette actions always use character-level fuzzy (SkimMatcherV2).
        // Tantivy's default tokenizer does not split CJK, which would treat the entire Chinese description as a single token for prefix matching,
        // causing search for "theme" in zh-CN to fail to match "open theme selector".
        // Meanwhile, fuzzy matching allows English keywords to match binding.name via subsequence matching (see the search implementation below).
    ...
    fn search(&self, search_term: &str) -> anyhow::Result<Vec<QueryResult<SearcherAction>>> {
        Ok(self
            ...
            .filter_map(move |binding| {
                ...
                //
                // Waz: Append binding.name (the action identifier, e.g., `workspace:show_theme_chooser`)
                // into the searchable string altogether, replacing `:` and `_` with spaces, to facilitate subsequence matching.
                // This way, if a user enters "theme" in zh-CN, it can still match "Open Theme Chooser" (Open Theme Chooser).
                ...
                .map(|mut result| {
                    // Highlighting targets the description. Indices that fall into the appended binding.name segment
                    // would cause the highlight to be drawn in the wrong position if out of bounds, so they are clipped here.
                    result
                        .matched_indices
