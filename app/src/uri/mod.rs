...
use crate::{drive::WazDriveObjectArgs, view_components::DismissibleToast};
...
/// Waz Wave 7-3: URI handler / Settings UI has been deleted, only for `update_environment_form` in agent UI
/// Acts as a transition before the major commit is completed (the latter is not used in spelling out the URL).
...
impl UriHost {
    fn handle(&self, primary_window_id: Option<WindowId>, url: &Url, ctx: &mut AppContext) {
        ...
        match self {
            UriHost::Auth => {
                safe_info!(
                    safe: ("Ignored auth url because Waz has no cloud login flow"),
                    full: ("Ignored auth url {url} because Waz has no cloud login flow")
                );
            }
            UriHost::Action => {
            ...
            UriHost::Settings => {
                ...
                if let Some(settings_sub_page) = settings_sub_page {
                    match settings_sub_page.as_str() {
                        "environments" => {
                            // Waz Wave 7-3:warp://settings/environments URI handler
                            // The ambient-agent UI subsystem is physically deleted. Also retain GitHub auth completion
                            // Notifications - other independent components may need to listen.
                        ...
                        // Waz Wave 3-1: "platform" URI routing original point
                        // `SettingsSection::OzCloudAPIKeys` (cloud API key management page),
                        // Physically deleted along with the UI. Keep arm to document original intent, physics handling is no-op.
                        "platform" => {
                            log::warn!(
                                "warp://settings/platform 路由在 Waz 中已下线,忽略该请求"
                            );
                        }
                        "appearance" => {
                            dispatch_action_in_new_or_existing_window(
    ...
    fn window_behavior_hint(&self) -> WindowBehaviorHint {
        ...
        match self {
            ...
            Self::NewTab => W::ShowPrimaryWindow(WindowActivationFallbackBehavior::Notify {
                ...
                description: "Go to Waz to see your new tab.".to_owned(),
            }),
...
enum OpenFileAction {
    ...
    Notebook,
    /// Open in Waz's code/text editor pane.
    Editor,
...
fn open_file(window_id: Option<WindowId>, path: PathBuf, ctx: &mut AppContext) {
    ...
    if action == OpenFileAction::Notebook {
        ...
        #[cfg(feature = "local_fs")]
        {
            ...
            // Open text/code files in Waz's code editor, respecting the user's layout preference.
