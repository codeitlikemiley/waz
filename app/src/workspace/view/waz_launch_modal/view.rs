...
const HERO_IMAGE_PATH: &str = "async/png/onboarding/waz_launch_banner.png";
...
fn feature_items() -> [FeatureItem; 3] {
    [
        FeatureItem {
            icon: Icon::HeartHand,
            title: crate::t!("waz-launch-contribute-title"),
            description: crate::t!("waz-launch-contribute-description"),
            inline_link: Some(InlineLink {
                text: crate::t!("waz-launch-contribute-link-text"),
                url: CONTRIBUTING_URL,
            }),
        },
        FeatureItem {
            icon: Icon::Oz,
            title: crate::t!("waz-launch-oad-title"),
            description: crate::t!("waz-launch-oad-description"),
            inline_link: None,
        },
        FeatureItem {
            icon: Icon::MessageChatSquare,
            title: crate::t!("waz-launch-auto-model-title"),
            description: crate::t!("waz-launch-auto-model-description"),
            inline_link: None,
        },
    ]
}
...
pub fn init(app: &mut AppContext) {
    ...
    app.register_fixed_bindings([FixedBinding::new(
        "escape",
        WazLaunchModalAction::Close,
        id!(WazLaunchModal::ui_name()),
    )]);
}

#[derive(Clone, Debug)]
pub enum WazLaunchModalAction {
    Close,
    VisitRepo,
}

#[derive(Clone, Debug)]
pub enum WazLaunchModalEvent {
    Close,
}
...
impl WazLaunchModal {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let close_button = ctx.add_view(|_ctx| {
            ActionButton::new("", CloseButtonTheme)
                ...
                .on_click(|ctx| ctx.dispatch_typed_action(WazLaunchModalAction::Close))
        });

        let cta_button = ctx.add_view(|_ctx| {
            ActionButton::new(crate::t!("waz-launch-visit-repo"), CtaButtonTheme)
                .with_full_width(true)
                .on_click(|ctx| ctx.dispatch_typed_action(WazLaunchModalAction::VisitRepo))
        });
    ...
    fn render_description(appearance: &Appearance) -> Box<dyn Element> {
        Text::new(
            crate::t!("waz-launch-description"),
            appearance.ui_font_family(),
            14.,
        )
...
impl Entity for WazLaunchModal {
    type Event = WazLaunchModalEvent;
}

impl View for WazLaunchModal {
...
impl TypedActionView for WazLaunchModal {
    type Action = WazLaunchModalAction;

    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            WazLaunchModalAction::Close => {
                ctx.emit(WazLaunchModalEvent::Close);
            }
            WazLaunchModalAction::VisitRepo => {
