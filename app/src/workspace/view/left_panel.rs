...
pub enum LeftPanelEvent {
    ...
    /// User clicked a file in the remote file tree -> Main window should open it as a remote buffer.
    ...
    /// User clicked a server in the SSH manager tree / double-clicked / right-clicked "Edit" -> Main window should open/focus
    /// `SshServerPane` in the central area (specifically `WorkspaceView::open_ssh_server`).
    OpenSshServerEditor {
        node_id: String,
    },
    /// User right-clicked "Connect" in the SSH manager -> Main window runs `ssh ...` in a new terminal pane
    /// and starts SecretInjector (implemented in Commit 3; currently placeholder event).
    OpenSshTerminal {
        node_id: String,
...
impl LeftPanelView {
    pub fn new(
        ...
        ctx.subscribe_to_view(&warp_drive_view, |_me, _, event, ctx| {
            ctx.emit(LeftPanelEvent::WazDrive(event.clone()));
        });
        ...
        let active_view = views.first().copied().unwrap_or(ToolPanelView::WazDrive);
        ...
        match view {
            ...
            ToolPanelView::WazDrive => {
                ...
                ToolbeltButtonConfig {
                    ...
                    action: LeftPanelAction::WazDrive,
                    render_with_active_state: false,
                    tooltip_keybinding: toolbelt_tooltip_keybinding(&tooltip_keybinding_names, ctx),
                    tooltip_keybinding_names,
                }
    ...
    pub fn is_warp_drive_active(&self) -> bool {
        self.active_view.get() == ToolPanelView::WazDrive
    }
    ...
    fn update_button_active_states(&mut self) {
        for button in &mut self.toolbelt_buttons {
            button.render_with_active_state = match &button.action {
                ...
                LeftPanelAction::WazDrive => self.active_view.get() == ToolPanelView::WazDrive,
...
impl LeftPanelView {
    pub fn handle_action_with_force_open(
        ...
        match action {
            ...
            LeftPanelAction::WazDrive => {
                active_view_state::set(self, ToolPanelView::WazDrive, ctx);
                if force_open {
                    send_telemetry_from_ctx!(
...
impl View for LeftPanelView {
    ...
    fn on_focus(&mut self, focus_ctx: &FocusContext, ctx: &mut ViewContext<Self>) {
        ...
        if focus_ctx.is_self_focused() {
            match self.active_view.get() {
                ...
                ToolPanelView::WazDrive => ctx.focus(&self.warp_drive_view),
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        let content_area: Box<dyn Element> = match self.active_view.get() {
            ...
            ToolPanelView::WazDrive => Shrinkable::new(
                1.0,
