...
enum SummaryPaneKind {
    ...
    Settings,
    EnvVarCollection,
    // Waz Wave 7-3: `EnvironmentManagement` variant is physically removed with the ambient-agent UI subsystem.
    AIFact,
    AIDocument,
...
fn resolve_icon_with_status_variant(
    ...
    match typed {
        ...
        // Waz Wave 7-3: `TypedPane::EnvironmentManagement` is physically removed with the ambient-agent UI subsystem.
        TypedPane::Settings => IconWithStatusVariant::Neutral {
            icon: typed.icon(),
            icon_color: main_text,
        },
        // Waz Drive object types use their established index colors
...
impl TypedPane<'_> {
    fn summary_pane_kind(&self, title: &str, app: &AppContext) -> SummaryPaneKind {
        match self {
            ...
            // Waz Wave 7-3: `TypedPane::EnvironmentManagement` arm is physically deleted with the variant.
    ...
    fn kind_label(&self) -> String {
        match self {
            ...
            // Waz Wave 7-3:`TypedPane::EnvironmentManagement` kind_label arm with variant
            // Physically delete.
    ...
    fn icon(&self) -> WarpIcon {
        match self {
            ...
            // Waz Wave 7-3: `TypedPane::EnvironmentManagement` icon arm is physically deleted with the variant.
...
fn render_summary_pane_kind_icon_circle(
    ...
    let (icon_element, background): (Box<dyn Element>, ElementFill) = match kind {
        ...
        // Waz Wave 7-3:`SummaryPaneKind::EnvironmentManagement` arm is physically deleted with the variant.
