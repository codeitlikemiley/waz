...
const UPLOAD_STAGING_DIR_NAME: &str = ".waz-upload-staging";
...
impl ServerFileBrowserView {
    ...
    pub fn set_remote_root(
        ...
        let should_load = host_changed
            ...
            || session_changed
            || session_id_changed;
        // When switching host or session, the old progress polling has nothing to do with the new connection. Stop it first to avoid
        // Redundant timer callbacks. If a new upload/download task is generated during subsequent loading,
        // `schedule_progress_poll` will be restarted.
    ...
    match response.result {
        Some(list_directory_response::Result::Success(success)) => {
            ...
            for entry in success.entries {
                // Compatible with older versions of the server: the `kind` field is introduced by the new protocol and returned by the old server
                // `FileSystemEntryKind::Unspecified` cannot be directly downgraded to `Other`,
                // Otherwise, double-clicking the directory will mistakenly go through the ResolvePath (old server does not support) process and report an error.
                // Here fallback to the old field `is_dir` when `Unspecified`; unknown kind(future server
                // Possibly new variants) are classified as `Other` to avoid misjudgment caused by guessing based on `is_dir`.
                let kind = match FileSystemEntryKind::try_from(entry.kind) {
                    ...
                    Ok(kind) => kind,
                    Err(_) => FileSystemEntryKind::Other,
                };
