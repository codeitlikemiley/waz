...
pub enum WorkspaceAction {
    ...
    /// Opens a new terminal pane in the center of the current tab and executes `ssh user@host` (exclusive to openWarp).
    /// Triggered by the Connect button in SshServerView or right-clicking "Connect" in SshManagerPanel.
    OpenSshTerminal {
        node_id: String,
        server: warp_ssh_manager::SshServerInfo,
    },
    /// Opens/closes the SSH manager view in the left panel (exclusive to openWarp).
    ToggleSshManager,
    /// Opens/closes the Skill manager view in the left panel (exclusive to openWarp).
    ToggleSkillManager,
    AddTabWithShell {
    ...
    DropTab,
    /// Toggles the left panel. In Code Mode V1 this toggles Waz Drive.
    /// In Code Mode V2 this toggles the left panel which contains both the project explorer and
    /// Waz Drive. This happens as explicit action from the user.
    ToggleLeftPanel,
    /// Toggles directly to the Waz Drive tab of the left panel in Code Mode V2
    ToggleWarpDrive,
    /// Unconditionally opens Waz Drive. This is used in the case of user lifecycle
    ...
    /// Immediately triggers an app crash when called.
    Crash,
    ...
    OpenLink(String),
    /// On WASM, opens a given URL in the desktop Waz app (if installed) or redirects to download page.
    ...
    TerminateApp,
    CloseWindow,
    /// Help the user call the Waz executable with the [`crate::args::DEBUG_DUMP_FLAG`].
    DumpDebugInfo,
    ...
    // Waz Wave 7-3: `OpenEnvironmentManagementPane` WorkspaceAction is physically deleted along with the ambient-agent UI subsystem.
    ...
    /// Install the Waz CLI command to /usr/local/bin
    #[cfg(target_os = "macos")]
    InstallCLI,
    /// Uninstall the Waz CLI command from /usr/local/bin
    #[cfg(target_os = "macos")]
    UninstallCLI,
    ...
    /// Open the Waz Launch Modal (for debugging)
    #[cfg(debug_assertions)]
    OpenWazLaunchModal,
