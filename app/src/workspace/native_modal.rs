...
/// Used to show a Waz-native modal dialog above a [`super::Workspace`]. The first button is [`ButtonVariant::Accent`].
...
impl View for NativeModal {
    ...
    fn render(&self, app: &warpui::AppContext) -> Box<dyn warpui::Element> {
        let Some(alert_dialog) = self.alert_dialog.as_ref() else {
            // Resident view + fill on demand; empty state = rendering Empty is a legal expected branch, do not warn.
