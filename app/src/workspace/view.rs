...
pub(crate) mod waz_launch_modal;
...
use crate::workspace::view::waz_launch_modal::{
    WazLaunchModal, WazLaunchModalEvent,
};
...
// Waz Wave 3-1: `AuthClient` trait is physically deleted along with server_api/auth.rs.
use crate::settings::{
...
// Waz Wave 7-3: `environments_page::EnvironmentsPage` import is physically deleted
...
// Waz: Remove SharedObjectsCreationDeniedModal (Cloud Drive quota denial modal)
...
use crate::drive::{
    DriveObjectType, DrivePanel, DrivePanelEvent, ObjectTypeAndId, WazDriveObjectSettings,
};
...
/// Height for all panel headers (tab bar, waz drive, resource center, theme chooser, etc.).
...
// We use the word "Waz" in the Update Ready button to make it obvious that the terminal is Waz.
// This can lead to free advertising when users screen-share Waz when an update is available.
...
enum SimplifiedWasmTabBarContent {
    /// Viewing a Waz Drive object (notebook, workflow, env vars, AI facts, MCP servers)
    WarpDriveObject,
...
pub struct Workspace {
    ...
    waz_launch_modal: ViewHandle<WazLaunchModal>,
    ...
    /// Notification center mailbox (dropdown popover for the Inbox button in the top-right corner of the title bar).
    /// Instantiated only when the `HOANotifications` feature flag is enabled.
    notification_mailbox_view: Option<ViewHandle<NotificationMailboxView>>,
    /// Notification toast stack (floating at the bottom-right corner). Gated on `HOANotifications` as above.
...
impl Workspace {
            ...
            ImportModalEvent::OpenTargetWithHashedId(server_id) => {
                ...
                if let Some(id) = id_to_force_expand {
                    self.open_notebook(
                        ...
                        &WazDriveObjectSettings::default(),
                        ctx,
                        true,
                    );
    ...
    pub fn new(
        ...
        let waz_launch_view = ctx.add_typed_action_view(WazLaunchModal::new);
        ctx.subscribe_to_view(&waz_launch_view, |me, _, event, ctx| {
            me.handle_waz_launch_modal_event(event, ctx);
        });
        ...
        // Show the Waz AI warm welcome iff the user hasn't dismissed it nor interacted with Waz AI before.
        ...
        // Don't automatically show the Waz AI welcome during onboarding if the block onboarding flow is being used.
        ...
        // Notification center mailbox + toast (instantiated only when the `HOANotifications` feature flag is enabled).
        ...
        // Subscribe to notification model events so the unread red dot on the Inbox button in the title bar refreshes in time.
        ...
        // Waz: Remove SharedObjectsCreationDeniedModal registration (Cloud Drive quota denial modal)
        ...
        ctx.subscribe_to_model(&OneTimeModalModel::handle(ctx), |me, model, event, ctx| {
            ...
            if *is_open {
                ...
                if model_ref.target_window_id() == Some(ctx.window_id()) {
                    if model_ref.is_waz_launch_modal_open() {
                        me.focus_waz_launch_modal(ctx);
        ...
        let mut ws = Self {
            ...
            waz_launch_modal: waz_launch_view,
            codex_modal,
            lightbox_view: None,
    ...
    fn restore_left_panel_for_tab(
        ...
        self.left_panel_view.update(ctx, |lp, ctx| {
            ...
            let active_view = match left_panel_snapshot.left_panel_displayed_tab {
                ...
                LeftPanelDisplayedTab::WazDrive => ToolPanelView::WazDrive,
    ...
    fn configure_empty_workspace(
        ...
        let open_warp_drive = if !show_warp_home {
            ...
            // If we can't start a terminal session to run the onboarding flow, show the Waz Home
            // placeholder along with Waz Drive.
            true
        };
        let initial_tab = self.active_tab_pane_group().clone();

        if open_warp_drive {
            // We open Waz Drive automatically in two cases:
            // * The user is new to Waz, and went through the overall onboarding flow
            ...
            ctx.spawn(initial_load_complete, move |me, _, ctx| {
                // New Waz users can have non-welcome objects if they were directly invited OR if
                ...
                if ObjectStoreModel::as_ref(ctx).has_non_welcome_objects() {
                    ...
                    // After opening Waz Drive, if we rendered the Waz Home placeholder panel, replace it with one of
    ...
    /// Compatible with the old server token entry point: only opens conversations that have already been restored locally.
    ...
    fn show_local_conversation_not_found_toast(&mut self, ctx: &mut ViewContext<Self>) {
        self.toast_stack.update(ctx, |view, ctx| {
            let new_toast = DismissibleToast::error(
                "Conversation is not available in local Waz history.".to_string(),
            );
    ...
    fn get_simplified_wasm_tab_bar_content(
        ...
        // Check if focused pane is a Waz Drive object
    ...
    fn toggle_ai_assistant_panel(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // If there is a modal open, it would sit above the Waz AI panel and we would end up
        // focusing the Waz AI panel _behind_ the floating modal. Instead, we opt for the normal
        // toggle behavior which will close the current modal view and then toggle Waz AI.
        ...
        // Close any other modals that could be floating on top of the Waz AI panel.
    ...
    /// Check if Waz Drive view is focused within.
    /// Routes to the appropriate Waz Drive panel.
    ...
    /// The current focusable panels are: Waz Drive, theme chooser, AI, and resource center (keyboard shortcuts page only)
    fn focus_left_panel(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // Starts from a left panel: Waz Drive
    ...
    fn focus_right_panel(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // Starts from a left panel: Waz Drive, theme chooser
        else if self.is_warp_drive_view_focused(ctx)
    ...
    fn handle_left_panel_event(&mut self, event: &LeftPanelEvent, ctx: &mut ViewContext<Self>) {
        match event {
            ...
            LeftPanelEvent::WazDrive(drive_event) => {
    ...
    /// Opens the edit pane for the given SSH node in the central area. MVP implementation: **always opens a new
    /// pane** (deduplication / find_pane is not done yet); from Phase 2 onwards, a manager singleton will be added to do
    /// deduplication and switch window focus.
    ...
    /// After clicking a file in the remote file tree, open it using the buffer-sync protocol.
    ///
    /// Both remote and local files uniformly go through [`Self::open_code`] / `CodePane` / `CodeView`:
    /// this way, remote files observe the `open_file_layout` (new tab / split pane) and multi-file
    /// grouping toggle (multiple remote files merged into the same inner tab of the code editor pane) just like local files.
    ///
    /// The buffer content is still opened via the `BufferLocation::Remote` path of `GlobalBufferModel`
    /// (sending `OpenBuffer` to the daemon, followed by `BufferEdit` / `BufferUpdatedPush` sync),
    /// and this part is completed inside `CodeView` by `LocalCodeEditorView::new_with_remote_buffer`.
    #[cfg(feature = "local_tty")]
    ...
    /// Identical to [`Self::open_remote_file`], but can carry a `line_col` (line:column jump).
    /// Used when Ctrl/Cmd+clicking a remote file path in the terminal, forwarding the line number all the way to `open_code`.
    #[cfg(feature = "local_tty")]
    ...
    /// Opens a new terminal pane in the current tab, runs the `ssh ...` command automatically, and spawns a
    /// SecretInjector to listen to PTY output, automatically injecting the secret from the keychain when the `password:` / `passphrase:` prompt appears.
    ///
    /// **Boundary for public key passwordless login**: The user configured authorized_keys on the server side, and the client
    /// handshake with the default private key succeeds -> no prompt will appear -> injector silently times out (15s) and exits,
    /// **without accidentally injecting into the shell after login**. This is a natural characteristic of SecretInjector: strict matching of
    /// line endings + one-off trigger + deadline.
    ///
    /// **shell bootstrap timing**: `execute_command_or_set_pending` puts the ssh command into
    /// the pending queue and flushes it upon the `BootstrapPrecmdDone` event — avoiding combining it
    /// with the bootstrap script and breaking the shell (tested on 2026-05-04).
    pub fn open_ssh_terminal(
        ...
        // Open a new tab (not split screen — previously using add_terminal_pane(Direction::Right) would split left/right,
        // which users didn't like). The new tab automatically becomes the active tab upon creation.
        self.add_new_session_tab_internal_with_default_session_mode_behavior(
            NewSessionSource::Tab,
            Some(window_id),
            None, /* chosen_shell */
        ...
        // 1. Synchronously read keychain (main thread is OK). auth_type determines whether to look up password or passphrase.
        ...
        //    will broadcast before spawn, and the injector won't receive it.
        ...
        // Start the command injector — wait for the shell to be ready, then automatically execute startup_command
        ...
        // su password injector — listen for the su password prompt, and automatically enter the root password
        ...
        // 3. Queue the ssh command, wait for bootstrap to complete, and then flush automatically.
        terminal_view.update(ctx, |view, ctx| {
    ...
    fn view_logs(&mut self, ctx: &mut ViewContext<Self>) {
        // Synchronously collect diagnostic info on the calling thread (version, platform, channel, execution mode, MCP/changelog path, etc.),
        // the actual zip packing is completed on the blocking thread to avoid thread-crossing when reading the `AppContext` global state.
    ...
    /// Different from `view_logs`: lets the user choose the saving location via the native system save-file dialog,
    /// then writes the log bundle directly to that location. The packaged content is identical to `view_logs`.
    ///
    /// Both success and failure are fed back through the `toast_stack`, so that views like the settings page
    /// which do not have their own toast area can also see the result.
    #[cfg(not(target_family = "wasm"))]
    fn export_logs_to_path(&mut self, ctx: &mut ViewContext<Self>) {
        use warpui::platform::SaveFilePickerConfiguration;

        // Synchronously collect extras on the calling thread (reading the `AppContext` global state). The save dialog
        // and the actual disk writing will take place in the subsequent async flow.
        let extras = Self::collect_log_bundle_extras(ctx);
        ...
        ctx.open_save_file_picker(
            move |path_opt, _me, ctx| {
                let Some(path_string) = path_opt else {
                    // User cancelled, do not disturb the user.
                    return;
                };
                ...
                ctx.spawn(
                    ...
                    |me, result, ctx| match result {
                        Ok(Ok(path)) => {
                            // i18n_embed_fl::fl! requires positional arguments to live until the macro expansion ends,
                            // hence we bind it with `let` to a local String first, then borrow its &str.
    ...
    ///   Included in the zip only if the file actually exists.
    ///
    /// Intentionally **not** packaged content (trade-offs):
    /// - `.dmp` minidump binary (could be extremely large, needs to be uploaded individually on demand);
    /// - `waz.prompt_chips.log` (contains command stdout/stderr, generated only in the debug channel, default privacy risk);
    /// - profiling products (`dhat-heap.json` / `profile.pb`, enabled only under special cargo features).
    #[cfg(not(target_family = "wasm"))]
    fn collect_log_bundle_extras(ctx: &AppContext) -> warp_logging::LogBundleExtras {
        ...
        // 1) manifest.txt: Readable diagnostic summary, the first thing to check when troubleshooting.
        // The log directory is desensitized using `home_relative_path` (replacing `$HOME` with `~` on Unix)
        // to avoid leaking the username / real home directory path when sharing the zip with troubleshooting personnel.
        let manifest = {
        ...
        // 2) Other products in the same directory: minidump service process logs and Windows updater logs for the same channel.
        if let Some(dir) = log_dir.as_ref() {
            let candidates: &[&str] = &[
                "warp-minidump.log", // Linux/Windows minidump service process
                "warp_update.log",   // Windows autoupdater (Inno Setup)
            ];
        ...
        // 3) MCP server current session stderr (`purge_on_startup: true`, exists only during runtime).
        // The path is obtained indirectly via `simple_logger::manager::resolve_log_path`: resolving a dummy filename
        // then calling `parent()` to get the namespace directory, avoiding exposing the `log_directory_path` private API.
    ...
    /// a user clicks "Update Waz" in the top right of the tab bar.
    ...
    fn check_and_trigger_onboarding(&mut self, ctx: &mut ViewContext<Self>) -> bool {
        // Waz: Remove the welcome block tutorial for agentic suggestions shown on first open. Still mark the user as
        // onboarded to avoid downstream components (such as the telemetry banner) treating them as a new user.
        if !self.auth_state.is_onboarded().unwrap_or_default() {
            AuthManager::handle(ctx).update(ctx, |auth_manager, ctx| {
                auth_manager.set_user_onboarded(ctx);
            });
    ...
    /// Opens the Waz Drive object identified by `uid` in a new pane
    ...
    pub fn open_notebook(
        ...
        settings: &WazDriveObjectSettings,
        ctx: &mut ViewContext<Self>,
        ...
        if let Some((window_id, locator)) = notebook_manager.as_ref(ctx).find_pane(source) {
            ...
            // TODO(waz-cloud-removal Phase 5): The invitee_email/source notebook invitation link has no UI exit,
            // but `WazDriveObjectSettings.invitee_email` is still passed in via the URL handler / drag-drop path.
            // When the invitee concept is retired in Phase 5, remove the field from the settings struct as well.
            let _ = settings;
            let _ = source;
        ...
        // Get notebook ID to set Waz drive index selected state
    ...
    /// Open a Waz Drive workflow in response to an intent URL.
    ...
    // Remote file browser cd: Different from local cd_to_directory, this executes the command directly instead of
    // filling it into the input box. The reason is that after switching the working directory, the remote session needs
    // to reflect it immediately in the session state, and the panel action is a clear intent triggered by the context menu,
    // so no secondary user confirmation is required.
    ...
    // Waz Wave 7-3: `open_environment_management_pane` is physically deleted along with the ambient-agent UI subsystem.
    ...
    /// Find an active session and pre-fill the input editor the Waz executable with the
    ...
    /// Install the Waz CLI by creating a symlink in /usr/local/bin
    #[cfg(target_os = "macos")]
    ...
    /// Uninstall the Waz CLI by removing the symlink from /usr/local/bin
    #[cfg(target_os = "macos")]
    ...
    pub fn open_or_toggle_warp_drive(
        ...
        // Closing all left panels will also close waz drive so we need to retrieve
        // whether waz drive was open first, and toggle based on the initial value.
        ...
        // Set selected object to None upon toggle close of Waz Drive
        if !self.current_workspace_state.is_warp_drive_open {
            self.set_selected_object(None, ctx);
            self.focus_active_tab(ctx);
        }

        // Reset focused index when opening/toggling Waz Drive open
        if self.current_workspace_state.is_warp_drive_open {
            self.reset_focused_index_in_warp_drive(true, ctx);
        }
        ...
        // Telemetry and welcome tip logic is only for when the user explicitly opens Waz Drive
        // AND waz drive wasn't open before. There are other scenarios where we open Waz Drive like:
    ...
    fn user_menu_items(&self, app: &AppContext) -> Vec<MenuItem<WorkspaceAction>> {
        // Decentralized branch: User menu no longer shows cloud account related items like account name / account CTA / Upgrade / Billing / Invite /
        // Log out, etc. Only locally available entry points (update, settings, docs, feedback, logs) are kept.
        ...
        // Decentralized branch: This place originally appended account-related items like account CTA / Upgrade / Billing / Invite / Log out,
        // which are all removed in local mode.
        items
    }
    ...
    fn build_worktree_sidecar_items(&self) -> Vec<MenuItem<NewSessionSidecarSelection>> {
        ...
        // PersistedWorkspace has been decommissioned, no longer pulling lists from "recent repositories";
        // _query is only here to preserve the variable usage semantics, actual items are always empty.
        let _ = query;
        ...
        self.new_session_sidecar_menu.update(ctx, |menu, view_ctx| {
            ...
            menu.set_pinned_footer_builder(move |app| {
                ...
                .on_click(|ctx: &mut warpui::elements::EventContext, _, _| {
                    // PersistedWorkspace has been decommissioned, no longer prompting the "Add Repository" selector here,
                    // only closing the current menu. The UI button is temporarily retained and can be adjusted later.
    ...
    fn open_repo_picker_for_tab_config_modal(
        ...
        ctx.open_file_picker(
            move |result, ctx| {
                ...
                // PersistedWorkspace has been decommissioned, no longer writing to the "recent repositories" list.
        ...
        if self.is_input_box_visible(ctx) {
            ...
            if open_workflow_id == Some(*workflow_id) {
                ...
                if let Some(workflow) = object_store_model.get_workflow(workflow_id) {
                    ...
                    self.run_cloud_workflow_in_active_input(
                        workflow.clone(),
                        WorkflowSelectionSource::WazDrive,
                        TerminalSessionFallbackBehavior::default(),
                        ctx,
                    );
                }
    ...
    pub fn snapshot(
        ...
        let warp_drive_index_width = modal_sizes.map(|ms| {
            ms.warp_drive_index_width
                .lock()
                .expect("should be able to lock waz drive resizable state handle")
                .size()
        });
    ...
    fn open_palette(
        ...
        match mode {
            ...
            PaletteMode::WazDrive => self.open_warp_drive_palette(ctx),
    ...
    /// We do not want to focus Waz Drive, instead we want to focus the editor of the open object.
    fn view_in_warp_drive(&mut self, item_id: WarpDriveItemId, ctx: &mut ViewContext<Self>) {
        self.open_left_panel(ctx);
        self.left_panel_view.update(ctx, |left_panel, ctx| {
            left_panel.handle_action(&LeftPanelAction::WazDrive, ctx);
        });
    ...
    /// This function is used when we want to view an item in Waz Drive AND focus Waz Drive.
    ...
    /// Updates the left panel's waz drive view.
    ...
    fn handle_changelog_event(&mut self, _event: &ChangelogEvent, _ctx: &mut ViewContext<Self>) {
        // Waz is a localized fork, does not rely on a private changelog service, and does not pop up a toast/resource-center after updates.
    }
    ...
    fn handle_settings_pane_event(
        ...
        match event {
            // Waz decentralized branch: `CheckForUpdate` / `WazDrive` event arms are physically deleted
            // along with the same-name variants in `SettingsViewEvent`. Manual check for updates can still be
            // triggered by `WorkspaceAction::CheckForUpdate` (`workspace:check_for_updates` binding);
            // Waz Drive can still be triggered by `WorkspaceAction::WazDrive`.
        ...
        match event {
                ...
                if let Ok(notification_data_str) = serde_json::to_string(&notification_data) {
                    ...
                    ctx.send_desktop_notification(
                        ...
                        move |workspace, notification_error, ctx| {
                            // Log unknown errors locally to facilitate troubleshooting of the notification system.
            ...
            pane_group::Event::WazDriveLink {
                ...
                match open_warp_drive_args.object_type {
                    ...
                    _ => {
                        log::warn!("Attempted to open an unsupported Waz Drive link")
                    }
                }
            }
            #[cfg(feature = "local_fs")]
            ...
            pane_group::Event::WazDriveObjectInPane(uid) => {
                ...
                let is_target_active =
                    self.left_panel_view
                        .read(ctx, |left_panel, _| match target_view {
                            ...
                            LeftPanelTargetView::WazDrive => left_panel.is_warp_drive_active(),
                        });
                ...
                if self.active_tab_pane_group().as_ref(ctx).left_panel_open && is_target_active {
                    ...
                    self.left_panel_view.update(ctx, |left_panel, ctx| {
                        let action = match target_view {
                            ...
                            LeftPanelTargetView::WazDrive => LeftPanelAction::WazDrive,
                        };
            ...
            // Waz: Ctrl/Cmd + click a remote SSH file path in the terminal, opened via the buffer-sync protocol.
            ...
            // Waz Wave 7-3: `pane_group::Event::OpenEnvironmentManagementPane` handler is physically deleted
            // along with the ambient-agent UI subsystem.
    ...
    fn update_active_session(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if let Some(terminal_handle) = pane_group_handle.as_ref(ctx).active_session_view(ctx) {
            ...
            {
                // Add the `ServerFileBrowser` guard to ensure that when this feature is disabled by
                // `ZAP_UNSTABLE_FEATURES`, it does not secretly pull the remote directory after the SSH session is activated, avoiding any related background activity.
                if is_remote
    ...
    fn handle_warp_drive_event(&mut self, event: &DrivePanelEvent, ctx: &mut ViewContext<Self>) {
        match event {
            ...
            DrivePanelEvent::OpenSearch => {
                self.open_palette_action(
                    PaletteMode::WazDrive,
                    PaletteSource::WazDrive,
                    None,
                    ctx,
                );
            }
            DrivePanelEvent::OpenNotebook(source) => {
                self.open_notebook(source, &WazDriveObjectSettings::default(), ctx, true)
            }
            ...
            DrivePanelEvent::OpenAIFactCollection => {
                ...
                send_telemetry_from_ctx!(
                    TelemetryEvent::KnowledgePaneOpened {
                        entrypoint: KnowledgePaneEntrypoint::WazDrive,
                    },
                    ctx
                );
            }
            DrivePanelEvent::OpenMCPServerCollection => {
                ...
                send_telemetry_from_ctx!(
                    TelemetryEvent::MCPServerCollectionPaneOpened {
                        entrypoint: MCPServerCollectionPaneEntrypoint::WazDrive,
                    },
                    ctx
                );
            }
            ...
            DrivePanelEvent::OpenSharedObjectsCreationDeniedModal(_, _) => {
                // Waz: Cloud Drive quota denial modal has been deleted, event is ignored directly
            }
    ...
    fn handle_update_manager_event(
            ...
            && matches!(result.operation, ObjectOperation::Create { .. })
        {
            ...
            {
                // TODO(waz-cloud-removal Phase 5): drive sharing onboarding
                // block is retired; `created_object` is still the cloud_object creation result,
                // and the StoredObject model itself will be retired in a subsequent phase.
    ...
    fn set_selected_object(&mut self, id: Option<WarpDriveItemId>, ctx: &mut ViewContext<Self>) {
        // Set Waz drive index selected state
        self.update_warp_drive_view(ctx, |drive_panel, ctx| {
    ...
    fn handle_ai_assistant_panel_event(
        ...
        match event {
            ...
            AIAssistantPanelEvent::PasteInTerminalInput(code) => {
                ...
                let workflow = Workflow::new("Command from Waz AI", command)
    ...
    fn handle_waz_launch_modal_event(
        &mut self,
        event: &WazLaunchModalEvent,
        ctx: &mut ViewContext<Self>,
    ) {
        match event {
            WazLaunchModalEvent::Close => {
                OneTimeModalModel::handle(ctx).update(ctx, |model, ctx| {
                    model.mark_waz_launch_modal_dismissed(ctx);
                });
                self.focus_active_tab(ctx);
                ctx.notify();
            }
    ...
    // Waz: Deleted open_shared_objects_creation_denied_modal (cloud Drive quota denial modal)
        ...
        let owner = match space {
            Space::Team { team_uid } => {
                if !UserWorkspaces::has_capacity_for_shared_workflows(team_uid, ctx, 1) {
                    // Waz: Cloud quota denial modal has been deleted, return directly
                    return;
                }
    ...
    fn render_ai_assistant_warm_welcome(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let body = appearance
            .ui_builder()
            .wrappable_text(
                "Ask Waz AI to explain errors, suggest commands or write scripts.".to_owned(),
                true,
            )
            ...
            if vertical_tabs_active {
                ...
                let tooltip = if self.left_panel_views.len() <= 1 {
                    match self
                        ...
                        .first()
                        .copied()
                        .unwrap_or(ToolPanelView::WazDrive)
                    {
                        ...
                        ToolPanelView::WazDrive => crate::t!("workspace-left-panel-warp-drive"),
    ...
    fn render_tab_bar_contents(
        ...
        // Simplified mode for viewing Waz Drive objects, shared sessions, or conversation transcripts on WASM
        #[cfg(target_family = "wasm")]
        if let Some(content_type) = self.get_simplified_wasm_tab_bar_content(ctx) {
            ...
            // Right: Info button + run history button (for agent sessions) + "Open in Waz" button
            ...
            // Hide "Open in Waz" button on mobile devices
    ...
    fn add_configurable_right_side_tab_bar_controls(
        ...
        if FeatureFlag::AvatarInTabBar.is_enabled() {
            ...
            // Decentralized branch: No longer rendering the Waz Essentials (lightbulb) button, only keeping the settings gear.
            target.add_child(
    ...
    fn banner_fields(&self, app: &AppContext) -> Option<WorkspaceBannerFields> {
        ...
        // banners.
        // Decentralized branch: reauth banner has been deleted.
    ...
    // Decentralized branch: `render_reauth_banner_element` has been deleted.
    ...
    fn process_updated_sync_state(&self, ctx: &mut ViewContext<Self>) {
        ...
        // If there is no active terminal (like when all Waz windows are
    ...
    fn focus_waz_launch_modal(&mut self, ctx: &mut ViewContext<Self>) {
        ctx.focus(&self.waz_launch_modal);
    }
    ...
    fn compute_left_panel_views(ctx: &AppContext) -> Vec<ToolPanelView> {
        ...
        if WarpDriveSettings::is_warp_drive_enabled(ctx) {
            views.push(ToolPanelView::WazDrive);
        }
        // Unique to openWarp: SSH manager, no feature flag, shown by default always.
        ...
        // Unique to openWarp: Skill manager, no feature flag, shown by default in local_fs builds.
        if cfg!(feature = "local_fs") {
    ...
    /// Opens a given URL in the desktop Waz app if installed, or redirects to download page.
    #[cfg(target_family = "wasm")]
    fn open_link_on_desktop(&mut self, url: &Url, ctx: &mut ViewContext<Self>) {
        ...
        if !is_app_installed {
            ...
            let toast_message = format!(
                "Have Waz installed but redirecting to download page?\nEnable Local Network Access for the Waz web launcher in your browser."
            );
            self.toast_stack.update(ctx, |toast_stack, ctx| {
...
impl TypedActionView for Workspace {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            // Decentralized branch: `LogOut` has been deleted.
            ...
            // Decentralized branch: `ShowUpgrade` / `ShowReferralSettingsPage` have been deleted.
            ...
            WazDrive => {
                if WarpDriveSettings::is_warp_drive_enabled(ctx) {
                    self.open_left_panel_view(&LeftPanelAction::WazDrive, ctx);
                }
            }
            ToggleLeftPanel => {
                let active_pane_group = self.active_tab_pane_group().clone();
                ...
                if !was_open && is_open {
                    ...
                    if file_tree_active {
                        ...
                        // Tools panel opened with Waz Drive as the active view
                        send_telemetry_from_ctx!(
                            TelemetryEvent::WarpDriveOpened {
            ...
            // Decentralized branch: `Reauth` has been deleted.
            // Decentralized branch: Cloud account registration / login actions have been deleted.
            ...
            // Decentralized branch: `AttemptLoginGatedAIUpgrade` has been deleted.
            #[cfg(all(enable_crash_recovery, target_os = "linux"))]
            ...
            // Waz Wave 7-3: `OpenEnvironmentManagementPane` WorkspaceAction handler is physically deleted
            // along with the ambient-agent UI subsystem.
            ...
            #[cfg(debug_assertions)]
            OpenWazLaunchModal => {
                // Force open the Waz launch modal for debugging
                OneTimeModalModel::handle(ctx).update(ctx, |model, ctx| {
                    model.force_open_waz_launch_modal(ctx);
                });
                ctx.notify();
            }
            #[cfg(debug_assertions)]
            ResetWazLaunchModalState => {
                // Reset the Waz launch modal dismissed state for debugging
                let old_value = *GeneralSettings::as_ref(ctx)
                    .did_check_to_trigger_waz_launch_modal
                    .value();
                GeneralSettings::handle(ctx).update(ctx, |settings, ctx| {
                    if let Err(e) = settings
                        ...
                        .set_value(false, ctx)
                    {
                        log::warn!("Failed to reset Waz launch modal dismissed setting: {e}");
                    }
                });
                let new_value = *GeneralSettings::as_ref(ctx)
                    ...
                    .value();
                log::info!(
                    "Waz launch modal state: old={}, new={}, feature_flag_enabled={}",
                    old_value,
                    new_value,
                    FeatureFlag::WazLaunchModal.is_enabled()
                );
            }
            #[cfg(debug_assertions)]
            ...
            ToggleWarpDrive => {
                if WarpDriveSettings::is_warp_drive_enabled(ctx) {
                    let is_showing =
                        self.left_panel_view.as_ref(ctx).active_view() == ToolPanelView::WazDrive;
                    self.toggle_left_panel_view(&LeftPanelAction::WazDrive, is_showing, ctx);
                }
            }
            ToggleSshManager => {
                let is_showing =
...
impl View for Workspace {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // For WASM simplified tab bar views (Waz Drive objects, shared sessions, conversation transcripts),
        ...
        if should_show_modal && one_time_modal_model.is_waz_launch_modal_open() {
            stack.add_child(ChildView::new(&self.waz_launch_modal).finish());
        }
        ...
        // Notification center overlay: mailbox dropdown panel + (when mailbox is not open) toast stack.
        if FeatureFlag::HOANotifications.is_enabled()
