...
// Waz (localization, Phase 5): `PreferencesSyncer` has been physically removed.
...
fn initialize_app(app: &mut App) {
    ...
    // Waz (localization, Phase 5): `PreferencesSyncer` has been physically removed and the test singleton is no longer needed.
...
fn test_open_or_toggle_warp_drive() {
    App::test((), |mut app| async move {
        ...
        let workspace = mock_workspace(&mut app);
        workspace.update(&mut app, |workspace, ctx| {
            // First, unconditionally open Waz Drive as a system action. WD should be open and welcome tips should not have opening waz drive.
            ...
            assert!(
                !workspace
                    ...
                    .as_ref(ctx)
                    .features_used
                    .contains(&Tip::Action(TipAction::WazDrive)),
                "Waz drive welcome tip should not be completed"
            );

            // Next, toggle waz drive as a user action. WD should be closed and tip should not be filled out.
            workspace.open_or_toggle_warp_drive(
                true, /* toggle */
            ...
            // Finally, toggle waz drive again as a user action. WD should be open and tip filled out.
            workspace.open_or_toggle_warp_drive(
                true, /* toggle */
                true, /* explicit_user_action */
                ctx,
    ...
    App::test((), |mut app| async move {
        ...
        workspace.update(&mut app, |view, ctx| {
            assert!(
                ...
                "Expected Waz Drive panel to be focused"
            );
        });
...
// The agent_management_view field and the entire agent setup guide functionality have been removed in Phase 2c.
