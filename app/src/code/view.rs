...
#[derive(Clone)]
pub struct TabData {
    /// Local file path. Only local files have values — remote files have no local path.
    /// Used only for local-specific logic such as display / repo detection / rename.
    path: Option<PathBuf>,
    /// The unified identity of the tab (local path or remote `RemotePath`), used for tab deduplication / focusing.
    /// For newly created files (no backing), it is `None`.
...
impl TabData {
    ...
    /// The unified identity of the tab (local path or remote `RemotePath`).
    pub fn location(&self) -> Option<&BufferLocation> {
        self.location.as_ref()
    }

    /// The file name displayed on the tab / header.
...
pub struct CodeView {
    ...
    /// Callback waiting for remote SaveBuffer to complete asynchronously, key is `EntityId` of `LocalCodeEditorView`.
    /// The remote buffer's `save_local` returns `Ok(())` immediately, but actual daemon disk writing awaits the asynchronous
    /// `LocalCodeEditorEvent::FileSaved` / `FailedToSave` events; the callback for "close tab" must be deferred until
    /// this event arrives, otherwise the tab would already be closed upon save failure and the user wouldn't see the error.
...
impl CodeView {
    ...
    /// Constructs an editor view bound to a remote buffer. The structure is identical to
    /// [`Self::construct_shared_buffer_editor_from_path`], but the buffer is opened via the
    /// `GlobalBufferModel`'s `BufferLocation::Remote` path, with its content asynchronously populated by the buffer-sync protocol.
    ...
    fn construct_shared_buffer_editor_from_remote(
        ...
        ctx.add_typed_action_view(|ctx| {
            ...
            // The remote buffer is empty when created, and the initial content arrives asynchronously via the buffer-sync protocol.
            // If editable during the loading window, user input would be overwritten and lost by replace_all() of OpenBufferResponse
            // — lock it as Selectable first (scrollable/selectable but not editable), and restore to Editable once `FileLoaded` arrives.
            editor.editor().update(ctx, |code_editor, ctx| {
                code_editor.set_interaction_state(InteractionState::Selectable, ctx);
            });

            // Consistent with local shared-buffer editors: add footer / save conflict UI.
            editor.add_footer(ctx);
            editor
        })
    }
    ...
    fn build_tab_data(
        ...
        // Local path (only present in the `Local` variant), used for display / repo detection / rename.
        ...
        ctx.subscribe_to_view(&code_editor, |me, emitter, event, ctx| match event {
            LocalCodeEditorEvent::FileLoaded => {
                // Once the initial content of the remote buffer arrives, release the edit lock applied during loading.
                // Local files are editable upon creation and never locked, so this is a no-op here.
            ...
            LocalCodeEditorEvent::FailedToLoad { error: err } => {
                ...
                // Upon load failure, close this erroneous file tab — avoiding leaving an unloadable empty pane/tab.
                // Locate which tab failed through the event emitter (LocalCodeEditorView handle),
                // and reuse the user's manual tab closing path `remove_tab_data_index`:
                // If one of multiple tabs in the tab group fails, only remove that tab; if it is the only tab,
                // `update_tab_bar_state` will emit `PaneEvent::Close` after the tab group is cleared to close the entire pane.
            ...
            LocalCodeEditorEvent::FileSaved => {
                ...
                ctx.notify();
                // Remote async Save completed: trigger the cached close-tab callback.
            ...
            LocalCodeEditorEvent::FailedToSave { error: err } => {
                ...
                // Remote async Save failed: the tab must remain, instructing the callback to go to the Failed branch.
                // The Save branch of `remove_tab_with_intent` only closes the tab when `outcome != Canceled`
                // — here we provide Failed, which will keep the tab according to strategy (i) below.
                if let Some(cb) = me.pending_remote_save_callbacks.remove(&emitter.id()) {
    ...
    fn focus_existing_tab_if_present(
        ...
        // A `None` location (new file) is never deduplicated against existing tabs.
    ...
    fn open_new_tab_for_location(
        ...
        if let (Some(location), Some(tab)) = (location, self.tab_group.get(active_tab_index)) {
            // `FileOpened` only makes sense for local files (repo detection/OpenedFilesModel).
            if let BufferLocation::Local(file_path) = &location {
                ctx.emit(CodeViewEvent::FileOpened {
    ...
    fn set_title(&self, _unsaved_changes: bool, ctx: &mut ViewContext<Self>) {
        ...
        // The title prefers the local path (full path). When a remote file has no local path, it falls back to
        // the display name derived from its `location`, avoiding displaying remote tabs as "Untitled".
    ...
    fn save_local(
        ...
        // The remote buffer's `save_local` only triggers the `SaveBuffer` protocol message, and the actual
        // daemon disk writing awaits the `LocalCodeEditorEvent::FileSaved` / `FailedToSave` asynchronous events.
        // Here we record the editor entity id and "whether remote", caching the callback as needed in the `Ok` branch.
        ...
        match result {
            ...
            Ok(()) => match (remote_save_target, callback) {
                (Some(editor_id), Some(cb)) => {
                    // Remote: cache the callback, to be triggered when FileSaved / FailedToSave events arrive.
    ...
    pub fn cleanup_all_tabs(&mut self, ctx: &mut ViewContext<Self>) {
        // Consistent with `remove_tab_data_index_inner`: actively close all buffers to avoid
        // `GlobalBufferModel` retaining stale buffers which could cause the next reopen to show unsaved memory content,
        // and to let the daemon release the remote buffer memory.
    ...
    fn remove_tab_with_confirmation(
        ...
        if let Some(tab) = self.tab_at(index) {
            // Derive the file name using `location`: remote files do not have a local `path`, otherwise the file name would be lost.
    ...
    /// Identical to [`Self::remove_tab_data_index`], but **does not actively close** the buffer.
    /// Used for dragging across panes/windows: the tab is just being moved, and the buffer state (as well as the remote buffer
    /// memory on the daemon side) must be retained for reuse by the new pane.
    ...
    fn remove_tab_data_index_inner(
        ...
        // Actively close the buffer: for tab closing scenarios, we must actively clean up the `InternalBufferState`
        // inside `GlobalBufferModel` (`remove_deallocated_buffers` only clears `WeakHandle`s that have already been dropped,
        // but at this point `TabData` still holds a strong indirect reference to the buffer). Otherwise,
        // reopening the remote buffer next time would reuse the stale state containing unsaved edits, creating the false
        // impression of "appears saved". See `GlobalBufferModel::close_buffer` for details.
        if close_buffer {
            ...
            Some(PendingSaveIntent::Save) => {
                self.save_local(
                    ...
                    Some(Box::new(move |outcome, me, ctx| {
                        // Only close the tab when saving is successful. Remote saving backfills
                        // `SaveOutcome::Failed` via async events, in which case the tab must remain, letting the user see
                        // the toast error to retry or explicitly Discard. Canceled is also not closed.
        ...
        match intent {
            Some(PendingSaveIntent::Save) => {
                self.save_local(
                    ...
                    Some(Box::new(move |outcome, me, ctx| {
                        // Consistent with `remove_tab_with_intent`: only advance to the next one upon success.
                        // Failures/cancellations stop at the current tab, leaving the toast for the user to decide.
                        if outcome == SaveOutcome::Succeeded {
    ...
    fn close_saved_tabs(&mut self, ctx: &mut ViewContext<Self>) {
        // Actively close the removed buffers (same as `cleanup_all_tabs`).
        ...
        for (i, tab_data) in source_code_view.tab_group.iter().enumerate() {
            if let Some(location) = tab_data.location() {
                if let Some(&index) = existing_locations_to_idx.get(location) {
                    ...
                    // `existing_locations_to_idx` misses tabs without location, so the length of tab_group is required.
                    if i == source_code_view.active_tab_index() {
