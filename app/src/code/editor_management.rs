...
pub enum CodeSource {
    ...
    /// Remote file opened by clicking from the remote file tree (unique to openWarp). The content is synchronized
    /// with the SSH daemon via the buffer-sync protocol. There is no corresponding local path, so the tab identity
    /// is identified using [`RemotePath`]. Panes are not persisted — the remote buffer depends on an active SSH connection.
    RemoteFileTree {
...
impl CodeSource {
    ...
    /// The local file path. **Does not include remote files** — remote files do not have a local path; use
    /// [`Self::location`] to get the unified identity.
    ...
    /// The unified identity of the file corresponding to this source (local path or remote `RemotePath`).
    ///
    /// Both local and remote files undergo the multi-file grouping logic of `CodeView`. Tab deduplication and focusing
    /// both use [`BufferLocation`] as the key.
    ...
    /// be restored. `RemoteFileTree` relies on an active SSH connection and is likewise not restored.
...
impl CodeManager {
    ...
    /// the given pane group. `location` uniformly covers both local and remote files.
