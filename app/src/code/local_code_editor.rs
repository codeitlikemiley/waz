...
//
// After the LSP full stack was retired, this file no longer hosts any logic related to LSP / hover / goto-definition /
// find-references / diagnostics decoration; it only retains local capabilities such as file load/save, diff accept/reject,
// selection context tooltips, version conflict banners, and the TabConfig footer.
use std::{
    ops::Range,
    path::{Path, PathBuf},
    rc::Rc,
...
pub enum LocalCodeEditorEvent {
    ...
    LayoutInvalidated,
    /// Emitted and propagated to the upper layer for processing when "/update-tab-config" is clicked on the TabConfig footer.
    RunTabConfigSkill {
        path: PathBuf,
    },
...
enum LoadedFileMetadata {
    ...
    /// Remote buffer: the file resides on an SSH host, synchronized via the buffer-sync protocol,
    /// and has no corresponding local path.
...
impl LocalCodeEditorView {
    pub fn new(
        ...
        ctx.subscribe_to_view(&editor, |me, _, event, ctx| match event {
            ...
            CodeEditorEvent::MouseHovered { .. } => {
                // After LSP is retired, mouse hovers no longer trigger hover/goto-definition; the event subscription is retained but not processed.
            }
    ...
    fn perform_save(&mut self, file_id: FileId, ctx: &mut ViewContext<Self>) {
        ...
        // Remote SSH file: write to disk via the buffer-sync `SaveBuffer` protocol. The local
        // `FileModel` path below cannot be used — Remote buffers have no local path and would yield `NoFilePath`.
        // User edits have already been synchronized to the daemon in real-time via `BufferEdit`, so this only triggers the daemon to write to disk.
    ...
    ///
    /// Opens the remote file via [`GlobalBufferModel::open`] with [`BufferLocation::Remote`],
    /// and its content is asynchronously populated by the buffer-sync protocol. Language detection reuses the extension of the remote path.
    #[cfg(feature = "local_tty")]
    ...
    where
        T: FnOnce(BufferState, &mut ViewContext<Self>) -> ViewHandle<CodeEditorView>,
    {
        // The remote path is used for language detection (extension).
    ...
    /// After LSP is retired, normal source files no longer render footers.
    ...
    fn subscribe_to_global_buffer_events(file_id: FileId, ctx: &mut ViewContext<Self>) {
        ...
        ctx.subscribe_to_model(&GlobalBufferModel::handle(ctx), move |me, _, event, ctx| {
            ...
            match event {
                ...
                // and the local editor view does not care about them.
    ...
    pub fn save_local(&mut self, ctx: &mut ViewContext<Self>) -> Result<(), ImmediateSaveError> {
        ...
        // After LSP is retired, LSP formatting is no longer called before saving.
    ...
    pub fn file_path(&self) -> Option<&Path> {
        match self.metadata.as_ref()? {
            ...
            // Remote files do not have local paths.
