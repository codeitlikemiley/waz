...
// Orphaned code left after upstream Waz pruning is temporarily retained, globally suppressing dead_code warnings.
#![allow(dead_code)]
...
// Waz (localization, Phase 4): `ServerVoiceTranscriber` was originally used for default `VoiceTranscriber` injection, but now uses `VoiceTranscriber::disabled()`. The import with the same name is temporarily kept.
...
/// Launch mode for how to start up Waz.
...
impl LaunchMode {
    ...
    /// Returns `true` if Waz should run headlessly, without a visible UI.
...
pub fn run() -> Result<()> {
    // POSIX locale fallback: when LANG/LC_* are all unset, provide a reasonable UTF-8 default value for C/Rust libraries
    // (chrono numeric formatting, libc strftime, etc.) that depend on these environment variables. Skip on Windows on purpose
    // — the Windows API (`GetUserPreferredUILanguages`) is the true source of UI locale; forcing `LANG=en_US.UTF-8` here would
    // make `DesktopLanguageRequester` return en regardless of what UI language the user chooses, thereby breaking the CJK Han
    // glyph fallback (the Japanese UI would instead receive Simplified Chinese glyphs).
    // On macOS, if the Bundle is opened from the desktop, it also lacks environment variables, but if `LANG` is set,
    // its `DesktopLanguageRequester` will return that value directly without querying the system, so it needs to be skipped as well.
    ...
    // i18n must be initialized before any `t!()` call in the UI; first use the system locale, which will be overridden by
    // LanguageSettings after settings are loaded. OnceLock is reentrancy-safe.
    i18n::init(None);
...
fn run_internal(mut launch_mode: LaunchMode) -> Result<()> {
    ...
    // SQLite prewarming: start init_db() (connection + migration) in a background thread before app_builder.run() is called,
    // allowing SQLite initialization to run concurrently with subsequent winit / wgpu initialization.
    // When the main thread reaches persistence::initialize, it will take over the prewarmed connection.
    // Only required for LaunchMode::App (CLI / Worker / Test take other paths and do not call persistence::initialize in initialize_app).
    ...
    // Collect run_internal() errors that occurred before app initialization, and write logs after local crash reporting is initialized.
    #[cfg_attr(
        not(all(
    ...
    if let LaunchMode::App { .. } = launch_mode {
        match app_services::linux::pass_startup_args_to_existing_instance(
            ...
            // we only want to run a single instance of Waz at a time.
            Ok(_) => std::process::exit(0),
            // If Waz isn't already running, we're good to go.
            ...
            // state where Waz refuses to run even a first instance.
            Err(err) => {
    ...
    // Sets up a Job Object that we associate with the Waz process to handle
...
fn initialize_app(
    ...
    // Warning: errors occurring before crash_reporting::init can only be written to local logs.
    // Only crash-reporting dependencies should be initialized here; other task failures should be written to pre_init_errors.
    ...
    // Waz: preserve AuthStateProvider singleton only for legacy call points to read local placeholder user state.
    ctx.add_singleton_model(|_ctx| AuthStateProvider::new(auth_state.clone()));

    // Waz Wave 3-1: AuthManager has been localized into a stub, no longer injecting server_api / auth_client.
    ...
    // The SSH manager opens its own write connection outside the main write thread (WAL + busy_timeout ensures safety).
    // The path must be set after persistence::initialize completes migration, otherwise the first SshManager
    // operation might hit missing-table.
    ...
    // The API key of custom Agent Providers is saved to secure storage by an independent singleton,
    ...
    // Issue #72: The Basic Auth password of the global HTTP proxy goes through the OS key store.
    // Reapply immediately after registration to cover the global slot placeholder set with an empty string during the settings::init phase with the real password.
    ...
    // Subscribe to password changes (upon UI write) to sync and re-push to the global slot.
    ...
    // Cloud Sync Token goes through the OS key store, not saved to TOML.
    ...
    if FeatureFlag::Autoupdate.is_enabled() {
        // Original: Synchronously call remove_old_executable() to clean up the old executable left by the last auto-update.
        // Changed: Defer to background_executor. Among cross-platform paths, only macOS actually needs to do work
        // (Linux/Windows are noops), and on macOS it is just fs::remove_dir_all, which has zero dependency with the main thread
        // logic. Failures originally only logged an error, so running in the background won't lose information.
        ctx.background_executor()
            .spawn(async {
    ...
    // Waz Wave 6-1: The `remote_server::wire_auth_token_rotation(ctx)` call has been physically deleted along with the server API token rotation event and the `wire_auth_token_rotation` function itself.
    ...
    // Rewrite recognized Waz web URLs (sessions, Drive, settings, home) into local
    ...
    if user_is_logged_in {
        // Waz's local auth facade has already loaded the identity snapshot during `AuthState::initialize`.
        // No extra cloud token refresh / auth refresh is triggered during startup.
        ...
        ctx.on_first_frame_drawn(move |ctx| {
            let timing_data = IntervalTimer::handle(ctx).update(ctx, |timer, _| {
                ...
                // Local optimization hook: print the full startup timing chart to stderr when WARP_STARTUP_TRACE=1.
        ...
        // Logged-out users also need to be able to view startup timing (mostly in BYOP scenarios), so print
        // the WARP_STARTUP_TRACE chart once after the first frame. No telemetry is sent, logic is not affected.
        ctx.on_first_frame_drawn(move |ctx| {
    ...
    // Waz Wave 7-2: `settings_view::update_environment_form::init` has been physically deleted along with the cloud ambient agent core subsystem.
    ...
    ctx.add_singleton_model(|ctx| {
        ...
        // Client app: subscribe to the buffer push events of RemoteServerManager. The daemon does not do
        // this step (the daemon does not register RemoteServerManager), so this block cannot be put into
        // `GlobalBufferModel::new`. RemoteServerManager has been registered beforehand.
        #[cfg(feature = "local_tty")]
    ...
    ctx.add_singleton_model(|_| {
        // Waz (localization, Phase 4): the original default injection of `ServerVoiceTranscriber` went to cloud-based Wispr STT.
        // Under the localization scenario, cloud voice transcription is unavailable, so we change it to `disabled()` to make the upper
        // `transcriber()` return None. The voice input UI then only collects audio without transcribing (to be filled when local STT is integrated later).
    ...
    // Waz (Wave 4): After the entire SyncQueue deletion, there is no longer `unsynced_actions` /
    // `objects_with_pending_changes` tracking; local write is equivalent to "completed".
    let _ = (&object_store_model, &object_actions);
    // Keep `ObjectTypeAndId` import for other modules in the crate to access via `crate::` path.
    ...
    }
    {
        ...
        // Clean up unconvertible persisted conversations from SQLite to avoid repeated attempts and logging warnings on every startup
    ...
    // Notifications center singleton model: must be registered after BlocklistAIHistoryModel
    // and CLIAgentSessionsModel because it subscribes to these two models during construction.
    ...
    // Waz: UpdateManager is only responsible for the in-memory/SQLite synchronization of local cloud objects, no longer injecting the cloud client.
    ...
    // Waz (localization, Phase 5): `PreferencesSyncer` has been physically deleted. The original syncer was responsible for two-way synchronization between local settings.toml and cloud preferences; under the localization scenario, only local TOML loading is retained.
    ...
    // Waz (localization, Phase 2d-4a-1): the original `Listener` singleton was responsible for the cloud-side cloud_objects RTC WebSocket.
    // After 2b-1, `start_listener` was already a no-op, and in this version the entire file along with the singleton injection is physically deleted.
...
fn app_callbacks(is_integration_test: bool) -> warpui::platform::AppCallbacks {
    warpui::platform::AppCallbacks {
        ...
        on_will_terminate: Some(Box::new(move |ctx| {
            ...
            // autoupdate, to ensure we're not running any extra Waz processes
        ...
        on_window_moved: Some(Box::new(move |ctx| {
            // During startup, winit triggers move/resize several times sequentially; save_app during this phase is meaningless and slows down startup.
...
pub fn enabled_features() -> HashSet<FeatureFlag> {
    ...
    // SSH remote-server: enabled via RELEASE_FLAGS in release bundle, but dev source builds
    // (`cargo run`) are not release bundles, so this flag would remain disabled — thus SSH sessions
    // would always fall back to the legacy path, the remote-server transport would not be activated, and the
    // dev mode auto-build and binary upload (see ssh_transport.rs) would have no chance to trigger. Explicitly
    // enabled here in debug builds to ensure remote file opening / buffer-sync can be debugged during
    // development. Windows does not support remote-server binaries for now, excluded to remain consistent
    // with RELEASE_FLAGS cfg.
    #[cfg(all(debug_assertions, not(windows)))]
    flags.insert(FeatureFlag::SshRemoteServer);
    #[cfg(all(debug_assertions, not(windows)))]
    flags.insert(FeatureFlag::ServerFileBrowser);

    // Issue #72: HTTP proxy settings page. Does not go through channel checks, enabled by default in all channels
    // including waz, as a basic capability for enterprise VPN / corporate proxy scenarios.
    ...
    let extra_flags: &[FeatureFlag] = &[
        ...
        // Waz Wave 7-2: `CloudEnvironments` FeatureFlag has been physically deleted along with the cloud ambient agent core subsystem — `warp environment` subcommand + `--environment` argument are taken offline in sync.
        ...
        // Physically deleted - `warp environment` subcommand + `--environment` argument are taken offline in sync.
        ...
        // Waz (localization): the cloud entrance for ambient agent / agent management view has been physically taken offline.
        // BYOP agent running locally does not rely on these entrances.
        ...
        FeatureFlag::WazNewSettingsModes,
    ...
    // Unstable feature toggle: globally enable features that are not yet officially released in release builds
    // through the `ZAP_UNSTABLE_FEATURES` environment variable. The value is a comma-separated list of unstable
    // feature names (snake_case), or `all`/`*` to enable all at once. The dev build already automatically
    // enables all current unstable features in the debug_assertions path, so this primarily serves release users.
...
/// `ZAP_UNSTABLE_FEATURES` accepted unstable feature names -> `FeatureFlag` mapping.
/// Features registered here are hidden by default in release builds and will only appear after the
/// corresponding token is set; dev builds go through the debug_assertions branch and are enabled by
/// default, without requiring this variable.
