...
/// Warm up SQLite connection + run migration on background thread. Should be in `app_builder.run`
/// before calling so that SQLite initialization can occur concurrently with winit/wgpu initialization,
/// This saves ~70–90ms on cold start. Call idempotent multiple times. If not called, it will fallback to the synchronization path.
#[cfg_attr(not(feature = "local_fs"), allow(dead_code))]
    ...
    pub fn terminate(&mut self) {
        if let Some(handle) = self.thread_handle.take() {
            ...
            if handle.join().is_err() {
                // When crash reporting is enabled, the panic hook has been written to the local log, and thread-level context is added here.
