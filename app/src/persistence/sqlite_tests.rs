...
#[cfg(target_os = "macos")]
#[test]
fn test_migrate_waz_app_group_sqlite_copies_newer_legacy_files() {
    use super::migrate_waz_app_group_sqlite_if_needed;
    ...
    migrate_waz_app_group_sqlite_if_needed(&target_db, &legacy_dir)
    ...
    assert!(state_dir
        .join(".waz-app-group-sqlite-migrated")
        .exists());
}

#[cfg(target_os = "macos")]
#[test]
fn test_migrate_waz_app_group_sqlite_copies_when_legacy_wal_is_newer() {
    use super::migrate_waz_app_group_sqlite_if_needed;
...
#[cfg(target_os = "macos")]
#[test]
fn test_migrate_waz_app_group_sqlite_marker_skips_copy() {
    use super::migrate_waz_app_group_sqlite_if_needed;
    ...
    fs::write(&target_db, "target-db").expect("target db should be written");
    fs::write(
        state_dir.join(".waz-app-group-sqlite-migrated"),
        "migrated\n",
    )
