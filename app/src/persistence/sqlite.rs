...
use crate::drive::WazDriveObjectSettings;
...
const ZAP_APP_GROUP_SQLITE_MIGRATION_MARKER: &str = ".waz-app-group-sqlite-migrated";
#[cfg(target_os = "macos")]
const WARP_APP_GROUP_ID: &str = "2BBY89MBSN.dev.warp";

/// Callback used when deleting a local stored object. The input parameter is the id of the object to be deleted.
/// The incoming conn has started a transaction.
...
/// SQLite connections warmed up in the background. After `prewarm_db_in_background` is started,
/// The background thread will save the `init_db()` result here; `initialize` will take it from here.
///
/// Type description:
/// - The outer layer `OnceLock` indicates "whether preheating has been started".
/// - The inner layer `Mutex<PrewarmState>` is used for the main thread to obtain the results and the background thread to store the results.
static PREWARMED_DB: OnceLock<Mutex<PrewarmState>> = OnceLock::new();

/// Warm up state machine. `Pending` -> (`Done` | `Joining`). `Joining` represents the main thread
/// The handle was taken but the background thread has not yet written to Done - the background thread should still be able to write.
/// `Taken` indicates that the main thread has taken away the result.
enum PrewarmState {
    Pending(std::thread::JoinHandle<()>),
    /// The main thread took the handle and went to join, but the background thread has not yet written the result.
    /// The background thread will change this status to Done.
    Joining,
    Done(Result<SqliteConnection>),
    Taken,
}

/// Warm up SQLite connections + perform migrations on a background thread. Should be in `app_builder.run`
/// Initiated before the call so that SQLite initialization can be combined with subsequent winit/wgpu initialization
/// Performed concurrently, ~70–90ms can be saved on cold start.
///
/// If this function is not called, `initialize` will fallback to the synchronization path (original behavior).
/// Multiple calls are idempotent - OnceLock guarantees that the background thread will only be started once.
pub fn prewarm_db_in_background() {
    // OnceLock::get_or_init will only jump to the closure once under multiple callers - background thread
    // Will only be created once. If spawn fails, the Failed status will be saved (unlikely).
            ...
            .spawn(|| {
                ...
                if let Some(cell) = PREWARMED_DB.get() {
                    if let Ok(mut guard) = cell.lock() {
                        // Convert to Done. Both Pending and Joining states require writing results.
                        // If the main thread has Taken (shouldn't happen), drop the connection.
                        match *guard {
...
/// If the background warm-up has been started, get the result (you may have to wait for the background thread to join).
/// Returning `None` means that preheating has never been initiated and the caller takes the synchronous path.
fn take_prewarmed_db() -> Option<Result<SqliteConnection>> {
    let cell = PREWARMED_DB.get()?;
    let take_start = Instant::now();

    // First take out the join handle (if it is still Pending), convert it to Joining, and then get the result after the join is completed.
    // This two-stage design prevents the main thread from holding the lock and ensures that the background thread can write the results.
    let handle = {
        let mut guard = match cell.lock() {
            Ok(g) => g,
            // Lock poisoning = a previous panic; take the synchronous path.
            Err(_) => {
                log::warn!("SQLite prewarm: mutex poisoned, falling back to sync");
                return None;
            }
        };
        // Let’s look at the status first: Done gets the result directly; Pending gets the handle and then joins.
        match std::mem::replace(&mut *guard, PrewarmState::Taken) {
            PrewarmState::Pending(h) => {
                // Transition to Joining state (the background thread will still write Done).
            ...
            PrewarmState::Joining | PrewarmState::Taken => {
                // Shouldn't happen - take will only be called once. Return None to take the synchronous path.
    ...
    // join background thread. If the background panics, join will return Err - this is the key to avoiding an infinite loop.
    if let Some(handle) = handle {
        match handle.join() {
            Ok(()) => {
                let join_us = take_start.elapsed().as_micros();
                log::info!("SQLite prewarm: main thread waited {join_us} µs for background");
                // After the background thread has finished running, the results should be in the Mutex.
                let mut guard = match cell.lock() {
                    Ok(g) => g,
                    Err(_) => return None,
                };
                match std::mem::replace(&mut *guard, PrewarmState::Taken) {
                    ...
                    // Background thread join succeeds but Done is not written - impossible, defensive handling.
                    _ => None,
                }
            }
            Err(_panic) => {
...
pub fn initialize(ctx: &mut AppContext) -> (Option<PersistedData>, Option<WriterHandles>) {
    ...
    // The background preheating result will be obtained first; if it cannot be obtained, init_db() will be synchronized (original behavior).
...
unsafe fn init_logging() {
    ...
    extern "C-unwind" fn log_callback(_data: *mut c_void, err_code: c_int, msg: *const c_char) {
        ...
        let level = match (primary_error_code, err_code) {
            ...
            // This is mostly outside of Waz's control (e.g. the user or some system program is
        ...
        // Local log paths should not panic, but still avoid unwinding across FFI boundaries.
        let _ = panic::catch_unwind(|| {
            // openWarp only writes local logs, and error codes/descriptions retain diagnostic value as usual.
            log::log!(
                level,
                "SQLite error {} ({}): {}",
                err_code,
...
pub(super) fn init_db() -> Result<SqliteConnection> {
    ...
    if warp_core::channel::ChannelState::channel() == warp_core::channel::Channel::Oss {
        if let Some(legacy_dir) = waz_legacy_app_group_sqlite_dir() {
            if let Err(err) = migrate_waz_app_group_sqlite_if_needed(&db_path, &legacy_dir)
                .context("Failed to migrate Waz SQLite database out of legacy App Group")
            {
                report_error!(err);
...
#[cfg(target_os = "macos")]
fn waz_legacy_app_group_sqlite_dir() -> Option<PathBuf> {
...
#[cfg(target_os = "macos")]
fn migrate_waz_app_group_sqlite_if_needed(target_db: &Path, legacy_dir: &Path) -> Result<()> {
    ...
    if !legacy_db.exists() {
        write_waz_app_group_sqlite_migration_marker(&marker)?;
        return Ok(());
    }

    if !should_copy_legacy_waz_sqlite(&legacy_db, target_db)? {
        write_waz_app_group_sqlite_migration_marker(&marker)?;
        return Ok(());
    }
    ...
    safe_info!(
        safe: ("Migrated Waz SQLite database out of legacy App Group"),
        full: ("Migrated Waz SQLite database from `{}` to `{}`", legacy_db.display(), target_db.display())
    );
...
#[cfg(target_os = "macos")]
fn should_copy_legacy_waz_sqlite(legacy_db: &Path, target_db: &Path) -> Result<bool> {
    if !target_db.exists() {
        return Ok(true);
    }
...
#[cfg(target_os = "macos")]
fn write_waz_app_group_sqlite_migration_marker(marker: &Path) -> Result<()> {
                ...
                for event in events {
                    match event {
                        ...
                        ModelEvent::Terminate => {
                            // Before exiting, proactively checkpoint all WALs back to the main library and clear them.
                            // Avoid sqlite3_open triggering "WAL mode database file was recovered" recovery at next startup.
                            if let Err(e) =
...
fn report_db_error(err_kind: &str, err: anyhow::Error, database_path: &Path) {
    // Databases are sometimes missing or inaccessible, and permissions and existence diagnostics are provided here.
...
fn save_pane_state(
    ...
    let kind = match &snapshot.contents {
        ...
        // Waz Wave 7-3: `EnvironmentManagement` arm is physically deleted along with the variant.
    ...
    match &snapshot.contents {
        ...
        // Waz Wave 7-3: `EnvironmentManagement` LeafContents arm is physically deleted along with the variant.
...
/// Helper that removes the local stored object identified by `sync_id`. If a valid object metadata row is found,
/// Just call `delete_object_fn` to delete the actual object.
                ...
                NOTEBOOK_PANE_KIND => {
                    ...
                    LeafContents::Notebook(match local_path {
                        ...
                        None => NotebookPaneSnapshot::NotebookObject {
                            notebook_id,
                            settings: WazDriveObjectSettings::default(),
                        },
                    })
                }
                WORKFLOW_PANE_KIND => {
