...
/// Manages the Waz notification plugin for a specific CLI agent.
///
...
pub(crate) trait CliAgentPluginManager: Send + Sync {
    /// The minimum plugin version required by this Waz build.
    ...
    /// Whether the Waz notification plugin is installed.
    ...
    /// Install the Waz notification plugin.
    ...
    /// Update the Waz notification plugin to the latest version.
    /// Default returns an error — only agents with `can_auto_install() == true` should override.
    ...
    fn install_success_message(&self) -> &'static str {
        "Waz plugin installed. Please restart the session to activate."
    }

    /// Toast message shown after a successful auto-update.
    fn update_success_message(&self) -> &'static str {
        "Waz plugin updated. Please restart the session to activate."
    }
...
/// doesn't have Waz notification plugin support.
