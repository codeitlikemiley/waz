...
/// A path to a file that can be opened in Waz, along with its type.
...
impl TerminalView {
    ...
    /// Whether or not the "Open in Waz" banner is open.
        ...
        match action {
                ...
                match &self.inline_banners_state.open_in_warp_banner {
                    Some(banner_state) => {
                        ActionAccessibilityContent::Custom(AccessibilityContent::new_without_help(
                            format!("Open {} in Waz", banner_state.target.path.display()),
            ...
            OpenInWarpBannerAction::LearnMore => {
                ActionAccessibilityContent::Custom(AccessibilityContent::new(
                    ...
                    "Learn more about opening Markdown files in Waz",
                    WarpA11yRole::UserAction,
                ))
            }
        }
...
/// Examines `command` for a file openable in Waz, returning the resolved path and type if found.
        ...
        if let Some(positionals) = positionals {
            for arg in positionals.iter() {
                ...
                if async_fs::metadata(&resolved).await.is_ok() {
                    // We've found a file that exists and can be opened in Waz.
