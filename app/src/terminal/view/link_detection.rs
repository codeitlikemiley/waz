...
#[cfg(feature = "local_fs")]
impl super::TerminalView {
    /// Waz: Determine whether the given session is a remote-server (SSH) session.
    ///
    /// When `local_tty` is not enabled / on wasm / `SshRemoteServer` feature flag is off,
    /// Always return `false`, which means the local behavior remains completely unchanged.
    ...
    /// Waz: Obtain the directory list verification context of a cwd of the remote session.
    ///
    /// If the cache is hit, `Remote(Some(..))` will be returned directly; if it is not hit, daemon will be initiated asynchronously.
    /// `ListDirectory` RPC pulls the directory list, and this round returns `Remote(None)` (not highlighted),
    /// After the pull is completed, it is written to the cache and `ctx.notify()` triggers re-render to light up the link.
    ///
    /// The cache remains bounded: when pulling a new cwd, all old entries are cleared and only the current cwd is retained.
    #[cfg(all(
        feature = "local_tty",
        feature = "local_fs",
    ...
    fn remote_dir_listing_context(
        ...
        // The cache is indexed by (session, cwd) composite key to avoid crosstalk between the same paths on different hosts.
        let cache_key = (session_id, cwd_path.clone());

        // Hit cache (ready or pulling) is returned directly.
        ...
        // Pull new cwd: When the upper limit of capacity is exceeded, the oldest entry will be eliminated in FIFO in the order of insertion.
        // Then insert the `None` placeholder (marked for pulling). `MAX_ENTRIES` select 8 for sufficient coverage
        // There are several working directories that users often switch to in the terminal to avoid RPC every time they switch back.
        const MAX_ENTRIES: usize = 8;
        while self.remote_dir_listing_cache.len() >= MAX_ENTRIES {
            // shift_remove_index maintains insertion order; FIFO head is oldest.
        ...
        ctx.spawn(
            ...
            move |me, result, ctx| {
                ...
                // During the pull, the user may have switched cwd/cleared the cache, and only the placeholders are written before they are still there.
                ...
                match result {
                    Ok(resp) => match resp.result {
                        Some(list_directory_response::Result::Success(success)) => {
                            ...
                            // When the list arrives, re-render is triggered to let the link be rescanned and lit.
                            ctx.notify();
                        }
                        Some(list_directory_response::Result::Error(err)) => {
                            ...
                            // Failed to pull: Remove the placeholder and try again the next time you hover.
    ...
    fn scan_for_file_path(
        ...
        // Waz: Determine whether the session to which the hovered block belongs is a remote-server session.
        // The files of the remote session are not on the local disk, so you need to use `LinkValidationContext::Remote`
        // Carry the real directory list pulled by the daemon for accurate verification.
        ...
        //
        // Waz: The block `pwd()` of the remote session is the remote cwd reported by shell-integration.
        // After splicing, the correct remote absolute path is obtained, so the remote block also participates in scanning (no longer skipped).
        let pwd_to_scan_for = match position {
            WithinModel::AltScreen(_) => {
                if is_remote {
                    // Remote session: `pwd()` returns the remote activity cwd reported by shell-integration.
                    self.pwd()
                } else {
        ...
        // Waz: The remote session is accurately verified against the cached cwd directory listing; the local session remains `Local`.
        ...
        match pwd_to_scan_for {
            ...
            Some(path) if matches!(from_editor, TerminalEditor::No) => {
                ...
                // Use hover to block your own launch data to avoid cross-session /host/WSL
                // Using wrong shell rules to parse paths.
