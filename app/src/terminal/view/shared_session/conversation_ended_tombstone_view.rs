...
struct TombstoneDisplayData {
    ...
    /// Log task execution harness when available from local data.
    #[cfg(not(target_family = "wasm"))]
...
impl TombstoneDisplayData {
    ...
    /// Supplement display information with data from AmbientAgentTask.
    #[cfg(not(target_family = "wasm"))]
...
impl ConversationEndedTombstoneView {
        ...
        ctx.subscribe_to_view(
            ...
            |_, _, event, ctx| match event {
                ArtifactButtonsRowEvent::OpenPlan { document_uid } => {
                    // openWarp localization: also go local in shared session tombstone
                    // AIDocument pane no longer relies on cloud notebook_uid.
    ...
    fn render_action_buttons(
        ...
        #[cfg(not(target_family = "wasm"))]
        {
            ...
            // forked into a local Waz conversation. Unknown harness (None) is
