...
impl TerminalView {
    ...
    fn update_shared_session_pane_header(&mut self, _ctx: &mut ViewContext<Self>) {
        // Waz Phase 2a: pane-header sharing UI is gone, so the pane no
    ...
    // Waz:Share Session path has been cut off, the following two methods retain the signature but no-op,
    // No longer emit `Event::OpenShareSessionModal{,DeniedModal}`, and no longer touch the cloud collaboration session service.
    ...
    // Waz: Shared Session network entrance has been cut off, attempt_to_share_session overall no-op,
    // No more set SharePending status, no more emit StartSharingCurrentSession, no more triggering telemetry.
    ...
    pub fn on_session_share_started(
        ...
        // TODO(waz-cloud-removal Phase 5): `self_handle` was originally fed to ShareableObject::Session
        // Used for sharing UI back-checking pane; sharing UI has been deleted but the entire shared_session link is still there.
        // Delete this ctx.handle() call when the shared_session is completely retired.
        ...
        self.pane_configuration.update(ctx, |pane_config, ctx| {
            ...
            // Waz Phase 2a: sharing dialog + pane-header `ShareableObject`
    ...
    pub fn on_session_share_joined(
        ...
        self.pane_configuration.update(ctx, |pane_config, ctx| {
            pane_config.refresh_pane_header_overflow_menu_items(ctx);
            // Waz Phase 2a: removed `set_shareable_object` (cloud sharing UI gone).
            pane_config.notify_header_content_changed(ctx);
        });
