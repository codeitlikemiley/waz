...
impl TerminalView {
    ...
    /// Insert a pending user query block for the local ambient agent run until the harness CLI is started.
    /// This block only displays the user prompt and queued status, and does not provide a local queue callback button.
