...
pub struct PromptSuggestionBannerState {
    ...
    /// Waz BYOP: The model actively adjusts the chip generated by the `suggest_prompt` tool to carry action_id,
    /// When accepting/rejecting, you need to adjust `complete_suggest_prompt_action` to close the oneshot channel
    /// Let the BYOP loop get the result and continue to the next round. `None` indicates that this chip comes from another path
    /// (For example, MAA server passive suggestion), there is no need to go through BYOP to complete the writeback.
