...
//! Waz's SSH extension on the remote host the shell just connected to,
...
//!   cards ("Install Waz's SSH extension" / "Continue without installing"),
    ...
    fn render_footer(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let checkbox_label =
            Hoverable::new(self.do_not_ask_again_label_mouse_state.clone(), move |_| {
                Text::new(
                    crate::t!("terminal-dont-ask-again"),
                    ui_font_family,
                    footer_font_size,
                )
        ...
        let manage_settings_link = appearance
            ...
            .link(
                ...
                None,
                Some(Box::new(|ctx| {
                    ctx.dispatch_typed_action(SshRemoteServerChoiceViewAction::WazifySettings);
                })),
...
impl TypedActionView for SshRemoteServerChoiceView {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            SshRemoteServerChoiceViewAction::WazifySettings => {
                ctx.emit(SshRemoteServerChoiceViewEvent::WazifySettings);
            }
        }
    }
}
