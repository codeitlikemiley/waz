...
pub fn initialize_app(app: &mut App) {
    ...
    // Waz:RepoOutlines has been deleted and is no longer registered.
    #[cfg(feature = "voice_input")]
...
#[test]
fn test_tmp_path_completions() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        let terminal = add_window_with_bootstrapped_terminal(&mut app, None, None).await;
        let input = terminal.read(&app, |terminal, _| terminal.input().clone());

        // Prepare a TmpFormPanel command entry
        let command_entry = warp_completer::signatures::tmp::CommandEntry {
            ...
            tokens: vec![
                warp_completer::signatures::tmp::TokenDef {
                    ...
                    required: true,
                    token_type: warp_completer::signatures::tmp::TokenType::File,
                    default: None,
                    values: None,
                    flag: None,
                    data_source: None,
        ...
        // Hitting tab inside file token field when suggestions are empty triggers completions
        input.update(&mut app, |input, ctx| {
            input.input_tab(ctx);
        });

        // Now mock completion results populated into input_suggestions
        input.update(&mut app, |input, ctx| {
            input.input_suggestions.update(ctx, |suggestions, _| {
                suggestions.set_items(vec![
                    Item::from_text("src/main.rs".to_string()),
                    Item::from_text("src/lib.rs".to_string()),
                ]);
            });
        });

        // Call tab again: it should cycle to the first item ("src/main.rs"), cycle through, and set editor buffer
        input.update(&mut app, |input, ctx| {
            input.input_tab(ctx);
        });

        // Verify the buffer is updated
        assert_eq!(
            input.read(&app, |input, ctx| input.buffer_text(ctx)),
            "git add src/main.rs"
        );

        // Verify selected item text is updated
        let selected = input.read(&app, |input, ctx| {
            input.input_suggestions.read(ctx, |s, _| s.get_selected_item_text().map(|t| t.to_string()))
        });
        assert_eq!(selected, Some("src/main.rs".to_string()));

        // Hitting EditorDown should cycle to the next item
        input.update(&mut app, |input, ctx| {
            input.editor_down(ctx);
        });

        // Verify the buffer and selection update to "src/lib.rs"
        assert_eq!(
            input.read(&app, |input, ctx| input.buffer_text(ctx)),
            "git add src/lib.rs"
        );

        // Hitting EditorUp should cycle back to the previous item
        input.update(&mut app, |input, ctx| {
            input.editor_up(ctx);
        });
        assert_eq!(
        ...
        // Confirm suggestion closes suggestions dropdown
        input.update(&mut app, |input, ctx| {
            let active_item_text = input.input_suggestions.read(ctx, |s, _| s.get_selected_item().unwrap().text().to_string());
            input.handle_suggestions_event(
                &InputSuggestionsEvent::ConfirmSuggestion {
                    suggestion: active_item_text,
                    match_type: MatchType::Exact { is_case_sensitive: true },
                },
                ctx,
            );
        });

        // Verify input suggestions are empty, but TmpFormPanel is still active
        input.read(&app, |input, ctx| {
            assert!(input.input_suggestions.read(ctx, |s, _| s.is_empty()));
            assert!(matches!(
                input.suggestions_mode_model.read(ctx, |m, _| m.mode().clone()),
                InputSuggestionsMode::TmpFormPanel { .. }
            ));
        });
    });
...
#[test]
fn test_tmp_form_panel_confirm_and_shift_tab() {
    App::test((), |mut app| async move {
        initialize_app(&mut app);
        let terminal = add_window_with_bootstrapped_terminal(&mut app, None, None).await;
        let input = terminal.read(&app, |terminal, _| terminal.input().clone());
        ...
        // Mock completion results populated into input_suggestions
        input.update(&mut app, |input, ctx| {
            input.input_suggestions.update(ctx, |suggestions, _| {
                suggestions.set_items(vec![
                    Item::from_text("src/main.rs".to_string()),
        ...
        // Call tab to cycle to first item ("src/main.rs")
        input.update(&mut app, |input, ctx| {
            input.input_tab(ctx);
        });

        // Call tab again to cycle to second item ("src/lib.rs")
        input.update(&mut app, |input, ctx| {
            input.input_tab(ctx);
        });

        // Verify the buffer is updated to "git add src/lib.rs"
        assert_eq!(
            input.read(&app, |input, ctx| input.buffer_text(ctx)),
            "git add src/lib.rs"
        );

        // Calling shift_tab: it should cycle backwards to "src/main.rs"
        input.update(&mut app, |input, ctx| {
            input.input_shift_tab(ctx);
        });

        // Verify the buffer is updated to "git add src/main.rs"
        assert_eq!(
            input.read(&app, |input, ctx| input.buffer_text(ctx)),
            "git add src/main.rs"
        );

        // Hitting enter: should confirm the suggestion and return instead of executing command
        input.update(&mut app, |input, ctx| {
            input.input_enter(ctx);
        });

        // Verify suggestions are closed/empty, and command execution didn't happen (meaning we are still in TmpFormPanel mode)
        input.read(&app, |input, ctx| {
            assert!(input.input_suggestions.read(ctx, |s, _| s.is_empty()));
            assert!(matches!(
                input.suggestions_mode_model.read(ctx, |m, _| m.mode().clone()),
            ...
            assert_eq!(input.buffer_text(ctx), "git add src/main.rs");
        });
    });
}
