            ...
            }) = running_command.as_ref()
            {
                ...
                let in_band_command = match shell.shell_type() {
                    ShellType::PowerShell => {
                        format!("Waz-Run-GeneratorCommand {id} '{escaped_command}' -ErrorAction Ignore")
                    }
                    ShellType::Fish => {
...
impl CommandExecutor for InBandCommandExecutor {
    ...
    /// `warp_run_generator_command`/`Waz-Run-GeneratorCommand` shell script API that is declared as
    /// part of Waz's bootstrap script.
    ///
...
pub fn is_in_band_command(command: &str) -> bool {
    ...
    trimmed.starts_with("Waz-Run-GeneratorCommand ")
