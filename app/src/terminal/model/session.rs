...
pub enum BootstrapSessionType {
    /// The session host is the same host where Waz is running.
    Local,

    /// The session host is a different host from where Waz is running.
    WarpifiedRemote,
}
...
/// Returns the hostname for the local machine where Waz is running.
