...
pub struct BlockList {
    ...
    /// SSH subshell-bootstrap has just been completed and the active block prompt has not yet arrived. Press Ctrl+L and it will appear.
    /// The sum tree stacks height=0 blocks, causing the gap to fail to shrink. received in this state
    /// The call to `clear_visible_screen` will record pending and wait for the next `precmd` (representing the remote
    /// prompt has been redrawn) before it is actually executed.
...
impl BlockList {
    ...
    pub fn clear_visible_screen(&mut self) {
        // Problem background: SSH subshell-bootstrap has just been completed, and the active block has not yet drawn the prompt.
        // (`height=0` + `state=BeforeExecution` + not started), press Ctrl+L at the moment, the gap will be
        // Insert before/after this 0-height active block. Then the remote high frequency distributes in-band
        // generator command, each will `command_finished` → `create_new_block` launch a
        // New active block, that new active block is also "just created, height=0, prompt has not arrived"
        // state. As a result, there is a row of hidden/empty blocks with height=0 piled at the end of the sum tree. Before them
        // It’s still the gap that’s the whole screen high. The user’s actual input block is pushed far behind the gap.
        // place. But the gap can only shrink with the increment of "total height after gap", and those 0 height blocks
        // It cannot contribute to the increment, causing the gap to be unable to be shrunk → the command block entered by the user is always outside the screen.
        //
        // Fix: If the active block is not ready yet and there are still in-band commands flying at the moment (height=0 +
        // BeforeExecution + !in_band itself + in_flight_in_band_command_count > 0),
        // Make a note of pending. Wait until the next `precmd` arrives (meaning the remote prompt has been redrawn and active
        // block has real content) and then clear the screen. Local normal state (no in-band command on the fly) does not enter
        // This branch prevents local Ctrl+L from being accidentally blocked.
    ...
    /// Whether the active block is in the state of "just created, prompt has not arrived yet". It should not be allowed in this state
    /// `clear_visible_screen` goes in, otherwise a row of height=0 blocks will be left on the sum tree.
    ///
    /// Note: This is only possible if in-band commands are still flying (typically such as those dispatched frequently during SSH subshell-bootstrap)
    /// Generator) only takes the defer path. Otherwise `height=0 + BeforeExecution + !in_band` is actually
    /// The **default state** of the local shell's empty prompt waiting for user input will mistakenly intercept all local Ctrl+L.
    /// After setting it to pending, it will never wait for precmd to clear up, which shows that the local screen clearing fails (the SSH remote end works normally,
    /// Because the active block has drawn the remote prompt, height > 0 directly bypasses this judgment).
    fn should_defer_clear_for_unprepared_active_block(&self) -> bool {
        ...
        // height=0 means that the prompt has not been drawn yet; BeforeExecution means that execution has not started yet;
        // The reason why `started()` is not taken is because it looks at whether the command has started, which is not synchronized with the prompt arrival status.
    ...
    fn do_clear_visible_screen(&mut self) {
        ...
        //
        // Exception: in-band commands (such as the generator sent by the remote high-frequency after SSH subshell-bootstrap) will
        // Temporarily "borrow" the user's active block, set `is_for_in_band_command` to true and start it.
        // This kind of block is hidden on the UI (`should_hide_block` → `height()=0`). If Ctrl+L
        // Hit this in-flight window, click "started branch" to insert the gap after the block, and then
        // `command_finished` → `create_new_block` will bypass the `update_live_block_height` gap
        // Shrinking, causing the gap in the model to be all hidden blocks with height=0, `max_scroll_top=0`,
        // The input box is laid out to a large blank gap at the top of the screen and below → visually "the input is missing".
        //
        // Treating the in-band borrowed block as unstarted, and inserting the gap before the block is equivalent to treating it as
        // The "Still waiting for user input" status is consistent with product semantics.
    ...
    fn seek_down_to_next_grid(
        ...
        match grid_type {
            GridType::Output => {
                ...
                // vs the next "command" (in the Waz prompt case), when using the combined grid, rather than
...
impl ansi::Handler for BlockList {
    ...
    fn clear_screen(&mut self, mode: ClearMode) {
        ...
        match mode {
            ...
            ClearMode::All => {
                ...
                // Waz's custom logic for "clear". It's not immediately straightforward because a
                ...
                // quite right with Waz's custom clear screen logic.
            }
            _ => {}
        }
    ...
    fn precmd(&mut self, data: PrecmdValue) {
        ...
        // If Ctrl+L was delayed before (active block prompt has not yet arrived), now the remote
        // The prompt has been redrawn, so you can actually clear the screen once. If the active block is still inappropriate (e.g.
        // Continuous in-band commands are still running), they remain pending, and precmd will try again next time.
