...
//! executing one of Waz's DCS hooks, etc. [`Handler`] should be implemented by
...
/// Marks an OSC as one that is sent by Waz logic registered in the shell.
///
...
/// of checks ensuring that Waz's grids and ConPTY's grid are in sync.
...
/// between each update.
///
/// There are two mechanisms to prevent Waz from falling too behind:
    ...
    W: io::Write + 'a,
{
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            // Received a Waz OSC used for in-band generators.
            WARP_IN_BAND_GENERATOR_OSC_MARKER => match params.get(1) {
                Some(&WARP_IN_BAND_GENERATOR_START_BYTE) => {
                    log::info!("Received a Waz OSC marker for starting in-band command output.");
                ...
                _ => {
                    log::warn!("Received a Waz OSC marker missing required param.");
                }
            },

            // Received a Waz OSC used for shell hooks.
            WARP_OSC_MARKER => {
                ...
                match json_marker_char {
                    HEX_ENCODED_JSON_MARKER => {
                        ...
                        let Some(data_str) = params
                            ...
                            log::error!("Waz OSC marker did not contain payload");
                            return;
                        };
                        safe_debug!(
                            safe: ("Received Waz OSC string for shell hook"),
                            full: ("Received Waz OSC string for shell hook with JSON payload: {:?}", data_str)
                        );
            ...
            WARP_RESET_GRID_OSC_MARKER => {
                log::debug!("Received Waz OSC string for reset grid");
                self.handler.on_reset_grid();
            }

            // Received a Waz OSC used for completions.
            WARP_COMPLETIONS_OSC_MARKER => match params.get(1) {
                Some(&WARP_COMPLETIONS_START_BYTE) => {
                    let Some(format) = params
                        ...
                        log::warn!("Waz start completions OSC marker contained invalid format.");
                        return;
                    };
                ...
                Some(&WARP_COMPLETIONS_MATCH_RESULT_BYTE) => {
                    ...
                    let Some(data_str) = params
                        ...
                        log::warn!(
                            "Waz completions match result OSC marker did not contain payload"
                        );
                        return;
                    };
                ...
                Some(bytes) if bytes.starts_with(WARP_COMPLETIONS_MATCH_UPDATE_METADATA) => {
                    ...
                    let Some(data_str) = params
                        ...
                        log::warn!(
                            "Waz completions match metadata OSC marker did not contain payload"
                        );
                        return;
                    };
                    ...
                    match &parameter[2..] {
                        ...
                        _ => {
                            log::warn!("Invalid Waz OSC marker parameter for completions match metadata: {parameter}");
                        }
                    }
                }
                ...
                _ => {
                    log::warn!("Received a Waz OSC completions marker missing required param.");
                }
            },
