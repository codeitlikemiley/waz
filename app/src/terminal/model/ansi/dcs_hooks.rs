...
/// Indicates that the following JSON-encoded message is hex-encoded for Waz's lifecycle hooks.
...
/// Indicates that the following JSON-encoded message is unencoded for Waz's lifecycle hooks.
/// In DCS, it is used as the final char in the DCS start sequence.
/// In OSC, it is used as the first parameter.
pub(super) const UNENCODED_JSON_MARKER: char = 'f';

/// Indicates that the following message is a ANSI-C quoted message for receiving Waz's lifecycle
/// hooks via key-value pairs.
/// In OSC< it is used as the first parameter.
pub(super) const UNENCODED_KV_MARKER: char = 'k';

/// Enum representing all possible JSON payloads for Waz's DCS's.
...
/// Received from the pty after the shell has finished executing Waz's
/// bootstrap script.
...
/// Received from the pty when Waz's SSH wrapper is executed, prior to
...
/// created; if the session is for a subshell, this triggers Waz's bootstrap process.
...
/// The binding is triggered when Waz writes the `ESC-i` escape sequence to the pty.
/// Waz usually does this after a block completes, to collect any typeahead
...
/// `logout`, Ctrl-D on an empty prompt, etc.). Lets the Waz client drop
