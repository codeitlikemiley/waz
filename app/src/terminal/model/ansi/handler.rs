...
pub trait Handler {
    ...
    /// Callback for the Waz CommandFinished hook.
    ...
    /// Callback for the Waz precmd hook.
    fn precmd(&mut self, _data: PrecmdValue) {}

    /// Callback for the Waz preexec hook.
    fn preexec(&mut self, _data: PreexecValue) {}

    /// Callback for the Waz bootstrapped hook - called once when the shell is
    /// bootstrapped
    fn bootstrapped(&mut self, _data: BootstrappedValue) {}

    /// Callback for the Waz pre-interactive SSH session hook - called once
    ...
    /// Callback for the Waz ssh hook - called once after successfully connecting to
    /// an SSH server
    ...
    /// Callback for the Waz exit-shell hook — emitted by the remote shell right
    /// before it exits. Gives the Waz client a chance to drop per-session
    ...
    /// input buffer (the reporting is itself triggered by Waz).
    ...
    /// created. If the session is for a subshell, this should triggers Waz's bootstrap process.
    ...
    /// These allow external programs to trigger notifications in Waz.
