...
impl WarpifySuccessBlock {
        ...
        let auto_warpify_snippet = auto_warpify_snippet.map(|(output_grid, can_write_to_rc)| {
            AutoWarpifySnippet {
                description: (if !output_grid.is_empty() {
                    ...
                    "In remote subshells, Waz runs commands in the background to power completions, syntax highlighting, and other features."
                }).into(),
    ...
    pub fn render_title_ui(&self, theme: &WarpTheme, appearance: &Appearance) -> Box<dyn Element> {
        let header_contents = render::build_header_row(
            "Session Warpified",
            Icon::new(UiIcon::Waz.into(), theme.active_ui_detail()),
            theme,
            appearance,
        )
        .with_margin_right(8.)
...
impl TypedActionView for WarpifySuccessBlock {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            WarpifySuccessBlockAction::WazifySettings => {
                ctx.emit(WarpifySuccessBlockEvent::WazifySettings);
            }
