...
/// restore the PS1 from the saved PS1 value (we had unset the PS1 for Waz prompt).
const SWITCH_TO_PS1_ESCAPE_SEQUENCE: &[u8] = &[escape_sequences::C0::ESC, b'p'];
/// Used to let the shell know we are switching to the Waz prompt via a bindkey \ew. This will
/// unset the PS1 to ensure we don't have a double prompt (PS1 and Waz prompt).
...
impl<T: EventLoopSender> PtyController<T> {
    pub fn new(
        ...
        ctx.subscribe_to_model(&model_event_dispatcher, |me, event, ctx| match event {
            ...
            ModelEvent::HonorPS1OutOfSync => {
                // We force re-sync the PS1 state of Waz settings with the shell's environment variable, $WARP_HONOR_PS1, via
    ...
    /// Sends bindkey to notify shell process to switch to Waz prompt logic for prompt
