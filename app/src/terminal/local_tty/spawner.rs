...
fn invoke_without_crash_reporting<T>(
    ...
    // Pause local crash reporting status before forking the child process to avoid leaking signal/exception handling status to the shell.
    #[cfg(feature = "crash_reporting")]
    crate::crash_reporting::suspend_crash_reporting_for_child_spawn();

    let retval = func();

    // The local crash reporting state is restored after the child process completes forking.
