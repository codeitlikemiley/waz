...
//!
//! This module owns everything specific to running a Waz shell inside a
...
///
/// Lives under the Waz per-user cache directory rather than `/tmp` so:
...
/// Resolves the absolute path to the `sbx` CLI binary using the Waz
/// process's `PATH`.
///
/// Waz's process `PATH` is minimal and often misses user-shell-installed
/// tools (e.g. homebrew on Apple Silicon when Waz is launched from Finder,
...
///
/// Each instance carries a unique `sandbox_id` so multiple Waz panes can run
...
impl DockerSandboxShellStarter {
    ...
    /// Host directory where Waz writes this sandbox's bash init script.
