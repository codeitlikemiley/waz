...
/// * On macOS, this includes `$APP_PATH/Contents/Resources/bin`, in which we put a wrapper around the Waz CLI.
...
/// executable binary for one of Waz's supported shell types (bash, fish, zsh).
...
impl ShellStarter {
    ...
    /// used to spawn a shell process for a new top-level Waz session. If a WSL Distribution is
...
pub struct DirectShellStarter {
    ...
    /// Arguments to be passed to the shell binary at [`shell_path`] when spawning a new Waz
    /// session.
    args: Vec<OsString>,
}
...
fn arguments_for_session_spawning_command(
    ...
    match shell_type {
        ShellType::Zsh => {
            ...
            // begins with a space. We use this to hide Waz bootstrap commands from the history.
            vec![
                "-c".to_owned().into(),
        ...
        ShellType::Bash => {
             ...
             * 5. Send an InitShell message to Waz through escape sequences.
        ...
        ShellType::Fish => {
            ...
            vec![
                ...
                format!(
                    ...
                    // Fish's implementation of this breaks Waz by emitting `OSC 133 A` but not
