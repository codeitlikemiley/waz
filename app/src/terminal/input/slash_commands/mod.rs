    ...
    pub(super) fn execute_slash_command(
        ...
        match command.name {
            ...
            compact_and if command.name == commands::COMPACT_AND.name => {
                ...
                // When the menu path has no parameters, the argument is Some(""), which is normalized to None to avoid the summary.
                // Triggering an empty user query wastes tokens.
            ...
            compact if command.name == commands::COMPACT.name => {
                // Waz:`/compact` shares the local session compression link with `/compact-and` —
                // dispatch `WorkspaceAction::SummarizeAIConversation`,initial_prompt: None
                // It means "no subsequent prompt will be sent after compression", only the summary will be silently dropped into the conversation.
                // Customize the command (`/compact <command>`) into the prompt field, in BYOP build_chat_request
                // The path will be spelled after SUMMARY_TEMPLATE as plugin_context.
                //
                // In non-BYOP paths, `SummarizeConversation { prompt }` still uses server protobuf
                // (`api::request::input::SummarizeConversation`), server-side processing summary —
                // The previous semantics of prefix injection are completely replaced by SummarizeConversation.
                if self
                    .ai_context_model
                    .as_ref(ctx)
                    .selected_conversation_id(ctx)
                ...
                // When the menu path has no parameters, argument is Some(""), which is normalized to None,
                // Avoid taking an extra empty filter branch when chat_stream spells summary prompt.
