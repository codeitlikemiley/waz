...
impl SearchItem for ModelSearchItem {
    ...
    fn render_details(&self, app: &AppContext) -> Option<Box<dyn Element>> {
        ...
        // BYOP uses dedicated score rendering: Context / Output (bar is normalized with log2) + Cost = BilledToApi.
        // The visuals are identical to the default Waz panel, only the semantics of the rows are different.
...
/// Discounts only apply when the user is billing through Waz credits,
