...
fn test_bash_vim_banner_already_shown() {
    App::test((), |mut app| async move {
        ...
        // Ensure Waz's vim keybindings are off.
...
fn test_bash_vim_banner_off() {
    App::test((), |mut app| async move {
        ...
        // Ensure Waz's vim keybindings are on.
        AppEditorSettings::handle(&app).update(&mut app, |editor_settings, ctx| {
...
// Behavior test: When the rich input is opened, the OpenCLIAgentRichInput action is triggered again
// (i.e. the handler dispatched after Ctrl-G actually hit the binding) should close the rich input.
// This overrides the core toggle path for this fix.
#[test]
...
// ---- OneKey search filter pure function test ----
//
// These tests only target the `filter_and_sort_onekey_candidates` pure path,
// No need to construct TerminalView/ctx. Unicode handling of skim algorithms by fuzzy_match
// The crate is responsible, here we only verify whether our use of it in view.rs is as expected.
...
fn onekey_query_filters_out_non_matches() {
    ...
    // "ap" should hit apple / apricot, banana does not.
    // The relative order between apple / apricot is not asserted here - the specific scoring is determined by skim,
    // We only ensure that banana is filtered out.
...
fn onekey_query_matches_subtitle() {
    ...
    let indices = rows_indices(result);
    // Only the subtitle of server-1 contains prod.
...
fn onekey_query_matches_chinese_characters() {
    // Chinese character sequence matching: skim algorithm processes Unicode char.
    let candidates = vec![
        ("生产数据库", "ops@db.example.com:22"),
