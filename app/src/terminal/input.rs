...
pub enum InputSuggestionsMode {
    ...
    TmpFormPanel {
        command_entry: warp_completer::signatures::tmp::CommandEntry,
        active_token_index: usize,
        token_values: Vec<String>,
    },
    ...
    fn to_telemetry_mode(&self) -> TelemetryInputSuggestionsMode {
        match *self {
            ...
            InputSuggestionsMode::TmpFormPanel { .. } => {
                TelemetryInputSuggestionsMode::TmpFormPanel
            }
...
pub enum CommandExecutionSource {
    /// A non-shared command execution request from Waz AI++.
...
pub struct Input {
    ...
    pub(super) input_suggestions: ViewHandle<InputSuggestions>,
...
pub fn init(app: &mut AppContext) {
    ...
    app.register_fixed_bindings(vec![
        ...
        FixedBinding::new(
            "space",
            InputAction::TmpPanelToggleBoolean,
            id!("Input") & id!("TmpFormPanelBooleanActive"),
        ),
        FixedBinding::new(
            "y",
            InputAction::TmpPanelSetTrue,
            id!("Input") & id!("TmpFormPanelBooleanActive"),
        ),
        FixedBinding::new(
            "n",
            InputAction::TmpPanelSetFalse,
            id!("Input") & id!("TmpFormPanelBooleanActive"),
        ),
        FixedBinding::custom(
...
impl Input {
    ...
    pub(crate) fn new(
        ...
        ctx.subscribe_to_view(&agent_input_footer, |me, _, event, ctx| {
            match event {
                ...
                // Waz Wave 7-3:`AgentInputFooterEvent::OpenEnvironmentManagementPane` handler
                // Physically removed with the ambient-agent UI subsystem.
                #[cfg(not(target_family = "wasm"))]
    ...
    fn agent_mode_hint_text(&mut self, app: &AppContext) -> String {
        ...
        let key = match (
            ...
            (InputType::AI, _) => {
                ...
                //  * `None` (no conversation, empty, passive, or untitled) => new conversation => "Waz anything"
    ...
    /// Predicts the next action using an AI model and past context on blocks within Waz.
    ...
    fn env_vars_command_prefix(&self, env_vars_id: &SyncId, ctx: &AppContext) -> Option<String> {
        ...
        if shell_type == ShellType::Fish {
            // Waz currently doesn't support newlines in Fish, just prepend the vars
    ...
    fn handle_suggestions_event(
        ...
        match event {
            InputSuggestionsEvent::ConfirmSuggestion {
                ...
                let is_tmp = matches!(self.suggestions_mode_model.as_ref(ctx).mode(), InputSuggestionsMode::TmpFormPanel { .. });
            ...
            InputSuggestionsEvent::Select(selected_item) => {
                ...
                match &mode {
                    ...
                    InputSuggestionsMode::TmpFormPanel {
                        ...
                        let active_token_index = *active_token_index;
    ...
    fn confirm_suggestion_internal(
        ...
        let mode = self.suggestions_mode_model.read(ctx, |model, _| model.mode().clone());
        match mode {
            ...
            | InputSuggestionsMode::DynamicWorkflowEnumSuggestions {
                ...
                let selected_ranges = selected_ranges;
                self.editor.update(ctx, |editor, ctx| {
                    editor.select_and_replace(
                        suggestion,
    ...
    fn editor_up(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let handled = match mode {
            InputSuggestionsMode::TmpFormPanel {
                ...
                if !self.input_suggestions.read(ctx, |s, _| s.is_empty()) {
                    self.input_suggestions.update(ctx, |s, ctx| {
                        s.select_prev(ctx);
                    });
                    if let Some(selected_text) = self.input_suggestions.read(ctx, |s, _| s.get_selected_item_text().map(|t| t.to_string())) {
        ...
        if self.suggestions_mode_model.as_ref(ctx).is_visible() && self.can_query_history(ctx) {
            if let InputSuggestionsMode::TmpFormPanel { .. } = self.suggestions_mode_model.as_ref(ctx).mode() {
    ...
    fn editor_down(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        if self.suggestions_mode_model.as_ref(ctx).is_visible() {
            ...
            } else if self.input_suggestions.as_ref(ctx).is_empty() {
    ...
    fn can_attach_on_filepaths_paste_or_dragdrop(&self, ctx: &mut ViewContext<Self>) -> bool {
        // Shared session viewers cannot attach images in Waz.
        let is_viewer = self.model.lock().shared_session_status().is_viewer();
        if is_viewer {
            return false;
        }
        ...
        if let InputSuggestionsMode::TmpFormPanel {
            ...
            if let Some(results) = results {
                if !results.suggestions.is_empty() {
                    let preselect_option = TabCompletionsPreselectOption::First;
                    let replacement_word = results.replacement_span.slice(&buffer_text);
                    self.input_suggestions.update(ctx, |input_suggestions, ctx| {
    ...
    fn input_tab(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let is_tab_bound_to_open_completions =
            bindings::keybinding_name_to_keystroke(OPEN_COMPLETIONS_KEYBINDING_NAME, ctx)
                ...
                .unwrap_or_default();
        println!("DEBUG: suggestions_mode = {:?}", self.suggestions_mode_model.as_ref(ctx).mode());
        println!("DEBUG: is_tab_bound_to_open_completions = {}", is_tab_bound_to_open_completions);
        println!("DEBUG: cursor_positioned_for_completion = {}", self.cursor_positioned_for_completion(ctx));
    ...
    fn maybe_open_completion_suggestions(&mut self, ctx: &mut ViewContext<Self>) {
        if !matches!(
            self.suggestions_mode_model.as_ref(ctx).mode(),
            InputSuggestionsMode::CompletionSuggestions { .. } | InputSuggestionsMode::TmpFormPanel { .. },
        ) && self.cursor_positioned_for_completion(ctx)
        {
            ...
            let cwd = self.active_block_metadata
                ...
                .and_then(|m| m.current_working_directory())
                .unwrap_or("");
            if let Some((matched_entry, _prefix)) = warp_completer::signatures::tmp::find_matching_tmp_command(&buffer_text, cwd) {
                let new_vals = warp_completer::signatures::tmp::extract_token_values(&matched_entry.command, &matched_entry.tokens, &buffer_text);
                self.suggestions_mode_model.update(ctx, |m, ctx| {
                    m.set_mode(
                        InputSuggestionsMode::TmpFormPanel {
    ...
    fn should_enter_accept_completion_suggestion(&self, app: &AppContext) -> bool {
        match self.suggestions_mode_model.as_ref(app).mode() {
    ...
    pub(crate) fn input_enter(&mut self, ctx: &mut ViewContext<Self>) {
            ...
            && self.ai_input_model.as_ref(ctx).is_ai_input_enabled()
        {
                ...
                .is_configuring_ambient_agent()
            {
                ...
                // Collect pending files for the spawn request. Cloud mode image attachments are disabled in Waz.
    ...
    fn predict_am_query(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let _ = (processed_input, processed_output); // The historical ServerApi path is used, BYOP one-shot only takes the last_block summary
                    ...
                    send_telemetry_from_ctx!(
                        TelemetryEvent::WorkflowExecuted(WorkflowTelemetryMetadata {
                            ...
                            workflow_selection_source: selected_workflow_state
                                .workflow_selection_source,
                            // This is only `Some()` for WazDrive workflows; we don't track
...
impl TypedActionView for Input {
    ...
    fn handle_action(&mut self, action: &InputAction, ctx: &mut ViewContext<Self>) {
        match action {
            InputAction::TmpPanelToggleBoolean => self.tmp_panel_toggle_boolean(ctx),
            InputAction::TmpPanelSetTrue => self.tmp_panel_set_boolean(true, ctx),
            InputAction::TmpPanelSetFalse => self.tmp_panel_set_boolean(false, ctx),
...
impl View for Input {
    ...
    fn keymap_context(&self, app: &AppContext) -> warpui::keymap::Context {
        ...
        if matches!(
            ...
            ctx.set.insert("TmpFormPanelActive");
            if let InputSuggestionsMode::TmpFormPanel {
                command_entry,
                active_token_index,
                ..
                ...
                if let Some(token) = command_entry.tokens.get(*active_token_index) {
                    if token.token_type == warp_completer::signatures::tmp::TokenType::Boolean {
                        ctx.set.insert("TmpFormPanelBooleanActive");
                    }
                }
            }
        }
...
impl Input {
    fn tmp_panel_toggle_boolean(&mut self, ctx: &mut ViewContext<Self>) {
        if let InputSuggestionsMode::TmpFormPanel {
            command_entry,
            active_token_index,
            token_values,
            ...
            let mut vals = token_values.clone();
            let cur_val = vals[active_token_index].as_str();
            vals[active_token_index] = if cur_val == "true" { "false" } else { "true" }.to_string();
            let assembled = warp_completer::signatures::tmp::build_assembled_command(&command_entry, &vals, false);
            self.editor.update(ctx, |editor, ctx| {
                editor.set_buffer_text_ignoring_undo(&assembled, ctx);
                let char_len = editor.buffer_text(ctx).chars().count() as u32;
                editor.reset_selections_to_point(&BufferPoint::new(0, char_len), ctx);
    ...
    fn tmp_panel_set_boolean(&mut self, val: bool, ctx: &mut ViewContext<Self>) {
        if let InputSuggestionsMode::TmpFormPanel {
            command_entry,
            active_token_index,
            token_values,
            ...
            let mut vals = token_values.clone();
            vals[active_token_index] = val.to_string();
            let assembled = warp_completer::signatures::tmp::build_assembled_command(&command_entry, &vals, false);
            self.editor.update(ctx, |editor, ctx| {
                editor.set_buffer_text_ignoring_undo(&assembled, ctx);
                let char_len = editor.buffer_text(ctx).chars().count() as u32;
