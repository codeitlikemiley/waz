...
pub enum WasmNUXDialogAction {
    ...
    OpenNativeAndClose,
    /// Open the Waz download page
    OpenDownloadDesktopAppLink,
    /// Open a link to learn more about Waz
    LearnMore,
}
...
/// * Download Waz if they haven't already
...
impl WasmNUXDialog {
    ...
    /// behavior: if Waz is installed, redirect to it; otherwise stay on the web.
...
impl View for WasmNUXDialog {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // 1. The user doesn't have Waz installed - treat them as a potential new user and encourage downloading Waz.
        // 2. The user has Waz installed, but clicked through to the web - ask if they want to always default to web.
...
impl TypedActionView for WasmNUXDialog {
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            WasmNUXDialogAction::OpenNativeAndClose => {
                // We intentionally do not set the native preference here, in case the user hasn't actually installed Waz.
                // If they have, on subsequent loads, we'll detect that Waz is installed and redirect to the desktop.
                ctx.emit(WasmNUXDialogEvent::Close);
