...
pub enum LeafContents {
    ...
    // Waz Wave 7-3: `EnvironmentManagement` LeafContents variant was physically deleted along with the
    ...
    GetStarted,
    /// SSH server editor pane (exclusive to openWarp). Loaded/saved by referencing the
    /// `ssh_servers.node_id` primary key. **Not persisted** - after restart, the user reopens it from the left SSH manager tree.
    SshServer {
        node_id: String,
    },
}
...
impl LeafContents {
    ...
    pub(crate) fn is_persisted(&self) -> bool {
        match self {
            // Waz Wave 7-3: `EnvironmentManagement` arm was physically deleted along with the variant.
            // SSH server editor: Data (host/user/...) is persisted in the `ssh_servers` table;
            // the pane itself is just a view, there's no difference between closing and reopening it.
            LeafContents::SshServer { .. } => false,
            // Remote file code pane: the remote buffer relies on an active SSH connection,
            // and the `RemoteFileTree` source cannot be restored (`is_restorable() == false`).
            // If it were written to persistence, it would leave an orphaned `Code` row that is
            // skipped during the restore phase, leading to the loss of the entire tab.
            // Therefore, code panes with remote sources are not persisted as a whole.
...
// Waz Wave 7-3: `EnvironmentManagementPaneSnapshot` was physically deleted along with the LeafContents variant.
...
impl From<ToolPanelView> for LeftPanelDisplayedTab {
    fn from(view: ToolPanelView) -> Self {
        match view {
            ...
            ToolPanelView::WazDrive => LeftPanelDisplayedTab::WazDrive,
