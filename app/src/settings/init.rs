...
pub fn register_all_settings(ctx: &mut AppContext) {
    ...
    // Waz Wave 7-3: `AmbientAgentSettings` is physically removed with the ambient-agent UI subsystem.
...
pub fn init(
    ...
    // Applies persistent language settings to i18n loader. run() has been initialized with the system locale earlier and is covered here.
    // Does not move when the user explicitly selects;Language::System.
    {
    ...
    // Global HTTP proxy (see Issue #72): only non-sensitive fields in NetworkSettings are read here,
    // Passwords are pushed from ProxyCredentials read from the OS keystore by `initialize_app` after post-registration.
    apply_network_settings_to_global_slots(ctx, "");
    ctx.subscribe_to_model(&NetworkSettings::handle(ctx), |_model, _event, ctx| {
        // The password may have been provided by ProxyCredentials at the time of change. lib.rs will additionally subscribe to that
        // singleton and push apply with password. Only non-password fields are pushed here.
        // Just keep your password unchanged.
...
/// Read the current `NetworkSettings` + the externally passed `password`, and update it at the same time
/// `http_client::set_global_proxy_config` and `websocket::set_global_proxy_config`,
/// Keep the same proxy semantics for both (see Issue #72).
///
/// The password parameter is passed in as `&str` rather than read from the singleton of `ProxyCredentials`, in order to
/// Prevent the singleton from not being registered during the settings::init phase. Caller responsibility: initiate early transfer
/// Empty string (subsequent UI / ProxyCredentials events will be pushed again), and the real password will be sent later. Rebuild existing
/// `Client` instances are the responsibility of the caller.
...
/// Called after `initialize_app` (`ProxyCredentials` has been registered): read the current password and then push again
/// Global proxy settings. Also used for NetworkSettings change subscriptions to keep passwords from being lost.
