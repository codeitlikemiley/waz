...
define_settings_group!(InputSettings,
    settings: [
        ...
        // Control whether shortcut key tips are displayed in the zero-state of the Agent view:
        // 1) Three lines of "ctrl+shift+enter to start a new conversation / /model to switch models / esc to return to the terminal" in zero_state_block;
        // 2) There are four items at the bottom of the Agent message bar: "? View help / / View commands / Open dialog / Enter code review".
        // After closing, the user can reopen it in "Settings → Waz Agent → AI Input".
...
impl InputSettings {
    pub fn input_type(&self, app: &AppContext) -> InputBoxType {
        ...
        // but the shell has not signalled PS1 support, fall back to Waz input.
