...
impl ActiveEnvVarCollectionData {
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        // Waz: The original `UpdateManager` subscription is used to receive cloud synchronization completion events (Create/Update/Trash
        // /Untrash::Success), cloudless = never trigger; `trash_object`/`untrash_object` is localized
        // Do not emit `ObjectOperationComplete`. Phase 2c‑1 Delete subscription + handler.
        // `ObjectStoreModel` subscriptions persist (local object changes still require breadcrumbs to be refreshed).
