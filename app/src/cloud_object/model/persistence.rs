...
/// The persistence model of [StoredObject] information. In Waz, it corresponds to the local object store in SQLite.
...
impl ObjectStoreModel {
    pub fn new(
        ...
        // Waz does not have initial fetching of cloud objects; it can be considered readable once SQLite restore is completed.
    ...
    /// Waits for the local object store to become readable. Under Waz, this condition is satisfied immediately after SQLite restore.
    ...
    /// The number of local objects that have entered an error state and will be displayed in the Waz Drive index.
    ...
    /// Given a StoredObjectLocation (folder or space), returns the local objects directly located at this position and not trashed.
    /// This function does not recurse into nested folders.
    ...
    /// Given a StoredObjectLocation (folder or space), returns the local objects directly located at this position and already trashed.
    /// This function does not recurse into nested folders.
    ...
    /// Given a StoredObjectLocation (folder or space), returns the local objects directly located at this position that entered the
    /// trash because their parent was trashed, but were not explicitly trashed by the user. This function does not recurse into nested folders.
    ...
    /// Returns all active (not trashed) local objects in the specified space.
    ...
    /// Returns all active (not trashed) and non-welcome local objects in the specified space.
    ...
    /// Returns all trashed local objects in the specified space.
    ...
    /// Returns all local objects in the specified space that were explicitly trashed by the user.
    ...
    /// Whether the next local object-store refresh needs to force a full traversal of the objects.
