...
// To reuse historical serialized values and SQLite read paths, action type implements `to_string()`.
//
...
/// ObjectActionHistory preserves the serialized shape of historical object actions: uid, actions (single or bundled),
/// and the timestamp of the most recent action. Currently, it only serves as a compatible type transmissible by the local update_manager.
#[derive(Clone, Debug, PartialEq)]
