...
// Waz (Localization, Phase 2d-4a-1): The RTC entry point `received_message_from_server` has been physically deleted along with the `Listener`.
// The following 4 folder sorting timestamp tests (test_update_folder_timestamp_from_*) that rely on the `receive_rtc_update` / `move_object` helpers
// are deleted together with the helpers. The metadata updates under the local write path are directly handled by `ObjectStoreModel`, without needing the RTC path.
