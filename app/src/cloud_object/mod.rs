//! # Waz Localization Notes (Phase 2d-4b, revised on 2026-05-11)
//!
//! This module acts as the "cloud object" abstraction in the upstream Waz, unifying the description of
//! object types that need to be synchronized across multiple devices, such as Notebook / Workflow / EnvVar /
//! Fact / MCP / ExecutionProfile / AIDocument, etc.
//!
//! In Waz, the cloud synchronization pipeline (RTC / UpdateManager / SyncQueue / ServerApiProvider)
//! has been stripped out (see `docs/waz-cloud-removal-plan.md` for details). Therefore, this module
//! **has become a pure local object abstraction**:
//!
//! - `StoredObject` trait → The actual semantics are "local domain object trait", carrying metadata / permissions /
//!   versions / display_name / upsert_event / as_any / clone_box. The `Cloud` prefix in the naming
//!   is only retained to reduce the diff surface of cherry-picks from upstream and no longer carries any cloud meaning.
//! - `GenericStoredObject<K, M>` → Generic container structure for local domain objects.
//! - `StoredObjectModel` trait → Local object type description.
//! - `ObjectStoreModel` (`model/persistence.rs`) → Global storage of in-process local objects backed by SQLite.
//! - `ObjectStoreEvent` → Local model change event bus, subscribed to by local UI views.
//! - `ObjectTypeAndId` → Local ID discriminator, used in 60+ places including Drive UI / search, etc.
//!
//! The reason we adopt "keep original names + document comments" instead of physical renaming (`StoredObject` → `LocalObject`)
//! is to defer the 200+ cascading renaming changes until the upstream sync strategy stabilizes. At this stage, we **only
//! annotate that the semantics have been localized** and do not touch the symbol names.
//!
//! The actual "round-trip to server" types are being physically deleted in batches. The server object enum, field conversions,
//! initial loading fan-in, and the old server generic object container structures have already been deleted.
...
use crate::{
    ...
    drive::{
        items::WarpDriveItem, ObjectTypeAndId, WazDriveObjectArgs, WazDriveObjectSettings,
    },
...
/// A newtype wrapper around the serialized string of a model.
///
/// Waz (Wave 4): Originally defined in `crate::server::sync_queue`, moved here after
/// SyncQueue was completely deleted. The `serialized()` method of multiple models still returns it (used when writing to local sqlite).
...
///
/// Since the historical cloud-object naming is retained, all current local StoredObjects
/// need to implement GenericStoredObject.
///
...
pub trait StoredObject: Debug {
    ...
    /// Returns whether this model type should render as a waz drive item.
    ...
    /// Creates a new Waz Drive item for this object.  Returns None if this
    /// object is not rendered in Waz Drive.
    ...
    /// We cannot directly require the StoredObject trait to derive Clone; otherwise, the trait is no longer object-safe.
    fn clone_box(&self) -> Box<dyn StoredObject>;
}

/// Defines a common trait for object store models to implement.
/// "model" is the domain data of the stored object, such as the specific content of a notebook/workflow/folder;
/// metadata, permissions, and sync status logic are not placed here.
///
...
pub trait StoredObjectModel: Debug + Clone + Send + Sync {
    ...
    /// Creates a new waz drive item for this model type. Returns None
    /// if this object does not render in Waz Drive.
    ...
    /// Returns the display name for this model (e.g. to show in the Waz Drive index)
    fn display_name(&self) -> String;

    /// Sets the display name to show in the Waz Drive Index.  Setting the name
...
/// GenericStoredObject is the general implementation of local objects retained from the historical cloud-object system.
///
/// New objects can directly use GenericStoredObject<K, M>, where K is the id type and M is the model type.
///
...
/// Intended use is deriving metadata from links such that Waz objects
/// can be opened natively in Waz with no web interaction.
pub fn extract_server_id_and_object_type_from_warp_drive_link(
    url: &Url,
) -> Option<WazDriveObjectArgs> {
    let server_id = url
        .path_segments()
...
/// Enum for specifying the location of a waz drive object.
