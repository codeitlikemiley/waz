...
impl<A: Action + Clone> Menu<A> {
    ...
    /// Switch menu variants at runtime (e.g., temporarily switching the same `Menu` instance to `Scrollable`
    /// for the OneKey password candidate popup, and then switching back to `Fixed` after closing).
...
impl<A: Action + Clone> SubMenu<A> {
    ...
    /// Route pointer events down into nested submenus.
    ///
    /// `handle_action` is always called on the root menu (`depth == 0`), but the `depth`
    /// of mouse hover / leaf / unhover events can be at any nested level.
    /// This function recursively searches along the "currently selected submenu" path
    /// for the level that should actually handle the event, and then forwards the event to it.
    /// If there is no selected submenu in the middle levels or if the depth does not match, it is discarded.
    /// Returns `true` to indicate that the event has been handled by some level,
    /// so the upper-level `handle_action` does not need to proceed with its own event dispatching.
    fn delegate_pointer_action_to_selected_submenu(
        ...
        if submenu.depth > event_depth {
            // The next level submenu has already exceeded the event target level, discarding the event.
            return false;
        }
