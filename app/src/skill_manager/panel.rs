...
// Filter tab: Keep the height compact, use 12px font size + strong contrast activation state to support readability.
// Avoid intercepting click events by shell ClippedScrollable.
...
impl SkillManagerPanel {
    ...
    fn render_filter_button(
        ...
        Hoverable::new(state, move |mouse| {
            // Text is vertically centered in a fixed row height using Flex::row + CrossAxisAlignment::Center.
            ...
            if is_active {
                // Use fg_overlay_4 in the active state to make the contrast stronger and avoid not seeing the selection change when switching quickly.
    ...
    fn render_filter_rows(
        ...
        // No longer coat ClippedScrollable / shell container:
        // 1. Horizontally scrolling containers will intercept mouse-down for drag judgment, resulting in delayed click response when quickly switching tabs.
        // 2. The number of providers in most scenes is ≤ 3,250px and can be accommodated by the width of the sidebar; if it overflows, it will be clipped by Clipped.
        //    The user can drag the sidebar wider.
