...
impl LaunchConfigSaveModal {
    ...
    fn render_save_config_button(
        ...
        self.save_modal_button(
            appearance,
            crate::t!("launch-config-save-configuration"),
    ...
    fn render_open_file_button(&self, appearance: &Appearance) -> Box<dyn Element> {
        self.save_modal_button(
            appearance,
            crate::t!("launch-config-open-yaml-file"),
        ...
        let header = Flex::row()
            .with_child(
                Shrinkable::new(
                    ...
                    Align::new(
                        Text::new_inline(
                            crate::t!("launch-config-save-current-configuration"),
        ...
        let info = match &self.save_state {
                    ...
                    self.render_formatted_text_line(
                        ...
                        vec![
                            FormattedTextFragment::plain_text(crate::t!(
                                "launch-config-save-success-prefix"
                            )),
                ...
                self.render_text_block(
                    ...
                    match failure_type {
                        FailureType::FileAlreadyExists => {
                            crate::t!("launch-config-save-failure-already-exists")
                        }
                        FailureType::Other => crate::t!("launch-config-save-failure-other"),
                    },
                )
                .with_padding_bottom(24.)
                .finish(),
            ...
            SaveState::NotSaved => {
                let text = if self.open_modal_keybinding_str.is_empty() {
                    crate::t!("launch-config-save-description-no-keybinding")
                } else {
                    crate::t!(
                        "launch-config-save-description-with-keybinding",
                        keybinding = self.open_modal_keybinding_str.as_str()
                    )
                };
                header
                    .with_child(
...
impl View for LaunchConfigSaveModal {
    ...
    fn accessibility_contents(&self, _ctx: &AppContext) -> Option<AccessibilityContent> {
        Some(AccessibilityContent::new(
            crate::t!("launch-config-save-modal-a11y-title"),
            crate::t!("launch-config-save-modal-a11y-description"),
...
impl TypedActionView for LaunchConfigSaveModal {
    ...
    fn handle_action(&mut self, action: &ActionRequest, ctx: &mut ViewContext<Self>) {
        ...
        // this will become moot when we put launch configs in Waz Drive.
