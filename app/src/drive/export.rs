...
/// Singleton model for exporting from Waz Drive.
...
impl ExportManager {
    ...
    fn handle_failure(
        ...
        let id = *export.key();
        // Export failure may contain user file paths or object names, so write only local secure logs; the most common cause is also local I/O issues.
        safe_warn!(
