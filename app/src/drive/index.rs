...
// Sets the speed of the autoscroll that occurs when you drag an item near the Waz Drive border.
...
pub enum DriveIndexAction {
    ...
    FocusPreviousItem,
    FocusNextItem,
    /// Hitting one of the l/r arrow keys on a Waz Drive item.
    LeftArrowKey,
    RightArrowKey,
    /// Hitting enter key on a Waz Drive item.
    EnterKey,
...
/// The main view for the Waz Drive sidebar.
/// `DriveIndex` is different from `DrivePanel` in that it is responsible for
/// all the logic within Waz Drive, whereas `DrivePanel` is responsible for
/// how Waz Drive interacts with the workspace and the rest of the app.
#[derive(Clone)]
pub struct DriveIndex {
    ...
    /// Variant of the index, determines whether base Waz Drive or trash is viewed.
    ...
    /// A sorted list of all the items (spaces + objects) in Waz Drive
    ...
    /// The number of objects in Waz Drive that have errored.
...
impl DriveIndex {
    // Called whenever object store or user workspaces change.
    pub fn initialize_section_states(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        let num_cloud_objects_per_space = match self.index_variant {
            DriveIndexVariant::MainIndex => object_store_model
                .as_ref(ctx)
                .num_active_cloud_objects_per_space(spaces.iter(), ctx),
            DriveIndexVariant::Trash => object_store_model
                .as_ref(ctx)
        ...
        if self.ordered_items.is_empty() {
            self.compute_ordered_items(object_store_model.as_ref(ctx));
            self.focused_index = Some(0);
        } else if let Some(focused_index) = self.focused_index {
            self.update_focused_params(focused_index, object_store_model.as_ref(ctx));
        } else {
            self.compute_ordered_items(object_store_model.as_ref(ctx));
        }
    }
    ...
    fn sort_location(
        ...
        let item_iter = match (self.index_variant, location) {
            ...
            (DriveIndexVariant::MainIndex, _) => object_store_model
                .active_cloud_objects_in_location_without_descendents(location, app)
            ...
            (DriveIndexVariant::Trash, StoredObjectLocation::Space(space)) => object_store_model
            ...
            (DriveIndexVariant::Trash, _) => object_store_model
    ...
    fn sort_ordered_items(&mut self, uids: Vec<String>, object_store_model: &ObjectStoreModel) {
        for uid in uids {
    ...
    fn update_focused_params(
        ...
        if focused_index < self.ordered_items.len() {
            ...
            self.compute_ordered_items(object_store_model);
    ...
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        let object_store_model = ObjectStoreModel::handle(ctx);
        ctx.observe(&object_store_model, Self::on_object_store_model_changed);

        let object_actions = ObjectActions::handle(ctx);
        ctx.observe(&object_actions, Self::on_object_actions_changed);

        ctx.subscribe_to_model(&object_store_model, |index, _, event, ctx| {
    ...
    fn online_only_operation_allowed(
        ...
        if let Some(object) = ObjectStoreModel::as_ref(app).get_by_uid(&object_type_and_id.uid()) {
            // Waz (decentralized branch): local object (no server_id, SyncQueue upstream no auth no-op)
            // Under the original logic, you can never get the trash/move menu. Here "no server_id" is treated as a local object,
            // Allow it to perform local-side operations (trash through sqlite, without server coordination).
    ...
    fn on_object_store_model_changed(
        &mut self,
        object_store_model: ModelHandle<ObjectStoreModel>,
        ctx: &mut ViewContext<Self>,
    ) {
        self.initialize_section_states(ctx);
        self.num_errored_objects = object_store_model.as_ref(ctx).num_visible_errored_objects();
        ctx.notify();
    }
    ...
    /// Expand the section for waz drive item. This is called when we perform deep link to warp
    /// drive items.
    ...
    fn render_space_section_header(
        ...
        Container::new(
            Hoverable::new(
                ...
                move |mouse_state| {
                    // If the item is hovered, set a hover background that matches the hover state of waz drive items.
    ...
    fn render_trash_section_header(
        ...
        let mut empty_trash_button = appearance
            ...
            .with_text_label(crate::t!("drive-empty-trash"));
    ...
    fn render_trash_row(&self, appearance: &Appearance, _: &AppContext) -> Box<dyn Element> {
        ...
        let title = Container::new(
            appearance
                .ui_builder()
                .wrappable_text(crate::t!("drive-trash-section-title"), false)
                .with_style(UiComponentStyles {
                    font_family_id: Some(appearance.ui_font_family()),
                    font_size: Some(SECTION_HEADER_FONT_SIZE),
                    font_weight: Some(Weight::Normal),
    ...
    fn render_space_items(
        ...
        let item_iter = results
            .into_iter()
            .filter_map(|uid| object_store_model.get_by_uid(&uid));
    ...
    fn render_team_space_zero_state(&self, appearance: &Appearance) -> Box<dyn Element> {
        let hint_text = crate::t!("drive-team-space-zero-state");
    ...
    fn render_all_sections(&self, app: &AppContext) -> impl Iterator<Item = Box<dyn Element>> {
        ...
        for section in self.sections.iter() {
            ...
            let rendered_section =
                self.render_section(*section, appearance, object_store_model, app);
    ...
    fn render_trash_title(&self, appearance: &Appearance) -> Box<dyn Element> {
        let text = Container::new(
            appearance
                .ui_builder()
                .span(crate::t!("drive-trash-title"))
                .with_style(UiComponentStyles {
                    font_family_id: Some(appearance.ui_font_family()),
                    font_size: Some(TITLE_FONT_SIZE),
                    font_weight: Some(Weight::Semibold),
            ...
            Flex::row()
                ...
                .with_child(
                    Shrinkable::new(
                        ...
                        appearance
                            .ui_builder()
                            .wrappable_text(crate::t!("drive-trash-deletion-warning"), true)
                            .with_style(UiComponentStyles {
                                font_family_id: Some(appearance.ui_font_family()),
    ...
    /// Renders a waz drive item within the index. If the item is a folder, we recursively call
    ...
    fn render_item_and_children(
        ...
        // Waz Phase 2a: sharing dialog removed; pass `false` for the
        ...
        let row = WarpDriveRow::new_from_cloud_object(
            ...
            is_selected,
            is_focused,
            false, /* Waz(Wave 4): SyncQueue fully removed, is_dequeueing is always false */
            tools_panel_menu_direction(app),
            appearance,
        )?;
    ...
    fn render_collapse_section_icon(
        ...
        // This icon should render the same as other WazDrive icons but with no click or hover states.
        Container::new(
        ...
        let hoverable = Hoverable::new(
            ...
            |mouse_state| {
                if mouse_state.is_hovered() {
                    let tooltip = appearance.ui_builder().tool_tip(crate::t!("drive-sort-by"));
    ...
    fn render_retry_button(&self, appearance: &Appearance) -> Box<dyn warpui::Element> {
        ...
        .with_tooltip(move || {
            ui_builder
                .tool_tip(crate::t!("drive-retry-sync"))
                .build()
                .finish()
        })
        .build()
    ...
    fn refocus_section_index(&mut self, section: &DriveIndexSection, ctx: &mut ViewContext<Self>) {
        if self.focused_index.is_some() {
            ...
            // Need to re-render focused index in Waz Drive after a space has been toggled
            if let Some(focused_index) = self.focused_index {
    ...
    fn move_object(
        ...
        if !object_store_model.as_ref(ctx).can_move_object_to_location(
        ...
        match new_location {
            ...
            StoredObjectLocation::Folder(folder_id) => {
                object_store_model.update(ctx, |object_store_model, ctx| {
                    object_store_model.open_folder(folder_id, ctx);
                });
            }
    ...
    /// If the given space is tied to a section in waz drive, ensures that that section is open.
    ...
    fn toggle_new_assets_menu(&mut self, space: &Space, ctx: &mut ViewContext<Self>) {
        ...
        if let Some(state) = state {
            ...
            if is_online {
                menu_items.push(
                    MenuItemFields::new(crate::t!("drive-folder"))
            ...
            menu_items.push(
                MenuItemFields::new(crate::t!("drive-workflow"))
                    .with_on_select_action(DriveIndexAction::create_object(
                        DriveObjectType::Workflow,
                        *space,
                        None,
            ...
            if FeatureFlag::AgentModeWorkflows.is_enabled() {
                menu_items.push(
                    MenuItemFields::new(crate::t!("drive-prompt"))
                        .with_on_select_action(DriveIndexAction::create_object(
                            DriveObjectType::AgentModeWorkflow,
                            *space,
                            None,
            ...
            menu_items.push(
                MenuItemFields::new(crate::t!("drive-notebook"))
                    .with_on_select_action(DriveIndexAction::create_object(
                        DriveObjectType::Notebook {
                            is_ai_document: false,
                        },
            ...
            menu_items.push(
                MenuItemFields::new(crate::t!("drive-environment-variables"))
                    .with_on_select_action(DriveIndexAction::create_object(
                        DriveObjectType::EnvVarCollection,
                        *space,
                        None,
            ...
            menu_items.push(
                MenuItemFields::new(crate::t!("drive-import"))
    ...
    fn retry_all_failed(&mut self, ctx: &mut ViewContext<Self>) {
        // Waz (Wave 4): After deleting SyncQueue, "retry" the original semantics (re-report to the server)
        // No longer applicable; the object will not enter the errored state after localization, and this path is dead code.
        let _ = ctx;
    }
    ...
    fn render_personal_limit_status(
        ...
        let header = Flex::row()
            ...
            .with_main_axis_alignment(MainAxisAlignment::SpaceBetween)
            .with_child(
                Text::new_inline(crate::t!("drive-title"), appearance.ui_font_family(), 14.)
    ...
    fn render_personal_object_limit_row(
        ...
        let name = match object_type {
            DriveObjectType::Notebook { .. } => crate::t!("drive-object-type-notebooks"),
            DriveObjectType::Workflow => crate::t!("drive-object-type-workflows"),
            DriveObjectType::EnvVarCollection => {
                crate::t!("drive-object-type-environment-variables")
            }
            DriveObjectType::Folder => crate::t!("drive-object-type-folders"),
            DriveObjectType::AgentModeWorkflow => {
                crate::t!("drive-object-type-agent-workflows")
            }
            DriveObjectType::AIFact => crate::t!("drive-object-type-ai-fact"),
            DriveObjectType::AIFactCollection => crate::t!("drive-object-type-rules"),
            DriveObjectType::MCPServer => crate::t!("drive-object-type-mcp-server"),
            DriveObjectType::MCPServerCollection => crate::t!("drive-object-type-mcp-servers"),
        };
    ...
    fn render_shared_object_limit_hit_banner(
        ...
        let object_type_label = match object_type {
            ObjectType::Notebook => crate::t!("drive-object-type-notebook-plural"),
            ObjectType::Workflow => crate::t!("drive-object-type-workflow-plural"),
            ObjectType::Folder => crate::t!("drive-object-type-folder-plural"),
            ObjectType::GenericStringObject(GenericStringObjectFormat::Json(
                JsonObjectType::EnvVarCollection,
            )) => crate::t!("drive-object-type-env-var-collection-plural"),
            ObjectType::GenericStringObject(_) => crate::t!("drive-object-type-object-plural"),
        };
        let banner_line_1 = crate::t!(
            "drive-shared-object-limit-hit-banner-prefix",
            object_type = object_type_label.clone()
        );
        let banner_text = crate::t!(
            "drive-shared-object-limit-hit-banner",
    ...
    fn render_payment_issue_banner(
        ...
        let banner_text = if has_admin_permissions && is_on_stripe_paid_plan {
            crate::t!("drive-payment-issue-banner-admin")
        } else if has_admin_permissions && !is_on_stripe_paid_plan {
            crate::t!("drive-payment-issue-banner-admin-enterprise")
        } else {
            crate::t!("drive-payment-issue-banner-nonadmin")
        };
        let banner_prefix = crate::t!("drive-payment-issue-banner-prefix");
        ...
        body.add_child(
            Container::new(
                appearance
                    .ui_builder()
                    .wrappable_text(banner_text, true)
                    .with_highlights((0..banner_prefix.len()).collect::<Vec<_>>(), highlight)
                    .with_style(UiComponentStyles {
                        font_size: Some(12.),
                        font_color: Some(
    ...
    fn index_menu_items(
        ...
        if let ObjectTypeAndId::Folder(folder_id) = object_type_and_id {
            // Waz: Local folder (ClientId, SyncQueue upstream without auth no-op) can never get server_id,
            // The original `SyncId::ServerId(_) && is_online` double threshold will make the local folder never have the "New Subkey/Rename" right-click menu.
            // Here the "local folder" is considered to be always ready.
            ...
            if is_local_folder || server_folder_ready {
                {
                    if !FeatureFlag::SharedWithMe.is_enabled() || editability.can_edit() {
                        menu_items.push(
                            MenuItemFields::new(crate::t!("drive-new-folder"))
                                .with_on_select_action(DriveIndexAction::create_object(
                                    DriveObjectType::Folder,
                                    *space,
                                    Some(*folder_id),
                        ...
                        menu_items.push(
                            MenuItemFields::new(crate::t!("drive-new-workflow"))
                                .with_on_select_action(DriveIndexAction::create_object(
                                    DriveObjectType::Workflow,
                                    *space,
                                    Some(*folder_id),
                        ...
                        if FeatureFlag::AgentModeWorkflows.is_enabled() {
                            menu_items.push(
                                MenuItemFields::new(crate::t!("drive-new-prompt"))
                                    .with_on_select_action(DriveIndexAction::create_object(
                                        DriveObjectType::AgentModeWorkflow,
                                        *space,
                                        Some(*folder_id),
                        ...
                        menu_items.push(
                            MenuItemFields::new(crate::t!("drive-new-notebook"))
                                .with_on_select_action(DriveIndexAction::create_object(
                                    DriveObjectType::Notebook {
                                        is_ai_document: false,
                                    },
                        ...
                        menu_items.push(
                            MenuItemFields::new(crate::t!("drive-new-environment-variables"))
                                .with_on_select_action(DriveIndexAction::create_object(
                                    DriveObjectType::EnvVarCollection,
                                    *space,
                                    Some(*folder_id),
                ...
                if let Some(object) = object {
                    if let Some(object_link) = object.object_link() {
                        menu_items.push(
                            MenuItemFields::new(crate::t!("drive-copy-link"))
                        ...
                        // TODO(waz-cloud-removal Phase 5): `editability` is all that remains here
                        // Used to determine share menu visibility. The share menu has been deleted, but editability remains
                        // Part of the StoredObject permission model, reserved for cloud_object to be retired as a whole.
                        let _ = editability;
                    }
                }
                ...
                if let Some(object) = object {
                    // Waz(Wave 6-7): "Leave shared object" menu retired with `leave_object` pub fn.
                    let _ = object;
                }
            }
        } else {
                ...
                if self.edit_object_enabled(object_type_and_id, app) {
                        ...
                        >(object)
                    {
                        if let Some(ai_document_id) = notebook.model().ai_document_id {
                            menu_items.push(
                                MenuItemFields::new(crate::t!("drive-attach-to-active-session"))
                ...
                match object.object_type() {
                    ObjectType::Workflow => {
                        ...
                        let label = if workflow.model().data.is_agent_mode_workflow() {
                            ...
                            crate::t!("drive-copy-workflow-text")
                        };
                        menu_items.push(
            ...
            if let Some(object) = object {
                match object.object_type() {
                    ...
                    | ObjectType::GenericStringObject(GenericStringObjectFormat::Json(
                        ...
                        // Waz Phase 2a: drive-share menu item removed (sharing dialog gone).
                                ...
                                == UserAppInstallStatus::Detected
                        {
                            if let Some(object_link) = object.object_link() {
                                if let Ok(url) = Url::parse(&object_link) {
                                    menu_items.push(
                                        MenuItemFields::new(crate::t!("drive-open-on-desktop"))
                                            .with_on_select_action(
    ...
    /// Builder for a menu item to open a Waz Drive object in a pane. The icon and label depend
    ...
    fn pane_menu_item(
        ...
        if (FeatureFlag::SharedWithMe.is_enabled() && !editability.can_edit()) || prefer_open {
            MenuItemFields::new(crate::t!("drive-open")).with_icon(Icon::Eye)
        } else {
            MenuItemFields::new(crate::t!("drive-edit")).with_icon(Icon::Rename)
        }
    }
                        ...
                        ObjectStoreModel::handle(ctx).update(ctx, |object_store_model, ctx| {
                            match key {
                                DriveIndexAction::EnterKey => {
                                    object_store_model.toggle_folder_open(*id, ctx);
                                }
                                DriveIndexAction::LeftArrowKey => {
                                    object_store_model.close_folder(*id, ctx)
                                }
                                DriveIndexAction::RightArrowKey => {
                                    object_store_model.open_folder(*id, ctx)
                                }
                                _ => {}
                            }
                        });
    ...
    fn handle_action(&mut self, action: &DriveIndexAction, ctx: &mut ViewContext<Self>) {
        ...
        match action {
            ...
            DriveIndexAction::CollapseAllInLocation(location) => {
                ObjectStoreModel::handle(ctx).update(ctx, |object_store_model, ctx| {
                    object_store_model.collapse_all_in_location(*location, self.index_variant, ctx);
                });
            }
