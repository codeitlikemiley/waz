...
pub struct EditorModal {
    ...
    /// Dropdown to select the separator for the Waz prompt, in the case of
    /// same line prompt. This separator is added at the end of the Waz prompt.
    warp_prompt_separator_dropdown: ViewHandle<Dropdown<EditorModalAction>>,
    /// The separator currently selected for the Waz prompt.
...
impl EditorModal {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        let warp_prompt_separator = match SessionSettings::as_ref(ctx).saved_prompt.value() {
            ...
            // If the "default Waz prompt" i.e. no context chips, is selected, then default to no Waz prompt separator.
    ...
    /// Updates the state of the Waz prompt separator dropdown to be enabled/disabled based on the current state of the modal.
    fn update_warp_separator_dropdown_state(&mut self, ctx: &mut ViewContext<Self>) {
        // If we are using the Waz prompt and SLP is enabled, then we enable the dropdown. Otherwise, disable it.
    ...
    fn save_prompt_to_settings(&mut self, ctx: &mut ViewContext<Self>) {
        if self.is_dirty {
            match self.prompt_type {
                PromptType::PS1 => {
                    // TODO: we need to stop the Waz prompt generators from running at this point
            ...
            let prompt_info = match self.prompt_type {
                ...
                PromptType::Waz => PromptChoice::Custom {
                    builtin_chips: self
                        .chip_configurator
                        .used_chips
                        .iter()
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            Self::Action::Chip(chip_action) => {
                ...
                if mutated {
                    self.is_dirty = true;
                    self.prompt_type = PromptType::Waz;
                }
                ctx.notify();
            }
            Self::Action::UsePS1 => {
                ...
                // Disable the Waz separator dropdown (only applies to Waz prompt).
            ...
            Self::Action::UseWarpPrompt => {
                ...
                // Enable the Waz separator dropdown, if SLP is on.
                self.update_warp_separator_dropdown_state(ctx);
                ctx.notify();
            }
            Self::Action::ResetWarpPrompt => {
                ...
                // Disable the Waz separator dropdown, since SLP is off for the default Waz prompt.
                self.update_warp_separator_dropdown_state(ctx);
            ...
            Self::Action::ToggleSameLinePrompt => {
                ...
                // In case we had previously picked default Waz prompt, but now the user toggled
...
impl EditorModal {
    ...
    // TODO: consider supporting SLP with the new Waz prompt.
    #[allow(dead_code)]
    ...
    fn render_warp_prompt_section(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        self.render_prompt_section(
            appearance,
            matches!(self.prompt_type, PromptType::Waz | PromptType::WarpDefault),
            header_row,
            None,
            body,
            self.mouse_state_handles
    ...
    fn render_buttons(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        // - the Waz prompt is used but there are no chips selected
        let save_disabled = !self.is_dirty
            || (matches!(self.prompt_type, PromptType::Waz)
