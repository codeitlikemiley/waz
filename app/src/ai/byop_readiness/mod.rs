//! Tool call readiness classification before BYOP request is sent.
//!
//! This module only processes the projected security metadata and does not read the original prompt, tool parameters or tool output.
//! Nor does it modify controller, serializer, or conversation state.
...
pub const BLOCKED_BYOP_REQUEST_MESSAGE: &str =
    "Can't continue this conversation: an earlier tool result is missing or corrupted in this conversation's history, so Waz can't safely send the request to your provider. Start a new conversation or fork from an earlier point to continue.";
...
pub enum RepairSource {
    /// When the user derives a new conversation from the source conversation through fork (/fork, rewind),
    /// The repair source documented by `byop_fork_repair_state_json` for discarded tool results.
    ForkedHistory,
    /// Fix sources for tool result gaps logged on restore that were written by legacy clients but cannot be played back by the current protocol.
    /// See `specs/byop-placeholder-tool-results/ISSUES.md` BYOP-PR-6: This variant is not currently generated in the recovery path,
    /// Unexplained gaps in old BYOP conversations are blocked by corrupted history by default.
    /// **Do not delete this variant at will**: `restored_legacy_history` may be deserialized in the persistence sidecar,
    /// Removal will break sidecar format compatibility.
...
impl<'a> Classifier<'a> {
    ...
    fn handle_tool_result(&mut self, result: ProjectedToolResult) -> Option<ReadinessState> {
        // Divide the judgment of "whether it belongs to the current active group" and "modify the active_call status" into two steps.
        // First pass the immutable borrowing filter, and if it fails, go directly to the unattached path.
        // Obtain variable borrowing after passing it to avoid expect/unwrap appearing on the production path.
    ...
    fn mark_stale_repair_records_for_visible_gap(&mut self, key: &ToolCallKey) {
        // The authorization semantics of Repair record require task_id + assistant_tool_call_message_id + tool_call_id
        // The three fields are completely equal; but when there is a real visible gap (that is, the same `tool_call_id` has been clearly marked as missing by the current conversation history),
        // Any repair record with "same tool_call_id but from other (task_id, assistant_message_id)" is no longer applicable,
        // Mark it as stale so that the upper layer will prompt in diagnostics that the record has been ignored.
