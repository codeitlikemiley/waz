...
pub struct AgentConversationsModel {
    ...
    /// Waz: No polling after localization; used only as a placeholder record for register_view_open/closed.
...
impl AgentConversationsModel {
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        // Waz (localization, Phase 3b-1 / Wave 6-6): AgentConversationsModel was originally responsible for polling/sniffing
        // remote ambient agent tasks and conversation metadata. Under the localized scenario:
        //   - No polling subsystem (physically deleted in Wave 6-6)
        //   - `has_finished_initial_load` is directly set to true, making UI queries return empty sets
        // BYOP agent local execution does not depend on this model.
        //
        // Issue #93 fix: Must subscribe to events from `BlocklistAIHistoryModel`, otherwise when the user deletes a conversation
        // from the history conversation list, the cached conversations in this model will not refresh, and the UI will continue to show deleted items.
    ...
    /// Read locally cached task data by task ID.
    ///
    /// Waz no longer fetches ambient agent tasks from the cloud. If the caller restores the old layout but the local model does not
    /// have the corresponding task, this returns `None`, which is handled by the existing panel degradation path.
