...
impl RuleEditorView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        // Waz (localization, Phase 2d-1): The original NetworkStatus subscription was used to redraw on online/offline status changes
        // (linked with the `is_delete_allowed` predicate). After localization, this predicate is always true, the subscription is dead code, and has been removed.
...
impl View for RuleEditorView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // Waz (localization, Phase 2d-1): The original `is_delete_allowed` depended on network online status + server_id.
        // After localization, a rule can be deleted as long as it is an existing rule being edited, and the predicate is removed.
