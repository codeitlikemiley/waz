...
// Retain English "Rules" for the top title (user preference, not translated to Knowledge Base).
...
impl RuleView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        // Waz (localization, Phase 2d-1): The original UpdateManager subscription was used to receive cloud creation/update ack
        // events, as well as panel redrawing driven by network status. After localization, ObjectStoreEvent already covers the
        // UI refresh requirements after local writes (sent in update_object/create_object in 2c-2/2c-3), so UpdateManager and
        // NetworkStatus subscriptions are dead code and removed altogether.
    ...
    fn render_description(&self, appearance: &Appearance) -> Box<dyn Element> {
        Container::new(
            appearance
                .ui_builder()
                .wrappable_text(crate::t!("rules-description"), true)
    ...
    fn render_scope_tabs(&self, appearance: &Appearance) -> Box<dyn Element> {
        let global_tab_label = crate::t!("rules-tab-global");
        let project_tab_label = crate::t!("rules-tab-project");
    ...
    fn render_disabled_banner(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let formatted_text = FormattedTextElement::new(
            FormattedText::new([FormattedTextLine::Line(vec![
                FormattedTextFragment::bold(banner_prefix),
                link,
                FormattedTextFragment::bold(banner_suffix),
            ])]),
    ...
    // Waz (localization, Phase 2d-1): The original `render_sync_status_icon` depended on `SyncQueue::is_dequeueing()`
    // and the `is_syncing` predicate. After localization, a "syncing" state will never occur, so it is entirely removed.
    ...
    fn render_project_based_row(
        ...
        row.add_child(
            appearance
                ...
                .with_text_label(crate::t!("common-open-file"))
                .build()
                .on_click(move |ctx, _, _| {
    ...
    fn render_global_rule_row(
        ...
        // Waz (localization, Phase 2d-1): The original `is_edit_allowed` depended on both network online status and server_id.
        // After localization, rules are always editable, and the click action is unconditionally attached.
        hoverable
            .with_cursor(Cursor::PointingHand)
            .on_click(move |ctx, _, _| {
    ...
    fn render_zero_state(&self, appearance: &Appearance) -> Box<dyn Element> {
        let text = match self.current_scope {
            RuleScope::Global => crate::t!("rules-zero-state-global"),
            RuleScope::ProjectBased => crate::t!("rules-zero-state-project"),
        };
