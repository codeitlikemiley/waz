//! Active AI model output parsing.
//!
//! Each sub-pipeline requires the model to reply in a specific format:
//! - prompt_suggestions: JSON `{"kind","query","should_plan_task"|"files"}`
//! - nld_predict: Plain text single line
//! - relevant_files: JSON `{"paths":[...]}`
//!
//! Models do not always adhere to the format cleanly — this is responsible for stripping fences, fault-tolerant parsing, failure → `None` / empty.
...
fn strip_code_fence(raw: &str) -> &str {
    ...
    if let Some(rest) = trimmed.strip_prefix("```") {
        // Skip possible language identifier (json/JSON/javascript/...) up to the newline
...
enum SuggestionDto {
    /// The model explicitly indicates "insufficient signal, no suggestion". Distinguishes from "parsing failure":
    /// the latter is mapped to `AgentModePromptSuggestion::Error` by the upper layer, while the former goes to
    /// `AgentModePromptSuggestion::None` cleanly without rendering a chip.
    None,
    Simple {
        query: String,
        #[serde(default)]
...
/// Failure → `None`, mapped to `AgentModePromptSuggestion::Error` by the caller.
/// `kind:"none"` → `Some(... suggestion: None)`, mapped to `AgentModePromptSuggestion::None` by the upper layer.
...
/// Trims, strips external quotes, rejects multiline / overly long → `None`.
pub fn sanitize_predict(raw: &str) -> Option<String> {
    ...
    if s.contains('\n') {
        // Take the first line (the model sometimes explains too much)
...
/// A pure DTO for workflow_metadata sub-pipeline (avoids this module depending on higher-level drive::workflows types).
#[derive(Debug, Clone)]
...
/// Failure / empty `command` → `None` (mapped to BadCommand by the caller).
...
/// Parses the JSON output of relevant_files, taking the intersection with input paths to filter out hallucinations.
...
mod tests {
    ...
    fn parse_none_kind_returns_empty_suggestion() {
        // `kind:"none"` means the model chooses to remain silent — must return `Some(suggestion: None)`,
        // cannot return `None` (the latter would be mapped to Error by the upper layer).
