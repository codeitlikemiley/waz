...
fn file_binary_content_self_closing_with_size() {
    ...
    let out = render_user_attachments(&[AIAgentContext::File(f)]).unwrap();
    // The new version of placeholder: path + mime_type + binary + size, both are indispensable.
...
fn file_binary_empty_omits_size_attr() {
    // The upper layer deliberately did not read the bytes (.exe / very large file) → the size attribute should be omitted, but the path / mime / binary must be present
    let f = FileContext::new(
    ...
    // Not strong assert specific value, only verify the existence of mime_type attribute
