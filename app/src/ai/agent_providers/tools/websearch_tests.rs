//! `web_runtime::run_websearch` Single test (mockito, no external network).
...
async fn passes_api_key_via_querystring() {
    // Do not directly verify mockito's querystring (because we use endpoint_override,
    // And endpoint_override has replaced endpoint_url).
    // Separately verify api_key through endpoint_url splicing.
...
// ---------------------------------------------------------------------------
// SearchToolArgs → SearchArgs populated by default
// ---------------------------------------------------------------------------

// ---------------------------------------------------------------------------
// Real endpoint smoke test (enabled by default; set WARP_SKIP_WEB_INTEGRATION=1 when CI network is limited)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Description document / opencode byte-level alignment + {{year}} placeholder regression
// ---------------------------------------------------------------------------

/// websearch.md must contain the `{{year}}` placeholder — `chat_stream::build_tools_array`
/// Will be replaced with the current year when building (aligned with opencode `websearch.ts:30-32`). Delete placeholder
/// Will cause the model to do time-sensitive searches using old years in the training data.
#[test]
fn websearch_description_contains_year_placeholder() {
    use super::super::websearch::WEBSEARCH;
    assert!(
        WEBSEARCH.description.contains("{{year}}"),
        "websearch description must contain {{{{year}}}} placeholder, which will be replaced during build"
    );
}

/// Lock websearch.md and opencode `packages/opencode/src/tool/websearch.txt`
...
/// `_byop_intercepted` sentinel must exist in search result (same as webfetch),
/// Let the controller know to trigger auto-resume, otherwise the model will get stuck and other consequences.
#[test]
