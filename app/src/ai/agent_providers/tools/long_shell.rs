//! Interactive tool for long-running shell commands:
//! - `write_to_long_running_shell_command`: write stdin/PTY to a running command
//! - `read_shell_command_output`: Take a snapshot of the current output of a command that is still running
//!
//! The `command_id` of these two tools comes from the initial snapshot of `run_shell_command`
//! (`LongRunningShellCommandSnapshot.command_id`). The model needs to see a
//! Long run the snapshot of the shell to get the ID.
...
fn write_parameters() -> Value {
    json!({
        ...
        "properties": {
            "command_id": {
                "type": "string",
                "description": "The command ID of the long-running command returned by run_shell_command previously."
            },
            "input": {
                "type": "string",
                "description": "The text/command to write to stdin/PTY. When mode=raw, control key tokens like <ESC>/<ENTER>/<CTRL-C> can be used."
            },
            "mode": {
                "type": "string",
                "enum": ["raw", "line", "block"],
                "description": "raw = raw bytes; line = as a single line (automatically appends newline); block = as a multi-line block.",
                "default": "line"
            }
        },
...
fn write_result_to_json(result: &api::message::tool_call_result::Result) -> Option<Value> {
    ...
    let value = match &r.result {
        ...
        // ShellCommandError now only has one variant: BlockNotFound
...
struct ReadArgs {
    command_id: String,
    /// "on_completion" (default) or number (seconds → Duration)
    #[serde(default)]
...
fn read_parameters() -> Value {
    json!({
        ...
        "properties": {
            "command_id": {
                "type": "string",
                "description": "The ID of the currently running command."
            },
            "delay_seconds": {
                "type": "integer",
                "description": "Optional: return the current output snapshot after the specified number of seconds; if omitted, waits until the command completes before returning.",
                "minimum": 0
            }
        },
        "required": ["command_id"],
