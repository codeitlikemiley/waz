...
pub const SERVER_OVERLOADED_TASK_FAILURE_MESSAGE: &str =
    "Waz is temporarily overloaded. Please try again shortly.";

/// JSON payload for starting an agent run. In Waz this is only used by local UI/CLI
...
impl AmbientAgentTaskId {
    /// Waz (localization, Phase 3b-4): Locally generate a UUID v4 as task_id to avoid local
    /// Harness relies on the remote pre-created task interface when starting a subtask.
    pub fn new_local() -> Self {
        let uuid = Uuid::new_v4();
        // UUID v4 is almost impossible to generate nil (probability ~ 1/2^122), and expect is used to indicate logical unreachability.
        let non_nil =
