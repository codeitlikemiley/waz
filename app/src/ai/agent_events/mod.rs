...
/// Entry point for the local agent event stream. Waz retains the interface to support local driver injection, with the default implementation disabling cloud RTC.
...
impl AgentEventStreamClient for DisabledAgentEventStreamClient {
    async fn stream_agent_events(
        ...
        Err(anyhow!(
            "Agent event stream disabled in Waz - RTC endpoint is removed"
        ))
    }
}
