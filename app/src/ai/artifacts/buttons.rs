...
pub enum ArtifactButtonsRowEvent {
    /// openWarp localization: Click the plan button to get the local AIDocumentId, no longer relying on the cloud notebook image.
    OpenPlan {
...
fn collect_buttons(
    ...
    for artifact in artifacts {
        match artifact {
            Artifact::Plan {
                title,
                notebook_uid: _, // openWarp no longer relies on cloud notebook_uid; use local document_uid
                document_uid,
            } => {
                // openWarp localization: as long as the local AIDocumentId can be parsed, the button will be displayed.
                // Click to open the local AIDocument pane; no longer rely on the cloud notebook image.
            ...
            | Artifact::File {
                ...
                // Waz no longer has cloud artifact storage, so file and screenshot
