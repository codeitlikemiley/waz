...
pub(crate) fn home_config_file_path(provider: MCPProvider) -> Option<PathBuf> {
    match provider {
        MCPProvider::Waz => warp_core::paths::warp_home_mcp_config_file_path(),
...
impl MCPProvider {
    ...
    pub fn home_config_path(&self) -> &'static Path {
        match self {
            MCPProvider::Waz => Path::new(".warp/.mcp.json"),
