...
/// updates to Waz's secure storage via a channel.
///
...
/// runtime token auto-refreshes are written back to Waz's secure storage.
///
...
pub async fn make_authenticated_client(
    ...
    oauth_state
        .start_authorization(&[], &redirect_uri, Some("Waz"))
        .await?;
    ...
    let config = match auth_manager.register_client("Waz", &redirect_uri).await {
        Ok(config) => config,
