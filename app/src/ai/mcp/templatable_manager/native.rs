...
/// An MCP server integration that Waz ships with bundled skills for.
...
impl TemplatableMCPServerManager {
    ...
    fn spawn_server_impl(
        ...
        if let TransportType::CLIServer(cli_server) = &mut server.transport_type {
            ...
            // continue to inherit Waz's process cwd.
...
async fn spawn_server(
    ...
    let service = match transport_type {
        TransportType::CLIServer(cli_server) => {
            ...
            // rmcp::transport::TokioChildProcess::builder strictly requires tokio::process::Command,
            // which cannot be replaced with the command::r#async::Command wrapper; instead, manually set CREATE_NO_WINDOW below
            // to keep Windows from flashing windows.
            ...
            .map_err(|err| {
                if err.kind() == std::io::ErrorKind::NotFound {
                    let cwd_display = cwd_for_log
                        .as_deref()
                        .unwrap_or("<inherited from Waz's process cwd>");
                    logger.log(format!(
