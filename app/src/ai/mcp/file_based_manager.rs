...
impl FileBasedMCPManager {
    ...
    /// - For `MCPProvider::Waz`: the logical root for `~/.warp*/.mcp.json`.
        ...
        self.file_based_servers_by_root
            ...
            .any(|(root_path, provider_map)| {
                provider_map.iter().any(|(provider, hashes)| {
                    ...
                    match provider {
                        MCPProvider::Waz => Self::is_global_warp_root(root_path),
    ...
    /// Waz config (`~/.warp/.mcp.json`). Global Waz servers always auto-spawn.
    ...
    fn spawn_file_based_servers(
        ...
        // - Global Waz: always auto-spawn.
        // - Global non-Waz: auto-spawn iff the toggle is on.
    ...
    fn handle_file_based_mcp_enabled_change(&mut self, ctx: &mut ModelContext<Self>) {
        ...
        // - Global Waz servers always spawn regardless of the toggle.
        ...
        if !AISettings::as_ref(ctx).is_file_based_mcp_enabled(ctx) {
            ...
            // Toggle on: spawn global third-party servers (global Waz servers are
    ...
    /// exception: global Waz installs are discovered in `~/.warp*/`, which
    ...
    pub fn spawn_root_for_installation(&self, uuid: Uuid) -> Option<PathBuf> {
        ...
        // Global Waz installs live under `~/.warp*/`, which is internal Waz
        // state rather than a meaningful working directory. Map them to the
        // home dir so all global installs (Waz and third-party) share a
        // consistent cwd.
