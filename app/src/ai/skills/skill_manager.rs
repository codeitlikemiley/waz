...
pub struct SkillManager {
    ...
    /// Skills bundled into Waz, each with activation condition and icon.
...
impl SkillManager {
    ...
    ///
    /// For BYOP `read_skill` tool: the model only sees `<name>` in the system prompt,
    /// The absolute path of SKILL.md is not known, so name → ParsedSkill parsing must be supported.
    ///
    /// When there are multiple copies of the same name, the first one is selected in ascending order by [`provider_rank`] (`Agents > Waz > Claude > …`),
    /// Consistent with `unique_skills`/`list_skill_inventory` priority. Bundle skill
    /// Without entering the `skills_by_name` index, we will traverse the details here alone.
    pub fn find_skill_by_name(&self, name: &str) -> Option<&ParsedSkill> {
        // Prefer filesystem skills: Choose the best one based on provider_rank from multiple copies with the same name.
        ...
        // Fallback: bundled skills (hit by name instead of id).
        self.bundled_skills
            .values()
    ...
    /// Load skill definitions bundled with Waz.
...
///
/// Supported variables:
/// - `{{warp_server_url}}` - Empty in Waz; retained for bundled skill compatibility.
...
/// branded icon; everything else falls back to the Waz logo.
