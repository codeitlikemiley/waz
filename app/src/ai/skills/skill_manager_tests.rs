...
// ============================================================================
// Tests for find_skill_by_name (BYOP read_skill tool checks ParsedSkill by name)
// ============================================================================
...
fn find_skill_by_name_prefers_higher_priority_provider() {
    // When a skill with the same name is registered in both .agents and .claude, find_skill_by_name should be
    // SKILL_PROVIDER_DEFINITIONS returns the copy of .agents(rank 0) in sequence.
    App::test((), |mut app| async move {
        app.add_singleton_model(DirectoryWatcher::new);
        app.add_singleton_model(|_| DetectedRepositories::default());
        app.add_singleton_model(RepoMetadataModel::new);
...
fn find_skill_by_name_returns_none_for_unknown_name() {
    App::test((), |mut app| async move {
        ...
        // To register a skill, make sure the manager index is not empty — otherwise a `mut app` warning will be
        // unused_mut triggers (no other manager operations will modify the app itself).
