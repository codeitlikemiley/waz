...
//!
//! Don't "optimize" template literals - this is a mutually portable contract with opencode, and any modifications require bi-directional synchronization.
...
/// Pin final user prompt — align `compaction.ts:121-132 buildPrompt`.
///
/// `previous_summary = Some(...)` → Take the "update" branch and use the existing summary as the `<previous-summary>` anchor;
/// `None` → Take the "new" branch. `context` comes from plugin hook (local implementation is temporarily empty vec).
...
/// Synthesize user "Continue..." synthetic message under `replay=false` + `auto=true` path —
/// Byte-level alignment `compaction.ts:533-537`.
///
/// When `overflow=true` is used, an additional "previous request exceeded ... attachments were too large" explanation will be spelled out in front.
