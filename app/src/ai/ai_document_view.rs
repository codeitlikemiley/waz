...
impl AIDocumentView {
    pub fn new(
        ...
        // Create sync button mouse state (for Waz Drive syncing)
    ...
    fn update_header_buttons(&mut self, ctx: &mut ViewContext<Self>) {
        // openWarp localization: plan is no longer synchronized to the cloud notebook, originally from sync_id
        // Get the server_id and pass it to the sharing UI; the sync_id has been deleted, and only the header overflow menu is resent here.
        self.pane_configuration.update(ctx, |pc, ctx| {
    ...
    fn render_sync_element(
        ...
        match save_status {
            // openWarp does not use cloud synchronization, Plan has been automatically saved to local SQLite,
            // The document is always Saved when it exists; a static "Local Saved" icon appears here.
            AIDocumentSaveStatus::Saved => {
                let appearance = Appearance::as_ref(app);
                let theme = appearance.theme();
                let color = theme.nonactive_ui_detail().into_solid();
                let ui_builder = appearance.ui_builder().clone();
                let tooltip_text = "Plan has been automatically saved locally.".to_string();
            ...
            // NotSaved in openWarp only means that the document does not exist (calling side placeholder), without drawing any UI.
    ...
    fn create_warp_drive_notebook(&self, _ctx: &mut ViewContext<Self>) {
        // openWarp localization: plan is no longer pushed to the cloud notebook, and this entry in the menu has been removed.
        // Only keep the enum variant to prevent vicious deserialization of old keybinding configurations; no-op here.
    ...
    fn handle_action(&mut self, action: &AIDocumentAction, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            AIDocumentAction::ShowInWarpDrive => {
                // openWarp localization: plan is no longer synchronized to Cloud Drive, originally sent to ViewInWarpDrive
                // The event jump to the cloud notebook path is completely unreachable. Keep only enum variants for anti-aging
                // Keybinding deserialization crashes, no-op here.
...
impl BackingView for AIDocumentView {
    ...
    fn pane_header_overflow_menu_items(
        ...
        // openWarp localization: cloud menu item "Copy Link" / "Show in Waz Drive"
        // Originally, it was only displayed after the cloud synchronization was successful. The local path was completely unreachable and was deleted directly.
