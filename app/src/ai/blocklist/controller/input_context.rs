...
lazy_static! {
    ...
    // Regex to match waz drive objects inserted via at-context. Ex: <notebook:[workflow_id]>
...
pub(super) fn input_context_for_request(
    ...
    // Reserved parameters: The signatures of multiple upper-level callers are temporarily unchanged; list_skills no longer requires session id deduplication.
    // But other potential uses may still require → leaving the _ prefix to avoid unused warnings.
    ...
    if FeatureFlag::ListSkills.is_enabled() {
        // After the project goes to the cloud, the system prompt is completely re-rendered on the client in each round (BYOP stateless).
        // Skills must be delivered in full each round, no difference will be made. Do not push when the list is empty, keep
        // The context is compact (the `{% if skills %}` guard on the template side can omit the section normally).
...
/// Get the object payload by UID and type from the ObjectStoreModel.
