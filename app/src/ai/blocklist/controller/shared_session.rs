// BlocklistAIController's shared session local display logic.
...
impl BlocklistAIController {
    ...
    fn on_shared_init(
        ...
        // If the shared session conversation token is already bound to the local conversation, it is reused directly.
        // Otherwise, when the current agent view conversation is empty, the local conversation ID is reused and the incoming shared token is bound.
        // This preserves the visibility of the created terminal block in the current agent view.
    ...
    fn on_shared_finished(
        ...
        self.handle_response_stream_finished(
            ...
            conversation_id,
            did_exchange_contain_user_query,
            // Shared session paths will not trigger local compression (synchronous streams from remote viewers), always None
            None,
            ctx,
        );
    }

    /// Find the local conversation corresponding to the shared conversation token.
    ...
    pub fn link_forked_conversation_token(
        ...
        // Get the new shared session conversation id from the StreamInit event.
