...
fn detects_interactive_session_commands_across_platforms() {
    for command in [
        ...
        " warp_run_generator_command 42 'ssh host'",
        "Waz-Run-GeneratorCommand 42 'ssh host' -ErrorAction Ignore",
...
fn does_not_detect_unrelated_or_non_interactive_ssh_commands() {
    for command in [
        ...
        "Waz-Run-GeneratorCommand 42 'git status' -ErrorAction Ignore",
        "rsync myfile.txt ssh://user@server.com",
        // Text is stuck to the closing quote, intentionally refuse to tokenize to avoid splitting into `ssh`
        // and then incorrectly judging it as an interactive session via `ssh hello-world`.
        r#""ssh"hello-world"#,
        // Unclosed quotes also refuse to tokenize.
        r#""ssh hello world"#,
    ] {
        assert_eq!(
            command_starts_non_terminating_session(command),
...
fn preemption_logic_covers_until_completion_timeout() {
    ...
    // BlockFinished is never preempted — it is the signal that the "command is truly finished".
    ...
    // ForceRefresh is always preempted, regardless of the delay.
    assert!(compute_is_preempted(ForceRefresh, UntilCompletion));
    assert!(compute_is_preempted(ForceRefresh, Default));

    // Timeout + OnCompletion / UntilCompletion is preempted.
    assert!(compute_is_preempted(
        Timeout,
        OnCompletion {
            timeout: Duration::from_secs(1)
    ...
    // #138: Pager hang fallback timeout must be marked as preempted to prevent the server from misinterpreting it as "command complete".
    assert!(compute_is_preempted(Timeout, UntilCompletion));

    // Timeout + Default / Duration is not preempted — the agent expects to get an intermediate snapshot.
