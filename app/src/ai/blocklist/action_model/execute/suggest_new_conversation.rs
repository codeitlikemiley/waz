...
impl SuggestNewConversationExecutor {
    ...
    pub(super) fn execute(
        ...
        // Waz: No "Open new conversation?" confirmation modal view component, call fast-fail Reject to prevent
        // the BYOP loop from hanging indefinitely waiting for the receiver. The model only needs to see Rejected, which doesn't affect the main flow.
        // Synchronously send a Reject to the channel once, which the receiver.recv() will get immediately.
        let _ = self
            .suggest_new_conversation_result_rx
            .0
            .try_send(NewConversationDecision::Reject);
