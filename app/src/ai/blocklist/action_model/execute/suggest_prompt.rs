...
impl PromptSuggestionExecutor {
    ...
    pub(super) fn execute(
        ...
        // Waz: Emit unconditionally (the original was gated under the PromptSuggestionsViaMAA cargo feature,
        // which OSS does not enable by default -> chip is never shown -> oneshot hangs forever). When the BYOP scenario model
        // actively calls suggest_prompt, the view layer needs to subscribe to this event to display the chip to the user,
        // and then call complete_suggest_prompt_action to close the channel when accepted. See details in
