...
impl PassiveSuggestionsModel {
    ...
    fn generate_prompt_suggestions(
        ...
        // BYOP path: Replace ServerApi calls with BYOP one-shot completion.
        // Waz has been stripped of Waz Inc cloud, silent no-op without BYOP configuration.
...
/// BYOP path prompt_suggestions request construction: extract block information + system context,
/// Delegate to `active_ai::prompt_suggestions::dispatch` to render the prompt and resolve BYOP configurations.
/// Returning `None` means there is no BYOP configuration or block content is lost, and the caller silently no-ops.
