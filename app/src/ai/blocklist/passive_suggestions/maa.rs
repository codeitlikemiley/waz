...
impl PassiveSuggestionsModel {
    ...
    fn send_request(
        ...
        log::debug!(
            "[passive-suggestions] skipped MAA request because the multi-agent endpoint is disabled in Waz"
        );
    ...
    fn handle_finished_stream(
        ...
        // Waz: Temporarily close passive suggestions cloud requests triggered by AgentResponseCompleted,
        // Avoid sending a failed HTTP to ${server_root_url}/ai/multi-agent after each round of Agent reply.
        // (No cloud auth + ShowSuggestions proto action BYOP cannot be synthesized).
        // TODO(byop-suggestions): If you want to reproduce the "Suggested Workflow / Rule" chip under BYOP later,
        // Need to add an independent BYOP one-shot link: mainstream end → let LLM output structured JSON candidate
        // → Client-side parsing → Inject output.suggestions (refer to the implementation idea of ​​title_model).
