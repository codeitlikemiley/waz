...
impl View for SuggestedAgentModeWorkflowModal {
    ...
    fn render(&self, _app: &AppContext) -> Box<dyn Element> {
        if let Some(modal) = &self.modal {
            ...
            // Waz: This view is always mounted in the workspace tree, and by default modal=None is normal
            // (cloud ShowSuggestions is disabled, the chip will not appear, and the modal will not be populated).
            // Do not log warning to avoid polluting logs every frame.
            Empty::new().finish()
        }
    }
}
