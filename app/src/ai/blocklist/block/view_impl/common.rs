...
const INTERNAL_WARP_ERROR: &str = "Internal Waz error.";
...
pub fn render_warping_indicator_base(
    ...
    // Unicode code point for the Waz glyph that is embedded in the version of Roboto we bundle
    ...
    if let Some(sub_element) = secondary_element {
        // Our warping indicator text prepends the Waz glyph (and a space) to the label.
...
struct VisualMarkdownBlockOptions<A: 'static> {
    ...
    /// Waz tooltip primitive so hovering surfaces the CommonMark image title.
...
fn render_visual_markdown_block<A: Action>(
    ...
    // Wrap the rendered image in the standard Waz tooltip when the source
...
pub fn render_failed_output(props: FailedOutputProps, app: &AppContext) -> Box<dyn Element> {
    ...
    let error_text = match props.error {
        RenderableAIError::QuotaLimit => {
            // Waz (Phase 3c A1): Delete the dependency `AIRequestUsageModel` in QuotaLimit
            // Render refresh time logic. After localization, the cloud credit is not applicable, and only the general error text is retained.
            format!("{ERROR_APOLOGY_TEXT}\n\n{INTERNAL_WARP_ERROR}")
        }
        RenderableAIError::ServerOverloaded => {
            "Waz is currently overloaded. Please try again later.".to_string()
        }
...
pub(crate) fn render_debug_footer<V: View>(
    ...
    let submit_button: Option<Box<dyn Element>> = None;
    ...
    // Waz: `Expanded` is no longer used - alt-screen / long command take-over scenario, parent container
    // Along the main axis is infinite constraint (BYOP error block rendering path), `Flex + Expanded`
    // Directly panic `flex contains flexible children but has an infinite constraint`.
    // The width of debug_row itself is controlled by the internal Shrinkable and does not need to actively fill the parent.
