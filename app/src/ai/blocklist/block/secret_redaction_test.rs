...
fn test_detect_secrets_single_secret_custom_with_multibyte() {
    // Set a custom secret regex that matches a Chinese multibyte secret, e.g., "secret"
    // Set as user secret (enterprise secrets is empty)
    secrets::set_user_and_enterprise_secret_regexes(
    ...
    // The Chinese secret "secret" starts at character index 4 and ends at character index 6
    assert_eq!(
        detected_secrets,
        vec![SecretRange {
            char_range: 4..6,
...
fn test_add_secret_redaction_to_text_with_multibyte_characters() {
    ...
    // Range for the secret "code 1234" in the multibyte text.
    let secret_range = SecretRange {
        char_range: 9..14,  // "Code 1234"
