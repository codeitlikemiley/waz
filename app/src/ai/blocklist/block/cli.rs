        ...
        let exchange_id = history_model
            ...
            .and_then(|c| {
                c.get_task(&task_id)
                    ...
                    .or_else(|| {
                        // Waz BYOP fallback:agent since LRC
                        // `cli_controller::FinishedAction` passes
                        // `create_silent_cli_subagent_task_for_conversation` real creation
                        // subtask but append exchange has not yet been given to it (no new query is triggered)
                        // `update_for_new_request_input`), use the last of root task
                        // exchange placeholder. Subsequent user follow-up queries are routed to this task →
                        // `AppendedExchange` → The above subscription (line 365-394) will automatically
                        // replace model to the real exchange, so the placeholder is only used when the window is created.
                        // Short-lived, invisible on UX.
                        let fallback = c.root_task_exchanges().last().map(|e| e.id);
                        if fallback.is_some() {
                            log::warn!(
                                "[byop] CLISubagentView::new task={task_id:?} no \
                                 exchange yet, falling back to root_task last_exchange;\
                                 waiting for AppendedExchange to trigger replace."
                            );
                        }
                        fallback
                    })
