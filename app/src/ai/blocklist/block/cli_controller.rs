    ...
    pub fn new(
        ...
        ctx.subscribe_to_model(action_model, |me, event, ctx| match event {
            ...
            BlocklistAIActionEvent::FinishedAction { action_id, .. } => {
                ...
                // Waz BYOP fallback: After the agent starts LRC, the upstream server path will be pushed
                // `BlocklistAIHistoryEvent::CreatedSubtask` triggers
                // `handle_history_model_event` upgrades the block to monitored state; BYOP does not
                // If this server event source is not filled in, active_block will always stop at
                // `Agent { long_running_control_state: None }` —— `is_agent_in_control()`
                // / `is_agent_monitoring()` / `is_agent_tagged_in()` all false, hit
                // `view.rs:6841-6853` `is_input_box_visible` The bottom false path of the long command branch,
                // The input box disappears; at the same time, `controller.rs:1112-1119` branch is read
                // `subagent_task_id`, fallback If the conversation task is not actually created, the next round
                // Query routing fails, triggering "Could not find conversation for response stream".
                //
                // Implement the silent path: `create_silent_cli_subagent_task_for_conversation`
                // Really create the subtask but do not emit `CreatedSubtask` (the emit timing of the original method is determined by the caller
                // Control), let this hook manually emit after upgrading the block. This way you can walk the entire upstream
                // Upgrade the link and create a SubagentView floating window, and match the root_task of `cli.rs:431-446`
                // Bottom line, when the task has no exchange yet, the view is created with the root last_exchange placeholder, and will be followed later
                // User follow-up query is routed to subtask to trigger `AppendedExchange`, view itself
                // Subscription (`cli.rs:355-399`) will automatically replace the model to the real exchange.
                //
                // Side effects: The floating window is rendered with root last_exchange the moment it is created, and the root content may briefly flash.
                // One frame; but `set_agent_interaction_mode_for_agent_monitored_command` upgrades
                // The subsequent filtering of SubagentView will only display task_id related content, and the experience is consistent with the upstream.
                ...
                // Waz BYOP: silent_create_for_byop does not emit CreatedSubtask, manual here
                // Trigger SpawnedSubagent to let terminal_view create a CLISubagentView floating window.
                // active_subagents_by_block.task_id is updated synchronously to ensure that the BlockCompleted hook is in
                // Clean up properly at the end of LRC.
                //
                // Note: the terminal_model lock has been dropped above to avoid the deadlock (emit) of the previous reproduce
                // SpawnedSubagent → terminal_view::handle_cli_subagent_controller_event
                // → CLISubagentView::new synchronization will also cause lock terminal_model and the like.
                // FairMutex reentrant).
