...
impl AIBlock {
    ...
    pub fn new(
        ...
        // Waz: Previously, the Success event of `GetRelevantFilesController` was subscribed to here
        // to call `ctx.notify` and refresh the UI after RAG completed. That controller has retired along with outline,
        // so the subscription point has also been removed.
        ...
        // Waz(Phase 3c A1): Removed subscription to `AIRequestUsageModelEvent::RequestBonusRefunded`.
        // After localization, this event will never be emitted (there is no `provide_negative_feedback_response`
        // RPC call), and the subscription itself has become dead code running in empty loop. The `request_refunded_count` field is kept
        // but will always be the initial value None.
...
pub enum AIBlockEvent {
    ...
    /// Open a Waz Text instance with the requested code diff.
    OpenCodeWithDiff {
        view: ViewHandle<CodeDiffView>,
    },
...
pub enum AIBlockAction {
    ...
    CopyDebugId(String),
    /// Open Waz feedback documentation
    OpenFeedbackDocs,
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            AIBlockAction::ViewScreenshot { action_id: _ } => {
                // Computer Use has been removed, screenshot lightbox is no longer available.
            }
        }
        ctx.notify();
    }
