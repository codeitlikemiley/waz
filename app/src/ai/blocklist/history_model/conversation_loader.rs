...
/// Representation of loaded local session data.
///
/// The exact format depends on the agent harness that spawned the session.
pub enum LoadedConversationData {
    /// A session generated by the Oz harness that is reducible to the [`AIConversation`] data model.
    Oz(Box<AIConversation>),
    /// Sessions generated by an external CLI agent harness.
