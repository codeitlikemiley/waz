...
impl BlocklistAIPermissions {
    ...
    /// Returns whether or not Waz can auto-read the given files (e.g. for codebase indexing).
    ...
    pub fn can_ask_user_question(
        ...
        // openWarp change: auto-approve (ctrl+shift+i) only automatically approves execution tools like shell/editor,
        // ask_user_question is always required to prompt the user to avoid the model's questions being silently swallowed.
        // Skip only when explicitly selecting `Never`.
