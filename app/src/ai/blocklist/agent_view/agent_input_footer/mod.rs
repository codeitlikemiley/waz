pub(super) mod chips;
pub mod editor;
// Waz Wave 7-3:`environment_selector` was removed with the hosted-mode footer.
...
// Waz Wave 7-3:`warp_cli::agent::Harness` import was removed with the hosted-mode footer.
...
// Waz Wave 7-3:`EnvironmentSelector` / `EnvironmentSelectorEvent` re-export was removed
...
// Waz Wave 7-3: removed the hosted-mode footer gap constant with the old footer.
...
pub struct AgentInputFooter {
    ...
    // Waz Wave 7-3:`environment_selector` field was removed with the hosted-mode footer.
...
impl AgentInputFooter {
    ...
    pub fn new(
        ...
        // Waz Wave 7-3:`EnvironmentSelector` initialization + subscription + ambient_agent
        ...
        let mut me = Self {
            ...
            // Waz Wave 7-3:`environment_selector` field init was removed with hosted-mode UI.
            // The subsystem is physically deleted.
    ...
    // Waz Wave 7-3: hosted-mode footer rendering was removed.
        ...
        Fut: std::future::Future<Output = Result<(), PluginInstallError>> + Send + 'static,
    {
        ...
        let (shell_path, shell_type) = match shell_data {
            ...
            // needing the Waz plugin to integrate with it.
    ...
    pub fn has_open_chip_menu(&self, app: &AppContext) -> bool {
        // Waz Wave 7-3:`environment_selector` is_menu_open() check was removed with hosted-mode UI.
        // The subsystem is physically deleted.
    ...
    pub fn toggle_cli_voice_input(
        ...
        match &self.cli_voice_input_state {
            CLIVoiceInputState::Stopped => {
                // Waz (Phase 3c A1): Remove `AIRequestUsageModel::can_request_voice`
                // Quota gate. After localization, voice input is not subject to cloud quota restrictions and can be sent uniformly.
...
impl View for AgentInputFooter {
    ...
    fn render(&self, app: &warpui::AppContext) -> Box<dyn warpui::Element> {
        ...
        // Waz Wave 7-3: removed environment-selector ambient-agent chip injection.
...
pub enum AgentInputFooterEvent {
    ...
    // Waz Wave 7-3:`OpenEnvironmentManagementPane` event was removed with hosted-mode UI.
    // Physically delete.
...
/// Green-accented theme for the "Install Waz plugin" chip.
