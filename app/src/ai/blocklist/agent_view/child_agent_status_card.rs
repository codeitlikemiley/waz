...
impl View for ChildAgentStatusCard {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        for child in &children {
            ...
            // T3-7: in_progress subagent surface current action in title,
            // "Untitled · ↳ Searching codebase..." / "Refactor X · ↳ Reading files..."。
            // The finished sub-agent still displays the original title unchanged.
...
/// T3-7: Extract the current/recent action presence summary of the subagent conversation,
/// Used for subtitles like surface "↳ Searching codebase..." on the parent ChildAgentStatusCard.
/// Visual alignment of the opencode TUI Task parent card `↳ Bash $ git status`.
///
/// Implementation: Traverse latest_exchange.output_status.output().messages and find the last one in reverse
/// `AIAgentOutputMessageType::Action`, takes `presence_continuous_summary`.
/// If not found, None is returned (card does not display subtitle).
