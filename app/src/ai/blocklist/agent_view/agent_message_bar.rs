...
impl AgentMessageBar {
    ...
    pub fn new(
        ...
        // Subscribe to the "Show Agent shortcut key hints" setting, and re-render the message bar after the switch is changed to hide/restore the 4 hints.
...
impl View for AgentMessageBar {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // Waz (Phase 3c A1): Remove ambient credits banner UI.
        // After localization, `ambient_only_credits_remaining` will always be None, and the original branch will only go to None.
...
impl MessageProvider<AgentMessageArgs<'_>> for ZeroStateMessageProducer {
    fn produce_message(&self, args: AgentMessageArgs<'_>) -> Option<Message> {
        ...
        // "Show Agent shortcut key hints" master switch: when false, skip the bottom 4 hints (? Help / / Command /
        // Open conversation/code review). resume / plan / fork are not affected because they are not prompts but stateful entries.
