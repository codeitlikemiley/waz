...
impl PromptAlertView {
    ...
    pub fn determine_state(app: &AppContext) -> PromptAlertState {
        ...
        // Waz: BYOP/local provider handles connection status by itself, including localhost such as Ollama
        // provider. Global offline status only blocks built-in cloud usage.
        ...
        // Waz (Phase 3c A1):`has_requests_remaining` is always true after localization,
        // The original if/else binary is only reachable by the SoftGate branch, and the true branch is used directly.
        ...
        // There is no cloud overages/billing upgrade path for the local version.
