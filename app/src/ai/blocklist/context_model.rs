...
/// The hard limit for inlining a single text-like PendingFile into the prompt. If exceeded, it is skipped directly (to avoid blowing up the context).
/// Difference from `attachment_utils::MAX_ATTACHMENT_SIZE_BYTES` (10MB, used for binary attachments):
/// That is a byte limit, while this is a token-friendly limit for inlining into the LLM prompt.
const MAX_INLINE_TEXT_FILE_BYTES: usize = 256 * 1024;

/// The hard limit for feeding a single binary PendingFile (PDF / audio / other) into the BYOP `Binary` ContentPart.
/// Uses the same 10MB limit as binary attachments to avoid blowing up the HTTP body after base64 encoding in a single request.
const MAX_INLINE_BINARY_FILE_BYTES: usize = 10 * 1024 * 1024;

/// Determines whether a PendingFile "looks like text" to decide whether to inline it.
/// Uses a double guarantee of mime + extension: `mime_guess` will return `application/octet-stream` for files without extensions like Dockerfile/Makefile,
/// requiring supplementary extension/filename matching.
fn is_text_like(file: &PendingFile) -> bool {
    let mime = file.mime_type.as_str();
    if mime.starts_with("text/") {
        return true;
    }
    // Common text-like application/* mime types
    matches!(
        mime,
        "application/json"
            | "application/xml"
...
/// Filename / extension fallback to cover files with conventional names but no extension (Dockerfile / Makefile / .env, etc.).
fn is_text_like_by_filename(file_name: &str) -> bool {
    let lower = file_name.to_ascii_lowercase();
    // Conventional files without extensions
    if matches!(
        lower.as_str(),
        "dockerfile"
            | "makefile"
...
/// Reads the content of a PendingFile and converts it into a `FileContext` consumable by both BYOP and warp-own paths.
///
/// Three tiers of paths:
/// 1. **text-like hit + UTF-8 ok + not exceeding text cap** -> `StringContent`, inlined into `<file>` XML block
/// 2. **multimodal mime (image/pdf/audio) + not exceeding binary cap** -> `BinaryContent(bytes)`,
///    which BYOP upgrades to `ContentPart::Binary` to actually send to the model
/// 3. **other binary (.exe / .zip / huge files)** -> `BinaryContent(empty Vec)` — does not read bytes
///    to avoid memory waste, but still creates FileContext so the AI can at least see the
///    path / mime / size in the prefix XML, and can use tools like read_files to process further
///
/// Critical fix: The `file_name` field is filled with the **full absolute path** instead of the basename. `FileContext.file_name`
/// is already used as `file_path` in `convert.rs:750`, and user_context is also rendered according to `path`.
/// Pushing the full path here allows the AI to locate the file directly using read_files / shell tools.
///
/// Design trade-off: In the warp-own protocol path, `BinaryContent` is directly discarded (returning empty vec)
/// by `Vec<api::FileContent>::from` in `convert.rs:759`, so even if we put all binaries into context here, it will not
/// contaminate the warp-own stream; only BYOP's `user_context::render_user_attachments` will
/// actually consume BinaryContent and upgrade it to `ContentPart::Binary`.
fn read_pending_file_for_context(file: &PendingFile) -> Option<FileContext> {
            ...
            if size as usize <= MAX_INLINE_TEXT_FILE_BYTES {
                match std::fs::read(&file.file_path) {
                    Ok(bytes) => {
                        ...
                        // text-like but content is not UTF-8 -> fall back to binary path
                    }
                    Err(e) => {
                        log::warn!(
    ...
    // 2) Multimodal binary (image/pdf/audio): needs to send bytes to the model, read and store into BinaryContent
    ...
    if is_multimodal_mime {
        if let Some(size) = metadata_size {
            if size as usize <= MAX_INLINE_BINARY_FILE_BYTES {
                ...
                // Huge multimodal files: fall back to empty BinaryContent, placeholder still carries size (from metadata)
                return Some(FileContext::new(
                    full_path,
    ...
    // Does not read bytes, avoiding 100MB exe taking up memory; AI gets path/mime/size via prefix XML
    // and can decide whether to call read_files or shell tools for further processing.
...
pub struct BlocklistAIContextModel {
    ...
    /// Context attachments displayed in the input box as visible @name.
...
impl BlocklistAIContextModel {
    ...
    pub fn pending_context(&self, app: &AppContext, is_user_query: bool) -> Vec<AIAgentContext> {
        ...
        // Waz: Previously checked RepoOutlines to determine if the repository under the current pwd was indexed,
        // so that "use codebase semantic search" could be selected as context. Now outline has retired, so it is always false.
        ...
        let project_rules = if let Some(pwd) = pwd.clone().and_then(|path| {
            ...
            // Prioritize normal path (zero IO, get results from HashMap after async indexing is done);
            // when not ready, fallback to sync fast-path stat + read rule files from cwd / ancestor directories.
            // Aligned with opencode `findUp` mode to ensure AGENTS.md is retrievable even if asked immediately after cd.
            // fast-path has internal cache + time budget, so UI is never blocked. See
        ...
        if is_user_query {
            ...
            // Waz P0/P1: Read PendingFile synchronously and push it into context as AIAgentContext::File.
            // - text-like (successful UTF-8 parsing) -> StringContent -> user_context.rs::render_file
            //   rendered as <file> XML block (BYOP) / api::input_context::File (warp-own)
            // - binary (PDF / audio / other) -> BinaryContent -> BYOP user_context Binary
            //   ContentPart upgrade path (warp-own discards in convert.rs:759 directly, no side-effects)
            for attachment in &self.pending_attachments {
    ...
    /// Register a context attachment that can be referenced as @name in subsequent queries.
    ...
    /// Returns the @ context attachments indexed by visible reference string.
    ...
    /// Returns the @ context attachments that still exist in the current query.
    ...
    /// Deletes the @ context attachments that no longer exist in the input box.
