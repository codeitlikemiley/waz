...
pub struct SessionContext {
    ...
    /// Waz: Connection info for legacy SSH session (user manually typed `ssh xxx@yyy` in local PTY,
    /// and warp shell hook is not installed on the remote). `session_type` is still `Local`,
    /// but the PTY is actually running on the remote. This needs to be communicated to the LLM in the prompt, otherwise the model defaults to the local OS.
    ssh_connection_info: Option<InteractiveSshCommand>,
    /// Whether it is a legacy SSH session (`IsLegacySSHSession::Yes`).
    is_legacy_ssh: bool,
}
...
impl SessionContext {
    ...
    /// Waz: legacy SSH connection info (host/port), only meaningful when `is_legacy_ssh()` is true.
    pub fn ssh_connection_info(&self) -> Option<&InteractiveSshCommand> {
        self.ssh_connection_info.as_ref()
    }

    /// Waz: Whether the session is legacy SSH (user manually typed ssh, remote has no warp hook).
    /// For this session, `session_type` is still `Local` but PTY is actually running on the remote,
    /// so profiles like `host_info`/`shell` reflect the local client instead of the remote shell.
...
/// preflight only fixes "one type" of issue per iteration (reclaimed persisted results / current input results / finished
/// results / cancellation result). Normally a single round of requests involves at most 2-3 types, but in cases of extreme concurrency or
/// when accumulating results of multiple tasks, more iterations may be required to converge. We keep a small upper bound here to return blocked early if a true infinite loop occurs, instead of
/// spinning silently.
...
pub struct BlocklistAIController {
    ...
    /// conversation, because the ambient agent task driver owns the entire Waz window working on a task, and any
    ...
    >,
    /// Stashed RequestInput of a BYOP request due to readiness reporting PendingToolResults.
    /// It is retrieved and resent by `flush_pending_byop_request_after_finished_action`
    /// after the live action of the conversation finishes and triggers FinishedAction,
    /// preventing user-submitted prompts from being silently discarded.
    /// At most 1 entry is kept per conversation; subsequent user requests will overwrite it.
...
impl BlocklistAIController {
    ...
    pub fn new(
        ...
        ctx.subscribe_to_model(&action_model, move |me, event, ctx| {
            ...
            // Prioritize resending the BYOP request cached by readiness (which contains the user's original prompt);
            // If none, follow the original follow-up path (sending only finished_action_results).
    ...
    fn maybe_populate_plans_for_ai_document_model(
        ...
        for attachment in referenced_attachments.values() {
            ...
            // openWarp localization: the plan is stored only in local SQLite. When restoring a session, if the plan is not in memory,
            // rebuild it from the content carried by the attachment. The title is not carried by the attachment caller,
            // so use a default planning title as a placeholder first; it will be overwritten by the title stored in SQLite when apply_persisted_content restores it.
    ...
    /// When the live action of the conversation is completed, if there was a BYOP request for this conversation previously cached
    /// due to readiness reporting PendingToolResults, prioritize resending this request (so that the user's original prompt is not lost),
    /// and merge the finished_action_results produced by the action completion into the same RequestInput to avoid sending two requests.
    /// Returns true if the pending request has been consumed and attempted to send; false if there is no pending request to flush.
    fn flush_pending_byop_request_after_finished_action(
        ...
        if let Err(e) = self.send_request_input(
            ...
            // Do not loop queue on secondary failures: if it still reports Pending, the Err has already been rewritten to
            // pending_byop_requests inside send_request_input; other errors are handled according to the controller's existing path.
        ...
        if let Some(running_command) = byop_get_running_command_for_lrc(&terminal_model) {
            ...
            log::warn!(
                "[byop-diag] LRC detected but byop_get_running_command_for_lrc \
                 returned None (active_block status mismatch)"
            );
        }
    }
    ...
    fn commit_byop_cancellation_results(
        ...
        // `ToolCallKey` is a triplet (task_id + assistant_tool_call_message_id + tool_call_id),
        // but the BYOP `AIAgentActionResult` and persisted `ToolCallResult` protocol layer only carry (task_id, tool_call_id),
        // without an explicit assistant_tool_call_message_id back-reference. To align with the ToolCallKey semantics
        // of the readiness module, we use (task_id, tool_call_id) as the outer matching key, and also record the set of
        // assistant_tool_call_message_ids expected by readiness; when the protocol assumption (tool_call_id is unique within the same conversation)
        // is broken, we only log a warning instead of blocking, avoiding user workflow hangs in edge cases.
        ...
        // A cancellation result that has already been persisted also counts as "having progress",
        // otherwise when readiness still reports NeedsCancellationCommit but there is no corresponding entry in request_input,
        // it will be incorrectly determined as zero progress and blocked; in fact, readiness will naturally converge in the next round.
    ...
    fn has_persisted_tool_result(
        ...
        // The persisted protobuf `ToolCallResult` protocol only carries `tool_call_id`, with no assistant_message_id back-reference,
        // so the strictest match can only be a (task_id, tool_call_id) pair. The outer `get_task(&task_id)` already
        // isolates the search scope to the target task, and here we additionally validate `msg.task_id` in case future callers get a non-isolated view.
    ...
    fn send_request_input(
        ...
        // A new round of requests starts, discard the old request cached due to readiness Pending first;
        // If this preflight encounters PendingToolResults again, it will be rewritten below.
        ...
        if let Err(error) = preflight_result {
            if let Some(pending) = error.downcast_ref::<PendingByopToolResultsError>() {
                ...
                // Temporarily cache the user's input, and after the live action in the conversation finishes and triggers FinishedAction,
                // resend it in `flush_pending_byop_request_after_finished_action` to prevent silent loss.
            ...
            // When the persistence path fails (shared session is not saveable, settings disable persistence, SQLite sender is unavailable,
            // or the channel is full), it would originally bubble up as a generic anyhow::Error to the caller and be silently swallowed by log::error!,
            // leaving the user without feedback. Here, we also route it through the blocked request UI path so the user can see the failure reason.
                ...
                error.downcast_ref::<crate::ai::agent::conversation::UpdateConversationError>()
            {
                if matches!(
                    ...
                    return self.complete_byop_blocked_request(
                        ...
                        request_params.model.clone(),
                        is_queued_prompt,
                        "Waz couldn't save the BYOP conversation state needed to send this \
    ...
    fn handle_response_stream_event(
        ...
        match event {
                ...
                match event {
                    Ok(event) => {
                        ...
                        match event {
                            ...
                            warp_multi_agent_api::response_event::Type::Finished(
                                ...
                                // Waz BYOP local session compaction: get the summarization flag before the stream finishes
                    ...
                    Err(e) => {
                        if matches!(e.as_ref(), AIApiError::QuotaLimit) {
                            // Waz(Phase 3c A1): Delete
                            // `AIRequestUsageModel::enable_buy_credits_banner` call.
                            // There is no "buy extra credits" business in the localized BYOP scenario.
                        }
            ...
            ResponseStreamEvent::AfterStreamFinished { cancellation } => {
                ...
                // Waz BYOP: Collect newly added message ids this round, to be used later in the EMPTY branch to detect
                // synthetic invalid_arguments error markers. **Only check this round's added** to avoid
                // hitting in history repeatedly and causing an auto-resume infinite loop (once the marker is persisted it is always there).
                ...
                if let Some(stream_cancellation) = &cancellation {
                    ...
                    // Waz: LRC tag-in first round automatically authorizes agent tool execution.
                    //
                    // Trigger condition: active_block is in InteractionMode::User { did_user_tag_in_agent: true } when initiating this round of requests.
                    // Cannot use current active_block's monitored metadata as fallback, otherwise subsequent normal requests in the same CLI subagent session
                    // will also be auto-accepted, resulting in the confirmation UI not being shown.
                    ...
                    // Waz BYOP: When from_args parsing fails, chat_stream takes fallback emit
                    // carrier ToolCall(tool=None) + synthetic error ToolCallResult(result=None,
                    // server_message_data is invalid_arguments JSON). Both go to NoClientRepresentation,
                    // not queued into actions_to_queue, exchange ends silently -> the model never receives error feedback,
                    // and the user must manually send a message again to force a retry.
                    //
                    // Check if the recent ~16 messages contain BYOP synthetic error markers; if so, reuse the
                    // auto-resume path at line 2695+ to trigger resending, allowing the model to immediately retry based on the error tool_result
                    // with corrected arguments. `can_attempt_resume_on_error=false` prevents infinite loops when LLM persistently outputs bad args.
                    // Only search for the synthetic error marker in messages newly added in this round to prevent repeated triggers
                    // from the same marker persisted in history.
                    // Waz BYOP: Synthetic ToolCallResults that did not enter the AIAgentAction queue
                    // need auto-resume; otherwise, the exchange ends silently and the model hangs waiting for results.
                    // 1. invalid_arguments — from_args parsing failure fallback (original).
                    // 2. _byop_intercepted — local intercept tool results like todowrite / webfetch / websearch.
                    //    These tools do not go through the protobuf executor, but directly synthesize the result,
                    //    with no AIAgentAction queued.
                ...
                // having no effect after localization.
            }
        }
    }
    ...
    // Waz(Phase 3c A1): Delete `maybe_refresh_ai_overages` function.
    // The original implementation was a tuning path to "fetch the latest overage state from the server when local limits are exhausted".
    // Under localized BYOP, there are neither limits nor overages, so both the function itself and its only call site need to be deleted.
    ...
    pub(super) fn handle_response_stream_finished(
        ...
        // Waz BYOP local session compaction: aggregate token_usage before moving into the closure below,
        // used for auto overflow check (used in the Done branch later).
        ...
        match finished_event.reason {
            Some(warp_multi_agent_api::response_event::stream_finished::Reason::Done(_)) | None => {
                // Waz BYOP local session compaction - write back summary
                ...
                // Waz BYOP local session compaction - auto overflow trigger (aligns with opencode `processor.ts:395-403`)
                // Only check when this stream is not the summary itself, to prevent recursion.
                if summarize_overflow.is_none() {
                    ...
                    if aggregate_count > 0 {
                        ...
                        if crate::ai::byop_compaction::is_overflow(&cfg, counts, model_limit) {
                            ...
                            // Triggered via SlashCommandRequest::Summarize (same link as /compact-and);
                            // overflow=true -> chat_stream carries the overflow flag when assembling the summarization request,
                            // and when commit_summarization writes back, it also sets overflow=true in the state (convenient for UI distinction).
...
/// Waz BYOP specific: extract RunningCommand in LRC tag-in / agent-monitored scenarios.
///
/// Upstream `get_running_command` returns None when `is_agent_monitoring()` — because under Waz's own
/// path, after LRC spawns a cli subagent, the server persists this state, and the client does not need to resend the running_command
/// in subsequent rounds. However, the BYOP direct-connect model has no server-side persistence, so **every round must bring the current PTY grid content
/// back to the model again** (otherwise the model can only see the blind spot after the first round of grid_contents).
///
/// The condition is relaxed to `is_agent_in_control_or_tagged_in()` — covering:
///   - tag-in: `InteractionMode::User { did_user_tag_in_agent: true }` (pre-spawn)
///   - monitored: `InteractionMode::Agent { ... }` (post-spawn)
///
/// Extraction logic is strictly aligned with upstream: when alt-screen is active, take `terminal_model.alt_screen().grid_handler()`
/// instead of `active_block.output_grid()` (the latter is empty during alt-screen,
/// do not use `output_to_string_force_full_grid_contents()` anymore, as that path returns an empty string under TUIs like nvim,
/// making the `<attached_running_command>` block empty, resulting in the model complaining about "not seeing command_id").
