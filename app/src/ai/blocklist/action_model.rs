...
impl BlocklistAIActionModel {
    ...
    pub(super) fn has_unfinished_action_for_tool_call(
        ...
        // In the action model, `AIAgentActionId` is the tool_call_id. We just rename the parameter explicitly here
        // to make it more intuitive at the call site (the byop preflight of the controller).
    ...
    /// Same as `queue_actions`, but with an added `auto_accept` parameter.
    ///
    /// When `auto_accept=true`, after preprocessing completes (`handle_preprocess_actions_results`)
    /// and each action is pushed into `pending_actions`, we **immediately call `execute_action` on each of them one by one**
    /// (internally following the `is_user_initiated=true` path, bypassing the `NeedsConfirmation` check),
    /// instead of the default `try_to_execute_available_actions` (`is_user_initiated=false`).
    ///
    /// Use case: Waz BYOP path LRC tag-in scenario - the user actively calls `SetInputModeAgent` to pass
    /// control to the agent, but the Accept button of `RequestedCommand` cannot be seen in alt-screen full screen mode.
    /// The controller detects the LRC state and uses this method to bypass the manual confirmation deadlock.
    ...
    fn handle_preprocess_actions_results(
        ...
        if auto_accept && !auto_accept_ids.is_empty() {
            // Waz: LRC tag-in automatic authorization path. Bypass the default
            // try_to_execute_available_actions (which requires manual confirmation),
            // and directly call `execute_action` (equivalent to user Accept) for each newly pushed action.
            log::info!(
        ...
        let Some(conversation_id) = found_conversation_id else {
            log::error!(
                "[byop-diag] handle_requested_command_accepted: action_id={action_id:?} NOT FOUND \
                 in pending_actions. pending_conversations=[{}] (action not correctly queued in pending_actions, \
                 chain broke between controller.queue_actions → action_model.try_to_execute_action)",
                self.pending_actions
                    .iter()
