...
pub enum AskUserQuestionPermission {
    ...
    Never,
    /// Equivalent to `AlwaysAsk` in openWarp: the auto-approve mode no longer silently skips user questions,
    /// and only auto-approves execution-type tools such as shell/edit. Variant name is retained for compatibility with serialized profiles.
    #[default]
...
pub struct AIExecutionProfile {
    ...
    /// Model used for generating conversation titles. Falls back to `base_model` when `None`.
    pub title_model: Option<LLMId>,
    /// Model used by active AI (prompt suggestions / NLD / relevant files).
    /// Falls back to `base_model` when `None`. It is recommended to choose a small, fast, and cheap BYOP model.
    pub active_ai_model: Option<LLMId>,
    /// Model used by Next Command (gray completion / zero-state suggestions).
    /// Falls back to `base_model` when `None`. Low-latency sensitive, it is recommended to choose the cheapest/fastest BYOP model.
    pub next_command_model: Option<LLMId>,

    pub context_window_limit: Option<u32>,

    /// Whether plans created by the agent should be automatically synced to Waz Drive
