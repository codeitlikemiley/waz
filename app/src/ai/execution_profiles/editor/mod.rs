...
// Decentralization branch: The original `render_upgrade_footer` was used to display the "frontier model
// needs to be upgraded to a paid plan" banner at the bottom of the model dropdown menu; in local mode,
// there is no longer a distinction between paid and free, and the entire section has been deleted.
...
impl ExecutionProfileEditorView {
        ...
        ctx.subscribe_to_model(&LLMPreferences::handle(ctx), |me, _, event, ctx| {
            ...
            match event {
                ...
                LLMPreferencesEvent::UpdatedActiveAgentModeLLM => {
                    ...
                    // title / active_ai models fallback to base, so they also need to be refreshed when base changes.
                    Self::refresh_filterable_model_dropdown(
                        &me.title_model_dropdown,
                        current_permissions.title_model.clone(),
                        |prefs| prefs.get_base_llm_choices_for_agent_mode().collect_vec(),
        ...
        D: FnOnce(&LLMPreferences) -> LLMId,
    {
        menu.update(ctx, |dropdown, ctx| {
            ...
            // Decentralization branch: no longer display upgrade footer based on RequiresUpgrade status.
