...
fn run_agent(
    ...
    match command {
        ...
        AgentCommand::List(_) => Err(anyhow::anyhow!(
            "Agent skill listing is disabled in Waz"
        )),
    }
}
...
impl AgentDriverRunner {
    async fn setup_and_run_driver(
        ...
        // Wait for Waz Drive to sync before building the task config, since
    ...
    ///
    /// Running Waz locally does not create a remote ambient-agent task record.
    /// The driver keeps `task_id` as `None`, so the downstream naturally skips the remote task branch.
...
fn launch_command(
    ...
    if !auth_state.is_logged_in() {
        return Err(anyhow::anyhow!(
            "No local user is available. Restart Waz and try again."
        ));
    }
...
/// Check if we're running within Waz (for example, if this is an invocation of the Waz CLI
/// within a Waz terminal session).
...
fn report_fatal_error(err: anyhow::Error, ctx: &mut AppContext) {
    ...
    #[cfg(not(target_family = "wasm"))]
    {
        if let Ok(path) = log_file_path() {
            let _ = write!(
                message,
                "\n\nFor more information, check Waz logs at {}",
                path.display()
            );
        }
    }
