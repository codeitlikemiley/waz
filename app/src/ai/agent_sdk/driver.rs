...
/// Signals to Claude child-harness hooks that Waz already owns the background
...
/// self-managed listener path so older Waz builds and standalone plugin
...
/// that Waz and the Claude hook scripts share.
...
/// `AgentDriver` is a model for driving an ambient Waz agent to completion.
///
...
impl AgentDriver {
    pub fn new(
        ...
        // Waz initializes the local user on startup; reaching here indicates the local auth singleton was not properly initialized.
    ...
    /// installing the Waz plugin and platform plugin, if applicable.
    ///
    ...
    fn execute_run(
        ...
        // openWarp does not sync plan to Waz Drive, the original "plan_artifact_created" CLI output depends on the cloud notebook_link,
        // so we no longer subscribe to the SaveStatusUpdated event of AIDocumentModel here.
