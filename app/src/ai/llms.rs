...
pub struct LLMPreferences {
    ...
    /// session-only, do not write settings.toml. Use `last_used_reasoning` to find out when the key is missing.
    /// If it is missing, use `default_reasoning_for(api_type, model_id)`.
    reasoning_effort_per_terminal: HashMap<EntityId, crate::settings::ReasoningEffortSetting>,
    /// Remember "the last gear used by a certain (api_type, model)" to do UX soft memory.
    /// session-only。
...
impl LLMPreferences {
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        // BYOP-only mode: picker is completely populated by user-configured agent_providers,
        // Completely no longer consume the upstream cloud model list.
        // The cache (MODELS_BY_FEATURE_CACHE_KEY) is also skipped - it is rebuilt directly from settings on startup.
        ...
        // Monitor secrets changes (API key additions and deletions) → Rebuild, because validity depends on whether api_key exists.
        ctx.subscribe_to_model(
    ...
    /// Recently used — written immediately after picker switch, new tab/restart will use it)> profile.base_model >
    /// default_llm_info()。
    ...
    /// Returns the LLM currently used for Session Title Generation.
    ///
    /// Priority:profile explicitly set `title_model` → otherwise fallback to `base_model`(active).
    ...
    /// Default title model - used when there is no independent setting, consistent with the base model.
    ...
    /// Returns the current LLM used for "proactive AI" (prompt suggestions / NLD / relevant files).
    ///
    /// Priority: profile explicitly set `active_ai_model` → otherwise fallback to `base_model`(active).
    /// The candidate set reuses `get_base_llm_choices_for_agent_mode()`.
    ...
    /// Default active AI model - used when there is no independent setting, consistent with the base model.
    ...
    /// Returns the current LLM used for "Next Command" (gray completion/zero-state suggestions).
    ///
    /// Priority: `next_command_model` set explicitly by profile → fallback to `base_model`(active) otherwise.
    ...
    /// Default next command model - used when there is no independent setting, consistent with the base model.
    ...
    pub fn update_preferred_agent_mode_llm(
        ...
        // Always write byop_last_used_model_id (even if changed=false, it will be overwritten to unify the new tab behavior).
        // picker explicit switching = user's strongest intention, new tabs/restarts will all be used.
    ...
    /// BYOP-only mode: The picker is completely populated by local `agent_providers`, and no longer goes to the warp backend to pull the model.
    /// These two refresh functions preserve signatures for existing call points (NetworkOnline / AuthComplete / TeamsChanged)
    /// Triggered, but internally noop.
    pub fn refresh_authed_models(&self, _ctx: &mut ModelContext<Self>) {}

    fn refresh_public_models(&self, _ctx: &mut ModelContext<Self>) {}

    /// Rebuild `models_by_feature` from settings.agent_providers + AgentProviderSecrets,
    ...
    /// Priority: per-terminal select > last-used (api_type, model) > default file for variants table > Auto.
    ...
    /// Set the reasoning effort of the specified terminal-view and update the last-used memory at the same time.
    /// And write the (api_type, model) → effort mapping immediately to the AISettings persistence layer
    /// (New tab/restart will read the latest value).
...
pub enum LLMPreferencesEvent {
    ...
    /// The reasoning effort of the current terminal-view changes (the picker selects a new file).
