...
fn test_harness_filter_matches_only_selected_harness() {
    App::test((), |mut app| async move {
        ...
        // Snapshot present but no harness set → Some(Oz), matches Waz Agent.
        ...
        // Local conversation: effectively Waz Agent.
        ...
        app.update(|ctx| {
            ...
            // Waz Agent / Oz → default-snapshot task and local conversation.
            ...
            assert_eq!(
                oz_items.len(),
                2,
                "expected 2 Waz Agent matches, got {oz_items:?}"
            );
            ...
            assert!(
                ...
                "stub task with no snapshot should not match the Waz Agent filter"
            );
        });
    });
}
