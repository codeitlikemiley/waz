...
impl ConvertAPIToolCallToAIAgentAction for api::message::ToolCall {
    fn to_action(
        ...
        let Some(tool) = self.tool else {
            // Waz BYOP: When from_args parsing fails, `make_tool_call_carrier_message`
            // intentionally emits a ToolCall with `tool: None`, serving only as a carrier to restore
            // the original fn_name + args_str to the upstream model in the next round of build_chat_request
            // (server_message_data carries the original content). The synthetic error ToolCallResult
            // that follows immediately is the actual content shown to the user.
            // Returning MissingTool would cause the entire conversation update to be rejected (`UpdateTask(ConversionError)`),
            // and the subsequent ToolCallResult could not be applied either → exchange would be permanently stuck in "Warping...".
            // Here, changed to NoClientRepresentation: the UI does not render this empty ToolCall,
            // task.messages is still persisted (with full server_message_data), keeping the upstream retransmission link complete in the next round.
        ...
        match tool {
            ...
            | api::message::tool_call::Tool::RequestComputerUse(_) => {
                // Computer Use has been removed, even if the model initiates these two types of calls, they will not be dispatched.
