...
pub(crate) fn convert_tool_call_result_to_input(
    ...
    match tool_call_result.result.as_ref() {
        ...
        Some(ToolCallResultType::UploadFileArtifact(_)) => {
            // Upload artifact is physically removed, discard cloud results directly
            let _ = tool_call_id;
            let _ = task_id;
            let _ = context;
            None
        ...
        | Some(ToolCallResultType::RequestComputerUseResult(_)) => {
            // Computer Use has been removed, directly ignore these two types of results encountered in history.
            None
        }
        Some(ToolCallResultType::FetchConversation(_)) => {
            // Cloud tools have been physically removed
            None
        }
