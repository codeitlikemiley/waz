...
impl Task {
    ...
    /// Waz BYOP exclusive: When the agent starts LRC on its own and receives a snapshot, directly create
    /// a Server-backed subagent task in the conversation.
    ///
    /// Cannot reuse `new_optimistic_cli_agent_subtask`: the generated `TaskImpl::Optimistic`
    /// directly returns `TaskNotInitialized` at entry points that require the source to exist,
    /// such as `add_messages` (`task.rs:649-651`) / `try_get_source` (`task.rs:627-631`). The upstream path relies on the server
    /// returning `ApplyClientAction::CreateTask` to upgrade optimistic tasks to server; BYOP has no
    /// server, so it needs to locally construct `api::Task` as the source, making the task Server-backed from the start.
    ///
    /// `subagent_params` uses the synthesized `Subagent { command_id, ... }` so that queries like
    /// `is_subagent_task_finished` do not all fall into the `SubagentTaskNotFound` branch.
    /// `tool_call_id` uses a new UUID. Since there is no corresponding ToolCall in root.messages,
    /// `is_subagent_task_finished` will always return `Ok(false)` (not finished) — which semantically makes sense
    /// as "unfinished" while LRC is running, and the `BlockCompleted` hook will clean it up when LRC actually finishes.
