...
/// Call BYOP next_command one-shot and wrap the result as `GenerateAIInputSuggestionsResponseV2`.
///
/// `byop_cfg` must be unwound by the caller from `&AppContext` before spawning (no longer available at runtime).
/// `byop_cfg = None` ⇒ Silent no-op, return Ok empty response (to avoid 401 error noise).
async fn byop_generate_input_suggestions(
    ...
    let Some(cfg) = byop_cfg else {
        // Waz has been stripped of the cloud, and no longer fallsback ServerApi when there is no BYOP configuration - returns an empty response,
        // Naturally, the UI will not display suggestions or flush error logs.
        return Ok(GenerateAIInputSuggestionsResponseV2::default());
    };
    // Map the flat fields of request to the input of the active_ai prompt template.
    // recent_blocks left blank — context_messages + history_context already contains serialized history commands.
    ...
    match suggestion {
        Some(cmd) => {
            // If the user has entered prefix, the model must start with prefix to be adopted.
...
impl NextCommandModel {
    ...
    pub fn generate_next_command_suggestion_with_prefix(
        ...
        // BYOP cfg must be resolved before spawn (&AppContext cannot be obtained in spawn).
        // Use None terminal_view_id to access the global currently active profile.
