...
fn init_local_crash_reporting(
    ...
    log::info!("openWarp: crash reporting uses local panic log, does not report to remote");
    ...
    PANIC_HOOK_INSTALLED.call_once(|| {
        ...
        std::panic::set_hook(Box::new(move |panic_info| {
            ...
            // Still call the original hook so that default behavior like stderr continues
...
/// Compatibility entry point to suspend local crash reporting state before a child process is spawned.
pub fn suspend_crash_reporting_for_child_spawn() {
    log::info!("openWarp: macOS native crash reporter stripped, skipping pause");
}

pub fn resume_crash_reporting_after_child_spawn() {
    log::info!("openWarp: macOS native crash reporter stripped, skipping resume");
}
...
/// Keep the public signature for login status callback usage, but local crash reporting does not record user identity.
