// Waz: telemetry sending layer and context provider have been removed.
// Only `TelemetryEvent` enum and its auxiliary types are kept here, serving as a type shell for numerous UI/model call sites.
...
// Compatibility metadata for local Waz Drive object event shells.
...
/// Common metadata retained for local Waz Drive event call sites that act on a specific object.
...
pub struct ObjectTelemetryMetadata {
    ...
    /// Legacy server UID slot. Waz keeps it optional while object-event call sites are being
    /// localized.
...
// Waz Phase 2a: `OpenedSharingDialogEvent` + `SharingDialogSource` and
...
pub enum TelemetryEvent {
    ...
    /// The native app was opened while logged out. Since Waz requires login,
    ...
    /// Opened legacy Waz AI.
    OpenedWarpAI {
        source: OpenedWarpAISource,
    },
    /// Issued legacy Waz AI request.
    WarpAIRequestIssued {
    ...
    // Toggled the legacy Waz AI side panel.
    ToggleWarpAI {
        opened: bool,
    },
    ...
    /// This is a telemetry event used to help track performance of Agent Predict in Waz,
    ...
    AgentModePrediction {
        ...
        // Waz leaves these optional; no telemetry sender consumes them.
    ...
    PromptSuggestionShown {
        ...
        /// suggestion. Waz keeps it optional for local diagnostics only.
    ...
    SuggestedCodeDiffBannerShown {
        ...
        block_id: Option<String>,
        request_duration_ms: u64,
        /// Legacy request token from the `/passive-suggestion` request. Waz keeps it optional
    ...
    /// Detected that Waz is running in an isolated sandbox.
