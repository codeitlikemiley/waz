...
pub enum SettingsPageViewHandle {
    ...
    // Waz Wave 3-1: `OzCloudAPIKeys` variant is physically deleted along with `platform_page`.
    // The cloud API key management UI completely represents the Waz Inc cloud account and has nothing to do with BYOP.
    // Waz Wave 6-8:`SharedBlocks` / `Referrals` variant comes with `ShowBlocksView` /
    // `ReferralsPageView` is physically deleted from the corresponding ServerApi client trait.
    // Waz Wave 7-3: `CloudEnvironments` variant has been physically removed with the ambient-agent UI subsystem.
    Warpify(ViewHandle<WarpifyPageView>),
    AI(ViewHandle<AISettingsPageView>),
    MCPServers(ViewHandle<MCPServersSettingsPageView>),
    WazDrive(ViewHandle<WarpDriveSettingsPageView>),
...
impl SettingsPageViewHandle {
    pub fn child_view(&self) -> Box<dyn Element> {
        ...
        match self {
            ...
            // Waz Wave 3-1: `OzCloudAPIKeys` arm is physically deleted along with `platform_page`.
            // Waz Wave 6-8: `SharedBlocks` / `Referrals` arm is physically deleted with the variant.
            // Waz Wave 7-3: `CloudEnvironments` arm is physically deleted along with ambient-agent UI.
            Warpify(view_handle) => ChildView::new(view_handle).finish(),
            AI(view_handle) => ChildView::new(view_handle).finish(),
            MCPServers(view_handle) => ChildView::new(view_handle).finish(),
            WazDrive(view_handle) => ChildView::new(view_handle).finish(),
...
pub enum SettingsPageEvent {
    ...
    // Waz Wave 7-3:`EnvironmentSetupModeSelectorToggled` /
    // `AgentAssistedEnvironmentModalToggled` is physically removed with the ambient-agent UI subsystem.
}
...
impl<V: warpui::View> PageType<V> {
    ...
    /// Get the scroll state handle (vertical, horizontal) of the current page and use it to retain the scroll position across instances during rebuild.
    /// The scroll state of `Monolith` is optional and is only returned when both non-empty handles exist.
    ...
    /// Replace the current page's internal handle with the old page's scroll handle, preserving the scroll position.
