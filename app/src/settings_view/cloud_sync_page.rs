...
use waz_sync::{GistClient, SyncEngine, SyncPlatform, SyncResult};
...
pub enum CloudSyncPageAction {
    ...
    /// Save the Token of the current platform
    SaveToken,
    /// Clear the Token of the current platform
    ClearToken,
    /// Token validation completed. platform/token is captured during SaveToken to avoid race conditions with SetPlatform
    TokenValidated {
        platform_setting: SyncPlatformSetting,
        token: String,
        result: Result<String, String>,
    },
    /// Request upload sync (pop up confirmation dialog to avoid accidentally overwriting cloud history)
    Upload,
    /// Download sync (using the currently selected platform)
    Download,
    /// Callback for asynchronous sync completion
    SyncComplete {
        platform: SyncPlatform,
        direction: SyncDirection,
    ...
    /// Confirm upload — token is captured in the View field, no need to pass via action (avoiding String clone overhead)
...
pub struct CloudSyncPageView {
    ...
    /// The token captured when entering the Conflict state, used during Force Upload to prevent the user from switching platforms during confirmation
    conflict_token: String,
    download_confirm_visible: bool,
    download_confirm_platform: SyncPlatform,
    /// A snapshot of the token captured when the download confirmation dialog is opened, used directly on Confirm,
    /// to avoid the spawn using wrong credentials if the user switches platforms or clears the token during confirmation
...
/// Construct Token password editor
...
/// Sync Dropdown selected state from CloudSyncSettings
...
/// Load current platform Token from CloudSyncTokenStore to the editor
...
/// Get the Token corresponding to the currently selected platform (read from OS keychain)
fn current_token(ctx: &AppContext) -> String {
    let platform = *CloudSyncSettings::as_ref(ctx).sync_platform.value();
    token_for_platform(ctx, platform.to_sync_platform())
}

/// Get the Token corresponding to the specified SyncPlatform, independent of the current dropdown selection.
/// Used for force_upload recapture scenario: must read the token of the platform that conflict belongs to,
/// rather than the new platform the user might switch to during the conflict.
...
impl CloudSyncPageView {
    ...
    /// Construct the modal that should currently be rendered as an overlay (conflict / download confirmation / upload confirmation).
    /// Centered by Stack within CloudSyncPageWidget::render using ParentOffsetBounds::WindowByPosition,
    /// must be called from the render path of this View to ensure click events can be routed back to handle_action
    /// (an overlay rendered by SettingsView would lose the view chain).
    fn build_modal_element(&self, appearance: &Appearance) -> Option<Box<dyn Element>> {
        ...
        if self.upload_confirm_visible {
            // Use Accent (theme main color) instead of Warn (yellow warning color); Force Upload uses Warn
            let confirm_button = Container::new(
                appearance
                    .ui_builder()
    ...
    /// Start upload sync. The token is captured by the caller when the dialog opens, ensuring it pairs with the platform.
    fn spawn_upload(&mut self, platform: SyncPlatform, token: String, ctx: &mut ViewContext<Self>) {
        ...
        // Save the current token as conflict_token, to be reused during Force Upload retry if upload returns Conflict
    ...
    /// Start download sync. The token is captured by the caller when the dialog opens.
    ...
    /// Start force upload sync (overwrite remote). The token comes from the snapshot when the conflict popped up.
    ...
    fn handle_action(&mut self, action: &Self::Action, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            CloudSyncPageAction::SaveToken => {
                ...
                // Capture platform + token when dispatching to prevent writing to the wrong keychain key if the user switches platforms during async operations
            ...
            CloudSyncPageAction::TokenValidated {
                ...
                match result {
                    Ok(username) => {
                        ...
                        // Use the platform / token captured during dispatch to write to keychain, not the current context
                        ...
                        // Update UI status only if the currently displayed platform matches the verified platform;
                        // otherwise, if the user has switched to another platform, the verification result should not overwrite the current UI
            ...
            CloudSyncPageAction::Upload => {
                ...
                // Return early: if token is empty, directly set to Failed, do not pop up the confirmation box that can only fail (PR #161 third round review)
                if token.is_empty() {
                    let label = platform.label();
                    self.sync_state = SyncState::Failed {
                        message: crate::t!("settings-cloud-sync-token-not-configured", platform = label.to_string()),
                ...
                // Upload is destructive as it overwrites cloud history, popping up double confirmation symmetrically with download.
                // Capture a token snapshot immediately when the dialog opens, to avoid spawn_upload using the wrong credentials
                // if the user switches platforms or clears the token during confirmation (PR #161 second round review)
            ...
            CloudSyncPageAction::SyncComplete {
                ...
                match result {
                    Ok(SyncResult::Success { version, .. }) => {
                        ...
                        // Non-conflict outcome: clear conflict_token to avoid PAT residing in view memory for a long time
                    ...
                    Ok(SyncResult::Conflict {
                        ...
                        // Refresh conflict_token when entering Conflict to avoid subsequent Force Upload using an empty token:
                        // - First conflict: spawn_upload has written token into conflict_token, re-overwriting here is harmless
                        // - force_upload returns Conflict again: previous mem::take cleared conflict_token,
                        //   must recapture based on the platform where the conflict belongs (rather than the current dropdown platform,
                        //   which the user might switch during conflict; PR #161 fourth round review)
            ...
            CloudSyncPageAction::CancelConflict => {
                self.conflict_visible = false;
                self.sync_state = SyncState::Idle;
                // Maintain symmetry with CancelUploadConfirm / CancelDownloadConfirm, clear residual PAT
                self.conflict_token.clear();
                ctx.notify();
            }
...
impl SettingsWidget for CloudSyncPageWidget {
    ...
    fn render(
        ...
        // Sync scope note — placed at the top of the page as the primary tip
        ...
        // Token Editor — use the text_input component to get consistent borders and layout constraints
        ...
        // Keep a 12px gap with the version info list below to prevent buttons from sticking to the Local Version label
        content.add_child(
        ...
        // Sync operation status (differentiated by color)
        ...
        // Conflict / download confirmation / upload confirmation dialog — constructed in the render path of this View,
        // using Stack overlay child (WindowByPosition + Center) to center the window,
