...
// For the description text of the AI ​​settings page, please refer to the settings-ai-* section of app/i18n/{en,zh-CN}/warp.ftl for i18n, key.
...
impl AISettingsPageView {
    pub fn new(ctx: &mut ViewContext<Self>) -> Self {
        ...
        // third-party coding agents rather than Waz's own AI.
        Self::update_editor_interaction_state(
    ...
    /// Rebuild the widget list of the current subpage.
    /// The internal state used for widgets relies on complex collections in `AISettings` (such as custom Agent
    /// Provider list), the ViewHandle held by the widget needs to be re-created when the collection size changes.
    pub fn rebuild_current_page(&mut self, ctx: &mut ViewContext<Self>) {
        // Reuse the scroll handle of the old page to avoid jumping back to the top after reconstruction.
    ...
    fn save_agent_provider_edits(
        ...
        AISettings::handle(ctx).update(ctx, |settings, ctx| {
            ...
            if let Some(p) = providers.iter_mut().find(|p| p.id == provider_id) {
                ...
                // Update according to model_index, skipping out-of-bounds indexes (the intermediate form of rebuild may be temporarily inconsistent with settings).
...
pub enum AISettingsPageAction {
    ...
    ToggleShowAgentTips,
    /// Switch the "Show Agent shortcut key hints" setting (zero status three-piece set + 4 hints at the bottom of the message bar).
    ...
    /// chat_stream explicitly binds genai AdapterKind accordingly, bypassing model name recognition.
    ...
    /// extra_headers/models). Replace the original "out of focus/Enter push field by field" UX - the user
    /// Settings_view Click the "Save" button and send it together.
    SaveAgentProviderEdits {
        ...
        /// Only the editable part is carried: `(model_index, name, id, context_window, max_output_tokens)`.
        /// reasoning / tool_call / image / pdf / audio are maintained by independent chip actions, do not go here.
    ...
    /// Update the context_window(tokens) of a single model, 0 = unspecified.
    UpdateAgentProviderModelContextWindow {
        provider_id: String,
        model_index: usize,
        context_window: u32,
    },
    /// Update max_output_tokens of a single model, 0 = not specified.
    ...
    /// Trigger a loading of the models.dev directory (disk cache + network refresh if necessary). Triggered when the Providers subpage is opened.
    EnsureModelsDevLoaded,
    /// Force refresh of models.dev directory (ignoring TTL). "Refresh" button fires.
    RefreshModelsDev,
    /// Create a new provider from the models.dev directory: backfill name/base_url/all models (including context).
    AddProviderFromModelsDev {
        catalog_provider_id: String,
    },
    /// Synchronize the existing provider's model list with models.dev (matched by base_url),
    /// Populate local entries with metadata such as context_window / reasoning / tool_call provided by catalog.
    SyncProviderModelsFromModelsDev {
        provider_id: String,
    },
    /// Collapse/expand the "quick add" chip row.
    ToggleModelsDevChipsExpanded,
    /// Set the search query for "quick add" chip rows (substring filtering provider name/id).
    SetModelsDevSearchQuery(String),

    // ----- Single model entry detail panel -----
    /// Toggle the expanded/collapsed state of the detail panel of a single model.
    ToggleAgentProviderModelExpanded {
        provider_id: String,
        model_index: usize,
    },
    /// The three-state cycle switches a certain multi-modal capability of a single model (image/pdf/audio).
    ...
    /// Toggle the reasoning flag of a single model (normal bool field, not tri-state).
    ToggleAgentProviderModelReasoning {
        provider_id: String,
        model_index: usize,
    },
    /// Toggle the tool_call flag of a single model.
...
/// model detail panel Three-state capability chip type.
        ...
        match action {
            ...
            AISettingsPageAction::SetAgentProviderApiType {
                ...
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    ...
                    if let Some(p) = providers.iter_mut().find(|p| p.id == *provider_id) {
                        p.api_type = *api_type;
                        // If base_url is empty, fill in the default endpoint of this type (convenient for novices).
                        // It does not move when the user has filled in the base_url himself.
            ...
            AISettingsPageAction::AddAgentProviderModel { provider_id } => {
                ...
                // Row-level add requires a new EditorView, so rebuild; rebuild_current_page already retains scrolling.
                self.rebuild_current_page(ctx);
            }
            AISettingsPageAction::RemoveAgentProviderModel {
                ...
                // Deleting one will cause subsequent index drift. Clear all expansion records of this provider to avoid accidental expansion.
                super::agent_providers_widget::clear_expanded_models_for_provider(provider_id);
                self.rebuild_current_page(ctx);
            }
                    ...
                    move |view, result, ctx| match result {
                        Ok(fetched) => {
                            AISettings::handle(ctx).update(ctx, |settings, ctx| {
                                ...
                                {
                                    // Merge retention: The existing id retains the name changed by the user, and the new id is appended.
                                    // Local redundant IDs will not be deleted (user manual ×).
                            ...
                            // The number of model rows may have changed, and the widget rows need to be rebuilt.
            ...
            AISettingsPageAction::AddAgentProviderHeader { provider_id } => {
                ...
                // After the number of header rows changes, you need to create/destroy the EditorView handle. Only notify will not refresh the rows.
                self.rebuild_current_page(ctx);
            }
            AISettingsPageAction::RemoveAgentProviderHeader {
                ...
                // Deletion will also cause the index to drift with the existing HeaderRow handle, requiring the page to be rebuilt.
                self.rebuild_current_page(ctx);
            }
            ...
            AISettingsPageAction::AddProviderFromModelsDev {
                ...
                let Some(cat_provider) = catalog.get(catalog_provider_id) else {
                    log::warn!("[models.dev] provider id not found in catalog: {catalog_provider_id}");
                    return;
                };
            ...
            AISettingsPageAction::SyncProviderModelsFromModelsDev { provider_id } => {
                ...
                // Matching strategy: First, base_url is completely equal/contained; otherwise, match catalog provider id or name by name (case-independent).
                ...
                let Some((_, cat_provider)) = cat_provider else {
                    log::warn!(
                        "[models.dev] no match found in catalog (base_url={}, name={})",
                        local.base_url,
                        local.name
                    );
                    return;
                ...
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    let mut providers = settings.agent_providers.value().clone();
                    if let Some(p) = providers.iter_mut().find(|p| p.id == *provider_id) {
                        // Existing ids are overwritten with catalog metadata; extra catalog items are appended; extra local items (user-defined) are kept.
                        for local_model in p.models.iter_mut() {
                            if let Some(cat_m) = cat_models.get(&local_model.id) {
                                ...
                                // Multimodal capability: **only fill None slots**, Some(_) is treated as explicitly overridden
                                // by the user and will not be touched by sync. This way:
                                // - First sync (untouched by user) → write all catalog inference results
                                // - User manually cycles to Some(true/false) then syncs → keeps override
                                // - User cycles three-state back to None (Auto) → next sync will fill it again
                ...
                AISettings::handle(ctx).update(ctx, |settings, ctx| {
                    ...
                    if let Some(p) = providers.iter_mut().find(|p| p.id == *provider_id) {
                        if let Some(m) = p.models.get_mut(*model_index) {
                            ...
                            // Three-state loop: None → Some(true) → Some(false) → None.
                            *slot = match *slot {
                                None => Some(true),
        ...
        let subtext = {
            let subtext_fragments = vec![
                FormattedTextFragment::plain_text(
                    "You haven't added any MCP servers yet. Once you do, you'll be able to control how much autonomy the Waz Agent has when interacting with them. ",
                ),
...
struct AIInputWidget {
    ...
    // Switch state handle corresponding to the "Show Agent shortcut hints" toggle.
...
impl SettingsWidget for AIInputWidget {
    ...
    fn render(
        ...
        // "Show Agent shortcut hints": controls the zero-state triple and the 4 hint items at the bottom of the message bar.
...
impl SettingsWidget for MCPServersWidget {
    ...
    fn render(
        ...
        let mcp_description = vec![
            FormattedTextFragment::plain_text(
                "Add MCP servers to extend the Waz Agent's capabilities. \
...
impl SettingsWidget for AIFactWidget {
    ...
    fn search_terms(&self) -> &str {
        "agent oz ai a.i. knowledge fact memory memories rules waz drive context workflows notebooks environment variables"
    }
    ...
    fn render(
        ...
        // Decentralized branch: no longer renders the "Waz Drive as agent context" switch.
        let _ = self;
        let _ = view;
...
impl VoiceWidget {
    fn render_voice_section(
        ...
        let voice_input_description_text_fragments = vec![
            FormattedTextFragment::plain_text(
                "Voice input allows you to control Waz by speaking directly to your terminal (powered by ",
            ),
...
impl SettingsWidget for CLIAgentWidget {
    ...
    fn render(
        ...
        // agents (Claude Code, Codex, Gemini CLI) rather than Waz's own AI.
