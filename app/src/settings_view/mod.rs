...
// Waz Wave 3-1: `platform` / `platform_page` are physically deleted along with the `OzCloudAPIKeys` settings entry point +
// Waz Inc cloud API key management UI.
// Waz Wave 6-8: `referrals_page` / `show_blocks_view` are physically deleted along with `ReferralsClient` /
// `BlockClient` trait -- both pages are stubbed to return Err / empty list, as they have no value locally.
mod settings_file_footer;
pub(crate) mod settings_page;
// Waz Wave 7-3: `telemetry` is physically deleted along with the only variant `EnvironmentsPageOpened` (ambient-agent UI).
// Waz Wave 7-2: `update_environment_form` is physically deleted along with the cloud ambient agent subject --
// `terminal::view::ambient_agent::first_time_setup` is retired along with `cloud_environments`.
mod warp_drive_page;
mod warpify_page;
...
pub enum SettingsViewEvent {
    Pane(PaneEvent),
    StartResize,
    // Waz decentralized branch: `CheckForUpdate` / `WazDrive` variants are physically deleted along with
    // the only emitter on the Account main settings page (`MainSettingsPageView`).
    ShowToast {
        message: String,
        flavor: ToastFlavor,
    },
...
pub enum SettingsSection {
    ...
    // Decentralized branch: The default page in Settings is changed to Waz Agent (local AI settings).
    #[default]
    WarpAgent,
    AgentProfiles,
    AgentMCPServers,
    /// Custom AI provider configuration (BYOP), the Providers subpage in the Agents secondary menu.
    AgentProviders,
    Knowledge,
    ThirdPartyCLIAgents,
    ...
    CloudSync,
    // Waz Wave 3-1: `OzCloudAPIKeys` enum variant is physically deleted along with the Waz Inc API key management UI.
    // Waz Wave 7-3: `CloudEnvironments` is physically deleted along with the ambient-agent UI subsystem.
}
...
impl Display for SettingsSection {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s: String = match self {
            ...
            SettingsSection::WazDrive => crate::t!("settings-section-warp-drive"),
            ...
            // Proxy settings page. The i18n key `settings-section-network` is complete in three languages: en / zh-CN / ja.
            SettingsSection::Network => crate::t!("settings-section-network"),
            // Waz Wave 3-1: `OzCloudAPIKeys` Display arm is physically deleted along with the variant.
            // Waz Wave 7-3: `CloudEnvironments` Display arm is physically deleted along with the variant.
        };
        write!(f, "{s}")
    }
}
...
impl SettingsSection {
    ...
    pub fn parent_page_section(&self) -> Self {
        match self {
            ...
            // Waz Wave 3-1: `OzCloudAPIKeys` is physically deleted along with the UI.
            // Waz Wave 7-3: `CloudEnvironments` umbrella is physically deleted along with the ambient-agent UI.
            other => *other,
        }
    }
...
impl FromStr for SettingsSection {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        match s {
            ...
            // Waz Wave 3-1: `OzCloudAPIKeys` is physically deleted along with the UI.
            // Waz Wave 7-3: `CloudEnvironments` FromStr arm is physically deleted along with the variant.
            _ => Err(()),
        }
    }
}
...
pub enum SettingsAction {
    ...
    WazDrive(warp_drive_page::WarpDriveSettingsPageAction),
...
macro_rules! update_page {
    ($handle:expr, $update:expr, $ctx:expr) => {
        match $handle {
            ...
            // Waz Wave 3-1: `OzCloudAPIKeys` arm is physically deleted along with the variant.
            // Waz Wave 6-8: `SharedBlocks` / `Referrals` arm is physically deleted along with the variant.
            // Waz Wave 7-3: `CloudEnvironments` arm is physically deleted along with the ambient-agent UI.
            ...
            SettingsPageViewHandle::WazDrive(handle) => $ctx.update_view(handle, $update),
...
impl SettingsView {
    pub fn new(page: Option<SettingsSection>, ctx: &mut ViewContext<Self>) -> Self {
        ...
        // Waz Wave 6-8: Shared blocks settings page is physically deleted along with `ShowBlocksView` / `BlockClient`,
        // handle / event subscriptions are removed as well.
        ...
        // Environments page
        // Waz Wave 7-3: `environments_page_handle` is physically deleted along with the ambient-agent UI subsystem.
        ...
        // Waz Wave 6-8: Referrals settings page is physically deleted along with `ReferralsPageView` / `ReferralsClient`,
        // handle / event subscriptions are removed as well.
        ...
        // Waz Wave 3-1: `platform_page_handle` is physically deleted along with `platform_page`.
        ...
        let mut settings_pages = vec![
            ...
            // Waz Wave 3-1: `platform_page_handle` is physically deleted along with the UI.
        ...
        // Assemble Network page only when the flag is enabled.
        ...
        // Decentralized branch: Under local mode, remove all settings entries related to
        // cloud accounts / billing / teams / synchronization / sharing.
        ...
        // Add Network page to sidebar only when the flag is enabled. Insert before About.
        if warp_core::features::FeatureFlag::HttpProxySettings.is_enabled() {
    ...
    pub fn focus(&mut self, ctx: &mut ViewContext<Self>) {
        self.focus_search_editor_if_no_child_is_focused(ctx);
        ctx.emit(SettingsViewEvent::Pane(PaneEvent::FocusSelf));
    }

    fn focus_search_editor_if_no_child_is_focused(&self, ctx: &mut ViewContext<Self>) {
        if ctx.is_self_focused() || !ctx.is_self_or_child_focused() {
    ...
    // Waz Wave 7-3: `handle_environments_page_event` is physically deleted along with the ambient-agent UI subsystem.
    ...
    // Waz Wave 3-1: `handle_platform_page_event` is physically deleted along with `platform_page::PlatformPageViewEvent`.
    ...
    pub fn set_and_refresh_current_page_internal(
        ...
        // Waz Wave 7-3: `SettingsTelemetryEvent::EnvironmentsPageOpened` is physically deleted along with the ambient-agent UI
        // subsystem.
    ...
    fn should_render_page(&self, settings_page: &SettingsPage, app: &AppContext) -> bool {
        match &settings_page.view_handle {
            ...
            SettingsPageViewHandle::WazDrive(v) => v.as_ref(app).should_render(app),
            // Issue #72: Global HTTP proxy settings page.
    ...
    fn get_modal_content_for_page(
        ...
        match page_handle {
            // Waz Wave 3-1: `OzCloudAPIKeys` modal arm is physically deleted along with the UI.
            SettingsPageViewHandle::MCPServers(view) => {
                view.read(app, |view, _| view.get_modal_content(app))
            }
            // CloudSync modal renders itself centered via Stack inside the page, not intercepted here
            // (since the SettingsView path cannot route CloudSyncPageAction back to CloudSyncPageView)
            _ => None,
        }
    }
...
impl View for SettingsView {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // Waz Wave 7-3: environment setup mode selector / agent-assisted environment
        // modal overlay rendering is physically deleted along with the ambient-agent UI subsystem.
...
impl TypedActionView for SettingsView {
    ...
    fn handle_action(&mut self, action: &SettingsAction, ctx: &mut ViewContext<Self>) {
        match action {
            ...
            SettingsAction::WazDrive(warp_drive_action) => {
                if let Some(warp_drive_page) = self.settings_page(SettingsSection::WazDrive) {
                    if let SettingsPageViewHandle::WazDrive(view) = &warp_drive_page.view_handle {
                        view.update(ctx, |view, ctx| {
