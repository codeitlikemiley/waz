...
///
/// Lists known repos from `PersistedWorkspace` that are not yet present in the user's
/// `directory_tab_colors` with a non-`Suppressed`
...
impl DirectoryColorAddPicker {
    ...
    fn refresh_items(&mut self, ctx: &mut ViewContext<Self>) {
        // PersistedWorkspace has been deprecated, and there are no longer candidate sources for "previously opened git repositories",
        // so we give the picker an empty collection, making the dropdown always fall back to only showing the `+ Add directory…` button.
