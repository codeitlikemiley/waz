...
impl StatusColor {
    fn to_color(&self, appearance: &Appearance) -> ColorU {
        match self {
            ...
            // Use the raw ANSI color for the status indicator dot to avoid mixing saturation in half onto the foreground via ansi_fg_*,
            // which would cause a blurry "gray-green/gray-yellow" appearance in low-saturation foreground themes like One Dark or Tokyo Night.
    ...
    fn from(status: ServerCardStatus) -> ServerCardOptions {
        match status {
            ...
            ServerCardStatus::Installed => ServerCardOptions {
                ...
                status_line: Some(crate::t!("settings-mcp-card-status-offline")),
                background: Background::Filled,
                full_card_clickable: false,
            },
            ServerCardStatus::StartingServer => ServerCardOptions {
                ...
                status_line: Some(crate::t!("settings-mcp-card-status-starting")),
                background: Background::Filled,
                full_card_clickable: false,
            },
            ServerCardStatus::Authenticating => ServerCardOptions {
                ...
                status_line: Some(crate::t!("settings-mcp-card-status-authenticating")),
                background: Background::Filled,
                full_card_clickable: false,
            },
            ...
            ServerCardStatus::ShuttingDown => ServerCardOptions {
                ...
                status_line: Some(crate::t!("settings-mcp-card-status-shutting-down")),
                background: Background::Filled,
                full_card_clickable: false,
            },
...
impl ServerCardView {
    ...
    fn render_tools_expandable(
        ...
        if tools.is_empty() {
            return Text::new(
                crate::t!("settings-mcp-card-tools-none"),
        ...
        Hoverable::new(toggle_mouse_state, move |_is_hovered| {
            Flex::row()
                ...
                .with_child(
                    Text::new(
                        crate::t!("settings-mcp-card-tools-available", count = tools.len()),
                        appearance.ui_font_family(),
                        appearance.ui_font_size(),
                    )
                    .with_color(text_color)
    ...
    fn render_actions_row(&self, state: &MouseState, appearance: &Appearance) -> Box<dyn Element> {
            ...
            if self.render_options.show_view_logs_icon_button {
                actions_row = actions_row.with_child(
                    self.build_icon_button(
                        appearance,
                        Icon::Code1,
                        crate::t!("settings-mcp-card-tooltip-show-logs"),
            ...
            if self.render_options.show_log_out_icon_button {
                actions_row = actions_row.with_child(
                    self.build_icon_button(
                        appearance,
                        Icon::LogOut,
                        crate::t!("settings-mcp-card-tooltip-log-out"),
            ...
            if self.render_options.show_share_icon_button {
                actions_row = actions_row.with_child(
                    self.build_icon_button(
                        appearance,
                        Icon::Share,
                        crate::t!("settings-mcp-card-tooltip-share-server"),
            ...
            if self.render_options.show_edit_config_icon_button {
                actions_row = actions_row.with_child(
                    self.build_icon_button(
                        appearance,
                        Icon::Pencil,
                        crate::t!("settings-mcp-card-tooltip-edit"),
        ...
        if self.render_options.show_view_logs_text_button {
            let view_logs_button = appearance
                ...
                .with_centered_text_label(crate::t!("settings-mcp-card-button-view-logs"))
                .build()
                .on_click(move |ctx, _, _| {
                    ctx.dispatch_typed_action(ServerCardAction::ViewLogs(item_id))
                })
        ...
        if self.render_options.show_edit_config_text_button {
            let edit_config_button = appearance
                ...
                .with_centered_text_label(crate::t!("settings-mcp-card-button-edit-config"))
                .build()
                .on_click(move |ctx, _, _| {
                    ctx.dispatch_typed_action(ServerCardAction::Edit(item_id));
                })
        ...
        if self.render_options.show_setup_text_button {
            let setup_button = appearance
                ...
                .with_centered_text_label(crate::t!("settings-mcp-card-button-set-up"))
                .build()
                .on_click(move |ctx, _, _| {
    ...
    fn render_update_available_icon(&self, appearance: &Appearance) -> Box<dyn Element> {
        ...
        let update_available_button = self
            .build_icon_button(
                appearance,
                Icon::Refresh,
                crate::t!("settings-mcp-card-tooltip-update-available"),
