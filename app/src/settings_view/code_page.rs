//! Code Settings Page: After the LSP full stack + persistent workspace history were deprecated in Waz,
//! this page only contains a few local toggles related to "editor and code review".
//!
//! Historically this page also carried the LSP management subpage + codebase indexing, but those have been deprecated;
//! `Code` is no longer an umbrella in the sidebar (no second subpage to attach to), and is now a single-level Page.
//! The page simply renders this group of toggles.
...
impl CodeSettingsPageView {
    pub fn new(ctx: &mut ViewContext<CodeSettingsPageView>) -> Self {
        // Subscribe to ProjectContextModel: rerender when project rules change
        // to keep any child components depending on the rules set up-to-date.
    ...
    /// Construct page widgets. Code is now a single page (no subpages, no category titles),
    /// rendering the "editor and code review" toggles directly.
    #[cfg(feature = "local_fs")]
    fn build_page(
        ctx: &mut ViewContext<Self>,
    ) -> (PageType<Self>, Option<ViewHandle<ExternalEditorView>>) {
        let (widgets, external_editor_view) = if FeatureFlag::WazNewSettingsModes.is_enabled()
        {
            ...
            // legacy view: under the old settings mode, the Code page renders nothing (originally CodePageWidget
            // only rendered a header from the LSP era, which has no practical meaning, so returns an empty page directly).
            (vec![], None)
        };
        (
    ...
    /// Under WASM build, there is no ExternalEditorView, so only 4 non-external editor toggles are rendered.
