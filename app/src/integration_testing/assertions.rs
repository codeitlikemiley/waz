...
pub fn join_a_workspace() -> TestStep {
    TestStep::new("Join a Waz Drive workspace")
        .with_action(move |app, _, _| {
...
// Waz (localization, Phase 2d-4a-1): the original `assert_websocket_has_started` / `assert_websocket_has_not_started`
// assertions depended on the physically deleted `Listener` singleton and had no callers, so they are removed together.
