...
/// Tracks the current request status for making Waz AI requests against server.
...
impl Requests {
    pub fn new(ctx: &mut ModelContext<Self>) -> Self {
        ...
        // Waz: No Waz Inc cloud backend, the initial request_limit_info is no longer pulled from the server;
        // just use the cached / default values above.
    ...
    /// Starts a Waz AI request against the server with the given request prompt.
    pub fn issue_request(&mut self, request: String, ctx: &mut ModelContext<Self>) {
        ...
        let response = "Waz AI Assistant cloud requests are disabled in Waz. Use Agent Mode with a configured BYOP model instead.".to_owned();
