...
/// Whether the title or message will be truncated by `truncate_text`.
...
/// Distinguishes between rendering differences of toast vs mailbox.
...
/// Button theme for the artifact chip inside notifications.
/// Uses an `outline` border so that it remains visible on `surface_2` backgrounds.
...
/// Callback when the user clicks to expand/collapse on truncated messages.
pub(crate) type OnExpandClick = Box<dyn Fn(&mut warpui::EventContext)>;

/// Renders the inner content of the notification item.
/// Dispatched to rich (with branch line) or simple layout depending on whether `item.branch` exists.
...
fn render_rich_text_column(
    ...
    let branch_right: Box<dyn Element> = match context {
        ...
        NotificationRenderContext::Toast => {
            // Do not render chevron if the content can be fully displayed.
            Flex::row().finish()
        }
...
/// Appends artifact buttons and extra content to the text column.
...
/// Title text, truncation length decided by the expanded state.
...
/// Message body text, truncation length decided by the expanded state.
...
/// Creates an `ArtifactButtonsRow` view with a notification-specific theme.
/// The caller needs to subscribe to events of the returned view themselves.
...
/// Handles artifact button events inside the notification view (toast / mailbox).
pub(crate) fn handle_notification_artifact_buttons_event(
    ...
    match event {
        ArtifactButtonsRowEvent::OpenPlan { document_uid } => {
            ...
            // openWarp localization: click the plan button to open the local AIDocument pane,
            // instead of jumping to the cloud notebook mirror.
