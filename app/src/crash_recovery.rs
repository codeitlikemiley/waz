...
struct CrashRecoveryProcess {
    ...
    /// Recent sequence of successful/failed renders for the current window; exits after writing to local logs if there are too many consecutive failures.
    ...
    fn handle_draw_frame_error(&mut self, window_id: WindowId) {
        ...
        *num_errors += 1;
        ...
        if *num_errors >= NUM_DRAW_ERRORS_BEFORE_EXITING {
            ...
            // Stop the local crash reporting process before exiting to avoid leaving behind background child processes.
