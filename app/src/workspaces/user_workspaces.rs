...
impl UserWorkspaces {
    ...
    /// are on the Waz Plan or the build is dogfood (both our internal Waz team and dogfood
    ...
    // Waz: Team space is the entrance to cloud collaboration, and the local version does not expose any Team space.
    pub fn team_spaces(&self) -> Vec<Space> {
        vec![]
    }

    // Waz:Drive only keeps local personal space. Team/Shared are both cloud collaboration interfaces.
    // Even if there is still workspace metadata in the old cache, you cannot re-enter Drive or Workflow UI.
    pub fn all_user_spaces(&self, ctx: &AppContext) -> Vec<Space> {
        let _ = ctx;
        vec![Space::Personal]
    }

    // Waz (localized branch) personal space owner is fixedly bound to the local placeholder user.
    // It must remain stable, otherwise the owner field of the old object will not match after restarting, and the old data will be "invisible" in the Personal Space list.
    fn effective_personal_user_uid() -> UserUid {
        UserUid::new(TEST_USER_UID)
    }

    // Returns the [`Owner`] for the user's personal drive.
    // Workflow / EnvVar / Folder / Notebook / Import under Waz:Drive Personal space
    // The Create actions are uniformly attributed to the local placeholder user (only local sqlite is persisted).
    ...
    pub fn owner_to_space(&self, owner: Owner, ctx: &AppContext) -> Space {
        ...
        match owner {
            Owner::User { user_uid } => {
                ...
                // Waz: Compare with effective_personal_user_uid to ensure there is no auth.
                // The local Owner(user_uid="waz") is also classified as Personal instead of Shared.
    ...
    pub fn remove_user_from_team(
        ...
        // Waz (localization): Remove member path in local without remote team write target → no-op.
    ...
    pub fn add_invite_link_domain_restrictions(
        ...
        // Waz (localization): The domain restriction path has no remote team/invite writing target in the local → Send Success event to prevent the UI from getting stuck.
    ...
    pub fn refresh_ai_overages(&mut self, _ctx: &mut ModelContext<Self>) {
        // Waz (localization, Phase 5): local cloud-free AI overages query, no-op.
        // The call site (`blocklist/controller.rs::maybe_refresh_ai_overages`) UI does not initiate meaningful updates.
    }
    ...
    pub fn is_ai_allowed_in_remote_sessions(&self) -> bool {
        // Waz does not have a managed organization policy and remote SSH sessions are always allowed to use local Agent capabilities.
        true
    }
...
// Waz (localization, Phase 5): `user_workspaces_tests.rs` all for team RPC paths (`MockTeamClient` / `mockall::Sequence`),
// After localization, these paths are unreachable and the entire file is physically deleted.
