...
pub struct ServerBufferTracker {
    /// Bidirectional mapping wire path ↔ FileId, search on both sides is O(1).
    /// `path_for_file_id` will be called every time `ServerLocalBufferUpdated` is called,
    /// Avoid linear scans with BiMap.
    open_buffers: BiMap<String, FileId>,
    /// Holds a **strong reference** to `ModelHandle<Buffer>` for each opened server-local buffer.
    ///
    /// `GlobalBufferModel` only stores `WeakModelHandle` internally, and the client relies on the editor view to hold it
    /// A strong reference keeps the buffer alive; but the daemon has no view - if it doesn't hold a strong reference here,
    /// After `handle_open_buffer` returns, the buffer reference count is reset to zero and will be used by WarpUI.
    /// `flush_effects` is recycled, causing the weak handle to be used when the subsequent `FileModel` asynchronous loading is completed.
    /// Failure (log "Cannot populate buffer with content due to deallocated model
    /// handle"). Drop when the buffer is closed (no connection).
...
impl ServerBufferTracker {
    ...
    /// Register a wire path → FileId mapping, and hold a strong reference to buffer so that it can
    /// The daemon side is alive (see the `buffer_handles` field description).
    ...
    /// Return owned `String` instead of `&str`, allowing the caller to borrow the result while holding it
    /// Others `&mut self` (Typical scenario: get the path in the event handler and then go back
    /// `send_server_message(...)`). The push frequency is not high and the clone overhead is negligible.
    ...
    pub fn remove_connection(
        ...
        // Discard all pending requests generated by the connection to avoid leaving stale entries after disconnection.
        ...
        for &file_id in &orphaned {
            ...
            // Release the strong reference, allowing the buffer to be recycled.
