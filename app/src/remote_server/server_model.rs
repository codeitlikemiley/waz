...
// Buffer-sync related: relies on GlobalBufferModel, the latter's server-local operation is only in
// Available under `local_fs`, so the entire set of server-side buffer processing is gated according to `local_fs`.
...
pub struct ServerModel {
    ...
    /// per SSH session / Waz tab connecting to this host.  Each entry maps
...
impl ServerModel {
        ...
        #[cfg(feature = "local_fs")]
        {
            ...
            ctx.subscribe_to_model(&gbm, |me, event, ctx| match event {
                ...
                GlobalBufferModelEvent::ServerLocalBufferUpdated {
                    ...
                    // state is inconsistent (an empty path will break the path↔buffer contract).
    ...
    pub fn handle_message(
        ...
        let outcome = match msg.message {
            ...
            // Waz: Directory enumeration of remote terminal file links (used to verify the path form).
            #[cfg(feature = "local_fs")]
    ...
    fn handle_open_buffer(
        ...
        // Track path → FileId mapping and connection. track_open_buffer also holds
        // Strong reference to buffer - daemon does not have an editor view. If it is not held, buffer will be in
        // FileModel is recycled before asynchronous loading is completed (see ServerBufferTracker::buffer_handles).
        self.buffers
    ...
    /// Waz: handles `ListDirectory` - synchronously enumerates direct children of a directory.
    ///
    /// For accurate verification of remote terminal file link detection: the client caches files under a certain cwd
    /// The real directory entry, which the link checker uses to cut out the correct filename from the entire `ls -l` line.
    /// `std::fs::read_dir` is a cheap synchronous call on the daemon side, so it returns directly
    /// `HandlerOutcome::Sync`, does not use asynchronous spawn.
    #[cfg(feature = "local_fs")]
        ...
        let result = match std::fs::read_dir(&path) {
            Ok(read_dir) => {
                ...
                for entry in read_dir.flatten() {
                    ...
                    // Prefer using `file_type()` (does not follow symbolic links, no additional stat required);
                    // Fallback to `metadata()` on failure (symlinks will be followed).
