...
pub enum StartupArgsForwardingError {
    /// There's no instance of Waz already running.
...
impl From<zbus::fdo::Error> for StartupArgsForwardingError {
    fn from(value: zbus::fdo::Error) -> Self {
        ...
        // registering a service, so this really means that Waz is not already
        // running.
...
// A D-Bus client for connecting to an already-running instance of Waz and
// invoking org.freedesktop.Application IPC methods.
//
// Since `default_service` / `default_path` are overridden on the builder via
// `.destination(well_known_name())` / `.path(application_service_path())` at the caller site
// (`pass_startup_args_to_existing_instance`), the constants here will not be actually used;
// but to avoid misleading future callers who might use default values via `Proxy::new` directly,
// we still point to the default OSS ID `dev.waz.Waz` here.
#[proxy(
    interface = "org.freedesktop.Application",
    default_service = "dev.waz.Waz",
    default_path = "/dev/waz/Waz",
    gen_blocking = false
)]
