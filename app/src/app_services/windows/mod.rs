...
pub enum StartupArgsForwardingError {
    ...
    #[error("there is no other instance of Waz")]
    NoExistingInstance,
...
pub(super) fn init(_ctx: &mut AppContext) {
    ...
    // Registry URI handler registration (warp:// protocol) is a purely synchronous operation writing multiple times to HKCU,
    // and has no dependency on any UI initialization in the main thread. Cold start takes about ~30-60 ms,
    // so we spawn it in a background thread to get it off the critical path. The failure logic was originally just log::error,
    // and running it in the background won't miss any information.
