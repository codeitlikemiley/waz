...
fn theme_chooser_items(theme_config: &WarpThemeConfig) -> Vec<ThemeChooserItem> {
    // Waz is decentralized: there is no referral concept anymore, all topics (including the two original referral-gated
    // Topics) are always visible to local users.
