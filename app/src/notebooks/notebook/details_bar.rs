...
/// * Interactive breadcrumbs for its location within Waz Drive
...
impl DetailsBar {
    ...
    fn render_mode_toggle(
        ...
        if matches!(editability, ContentEditability::RequiresLogin) {
            ...
            // openWarp UX: Add a tooltip to the "pencil" toggle button to prevent users (especially first-time users)
            // from not knowing that this icon is clickable and what happens when clicked. The tooltip expresses
            // "the action that will occur after clicking" based on the current mode, rather than repeating the state text next to it.
