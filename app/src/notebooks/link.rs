...
pub enum LinkTarget {
    ...
    LocalFile {
        ...
        session: Arc<Session>,
        /// Whether or not this file is a Markdown file viewable in Waz.
        is_markdown: bool,
    },
    LocalDirectory {
        path: PathBuf,
...
impl NotebookLinks {
    ...
    /// * Markdown files are opened in Waz (if the `FileNotebooks` feature flag is enabled).
        ...
        match link {
            LinkTarget::Url(url) => {
                if let Some(WarpWebLink::DriveObject(args)) = get_item_data_from_warp_link(&url) {
                    return ctx.emit(LinkEvent::WazDriveLink {
    ...
    pub fn secondary_action(&self, link: &LinkTarget, ctx: &mut ModelContext<Self>) {
        match link {
            ...
            LinkTarget::LocalFile {
                ...
                // The default action for Markdown file links is to open them in Waz. As a
...
pub enum LinkEvent {
    ...
    RefreshLinks,
    #[cfg(feature = "local_fs")]
    /// Emitted when a file should be opened in Waz (code editor or markdown viewer).
    OpenFileWithTarget {
        path: PathBuf,
        target: FileTarget,
