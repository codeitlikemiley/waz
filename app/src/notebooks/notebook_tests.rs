...
fn open_notebook(
    ...
    let load_future = handle.update(app, |view, ctx| {
        view.load(notebook, &WazDriveObjectSettings::default(), ctx)
    });
...
fn test_close_unmodified() {
    ...
    App::test((), |mut app| async move {
        ...
        // Waz (Wave 4): SyncQueue completely removed; the original stop_dequeueing + assert queue length are no longer applicable.
        ...
        app.read(|ctx| {
            ...
            // Waz (Wave 4): SyncQueue completely removed; the original `sync_queue.is_empty()` assertion is no longer applicable.
        })
    });
}
