...
/// A view that allows viewing/execution and editing of a Waz notebook.
...
impl NotebookView {
    ...
    fn title_from_editor(title_editor: &ViewHandle<EditorView>, app: &AppContext) -> String {
        ...
        if title.is_empty() {
            title.push_str(&crate::t!("common-untitled"));
        }
        title
    }
    ...
    fn handle_active_notebook_event(
        ...
        match event {
            ...
            ActiveNotebookDataEvent::CreatedOnServer => {
                ...
                {
                    // TODO(waz-cloud-removal Phase 5): Same as above. The sharing UI has been retired.
                    // Remove ShareableObject injection for notebooks; clean up the ID flow when cloud_object is retired.
                    let _ = id;
                }
            }
    ...
    pub fn load(
        ...
        // TODO(waz-cloud-removal Phase 5): The sharing UI has been retired.
        // Remove ShareableObject injection for notebooks; keep the cloud_object server_id path until Phase 5.
        let _ = notebook.id;
        ...
        self.update_breadcrumbs(ctx);
        // Waz Phase 2a: invitee-driven sharing dialog removed.
                    ...
                    Container::new(
                        Align::new(
                            appearance
                                ...
                                .with_tooltip(move || {
                                    ui_builder
                                        .tool_tip(crate::t!("notebook-tooltip-copy-to-personal"))
                                        .build()
                                        .finish()
                                })
                                .with_text_label(crate::t!("notebook-copy-to-personal"))
                                .build()
                                .on_click(|ctx, _, _| {
                ...
                .wrappable_text(
                    match sync_error {
                        NotebookSyncError::FeatureNotAvailable => {
                            crate::t!("notebook-sync-feature-not-available-message")
                        }
                        NotebookSyncError::InConflict => {
                            crate::t!("notebook-sync-conflict-resolution-message")
                        }
                    },
                    true,
                )
