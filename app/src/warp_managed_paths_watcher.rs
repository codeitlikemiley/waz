...
/// Duration between filesystem watch events for the Waz managed paths watcher, in milliseconds.
#[cfg(not(target_family = "wasm"))]
...
pub(crate) fn ensure_warp_watch_roots_exist() {
    ...
    if let Err(err) = fs::create_dir_all(&data_dir) {
        log::warn!(
            "Failed to create Waz data directory {}: {err}",
            data_dir.display()
        );
    }
    ...
    if config_local_dir != data_dir {
        if let Err(err) = fs::create_dir_all(&config_local_dir) {
            log::warn!(
                "Failed to create Waz config directory {}: {err}",
...
mod tests {
    ...
    fn warp_managed_mcp_config_path_contains_only_warp_home_path() {
        match (
            ...
            (_, _, None) => {}
            _ => panic!("Expected Waz MCP path when home directory is available"),
        }
    }
