//! Pure function single test of `candidates::CandidatesViewModel::rows()`.
//!
//! Only the mapping of "status → row list" is tested here; real IO(`load_candidates`), warpui
//! The runtime is not involved. Coverage matrix: status 5 (not loaded / NotFound / Error /
//! Empty / non-empty Loaded)× fold 2 types × `added_aliases` hit/miss.
...
fn rows_when_state_is_none_is_empty() {
    // The model has just been created and has not been refreshed yet - the panel therefore does not render the section at all.
...
fn rows_marks_added_when_alias_in_added_set() {
    // PRODUCT.md decision E: Imported → `added=true`, the UI displays the "Added" badge.
...
fn rows_propagates_all_candidate_fields() {
    ...
    match &rows[1] {
        CandidateRow::Candidate {
            ...
            // PathBuf::display() delimiters may differ across platforms - use contains instead
            // Literally equal, it is enough to assert that the critical path segment is inside.
...
fn rows_when_collapsed_returns_header_only() {
    // After folding, body(NotFound / Empty / Error / Candidate) is not rendered.
    let vm = CandidatesViewModel::with_state(
        Some(fake_load_result_loaded(
            "/home/u/.ssh/config",
            vec![cand("a"), cand("b")],
...
fn path_display_reflects_state_path() {
    ...
    // path_display returns None when status is empty
