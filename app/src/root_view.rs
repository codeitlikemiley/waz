...
use crate::drive::{ObjectTypeAndId, WazDriveObjectArgs, WazDriveObjectSettings};
...
/// Returns the product name of the current channel, used as the initial window title and the quake/transferred window title.
///
/// Taking `ChannelState::app_id().application_name()` is to make the OSS build show `Waz`,
/// while upstream channels like Stable/Preview/Dev still show their respective `Waz` / `WarpPreview` / `WarpDev`,
/// avoiding hardcoded strings in multiple places in the fork (Windows Task Manager groups processes by window title,
/// and a hardcoded `"Waz"` would make Waz display as `Waz(N)` in the Task Manager).
///
/// Note: After the window is created, `Workspace::update_window_title()` will override this value with the tab title upon every tab switch/rename.
/// Therefore, this function only determines the initial title when the window is first opened and before any tab is attached.
...
///
/// This is the canonical way to open a new Waz window - all other entrypoints should delegate to
/// it if possible.
...
fn open_warp_drive_object(arg: &WazDriveObjectArgs, ctx: &mut AppContext) {
...
/// This action will show or hide all of Waz's windows except the quake window
///
/// - If Waz is active and has any windows, hide those windows.
/// - If Waz is hidden, show all windows.
/// - If Waz is active but has 0 normal windows, create a new window with a new session.
fn show_or_hide_non_quake_mode_windows(_: &(), ctx: &mut AppContext) {
    ...
    // Now there is at least one window. If a Waz window is active, hide the app.
...
impl RootView {
    pub fn new(
        ...
        // Waz (localization, Phase 5): `PreferencesSyncer` has been physically deleted.
        // The original `InitialLoadCompleted` event was used to call
        // `apply_onboarding_settings` after the cloud preferences synchronization completed.
        // In the localized scenario, onboarding settings are applied directly locally.
        ...
        // Decentralized branch: `is_logged_in` is always true in local mode. The original structure is retained here
        // to maintain compilation feasibility/reachability of other branches like wasm/onboarding, and unneeded sub-branches will never be triggered.
        let auth_onboarding_state = if auth_state.is_logged_in() {
            ...
            cfg_if! {
                if #[cfg(target_family = "wasm")] {
                    ...
                    // When WazNewSettingsModes is enabled, show onboarding before login for
                    // users who haven't completed it yet (tracked via a local UserPreferences key).
                    let has_completed_local_onboarding = FeatureFlag::WazNewSettingsModes.is_enabled()
                        && has_completed_local_onboarding(ctx);
                    let should_show_pre_login_onboarding = FeatureFlag::WazNewSettingsModes.is_enabled()
    ...
    pub fn open_warp_drive_object_in_existing_window(
        ...
        if let AuthOnboardingState::Terminal(handle) = &self.auth_onboarding_state {
            ...
            log::warn!("Auth not complete before trying to open waz drive object");
        }
        true
    }
    ...
    /// If the user has completed local onboarding before entering the terminal, populate the local
    /// `is_onboarded` flag immediately after the auth facade enters the "usable" state.
    ///
    /// This logic runs on every `AuthComplete`, thus also covering the path of "skipping login first,
    /// then entering the authenticated state from other entry points"; the entire process only updates
    /// the local auth facade without performing any server-side synchronization.
    ...
    fn handle_auth_manager_event(&mut self, event: &AuthManagerEvent, ctx: &mut ViewContext<Self>) {
        ...
        match event {
            AuthManagerEvent::AuthComplete => {
                // If onboarding has finished before entering the auth completed state, fill in the local
                // `is_onboarded` bit here to prevent subsequent behavior based on the "onboarding not completed" branch.
                ...
                } else if let AuthOnboardingState::NeedsSsoLink { .. } = &self.auth_onboarding_state
                {
                    ...
                    // since we should err on the side of them _not_ being able to use Waz.
                ...
                UserAuthenticationError::UserAccountDisabled => {
                    cfg_if! {
                        if #[cfg(target_family = "wasm")] {
                            ...
                            // Waz has removed the log_out UI entry point; native no longer forces logout.
                            log::warn!("User account disabled; ignoring (Waz has removed log_out)");
                        }
                    }
                }
    ...
    fn handle_auth_override_warning_modal_event(
        ...
        match event {
            AuthOverrideWarningModalEvent::Close => {
                // Waz has removed the log_out entry point; closing no longer triggers logout.
            }
    ...
    /// Waz (localization, Phase 5): The original `handle_preferences_syncer_event` applied onboarding settings
    /// after the initial loading of cloud preferences synchronization completed, and was physically deleted along with the syncer.
    /// Onboarding settings are now applied directly when onboarding is completed, without waiting for cloud sync.
...
impl AuthOnboardingState {
    fn complete_auth_and_create_workspace(&mut self, ctx: &mut ViewContext<RootView>) {
        ...
        // The local `is_onboarded` flag will be filled in upon `AuthComplete` by
        // `RootView::finalize_local_onboarding_after_auth`.
