...
///
/// This will spawn `go tool pprof` to fetch and symbolicate the heap profile from the local HTTP server.
#[cfg(feature = "heap_usage_tracking")]
pub async fn dump_jemalloc_heap_profile(memory_breakdown: serde_json::Value) {
    // openWarp only writes the profile data and memory_breakdown to local logs.
    let result = dump_jemalloc_heap_profile_inner().await;
    match result {
        Ok(profile_data) => {
            log::warn!(
                "openWarp: abnormal memory usage detected (heap profile size {} bytes, memory breakdown: {})",
                profile_data.len(),
                memory_breakdown
            );
        }
