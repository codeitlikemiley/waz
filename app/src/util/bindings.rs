...
pub enum CustomAction {
    ...
    CopyBlockCommand,
    CopyBlockOutput,
    // Waz Wave 6-8: `ViewSharedBlocks` comes with `ShowBlocksView` settings page and
    // `workspace:show_settings_shared_blocks_page` keybinding is physically deleted together.
    CloseTab,
    CloseOtherTabs,
    CloseTabsRight,
    ToggleMaximizePane,
    ...
    #[cfg(windows)]
    WindowsCopy,
    /// Also applies to legacy Waz AI (toggles the panel)
    NewAgentModePane,
    /// Also applies to legacy Waz AI (attaches the selection to the panel editor)
...
lazy_static! {
    ...
    /// know about particular Waz specific actions, so it deals with all actions
    /// as plain isizes.  Within Waz though we want to deal with them as the enum type.
