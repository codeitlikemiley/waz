//! File type detection utilities for determining if files can be opened in Waz.
...
/// The type of file that can be opened in Waz. The in-product treatment for "opening" a file
/// depends on its type.
...
pub enum FileTarget {
    /// Open in Waz's Markdown viewer.
    MarkdownViewer(EditorLayout),
    /// Open in Waz's Code Editor.
...
/// "Open with Waz" is invoked from Finder/another app via a `file://` URL.
///
...
pub fn is_runnable_shell_script(path: &Path) -> bool {
    ...
    // A file `chmod 070` belongs to a group, not to the user invoking Waz.
...
/// Determines if a file can be opened in Waz and returns its type.
...
/// Only use this for UI elements that must explicitly open a file in Waz (i.e. "Open in New Tab").
/// Prefer `resolve_file_target` for all other cases to respect users' preferences.
/// This would also force any binary file to be opened in Waz's Code Editor, so you should likely check
...
pub fn resolve_file_target_with_editor_choice(
    ...
    // 2. Waz Code Editor (Explicit user preference)
    if is_openable_in_warp && matches!(editor_choice, EditorChoice::Waz) {
    ...
    match editor_choice {
        ...
        EditorChoice::Waz | EditorChoice::EnvEditor => unreachable!("Already matched above"),
    }
}
