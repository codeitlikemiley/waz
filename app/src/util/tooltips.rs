    ...
    let tooltip_element = if is_enterprise_secret_redaction_enabled {
        let tooltip_column = Flex::column()
            ...
            .with_child(
                appearance
                    .ui_builder()
                    .span(crate::t!("tooltip-secrets-not-sent-to-warp-server"))
                    .with_style(UiComponentStyles {
                        font_size: Some(12.),
...
/// Returns whether "Open in Waz" should be offered for the given file path.
///
/// This checks:
/// - Whether Waz is already the default editor (skip if so)
/// - Whether this file is openable in Waz (skips binary files and directories)
/// - Whether Waz is an OS-level default editor (skips Markdown files)
#[cfg(feature = "local_fs")]
pub fn should_show_open_in_warp_link(path: &Path, app: &AppContext) -> bool {
    ...
    if matches!(*settings.open_file_editor, EditorChoice::Waz) {
        return false;
    }
