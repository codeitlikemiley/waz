...
pub fn open_file_path_with_line_and_col(
    ...
    if full_path.is_file() {
        ...
        // Waz channel (e.g. Stable handling files while Preview is running).
        // When the resolved default is a different Waz, open with the
...
fn is_waz_bundle(bundle_id: &str) -> bool {
    AppId::parse(bundle_id)
        .map(|id| {
            id.qualifier() == "dev" && matches!(id.organization(), "warp" | "goldcoders")
        })
        .unwrap_or(false)
}
