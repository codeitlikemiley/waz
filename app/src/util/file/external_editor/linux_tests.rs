...
fn test_remaining_substitutions() {
    ...
    with_files("test_remaining_substitutions", data, |desktop, content| {
        ...
        // When constructing a command based on argv, each token is an independent parameter.
        // %c → "Waz Test Application" (single parameter, leave spaces)
        // %i → "--icon" and "/foo/bar/icon.png" (two independent parameters)
        // %k → desktop file path
        // %% → "%"
        let cmd = result.unwrap();
        let args: Vec<_> = cmd.get_args().collect();
        assert_eq!(args[0], "Waz Test Application");
        assert_eq!(args[1], "&&");
