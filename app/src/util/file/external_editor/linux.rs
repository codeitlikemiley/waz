...
impl EditorMetadata {
    ...
    /// Constructs a generic implementation of external editor commands.
    ///
    /// - First split the Exec field into shell words, and then expand the field code in each token.
    ///   get the correct argv vector
    /// - Set program and args directly to avoid going through the shell
    ///
    /// Field code replacement is handled by the `field_code_processor` callback. callback received
    /// `&mut Vec<String>`: Modify the last element to append to the current parameter, push new
    /// `String` represents new parameters.
        ...
        T: Fn(&Self, &mut Vec<String>, char),
    {
        ...
        // First split Exec by shell words, and then expand the field code within each token.
        // This preserves spaces in replacement values ​​(such as file paths) while respecting .desktop Exec
        // Citation rules in original content.
        let tokens =
        ...
        for token in &tokens {
            ...
            while let Some(ch) = iter.next() {
                ...
                // The field code processor can append additional argv items, for example %i will
                // "--icon" and the icon value are written as two independent parameters.
    ...
    /// field code default replacement logic.
    ///
    /// Process `field_code` according to the FreeDesktop specification, appending the replacement value to the current argv argument
    /// Or add a new argv parameter:
    /// https://specifications.freedesktop.org/desktop-entry-spec/latest/ar01s07.html
    /// %f, %F, %u, %U that rely on file paths use the `file_path` parameter.
    ///
    /// Maintain the original behavior when information is missing or processing fails: the corresponding replacement value is silently skipped.
    fn process_field_code(&self, parts: &mut Vec<String>, field_code: char, file_path: &Path) {
        match field_code {
            ...
            // URI
            'u' | 'U' => {
                // TODO(daprahamian): canonicalize is used here, so it will fail if the file does not exist,
                // An additional file system check will also be triggered. In the future, you can use std::path::absolute instead.
                //
                // Reference https://github.com/rust-lang/rust/issues/92750
            ...
            // The icon parameter needs to be split into independent argv items
            'i' => {
            ...
            // Unknown field code This character is reserved according to the specification
