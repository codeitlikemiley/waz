...
pub struct PromptConfiguration {
    ...
    /// The separator to use as a trailing character at the end of Waz prompt, if any.
...
impl Prompt {
    ...
    /// Reset to the default Waz prompt.
    ...
    /// Whether same line prompt is enabled for Waz prompt.
    pub fn same_line_prompt_enabled(&self) -> bool {
        self.config.same_line_prompt_enabled
    }

    /// The separator to be used for the Waz prompt.
    pub fn separator(&self) -> WarpPromptSeparator {
...
impl PromptConfiguration {
    /// The default Waz prompt, synthesized from legacy prompt settings.
