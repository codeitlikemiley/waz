...
use crate::drive::{ObjectTypeAndId, WazDriveObjectArgs};
...
// Waz: Remove ShareSessionModal import (cloud shared session modal)
...
// Waz Wave 7-3: `EnvironmentManagementPane` physically deleted along with the ambient-agent UI subsystem.
...
///    MSYS2/WSL on Windows) that a raw `PATH` lookup would miss when Waz is
...
pub enum Event {
    ...
    /// tell the workspace to open a file within Waz.
    OpenFileInWarp {
    ...
    WazDriveLink {
        open_warp_drive_args: WazDriveObjectArgs,
    },
    #[cfg(feature = "local_fs")]
    OpenCodeInWarp {
        source: CodeSource,
    ...
    WazDriveObjectInPane(ObjectUid),
    ...
    OpenAddRulePane,
    // Waz Wave 7-3: `OpenEnvironmentManagementPane` event physically deleted along with the ambient-agent UI
    // subsystem.
    OpenFilesPalette {
        source: PaletteSource,
    },
    ...
    /// Waz: Emitted when Ctrl/Cmd+clicking on a remote SSH session output file path in the terminal,
    /// and opened as a remote file in the editor by the workspace via the buffer-sync protocol.
...
pub struct PaneGroup {
    ...
    // Waz: Remove terminal_with_open_share_block_modal / share_block_modal fields (cloud share block)
    dragged_border: Option<DraggedBorder>,
    user_default_shell_changed_banner: ViewHandle<Banner<PaneGroupAction>>,

    // Waz: Remove terminal_with_open_share_session_modal / share_session_modal fields (cloud shared session)
    ...
    // Waz Wave 7-3:`pane_with_open_environment_setup_mode_selector` /
    // `pane_with_open_agent_assisted_environment_modal` physically deleted along with the ambient-agent UI subsystem.
...
impl PaneGroup {
    ...
    fn restore_pane_leaf(
        ...
        let result = match leaf.contents {
            ...
            } // Waz Wave 7-3: `EnvironmentManagement` LeafContents arm physically deleted along with the ambient-agent UI
              // subsystem.
        };
    ...
    /// is used for intra-session prompt switching between Waz prompt and PS1.
    ...
    fn open_share_session_denied_modal(
        ...
        // Waz: share_session_modal deleted, no-op
        ctx.notify();
    }
    ...
    // Waz: Remove handle_share_session_modal_event (cloud shared session modal)
    ...
    fn new_internal(
        ...
        // Waz: Remove share_block_modal registration (cloud share block)
            ...
            Banner::<PaneGroupAction>::new_permanently_dismissible(
                BannerTextContent::formatted_text(vec![
                    FormattedTextFragment::plain_text(
                        "Waz doesn't currently support your default shell, falling back to zsh.  ",
                    ),
        ...
        // Waz: Remove share_session_modal registration (cloud shared session modal)
        ...
        let mut pane_group = Self {
            ...
            // Waz Wave 7-3: Pane-level modal tracking fields in ambient-agent UI subsystem physically deleted along with the UI.
    ...
    fn load_data_into_transcript_viewer(
        ...
        // Insert the conversation ended tombstone (includes Open in Waz button on WASM).
    ...
    // Waz: Remove handle_share_block_modal_event (cloud share block)
    ...
    pub fn close_pane(&mut self, pane_id: PaneId, ctx: &mut ViewContext<Self>) {
        ...
        if FeatureFlag::UndoClosedPanes.is_enabled() {
            ...
            // Waz: Remove share_block_modal cleanup (cloud share block)
            // Waz Wave 7-3: Pane-level modal tracking fields cleanup in ambient-agent UI subsystem physically deleted along with the UI.
    ...
    pub fn close_overlays(&mut self, ctx: &mut ViewContext<Self>) {
        ...
        // Waz: Remove terminal_with_open_share_block_modal clear (field no longer exists)
        ctx.notify();
    }
...
impl View for PaneGroup {
    ...
    fn render(&self, app: &AppContext) -> Box<dyn Element> {
        ...
        // Waz: Remove rendering branches for share_block_modal / share_session_modal / role-change modal (fields no longer exist)
        ...
        // Modal overlay rendering at tab level physically deleted along with ambient-agent UI subsystem.
