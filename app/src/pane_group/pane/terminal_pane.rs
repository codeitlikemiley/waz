...
fn handle_terminal_view_event(
    ...
    if group.pane_contents.contains_key(&pane_id) {
        match event {
            ...
            Event::WazDriveObjectInPane(uid) => {
                ctx.emit(pane_group::Event::WazDriveObjectInPane(uid.clone()));
            }
            ...
            // Waz Wave 7-3: `Event::EnvironmentSetupModeSelectorToggled` handler physically deleted
            ...
            // Waz: Pass through the "open remote file" event emitted by the terminal to pane_group -> workspace.
            ...
            // Waz Wave 7-3: `OpenEnvironmentManagementPane` event forwarding physically deleted along with the ambient-agent UI
            // subsystem.
            #[cfg(feature = "local_fs")]
