...
pub(super) async fn prepare_local_harness_child_launch(
    ...
    let command = match harness {
        ...
        Harness::Claude => {
            ...
            // but there are no Waz-managed secrets to materialize into the
            // hidden child pane.
    ...
    // Waz (Localization, Phase 3b-4): Local harness starting a child task no longer goes through the cloud
    // `create_agent_task` mutation, and directly generates a local UUID v4 as the task_id.
