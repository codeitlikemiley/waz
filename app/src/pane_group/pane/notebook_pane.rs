...
use crate::{
    ...
    cloud_object::Space,
    drive::{items::WarpDriveItemId, ObjectTypeAndId, WazDriveObjectSettings},
    notebooks::{
...
pub(super) fn subscribe_to_link_model(
    ...
    ctx.subscribe_to_model(handle, move |pane_group, _, event, ctx| match event {
        ...
        LinkEvent::WazDriveLink {
            open_warp_drive_args,
        } => ctx.emit(crate::pane_group::Event::WazDriveLink {
        ...
        LinkEvent::OpenFileWithTarget {
            ...
            // Emit event to workspace to handle opening in Waz
