...
/// SHA-256 verification shared by three platforms after OSS download is complete:
/// 1. If no matching asset is found in cached release, skip (fallback to no verification);
/// 2. If the asset does not have a digest field (old release), skip;
/// 3. If the digest is not in the sha256:xxx format or has an incorrect length, skip;
/// 4. Under normal circumstances, read the file and stream calculate its sha256. If it matches, return Ok. If not, return Err and mark the file
///    as invalid (it is up to the caller to decide whether to delete it).
///
/// This function should not be called outside the OSS path: other channels use codesign / Inno built-in verification.
pub(crate) fn verify_oss_asset_sha256(
    ...
    let Some(asset) = release.find_asset(asset_name) else {
        log::warn!("openWarp: Cannot find {asset_name} in cached release, skipping SHA-256 verification");
        return Ok(());
    };
...
pub enum AutoupdateStage {
    ...
    /// A new version was installed, but Waz hasn't restarted yet.
    ///
...
/// Download progress, shared across platforms. `total` is `None` if the server does not return `Content-Length`.
#[derive(Clone, Debug)]
...
/// Platform download functions push progress back to the UI thread via this callback.
...
pub struct AutoupdateState {
    ...
    /// The latest progress of the current ongoing download; cleared by this module when Stage leaves DownloadingUpdate.
    /// The UI reads this via `download_progress()` and renders a progress bar.
...
impl AutoupdateState {
    ...
    /// Current download progress; only meaningful when stage is in DownloadingUpdate.
    ...
    pub fn register(ctx: &mut AppContext, http_client: Arc<http_client::Client>) {
        ctx.add_singleton_model(move |ctx| {
            ...
            {
                // Start polling loop; whether to actually initiate background checks is controlled by user settings.
                me.poll_for_update(ctx);
                // Enqueue a potential daily check when the app is activated, which will also read user settings before execution.
    ...
    fn should_update(&mut self, version: VersionInfo, update_id: String) -> UpdateReady {
        ...
        // openWarp Note: `ChannelState::app_version()` returns the injected raw value of `GIT_RELEASE_TAG`,
        // retaining the `v` prefix (e.g. `v2026.05.10.preview`); whereas `VersionInfo.version`
        // in the OSS branch is provided by `github::GithubRelease::version()`, which has already been stripped of `v`
        // using `trim_start_matches('v')` (e.g. `2026.05.10.preview`). Directly comparing strings for equality would always return false, causing
        // "already up to date" to be misidentified as "new version detected". Here, perform idempotent prefix normalization:
        // - Official Waz: tags always carry `v`, after trimming both sides they are still equal, behavior unchanged.
        // - openWarp: equal only after trimming, correctly identifying the same version.
    ...
    fn on_update_check_complete(
        ...
        match &update_available {
            Ok(UpdateReady::CanDownload {
                ...
                // openWarp(Channel::Oss): Follows the same download workflow as the official version. The platform-specific
                // download_update_and_cleanup selects appropriate assets and bypasses codesign verify in the OSS branch.
    ...
    fn download_new_update(
        ...
        // Initialize to 0/unknown when entering the downloading phase, ensuring the UI immediately displays "downloading" feedback;
        // below, actual progress for each chunk is pushed back via mpsc to overwrite it.
        ...
        // Progress channel: the download future pushes DownloadProgress to tx, which is received by spawn_stream_local
        // on the model main thread, written to self.download_progress, and notified.
        // Use an unbounded channel to prevent the download from blocking on send; the UI reads only the latest progress each time,
        // and intermediate backlog is directly overwritten when processed by the model to prevent stuttering.
        let (progress_tx, progress_rx) =
            futures::channel::mpsc::unbounded::<DownloadProgress>();
        let on_progress: ProgressCallback = Arc::new(move |p| {
            // Ignore when the receiver disconnects (download complete / model destroyed), without affecting the download itself.
            let _ = progress_tx.unbounded_send(p);
        });

        // Note race condition: after the future spawned by ctx.spawn completes, the main thread immediately executes the callback;
        // at this point, progress_rx might still have buffered items that haven't been consumed by spawn_stream_local.
        // If we crudely set `download_progress = None` in the callback, the subsequent stream's on_item
        // would modify None back to Some, causing a "ghost progress" effect. Fix: only write on_item when the stage is still
        // DownloadingUpdate; expired items are automatically discarded. When the stream terminates naturally (all tx Arcs are dropped),
        // on_done will clear the progress as a fallback.
        ...
        let _ = ctx.spawn(
            ...
            move |autoupdate_state, download_ready, ctx| {
                // Do not clear download_progress here: leave it to spawn_stream_local's on_done
                // for unified cleanup (to avoid race conditions, see the comment above).
...
pub fn accessibility_content(
    ...
    match (request_type, update_available) {
        ...
        (RequestType::ManualCheck, Ok(UpdateReady::Yes { .. })) => Some(AccessibilityContent::new(
            "Update available.",
            "Use the command palette to install and relaunch Waz",
...
async fn fetch_version(
    ...
    // openWarp uses GitHub Releases (zerx-lab/warp), completely bypassing Waz's official
    // channel_versions / GCS. Return early to avoid the subsequent fetch_channel_versions inevitably failing.
...
pub fn apply_update(
    ...
    // openWarp: The apply_update for all three platforms goes through the same entry point as the official version, except relaunch()
    // on each platform selects different "installation actions" for the OSS branch (mac: open dmg / Windows: non-silent
    // Inno installation / Linux AppImage: in-place overwrite + restart).
...
/// Relaunch Waz to apply an update.
///
/// This will:
...
{
    // openWarp(Channel::Oss): The mac platform dmg has been downloaded, no mount/swap/verify is needed, and
    // relaunch() will directly use `open <dmg>` to trigger Finder installation. Skip apply_update_async here,
    // otherwise mac::apply_update_async will attempt codesign verification (which fails without Apple Team ID).
