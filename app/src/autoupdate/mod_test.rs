...
fn test_should_update() {
    App::test((), |mut app| async move {
        ...
        app.update_model(&autoupdate_state, |autoupdate, _| {
            ...
            // `ChannelState::app_version()` under openWarp(Channel::Oss) retains the `v` prefix,
            // And `github::GithubRelease::version()` has trimmed out `v`. The only difference between the two is `v`
            // The prefix should be recognized as `UpdateReady::No`, otherwise the About page will always prompt "New version found".
            // Test 6: Same version of openWarp (local with v, remote without v)
            ...
            assert!(
                matches!(result, UpdateReady::No),
                "openWarp only 'v' prefix difference should be identified as up-to-date, actual: {result:?}"
            );

            // Test 7: Reverse (local without v, remote with v) should also be recognized as the same version
            ...
            assert!(
                matches!(result, UpdateReady::No),
                "Reverse prefix difference should be identified as up-to-date, actual: {result:?}"
            );

            // Test 8: Different versions must still be recognized as downloadable (not accidentally damaged by normalization)
            ChannelState::set_app_version(Some("v2026.05.10.preview"));
            ...
            assert!(
                matches!(result, UpdateReady::CanDownload { .. }),
                "Different versions should be identified as CanDownload, actual: {result:?}"
            );
        });
    });
}
