...
pub(super) fn relaunch() -> Result<()> {
    ...
    // openWarp(Channel::Oss): There is no code signature, so we cannot replace the bundle in-place using RENAME_SWAP.
    // Instead, call `/usr/bin/open <dmg>` to let Finder pop up the standard mount window, allowing the user to drag it
    // to the Applications directory to complete installation. We do not call `open -n bundle` here to relaunch ourselves, because
    // the current process has already requested termination during the apply_update stage, and the UI already knows to wait for the user to manually
    // close and reopen it. The dmg will also launch Finder after the current process exits.
    ...
    // Before launching the new version of Waz, wait for the current process to exit to avoid multiple icons appearing briefly in the Dock.
    // Here, an intermediate shell process is used to poll the current PID, and launch the new version of the app after the process exits.
    //
    // Check if the current process is still running every 200ms; launch the new version after the process exits.
    //
    // Shell commands need to be carefully concatenated: `pid` comes from the current process and is a number, while the bundle path
    // and environment variable values must be shell-escaped to prevent metacharacters in the path from causing injection.
...
/// OSS macOS installation entry point: scan `cache_dir/autoupdate/<id>/` to find the newly downloaded dmg,
/// wait for the current process to exit, then use `/usr/bin/open <dmg>` to trigger standard Finder mounting.
fn oss_open_installer() -> Result<()> {
    // Before entering this path, AutoupdateState.stage must be UpdateReady / Updating,
    // downloaded_update.update_id must exist; however, we do not access AutoupdateState in stateless functions,
    // and instead scan the disk: traverse cache_dir/autoupdate/ to find the latest dmg.
    ...
    // Wait for the current process to exit before opening the dmg. `open` is non-blocking by default; after Finder gets the dmg, it will
    // automatically mount and display the mount window; the user drags it to Applications in Finder to complete the upgrade.
    let script = format!(
...
/// Find the latest downloaded dmg in the `autoupdate/` directory. OSS only downloads dmg and no other files,
/// to find the latest dmg, sorting by file mtime is sufficient. Returning None indicates no available dmg (an exceptional case).
...
/// Determines if the user needs authorization in order to update Waz.
...
/// * Waz can create files in the directory
/// * Waz can modify the permissions of created files
...
/// team identifier matches Waz's team identifier.
...
pub(super) async fn download_update_and_cleanup(
    ...
    // openWarp(Channel::Oss): There is no Apple Developer ID signature, so we cannot go through the official
    // download_and_extract_binary (mount + cp + codesign verify + RENAME_SWAP).
    // The OSS path only streams the dmg to cache_dir/autoupdate/<id>/, and when applying, `relaunch()`
    // calls `open <dmg>` to let Finder pop up the standard mount window, allowing the user to drag it to Applications.
...
/// OSS-specific download: only streams the dmg to disk at `cache_dir/autoupdate/<update_id>/<dmg>`,
/// without mounting or verifying code signatures. Returns `DownloadReady::Yes` to indicate the installer is ready,
/// and the upper layer will switch to `UpdateReady`, waiting for the user to click "Install Now" to trigger `relaunch()`.
async fn oss_download_dmg(
    ...
    // Intentionally bypass hdiutil mount / verify_code_signature: OSS does not have Apple
    // codesign, nor does it need to copy the .app into the current bundle. The dmg itself is the object the user wants to "open".
    // However, verify the SHA-256 in the GitHub Release metadata to defend against CDN man-in-the-middle attacks / asset corruption.
    let asset_name = dmg_name(channel);
    if let Err(e) = super::verify_oss_asset_sha256(&dmg_path_buf, &asset_name) {
        // If verification fails, delete the downloaded file immediately to prevent the user from opening a corrupted dmg after clicking "Install".
...
async fn apply_update(channel: Channel, version_info: &VersionInfo, update_id: &str) -> Result<()> {
    ...
    if async_fs::metadata(executable_temp_file.as_path())
        ...
        .is_ok()
    {
        // If we performed this process already but didn't relaunch Waz, the old executable will
...
async fn download_dmg(
    ...
    let res = client
        ...
        .await?
        .error_for_status()?;
    // http_client::Response does not have content_length(), can only get from headers.
    let total = res
        .headers()
    ...
    // Report 0/total to let UI render progress bar immediately; thereafter, throttle reporting as each chunk is written.
    ...
    // Throttle: do not report every chunk (reqwest chunk might be very small, which would cause heavy UI repaints).
    // Only report when accumulating 64 KiB or after 250ms; the final one is forced to flush outside the loop.
...
async fn mount_dmg(dmg_dir: &Path, update_id: &str) -> Result<PathBuf> {
    ...
    // -nobrowse: Do not show the Waz DMG in Finder or similar apps.
    // -noautoopen: Do not open the Waz DMG in Finder.
...
fn update_url(channel: Channel, version: &str) -> String {
    ...
    if matches!(channel, Channel::Oss) {
        // OSS uses GitHub Releases: prioritize the real browser_download_url in the fetch_latest_release cache
        // (in case the repo is redirected / asset is renamed). If the cache is empty,
        // construct a standard `releases/download/<tag>/<asset>` fallback URL, where the tag is directly
        // VersionInfo.version with a `v` prefix (VersionInfo is already trimmed of `v`).
        ...
        return format!(
            "https://github.com/codeitlikemiley/waz/releases/download/v{version}/{asset}"
        );
    }
    format!(
        "{}/{}",
...
fn dmg_name(channel: Channel) -> String {
    ...
    // openWarp GitHub Release asset names are fixed to use `Waz-arm64.dmg` / `Waz-intel.dmg`
    // (from the naming convention of .github/workflows), which is inconsistent with `app_name_prefix("waz")`.
    // Here we only hardcode it for OSS, without affecting the universal naming of official channels.
    if matches!(channel, Channel::Oss) {
        return if is_arm64 {
            "Waz-arm64.dmg".to_string()
        } else {
            "Waz-intel.dmg".to_string()
        };
    }
