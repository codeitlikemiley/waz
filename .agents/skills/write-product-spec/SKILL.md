...
Write a `PRODUCT.md` spec for a significant feature in Waz.
...
"User" is not limited to the end user of the Waz app. It means whoever consumes the surface being designed:

- For UI / UX features: the human using Waz.
...
14. Sharing or exporting a block (Waz Drive, share link, save as file) preserves the original markdown source, not the rendered form.

15. Theming: table borders, header backgrounds, alternating row shading (if any), and link/code styles all come from the active Waz theme. No hard-coded colors.
