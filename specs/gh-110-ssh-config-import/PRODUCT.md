...
Tracks: [GitHub issue #110](https://github.com/codeitlikemiley/waz/issues/110).
Technical implementation details are in `TECH.md`.

## 1. Problem

Waz's SSH Manager (left panel, `Alt+5` / `Ctrl+5`) stores every server in its
...
1. **Setup friction.** A developer with N hosts in `~/.ssh/config` has to
   ...
   useful to them.
2. **Inconsistency across SSH entry paths.** Waz has two SSH paths:
...
owned by Waz and is decoupled from the source file.
...
1. As a developer who already maintains `~/.ssh/config`, I want Waz to show
   my hosts in the SSH Manager without re-entering them, so I can start using
   the panel immediately.
2. As a user who imported a host, I want to edit it in Waz freely without
   affecting `~/.ssh/config`, so my hand-written file stays exactly as I
   wrote it.
3. As a user who deletes a host from `~/.ssh/config`, I want my Waz-side
   imported copy to remain, so I don't lose work I've already done in Waz.
...
7. As a user who adds a new host to `~/.ssh/config` in `vim`, I want to come
   back to Waz, click Refresh, and see the new host immediately.
...
10. As a user, I want the resolved path to my config file shown in the UI,
    so I'm not guessing which `.ssh/config` Waz is reading.
...
| C | What happens if the source host changes after import? | Nothing on the Waz side. Candidate row stays "Added"; the candidate is not re-evaluated against the saved entry. | Same reason as B — no implicit sync. |
