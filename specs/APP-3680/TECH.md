...
**Feature flags:** `TabConfigs`, `AgentOnboarding`, `WazNewSettingsModes`, and `AgentView` are the relevant flags (`warp_core/src/features.rs`). We'll add a new flag for this modal.
...
No new feature flag needed. Gate the modal behind both `FeatureFlag::WazNewSettingsModes` (this is the new onboarding path) and `FeatureFlag::TabConfigs` (the modal produces a tab config, so the tab config system must be enabled). Both flags must be on for the modal to appear. When either is off, the old onboarding flow runs unchanged.
...
In `handle_agent_onboarding_event` (`app/src/root_view.rs:2080`), after the existing `OnboardingCompleted` handling, when both `FeatureFlag::WazNewSettingsModes.is_enabled()` and `FeatureFlag::TabConfigs.is_enabled()`:
...
**Risk: Breaking existing onboarding.** All new behavior is gated behind both `FeatureFlag::WazNewSettingsModes` and `FeatureFlag::TabConfigs`. When either is off, `handle_agent_onboarding_event` follows the identical code path as today. No changes to `OnboardingTutorial`, `SelectedSettings`, or `apply_onboarding_settings`.
...
- When `WazNewSettingsModes` is off, `DefaultSessionMode` is not touched by this code path.

### Feature flag gating (integration)
- When either `WazNewSettingsModes` or `TabConfigs` is off, `OnboardingCompleted` follows the old tutorial path — modal is never shown.
- When both `WazNewSettingsModes` and `TabConfigs` are on, `OnboardingCompleted` dispatches `ShowSessionConfigModal`.
