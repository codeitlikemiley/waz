# TECH.md — Rename Oz to Waz Agent in settings and onboarding
...
This is a rename of the in-app agent from "Oz" to "Waz Agent" across user-facing
...
1. `app/src/settings_view/mod.rs`
   ...
   - In the `Display` impl, the `WarpAgent` arm writes `"Waz Agent"`.
   - In the `FromStr` impl, accept both `"Oz"` (backward-compat legacy name)
     and `"Waz Agent"` as parseable forms that map to
...
2. `app/src/settings_view/ai_page.rs`
   ...
   - In `GlobalAIWidget::render`, replace `Text::new_inline("Oz", ...)` with
     `Text::new_inline("Waz Agent", ...)`. Keep every other argument, style,
   ...
   - Replace all remaining user-visible description strings that referenced
     "Oz" or "Oz agent" with "the Waz Agent" / "Waz Agent" as appropriate.
     ...
     allowlist/denylist descriptions, Rules description, Waz Drive context
...
3. `crates/onboarding/src/slides/agent_slide.rs`
   - `render_header`: change paragraph text from `"Customize your Agent, Oz"`
     to `"Customize your Waz Agent"`. Keep font size, weight, layout, and
     surrounding subtitle unchanged.
   - `render_disable_oz_section`: change checkbox label from `"Disable Oz"` to
     `"Disable Waz Agent"`. Keep styling, spacing, `disable_oz_mouse` state
...
- `cargo nextest run -p warp_app --no-fail-fast` or the relevant subset covering
  ...
  `SettingsSection::WarpAgent.to_string() == "Waz Agent"`. The `FromStr` test
  covers both `"Oz"` and `"Waz Agent"` resolving to
...
- Behavior #1, #2, #3, #9: manually open the settings UI and confirm the
  sidebar entry reads "Waz Agent", the subpage renders unchanged content, the
  heading above the global toggle reads "Waz Agent", and the "Oz Cloud API
...
- Behavior #8: confirm both `"Oz"` and `"Waz Agent"` resolve to
...
- Behavior #11: no automated accessibility test exists for these labels; manual
  ...
  accessible label.
- Behavior #12: toggle the `WazNewSettingsModes` feature flag and confirm
  the disable row only appears when enabled and always reads "Disable Waz
...
- Risk: external deep links or persisted telemetry strings reference `"Oz"` and
  break. Mitigation: `FromStr` accepts both `"Oz"` and `"Waz Agent"` mapping to
