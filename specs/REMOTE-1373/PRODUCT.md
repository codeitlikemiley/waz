...
- **Saves continue in place** — follow-up periodic/final saves write to the same server conversation id, same GCS objects, same Waz Drive object, same artifacts list.
## Non-goals
...
- New PR / plan / file artifacts from the resumed run attach to the same Waz Drive conversation object as the original.
