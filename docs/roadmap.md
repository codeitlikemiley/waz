...
Waz's agent stack will be built as a standalone open-source service, independent of the Warp client. The terminal becomes one client among many — a TUI, an IDE plugin or a cloud worker can all drive the same engine.

## Phase 1 — Build the agent harness core

- Design and build a standalone open-source agent service from scratch — agent loop, tool runtime, conversation/session state, prompt templating, provider routing — not tied to Warp's existing client code. Waz becomes its first consumer.
...
- Single account / identity shared across Waz terminal, headless TUI, IDE plugins and web UI.
...
[Simplified Chinese](./roadmap.zh-CN.md) · [Japanese](./roadmap.ja.md)
