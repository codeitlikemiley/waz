# Migrating settings to Waz

[Simplified Chinese](./migrate-from-warp.zh-CN.md) · [Japanese](./migrate-from-warp.ja.md)
This guide is for people who want to bring **settings-style configuration**
(custom keybindings, themes, workflows, MCP config, etc.) into Waz from a
previous install.

There are two source installs this might apply to:

1. **OpenWarp** — Waz's own previous name.
2. **Upstream [Warp](https://github.com/warpdotdev/warp)** — the project Waz
   is forked from.
...
Waz (and OpenWarp / upstream Warp before it) splits its on-disk state into
...
(`~/.warp/`, `~/.openwarp/`, or `~/.waz/`). On Linux and Windows they live in
...
OpenWarp **was** Waz. The rename commit (`feat: rename project Warp/OpenWarp →
Waz`) only renamed identifiers and on-disk paths — the **configuration file
...
> Quit Waz before copying, so no process is holding the files open.
...
you might have already set in Waz. Drop it if you'd rather have OpenWarp's
values win.

After verifying Waz looks right, you can delete the OpenWarp directories above
...
"Upstream Warp source paths" table above). Waz is built with channel `Oss`,
which gives it its own app ID (`dev.waz.Waz`) and its own per-platform layout.
The two installations cannot see each other's files, which is also what keeps
your Warp account / cloud state out of Waz.
...
Waz, and binary / private stores can be tied to Warp's auth and bundle
identity.
...
- **`user_preferences.json`** — a private store under
  ...
  confuse Waz's auth state. Waz defaults are already privacy-friendly.
- **`warp.sqlite`** (and its `-wal` / `-shm` sidecars) — schema is coupled
  to upstream Warp and not guaranteed to be compatible with Waz's migrations.
- **Keychain / DPAPI / libsecret entries** — bound to the Warp bundle /
  service name, useless to Waz.

### Steps

> Quit both Warp and Waz before copying.
...
Start Waz. You should see your custom themes in the theme picker, your
...
it in a text editor, or just delete it and let Waz fall back to defaults.

## Rolling back

Nothing in this guide is destructive: every file copied is something Waz will
