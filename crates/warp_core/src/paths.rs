...
/// The name of the directory in which to put non-global Waz-specific files.
///
...
/// The name of the folder that stores Waz execution logs and network logs.
...
/// Returns the home-relative Waz config directory name for the current channel and data profile.
///
...
/// Returns the home-relative Waz config directory for the current channel and data profile.
///
/// Unlike [`data_dir`] and [`config_local_dir`] on non-macOS platforms, this intentionally keeps
/// Waz-authored, user-facing config under a `.warp*` directory in the home directory instead of
...
/// the user had open and cached state of known Waz Drive objects.
...
pub fn secure_state_dir() -> Option<PathBuf> {
    ...
    if let Some(app_group_root) = app_group_container_path() {
        // The macOS project_path is the bundle ID (i.e. `dev.warp.Waz-Stable`).
...
fn project_dirs_for_app_id(
    ...
    cfg_if::cfg_if! {
        if #[cfg(any(target_os = "linux", target_os = "freebsd"))] {
            ...
            // a directory like "waz" matching our Linux package name.
            let base_app_name = match app_id.application_name() {
                "Waz" => "waz".to_owned(),
                other if other.starts_with("Waz") => other.replace("Waz", "waz-"),
...
/// Returns the path to resources included in the Waz distribution.
///
/// Unlike [`warpui::AssetProvider`] assets, which are generally embedded in the binary, these are
/// stored on the filesystem alongside the rest of Waz.
///
/// ## macOS
/// The resources directory is `$APP_DIR/Contents/Resources` (e.g. `/Applications/Waz.app/Contents/Resources`).
///
/// ## Linux
...
/// containing the Waz executable (e.g. `C:\Program Files\WarpDev\resources`).
