...
/// Execution mode that Waz is running under.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ExecutionMode {
    /// Waz is running as a normal desktop app.
    App,
    /// Waz is running as a CLI.
    Sdk,
}
...
/// Model tracking the mode that Waz is running in.
///
/// This gates functionality that's disabled when Waz is running in SDK mode.
#[derive(Clone, Debug)]
...
impl AppExecutionMode {
    ...
    ///
    /// Waz has no telemetry sender, so shutdown must never wait on telemetry work. The method
    ...
    /// If true, Waz is running in a sandbox like a Docker container or VM, rather than directly
    /// on a user machine.
