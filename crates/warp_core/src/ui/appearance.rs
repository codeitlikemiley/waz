...
/// Default UI font size (baseline value). Other semantic font size methods scale proportionally based on this.
pub const DEFAULT_UI_FONT_SIZE: f32 = 12.0;
/// Legal lower bound of UI font size (callers of `set_ui_font_size` should clamp between this and `UI_FONT_SIZE_MAX`).
pub const UI_FONT_SIZE_MIN: f32 = 8.0;
/// Legal upper bound of UI font size.
...
impl Appearance {
    ...
    /// but the literal is no longer coupled to external constants. It is recommended that new invocation points use this method,
    /// while `overline_font_size` is gradually kept as a historical invocation point.
    ...
    /// Calculates the minimum height required for the dropdown top bar based on the UI font size.
