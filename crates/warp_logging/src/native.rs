...
/// Extra content to be packaged into the log zip, collected and passed in by the caller.
///
/// `warp_logging` itself only knows the main log file; diagnostic summaries, logs of other subsystems (like MCP)
/// paths, auto-update logs, etc., are collected by the `app` layer and passed in through this struct, preventing this crate
/// from reversely depending on upper-level modules.
#[derive(Debug, Default)]
pub struct LogBundleExtras {
    /// Extra disk files that need to be packaged into the zip as-is; non-existent files will be silently skipped.
    pub extra_files: Vec<ExtraFile>,
    /// Virtual files directly written into the zip as memory strings (such as `manifest.txt`).
    pub inline_files: Vec<InlineFile>,
}

/// Description of the extra packaged disk file.
#[derive(Debug)]
pub struct ExtraFile {
    /// Real disk path.
    pub source_path: PathBuf,
    /// Relative path saved in the zip (supports subdirectories, such as `mcp/<uuid>.log`).
    pub entry_name: String,
}

/// Virtual file written to the zip using memory content.
#[derive(Debug)]
pub struct InlineFile {
    /// Relative path saved in the zip.
    pub entry_name: String,
    /// File content (UTF-8).
    pub contents: String,
}

/// Default zip filename (used for the "export to log directory" flow, and as the default filename
/// for the save-file picker). Formatted as `waz-20260518-093000.zip`.
...
/// Sanitizes the entry name provided by the caller into a safe zip internal relative path, defending against path traversal:
/// - Backslashes are unified as `/`;
/// - Rejects absolute paths / Windows drive letters;
/// - Strips `..` parent components as well as consecutive `/` / `.`;
/// - Empty strings are considered invalid.
///
/// Returns `None` to indicate that this entry should be skipped (the caller will log::warn! and continue).
fn sanitize_zip_entry_name(name: &str) -> Option<String> {
    ...
    // Check Windows drive letter, e.g., `C:/foo`.
    ...
    for segment in normalized.split('/') {
        match segment {
            ...
            ".." => return None,  // Do not allow jumping out.
...
/// Core implementation that actually writes logs + extras to the specified zip output path.
/// Public `create_log_bundle_zip` and `write_log_bundle_zip_to` both delegate here.
fn write_log_bundle_zip_inner(zip_path: &Path, extras: &LogBundleExtras) -> Result<()> {
    ...
    // Main log + rotated old logs, laid flat in the zip root directory.
    ...
    // Extra disk files: package them if they exist; non-existence or read failures only print warn and do not affect the main flow.
    ...
    // Memory content (`manifest.txt` etc.): always attempt to write.
...
///
/// Entry point for "packaging and displaying in file manager" (Help menu -> View Waz Logs).
///
/// `extras` allows the caller to append other diagnostic artifacts (MCP logs, auto-update logs, diagnostic summaries, etc.);
/// any non-existent or unreadable extra files will be skipped and recorded via `log::warn!`, and
/// will not cause the entire export to fail.
...
///
/// Entry point for "user selects path in save dialog" (Settings -> About -> Export Logs).
/// Shares the same packaging content and failure tolerance policy with `create_log_bundle_zip`.
