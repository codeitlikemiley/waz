//! Formats `SshServerInfo` into an `ssh ...` command. Pure function, easy to test.
//!
//! When writing to PTY, call `build_ssh_command_line` to shell-escape each argument,
//! preventing spaces or single quotes in the username, host, or key_path from breaking the command line.
...
async fn run_ssh_test(args: &[String]) -> Result<String, std::io::Error> {
    // Standardize child process spawning using command::r#async. On Windows, it includes CREATE_NO_WINDOW,
    // avoiding flashing console windows (see .clippy.toml for tokio::process::Command disallowance).
    ...
    // Success criteria: the process exit code is 0, or the output of remote `echo ok` has been returned
    // (some sshpass warnings cause non-zero exit codes, but stdout still contains "ok").
...
mod tests {
    ...
    fn default_port_omitted() {
        ...
        // shell-escape conservatively wraps user@host in single quotes, which is a valid and
        // shell-equivalent format — we do not strictly enforce the unquoted version.
    ...
    fn test_connection_requires_password_for_password_auth() {
        let s = server();
        // test_connection should return an error when password is not provided
    ...
    fn test_connection_key_auth_uses_batch_mode() {
        ...
        // For key authentication, BatchMode=yes should be used
        // Since we cannot actually connect, only test parameter construction here
