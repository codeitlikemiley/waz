...
pub enum FeatureFlag {
    ...
    /// Whether the user is part of the Waz Alpha Program (AI Trusted Testers).
    ...
    /// Enable multiselect in Notebooks and Waz Text.
    RichTextMultiselect,
    ...
    /// Enables next action prediction within Waz, powered by AI.
    AgentPredict,

    /// Enables receiving shared Waz Drive objects.
    SharedWithMe,
    ...
    /// Playground for reducing Waz UI clutter.
    MinimalistUI,
    ...
    /// Adds aliases for executing Waz Drive workflows.
    WorkflowAliases,
    ...
    /// Enables Waz Drive objects (like workflows) as context in AI context menu
    ...
    /// Enables auto-syncing ambient plans to Waz Drive.
    SyncAmbientPlans,
    ...
    /// Enables the Waz launch modal announcing Waz going open-source.
    ...
    /// Ctrl-G intercepts the keystroke and opens Waz's input editor instead of $EDITOR.
    CLIAgentRichInput,
    ...
    /// Enables the new settings to control visibility of Waz Drive, Code Review Panel,
    ...
    /// CLIs (e.g. `claude`) to execute prompts instead of Waz's agent harness.
    AgentHarness,

    /// Enables the upgraded CLI agent session tracking and notifications infrastructure.
    HOANotifications,

    /// Enables the install/update chip for the OpenCode Waz plugin.
    /// Requires HOANotifications to also be enabled.
    OpenCodeNotifications,

    /// Enables the install/update chip for the Codex Waz notification plugin.
    /// Requires HOANotifications to also be enabled.
    CodexNotifications,

    /// Enables the install/update chip for the Gemini CLI Waz extension.
    /// Requires HOANotifications to also be enabled.
    GeminiNotifications,
    ...
    /// Show saved SSH credential candidates when local PTY prompts for a password.
    OneKeyPrompt,
    ...
    /// Enable the global HTTP proxy settings page and proxy override logic in `Client::new()`.
    /// When disabled, the UI entrypoint is hidden, and `Client::new()` falls back to reqwest's default (reading environment variables).
    /// See Issue #72.
    HttpProxySettings,
}
...
/// Features enabled for feature preview build users (e.g.: Friends of Waz).
...
pub const RELEASE_FLAGS: &[FeatureFlag] = &[
    ...
    // winit's IME path has support for marked text on both macOS and Windows.
    // Windows must enable this flag to render IME preedit / input composition text,
    // otherwise only the OS candidate window is visible, and Waz will discard the marked text update entirely.
...
impl FeatureFlag {
    pub fn is_enabled(&self) -> bool {
        // Decentralized branch: In local mode, always disable the following flags related to account / login / cloud Agent,
        // and no longer be affected by channel / preview configurations.
        if matches!(
            self,
    ...
    pub fn flag_description(&self) -> Option<&'static str> {
        ...
        match self {
            ...
            SettingsFile => Some("Enables configuring Waz via a user-editable `settings.toml` file, with hot reload and error reporting for invalid values."),
