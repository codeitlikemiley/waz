...
fn test_cjk_input_detection() {
    futures::executor::block_on(async move {
        ...
        // Triggered from Shell mode by default (stricter scenario, validating that CJK still switches to AI).
        let context = Context {
            current_input_type: InputType::Shell,
            is_agent_follow_up: false,
        };

        // Single CJK word is also classified as AI (default logic would classify it as Shell due to token count < 2).
        ...
        // Mixed Chinese and English, goes to AI as long as it contains CJK.
        ...
        // Chinese punctuation (full-width comma, period, question mark) also matches.
