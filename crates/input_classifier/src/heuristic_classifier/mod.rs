...
impl InputClassifier for HeuristicClassifier {
    async fn detect_input_type(&self, input: ParsedTokensSnapshot, context: &Context) -> InputType {
        // Direct to AI if input contains CJK characters: The underlying vocabulary and ML models are trained on English,
        // so Chinese/Japanese/Korean inputs would be misclassified as Shell under default logic.
