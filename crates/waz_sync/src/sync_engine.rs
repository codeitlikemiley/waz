...
/// Data provider trait, implemented by business modules to connect with synchronization
pub trait SyncDataProvider: Send + Sync {
    /// Section key to which the data belongs (e.g. "ssh")
    fn section_key(&self) -> &str;

    /// Collects local data and returns the JSON Value of this section
    fn collect_data(&self, token: &str) -> Result<serde_json::Value, SyncEngineError>;

    /// Applies remote cloud data to the local state
    fn apply_data(&self, token: &str, data: &serde_json::Value) -> Result<(), SyncEngineError>;
}

/// Sync engine, responsible for uploading/downloading sync data to/from Gist
pub struct SyncEngine<C: GistOps> {
    client: C,
}

impl SyncEngine<GistClient> {
    /// Creates a new SyncEngine instance (using the real GistClient)
    pub fn new() -> Self {
        Self {
...
impl<C: GistOps> SyncEngine<C> {
    /// Creates an engine using a custom GistOps implementation
    pub fn with_client(client: C) -> Self {
        Self { client }
    }

    /// Uploads data to the specified platform
    pub async fn upload(
        &self,
            ...
            .map_err(|e| SyncEngineError::Gist(e.to_string()))?
        {
            ...
            // Strict > counts as a conflict; == indicates that the local and remote are on the same version, no need to upload again
    ...
    /// Downloads data from the specified platform
    ...
    pub async fn force_upload(
        ...
        // Upload, roll back version number on failure
        ...
        if let Err(e) = upload_result {
            // Aggregate and return both the upload error and the rollback error, rather than swallowing the rollback error.
            // If the rollback fails, the local sync_version gets stuck at new_version, causing subsequent uploads to
            // unexpectedly overwrite the remote with the local state, which must be visible to the caller.
            let rollback_msg = match tokio::task::block_in_place(|| {
                version_store.set_sync_version(local_version)
            }) {
                Ok(()) => String::new(),
                Err(rollback_err) => {
                    log::error!("Failed to roll back version number after a failed force upload: {rollback_err}");
                    format!(
                        " (Rollback of sync_version also failed: {rollback_err}, local version stuck at {new_version}, please manually revert to {local_version})"
                    )
                }
            };
...
mod tests {
    ...
    struct MockGistOps {
        ...
        /// Records the platform passed during the last call, making it easy to test and assert platform-specific paths
    ...
    async fn do_test_upload_creates_new_gist(platform: SyncPlatform) {
        ...
        // Assert that platform is actually passed to GistOps, avoiding mock swallowing parameters
    ...
    async fn test_upload_equal_version_is_no_op() {
        // When local and remote have the same version, uploading should return AlreadyUpToDate instead of Conflict
        // (Fix: previously it would fall into a false conflict loop, see PR #161 review)
        ...
        // Remote version has not changed, should not trigger any write operations
        assert!(!*engine.client.update_called.lock().unwrap());
        assert!(!*engine.client.create_called.lock().unwrap());
    }
