...
#[derive(Debug, Copy, Clone, ValueEnum, Eq, PartialEq, Default)]
pub enum Harness {
    /// Use Waz's built-in MAA infrastructure (default).
    #[default]
    #[value(name = "oz")]
    Oz,
...
pub struct RunAgentArgs {
    ...
    /// Display agent progress in the Waz interface.
    ...
    // Waz Wave 7-2: `--environment` parameter is physically deleted along with the cloud ambient agent main subsystem.
    ...
    ///
    /// "oz" (default) uses Waz's built-in agent infrastructure.
