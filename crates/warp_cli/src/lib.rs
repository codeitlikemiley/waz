...
pub mod completions;
pub mod config_file;
// Waz Wave 7-2: `environment` CLI is physically deleted along with the cloud ambient agent main subsystem.
pub mod json_filter;
pub mod mcp;
pub mod model;
pub mod provider;
...
/// Options related to the parent process that spawned this Waz instance.
#[derive(Debug, Default, Clone, clap::Args)]
pub struct ParentOpts {
    /// The ID of the Waz process that spawned this one.
    ///
    /// Used by codepaths that attempt to detect when the parent Waz process
    /// has terminated. Guaranteed to be [`None`] when this is the initial
    /// Waz process, but may also be [`None`] for Waz child processes if the
...
/// Waz identity without exposing credentials.
...
/// Command-line argument parser for the main Waz binary. This is used across all channels.
#[derive(Debug, Default, Parser, Clone)]
#[command(
    name = "oz",
    display_name = "Oz",
    about = r#"Waz local agent CLI
...
/// Flags for the Waz application. Additional binaries, like test runners, may use this type
/// along with their own flags, or convert their flags into an `AppArgs` value.
#[derive(Debug, Default, clap::Args, Clone)]
pub struct AppArgs {
    /// True if this instance of Waz was launched at the end of the auto-update process.
...
impl Args {
    ...
    pub fn from_env() -> Self {
        cfg_if::cfg_if! {
            ...
            if #[cfg(target_family = "wasm")] {
                ...
                // Waz Wave 7-2: `warp environment` subcommand is physically deleted along with the cloud ambient agent main body --
                // Previously there was a reverse interception here: check and error early before clap parsing.
                // Now that the enum variant is deleted, clap will naturally report "unrecognized subcommand".
    ...
    pub fn clap_command() -> clap::Command {
        ...
        // Waz Wave 7-2: `environment` subcommand and `--environment` argument are physically deleted along with the cloud ambient agent
        // main body -- the enum variant has been removed from `CliCommand` and `RunAgentArgs`.
...
"#
    ...
    /// Args for the main Waz application, if not running a subcommand.
    pub fn app_args(&self) -> &AppArgs {
        &self.args
    }

    /// Extract the main Waz application args.
...
/// Waz may spawn several worker processes - mostly servers that support the main application.
///
/// These subcommands run those worker processes, which are bundled into the Waz binary.
...
/// CLI-related subcommands. The command-line interface to Waz isn't a full SDK (e.g. with language bindings),
/// but it allows scripting some Waz functionality.
#[derive(Debug, Clone, Subcommand)]
pub enum CliCommand {
    /// Interact with Oz.
    #[command(subcommand)]
    ...
    // Waz Wave 7-2: `Environment` variant is physically deleted along with the cloud ambient agent main subsystem.
    /// Manage MCP servers.
    #[command(subcommand)]
...
/// A subcommand of the main Waz application. This includes all [`WorkerCommand`]s as well as app-specific debugging tools.
#[derive(Debug, Clone, Subcommand)]
pub enum Command {
    #[clap(flatten)]
    Worker(WorkerCommand),

    /// Commands that make up the Waz CLI.
    #[clap(flatten)]
    ...
    ///
    /// If no shell is provided, this defaults to the shell that Waz was run from.
...
/// Returns a flag that sets the current process as the parent of a Waz subcommand to spawn.
