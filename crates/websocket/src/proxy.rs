...
//! HTTP proxy support for WebSocket connections.
//!
//! Prioritizes reading the global singleton `http_client::current_proxy_config()`. If it is
//! `ProxyMode::Custom` or `Off`, it is directly applied; if it is `ProxyMode::System`, it
//! falls back to the original environment variable parsing logic (`HTTPS_PROXY` / `HTTP_PROXY` / `ALL_PROXY` /
//! `NO_PROXY`). This allows the Custom URL / Off settings on the settings page to cover WebSockets. See Issue #72.
//!
...
/// Proxy mode mirror. It has a one-to-one mapping with `http_client::ProxyMode`, but is mirrored locally here
/// to avoid circular dependency: `websocket -> http_client -> warp_core -> websocket`.
/// When `app` starts or settings change, both `http_client::set_global_proxy_config` and
/// `websocket::set_global_proxy_config` are called to ensure both remain consistent.
...
/// A mirror of `http_client::ProxyConfig`, see the description of [`ProxyMode`].
...
/// Setup global WebSocket proxy configuration. This should be called at startup and on settings changes,
/// along with `http_client::set_global_proxy_config`.
pub fn set_global_proxy_config(cfg: ProxyConfig) {
    if let Ok(mut guard) = slot().write() {
        *guard = cfg;
    } else {
        log::error!("Failed to write WebSocket proxy config: RwLock has been poisoned");
    }
}
...
fn current_proxy_config() -> ProxyConfig {
    match slot().read() {
        ...
        Err(err) => {
            log::error!("Failed to read WebSocket proxy config: RwLock has been poisoned ({err})");
            ProxyConfig::default()
        }
    }
}
...
///    - `Custom`: Use the URL / auth / no_proxy entered in settings.
///    - `Off`: Return `None`.
///    - `System`: Fall back to environment variable parsing.
/// 2. Environment variable parsing (maintaining backward compatibility):
...
pub fn resolve_proxy(uri: &http::Uri) -> anyhow::Result<Option<ProxyInfo>> {
    ...
    // Prioritize global settings (mirroring http_client to avoid circular dependency).
    let global_cfg = current_proxy_config();
    match global_cfg.mode {
        ProxyMode::Off => return Ok(None),
        ProxyMode::Custom => {
            // Reuse the comma-separated no_proxy list in settings.
            ...
            if trimmed.is_empty() {
                // Custom but URL is empty: Consistent with http_client, silently falls back to environment variable parsing.
                log::warn!("WebSocket: HTTP proxy is set to Custom but URL is empty, falling back to environment variables");
            } else {
        ...
        ProxyMode::System => {
            // Fall back to environment variable parsing.
        }
    }
...
/// Split the comma-separated no_proxy list and also apply the same rules in `is_no_proxy`.
...
/// A variant of `parse_proxy_url`, appending the explicit username / password from settings (overriding those in the URL).
