...
pub fn test_suggestions_menu_positioning() -> Builder {
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Open Waz Drive")
...
precmd_functions+=(_p9k_precmd)
        ...
        // If the user then switches back to the Waz prompt, we should close the banner.
        .with_step(
...
/// Default auto title. We test that Waz's auto title is used and verify that that
...
/// Validate that disabling Waz's auto title feature will not mess with oh-my-zsh settings.
...
"#,
        ...
        .with_step(wait_until_bootstrapped_single_pane_for_tab(0))
        // If Waz title is disabled, we don't set the DISABLE_AUTO_TITLE env variable
        .with_step(execute_command_for_single_terminal_in_tab(
            0,
            "echo $DISABLE_AUTO_TITLE".to_string(),
            ExpectedExitStatus::Success,
...
/// Checks that the tab title set by the user takes precedence over the Waz's default title and
...
// the workspace view, but we DO force waz drive open to show the dialog, so we can look for that
...
pub fn test_context_chips_prompt_at_bootstrap() -> Builder {
    new_builder()
        ...
        .with_step(
            new_step_with_default_assertions("Check Waz prompt")
