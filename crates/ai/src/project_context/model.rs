...
/// Default list of rule files. Order = Priority (earlier takes precedence); when multiple files in the same directory
/// exist simultaneously, `RuleAtPath::respected_rule()` only takes the one with the highest priority.
///
/// - WARP.md   Native project convention.
/// - AGENTS.md Community common convention (recognized by opencode / Cursor / Cline, etc.).
/// - CLAUDE.md Claude Code native convention, making projects migrating from Claude Code works out-of-the-box.
///
/// To extend a new name, just adjust this array (insertion index = priority), `RuleAtPath`
/// is implemented as a priority-indexed slot array, so no changes to if-else logic are needed.
///
/// Defined outside of `cfg_if` so that paths not compiling `local_fs` (WASM / tests) can also reference it.
...
cfg_if::cfg_if! {
    if #[cfg(feature = "local_fs")] {
        ...
        // `instant::Instant` is the globally agreed cross-platform (including WASM) starting point in this repository,
        // replacing `std::time::Instant`. Enforced using `disallowed_types` in `clippy.toml`.
        use instant::Instant;
        ...
        // —— Fast-path (aligned with opencode `findUp` mode) ——
        //
        // Main purpose: After `cd`ing into a new git repository, within the time window before the async
        // `index_and_store_rules` completes, `pending_context` synchronously calls this fast-path to directly
        // stat + read the rule files in the `cwd` and its ancestor directories, ensuring that
        // AGENTS.md / WARP.md / CLAUDE.md **will not be missed due to asynchronous races**.
        // Once the normal path (`find_applicable_rules`) is available, the fast-path gives way and clears its cache.
        //
        // UI smoothness guarantee:
        //   - Worst case for a single execution is `MAX_WALK_DEPTH * RULES_FILE_PATTERN.len()` metadata calls
        //     + `read_to_string` on hit files (rule files are generally a few KB, Windows NTFS < 1ms/file).
        //   - `FAST_PATH_BUDGET` hard time budget truncation, times out immediately returning the collected part, never blocking.
        //   - Steady-state hits (no directory changes) only perform stat, not re-reading files; re-scans if any of
        //     mtime / size / parent-dir-mtime changes.
        const MAX_WALK_DEPTH: usize = 6;
        const FAST_PATH_BUDGET: Duration = Duration::from_millis(20);
    }
}

/// Fast-path cache entry. `stamps` records the (path, mtime, size) of matched files,
/// and `walked_dir_stamps` records the (path, mtime) of traversed directories, used to detect
/// two types of invalidations: "new / deleted / modified rule files in directory". A `negative` cache indicates
/// no rules were found in the last scan, allowing subsequent identical stamps to be reused directly without IO.
#[cfg(feature = "local_fs")]
#[derive(Clone, Debug)]
struct FastPathEntry {
    rules: Vec<ProjectRule>,
    /// "root" used by fast-path — takes the directory of the **first level hit**; on complete miss, takes the cwd itself.
...
pub struct ProjectContextModel {
    ...
    /// Fast-path synchronous rule cache (aligned with opencode `findUp` mode).
    ///
    /// Used only as a fallback when `find_applicable_rules` returns None (async index not ready / not under the indexed root),
    /// to avoid missing injection of AGENTS.md / WARP.md when sending AI requests immediately after cd.
    /// Accessed by a single thread (WarpUI Singleton on the main thread), uses `RefCell` instead of a lock
    /// to satisfy the `&self` invocation pattern of `pending_context(&self, app: &AppContext)`.
    #[cfg(feature = "local_fs")]
...
impl ProjectContextModel {
    ...
    pub fn new_from_persisted(
        ...
        ctx.spawn(
            ...
            |me, mut res, ctx| {
                // Waz: Originally, this would call `try_initialize_and_register_watcher` for each persisted root,
                // which internally goes through `DetectedRepositories::detect_possible_git_repo(ProjectRulesIndexing)`
                // to trigger events, causing RepoMetadataModel to index 6 persisted repositories in full
                // (which is the largest background CPU cost during cold startup for Waz BYOP).
                //
                // Now it only populates the in-memory `path_to_rules` cache and does not actively trigger
                // detect events. When the user later enters the repository via terminal cd,
                // `RepoDetectionSource::TerminalNavigation` will naturally trigger a separate detection,
                // at which point `register_watcher_for_path` will be called.
                //
                // Practical impact: Persisted rules are not watched in real-time until the user enters that repository.
                // The cache itself remains available, and AI rule lookups are unaffected.
    ...
    // Waz: Originally `try_initialize_and_register_watcher` was the entry point to force detect a repo
    // when starting from a persisted rule path, triggering subsequent full indexing by RepoMetadataModel.
    // It has been deleted along with the detect call in `new_from_persisted`; now it passively runs
    // `register_watcher_for_path` solely via the `RepoDetectionSource::TerminalNavigation` path triggered by terminal cd.
    ...
    /// Unified entry point for rule querying: normal path preferred, synchronous fast-path fallback when async index is not ready.
    ///
    /// Aligns with the `findUp` behavior of opencode `Instruction.systemPaths()` (
    /// `opencode/packages/opencode/src/session/instruction.ts`): stat rule files level by level starting from cwd,
    /// stopping at the first level hit. The fast-path and normal path **never coexist**:
    /// once the normal path returns `Some`, it immediately clears the corresponding entry in the fast-path cache to ensure
    /// subsequent requests go 100% through the normal path after indexing is complete (capable of retrieving subdirectory rules + real-time watcher updates).
    #[cfg_attr(not(feature = "local_fs"), allow(unused_variables))]
    pub fn find_rules_with_fast_path(&self, cwd: &Path) -> Option<ProjectRulesResult> {
        if let Some(found) = self.find_applicable_rules(cwd) {
            #[cfg(feature = "local_fs")]
            {
                // Normal path is now available, discard fast-path cache (to avoid getting stale data later).
    ...
    /// Fast-path synchronous lookup + reading of rule files in cwd and ancestor directories. Called only when the normal path is None.
    ///
    /// Return semantics are consistent with `find_applicable_rules`:
    ///   - Some(ProjectRulesResult) with at least 1 active rule
    ///   - None means no rules were found (negative cache written, subsequent identical stamps will not trigger IO)
    #[cfg(feature = "local_fs")]
    fn fast_path_lookup(&self, cwd: &Path) -> Option<ProjectRulesResult> {
        // 1) Cache hit path: stat all stamps, reuse cache if all match (no file re-reading).
        ...
        // 2) Cache miss / invalidation: synchronous scan. Hard budget truncation with `FAST_PATH_BUDGET`, ensuring UI never hangs.
    ...
    /// Synchronously stat + read rule files level by level starting upwards from `start`. Aligns with opencode `findUp`,
    /// but adds double protection with `MAX_WALK_DEPTH` + `FAST_PATH_BUDGET` to ensure the UI never blocks.
    ///
    /// On each level, takes the first match according to `RULES_FILE_PATTERN` (WARP.md > AGENTS.md), aligning
    /// with the semantics of `RuleAtPath::respected_rule()`.
    #[cfg(feature = "local_fs")]
    fn scan_fast_path(start: &Path) -> FastPathEntry {
        ...
        for _ in 0..MAX_WALK_DEPTH {
            ...
            // Record directory mtime, so we can detect two types of changes: "rules files added/deleted in directory" later.
            ...
            // Search for the first rule file at this level by priority. Aligns with `RuleAtPath::respected_rule()` semantics.
            for filename in RULES_FILE_PATTERN {
                ...
                break; // Only take 1 from this layer
            }
    ...
    /// Invalidation check for cache. Only stats, does not read file contents.
    /// - matched file mtime/size unchanged -> contents can be reused
    /// - traversed directory mtime unchanged -> no new/deleted rule files
    ///
    /// With `FAST_PATH_BUDGET` budget, timeout during stat is treated as invalidation to trigger a re-scan.
    #[cfg(feature = "local_fs")]
    ...
    /// Converts `FastPathEntry` to the unified `ProjectRulesResult`.
    /// Returns None for empty rules, semantically aligning with `find_applicable_rules`.
    #[cfg(feature = "local_fs")]
