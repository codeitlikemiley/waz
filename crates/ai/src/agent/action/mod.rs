...
impl AIAgentActionType {
    ...
    /// Continuous phrase used to surface "what the AI is currently doing" —
    /// for sub-agent status cards, status bar, parent card surfacing, etc.
    /// Short and specific, the frontend can append things like `↳ Reading files...` / `↳ Searching codebase...`.
    /// Style matches opencode TUI (e.g., Bash="Writing command...", Read="Reading file...").
