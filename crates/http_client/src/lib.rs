...
pub mod headers {
    /// Custom Waz header indicating the version of the Waz app.
    pub const CLIENT_RELEASE_VERSION_HEADER_KEY: &str = "X-Waz-Client-Version";

    /// Custom Waz header indicating the OS category the request was sent from.
    pub(crate) const WARP_OS_CATEGORY: &str = "X-Waz-OS-Category";
    /// Custom Waz header indicating the OS name the request was sent from. On Linux this is the
    /// name of the distribution. On all other platforms it should be equivalent to
    /// `WARP_OS_CATEGORY`.
    pub(crate) const WARP_OS_NAME: &str = "X-Waz-OS-Name";
    /// Custom Waz header indicating the version of the operating system. On Linux this is the
    /// version of the distribution, not the Linux kernel version.
    pub(crate) const WARP_OS_VERSION: &str = "X-Waz-OS-Version";

    /// Custom Waz header indicating the linux kernel version. This is only sent from Linux.
    pub(crate) const WARP_OS_LINUX_KERNEL_VERSION: &str = "X-Waz-OS-Linux-Kernel-Version";

    /// Custom Waz header indicating the client role. We don't use the User-Agent header
    /// because it can't be set from WASM.
    pub(crate) const WARP_CLIENT_ID: &str = "X-Waz-Client-ID";
}
...
impl Client {
    pub fn new() -> Self {
        ...
        // Apply global HTTP proxy configuration (see Issue #72).
        // reqwest::Proxy API is not available on WASM targets, skip.
        #[cfg(not(target_family = "wasm"))]
        {
    ...
    /// Determines whether the request should carry Waz-specific headers. Only same-origin requests carry custom headers.
    ...
    fn include_warp_http_headers<U: IntoUrl + Clone>(url: U) -> bool {
        url.into_url().is_ok_and(|url| {
            url.host_str().is_some_and(|dest_host| {
                ...
                // If the request is sent to the current host, the destination host should exactly match the window host.
...
/// This allows callers to inspect headers (like X-Waz-Error-Code) when handling errors.
#[derive(Debug)]
