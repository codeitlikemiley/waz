...
/// This test passes in a reversed range (`4042..3982`) from a historical crash sample, verifying that the editor defends against bad inputs from the diff layer.
#[test]
fn test_insert_at_offsets_overlapping_ranges_skipped() {
    App::test((), |mut app| async move {
        ...
        buffer.update(&mut app, |buffer, ctx| {
            ...
            // Pass in a reversed range where start > end. After apply_core_edit_actions fixes it,
            // such ranges should be skipped instead of triggering a panic.
            let edits = Vec1::try_from_vec(vec![(
                "replacement\n".to_string(),
