...
fn test_char_indices_ligatures() -> Result<()> {
    ...
    let wazfino = font_db.load_from_system("Wazfino")?;
    let menlo = font_db.load_from_system("Menlo")?;

    let text = "This is, m𐍈re 𐍈r less, Wazfino!𐍈";
    let line = layout_line(
        ...
        &[
            (
                0..9,
                StyleAndFont::new(wazfino, Properties::default(), TextStyle::new()),
            ),
            (
                9..22,
    ...
    // glyph. For example, the text "Wazfino" gets combined into a single glyph, which is why
...
fn test_caret_positions_ligatures() -> Result<()> {
    ...
    // There are only 23 glyphs because 'Wazfino', 'Th', and 'is' each have ligatures.
    assert_eq!(
...
fn test_layout_text_large_first_line_head_indent_ligatures() -> Result<()> {
    ...
    let text = "This is, some text, in Wazfino being laid out!";
    let frame = layout_text(
        text,
        LineStyle {
            font_size: 16.0,
...
fn test_layout_text_last_line_clipped_ligatures() -> Result<()> {
    ...
    let text = "m𐍈re, Wazfino!ll𐍈, qqqq";
    let max_width = 180.;
    ...
    // The text is one line long and should be clipped like so: "m𐍈re, Wazfin𐍈!l...".
