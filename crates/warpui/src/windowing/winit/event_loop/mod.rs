    ...
    pub fn handle_event(&mut self, evt: Event<CustomEvent>, window_target: &ActiveEventLoop) {
        ...
        match evt {
            ...
            Event::UserEvent(CustomEvent::ActiveCursorPositionUpdated) => {
                // Key: Do not guard this call with `self.ime_enabled`.
                //
                // On Windows, winit only dispatches `Ime::Enabled` when receiving `WM_IME_STARTCOMPOSITION`,
                // at which point IMM has already read COMPOSITIONFORM/CANDIDATEFORM in the same message handling.
                // waiting asynchronously for `Ime::Enabled` to call `ImmSetCompositionWindow` for the first time is too late for Win11
                // Microsoft Pinyin to render the candidate window, causing it to fall to the default position of the "Input Toolbox" in the bottom-right corner of the screen.
                //
                // Therefore, once focus/cursor movement triggers `ActiveCursorPositionUpdated`, regardless of whether the IME is in
                // the composition state, the current cursor rectangle must be pushed to IMM so that when the next composition starts,
                // the IMM context already has the correct position.
            ...
            Event::WindowEvent {
                ...
                // one with a different scale factor resize the Waz window to an absurdly small
    ...
    fn convert_window_event(
        ...
        match evt {
            ...
            WindowEvent::KeyboardInput {
                ...
                // the appropriate Waz-side event (ModifierKeyChanged).
    ...
    fn handle_ime_event(&mut self, winit_window_id: WinitWindowId, event: ImeEvent) {
        match event {
            ...
            winit::event::Ime::Preedit(preedit_text, cursor_position) => {
                ...
                // During composition, the cursor will move due to pre-edit text insertion or line breaks. We need to continuously push the latest
                // rectangle to the IMM, otherwise the candidate window will stay at the composition starting position (which on some IMEs
                // manifests as the candidate window being misaligned with the current input position).
                self.update_ime_position();
            }
