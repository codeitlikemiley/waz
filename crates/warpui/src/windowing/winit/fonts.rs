...
// We use font-kit's family handle to load fonts that come with Waz as
...
impl FontDB {
    pub fn new() -> Self {
        ...
        // Warm-up is Windows-only: PR #62 only adds locale-aware CJK fallback on the Windows path,
        // and this regression (zerx-lab/warp#68) also only appeared on Windows. macOS/Linux walk their respective fallback paths,
        // without requiring warm-up.
        #[cfg(target_os = "windows")]
...
/// Rebuilds cosmic-text `FontSystem` in-place with the new locale, reusing the existing `fontdb::Database`,
/// so font data will not be reloaded. `cosmic_text::FontSystem` has no public `set_locale`,
/// therefore we replace the entire structure via `std::mem::replace` and hook it back to the db.
///
/// Cost: `into_locale_and_db` only preserves `(locale, db)` across the replacement. The new `FontSystem`
/// will rebuild instance-level caches from scratch: `font_cache`, `monospace_font_ids`,
/// `per_script_monospace_font_ids`, `font_codepoint_support_info_cache`,
/// `font_matches_cache`, `shape_plan_cache`, as well as `shape_run_cache` when the `shape-run-cache` feature is enabled.
/// The constructor will also re-run `cache_fonts` on the monospace font collection.
/// On Windows with many system fonts, the first rendering after reconstruction can take hundreds of ms to about 1 s.
/// The font file data backed by mmap itself will not be re-read.
///
/// This is acceptable because UI locale switching happens rarely (currently driven by Settings -> Language and
/// the "restart Waz" prompt), and the user already expects an interruption. If low-latency hot reloading is needed later,
/// the hook is in `app::i18n::set_locale`.
