...
/// Windows-specific fallback: only enumerates clipboard formats that look like images, and looks for
/// known image bytes such as PNG/JPEG/GIF/WebP.
///
/// This is used to maintain compatibility with screenshot tools that do not write CF_DIB but only register format names like "PNG" or "image/png".
/// Do not probe arbitrary registered formats: clipboard-win must call GlobalSize/GlobalLock when reading raw bytes,
/// and some external clipboard formats cannot be safely read as HGLOBAL byte buffers.
#[cfg(target_os = "windows")]
fn try_read_image_via_custom_windows_formats(
    ...
    // Collect candidate formats first, avoiding raw-reading every custom format placed by external applications just for diagnostics.
...
fn is_windows_image_clipboard_format_candidate(format: u32, name: &str) -> bool {
    ...
    // Only registered formats whose format names explicitly indicate image bytes are suitable enough for raw-read.
    // This preserves support for common screenshot tools while avoiding touching arbitrary private formats of browsers/editors during text pasting.
