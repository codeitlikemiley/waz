...
impl Scene {
    ...
    pub fn visible_rect(&self, origin: Point, size: Vector2F) -> Option<RectF> {
        // TODO: Investigate under what circumstances a z-index that does not exist in the scene is passed.
        // Historical crash samples show this is relatively common, but does not significantly affect normal use; it might occur during a view teardown frame.
    ...
    fn validate_rect(
        rect: &RectF,
        #[cfg_attr(not(debug_assertions), allow(unused_variables))] location: Option<
