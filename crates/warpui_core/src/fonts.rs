...
pub struct Cache {
    ...
    /// In-flight waiters: When `available_system_fonts` is not ready yet and the initial scan is still running asynchronously,
    /// subsequent `all_system_fonts` calls will attach their senders here to reuse the result of the same scan,
    /// avoiding duplicate spawns and duplicate `process_loaded_system_fonts` execution on the main thread.
    #[cfg_attr(target_family = "wasm", allow(dead_code))]
...
impl Cache {
    ...
    pub fn all_system_fonts(
        ...
        if already_in_flight {
            // A scan of the same type is already in flight, reuse its result without duplicate spawns or duplicate main-thread
            // execution of `process_loaded_system_fonts`.
