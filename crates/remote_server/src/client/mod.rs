...
/// so that `kill_on_drop` fires when teardown occurs. In Waz this is
...
impl RemoteServerClient {
    ...
    /// session's (in Waz, on the `RemoteServerManager`'s
...
impl RemoteServerClient {
    ...
    /// Waz: List immediate children of a directory on the remote host.
    ///
    /// Used by terminal file link detection to precisely validate the remote path structure
    /// (local sessions rely on `fs::metadata` to do this, as remote files are not on the local disk).
    ...
    ///
    /// Waz: Unlike other fire-and-forget notifications, buffer edit delivery failures must be reported.
    /// If silently swallowed when `outbound_tx` is closed (connection dead), the local buffer would keep advancing
    /// while the daemon receives no edits, causing an invisible desync. Return `Err` on failure for the caller to handle.
