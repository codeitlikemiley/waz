...
//!
//! Note: The remote-server binary is now statically linked with musl (see `preinstall_check.sh`),
//! and does not depend on the host's libc. The parsed libc family/version here is no longer used for
//! installation gating, but is kept solely for telemetry/diagnostics. `RemoteLibc` and `GlibcVersion`
//! continue to be used by [`crate::setup::PreinstallCheckResult`] and `UnsupportedReason`.
