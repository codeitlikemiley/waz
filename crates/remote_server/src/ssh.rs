...
/// Uploads a single file to the remote host via the existing ControlMaster socket.
pub async fn scp_upload(
    ...
    let output = async {
        Command::new("scp")
            // -C: Compress transfer. The remote-server binary uploaded in dev mode is hundreds of MBs,
            // so compression can significantly shorten upload times and reduce timeout risks.
            .arg("-C")
            .arg("-o")
