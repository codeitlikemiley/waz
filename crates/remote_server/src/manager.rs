...
/// Whether strict tag matching should be enforced on the remote `server_version`.
///
/// For [`Channel::Oss`](Waz), local source builds do not have a `GIT_RELEASE_TAG`,
/// but the SSH Extension might install the latest release of the remote-server.
/// If strict checking is enforced, a mismatch between a client with `None` and a server with a non-empty tag
/// will trigger a delete-reinstall-mismatch loop. Release builds avoid stale binaries by using versioned install paths,
/// while local builds continue to skip strict version checks.
#[cfg(not(target_family = "wasm"))]
...
impl RemoteServerManager {
    ...
    async fn run_connect_and_handshake(
        ...
        //
        // Under `Channel::Oss` (Waz), we temporarily reuse the official release binary. The client itself
        // lacks a `GIT_RELEASE_TAG` and will never match the server, so we skip strict checking.
        // See the comments on [`should_enforce_remote_version_check`] for details.
    ...
    pub(crate) fn mark_session_disconnected(
        ...
        } = prev
        {
            ...
            drop(_child);
            // If the child process has already exited (`exit_status.is_some()`), it means the remote daemon
            // is truly gone — for example, if the user typed `exit` in the remote shell, and the ssh ControlMaster
            // closed all slave channels together, causing `remote-server-proxy` to exit as well.
            // In this case, reconnecting immediately will likely fail, and might instead cause the terminal pane
            // to hang for 2-4 seconds before fully terminating; thus we skip auto-reconnection and go straight to the Disconnected path.
            // Reconnection logic is preserved only for `exit_status.is_none()` (where the child is still running but the reader received EOF),
            // which represents a "true, transient network blip".
