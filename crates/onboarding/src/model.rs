    ...
    pub fn is_ai_enabled(&self) -> bool {
        ...
        match self {
            ...
            SelectedSettings::Terminal { .. } => {
                // With old onboarding (no WazNewSettingsModes), Terminal
...
impl OnboardingStateModel {
    ...
    pub(crate) fn settings(&self) -> SelectedSettings {
        use warp_core::features::FeatureFlag;
        let ui_customization = if FeatureFlag::WazNewSettingsModes.is_enabled() {
    ...
    pub(crate) fn set_models(
        ...
        let is_past_agent_slide = if FeatureFlag::WazNewSettingsModes.is_enabled() {
            matches!(
                self.step,
    ...
    pub(crate) fn back(&mut self, ctx: &mut ModelContext<Self>) {
        use warp_core::features::FeatureFlag;
        let theme_picker_last = FeatureFlag::WazNewSettingsModes.is_enabled();
