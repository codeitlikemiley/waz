...
#define MyAppPublisher "Waz"
#define MyAppURL "https://waz.dev/"
#ifndef MyAppName
  ...
  ; Default follows the official channel `dev.warp.*` naming; OSS overrides to `dev.goldcoders.waz` in bundle.ps1.
...
#define ChannelPascalCase \
  ...
  "Unknown")))))
#define AppMutexName "Local\Waz" + ChannelPascalCase + "_SingleInstance"


[Setup]
; NOTE: The value of AppId uniquely identifies this application. Do not use the same AppId value in installers for other applications.
; (To generate a new GUID, click Tools | Generate GUID inside the IDE.)
; bundle.ps1 passes `InnoAppId=waz` for OSS; other channels use the default `warp-terminal-{ReleaseChannel}`.
#ifndef InnoAppId
...
; Force close previous Waz if it hasn't shut down yet.
; In the update flow we already warn the user if they have something running and make them confirm
; before running this installer. Therefore, we are good to force close Waz without fear of losing
; unsaved work.
...
; For manual installs: if Waz is running, show a dialog prompting the user to close it
...
[Registry]
Root: HKCU; Subkey: "SOFTWARE\Waz.dev\{#MyAppName}"; Flags: uninsdeletekey
; cleanup "Open Waz Here" registry entries
...
; Add "Open Waz in new tab" to directory context menu
...
; Add "Open Waz in new tab" to directory background context menu
...
; Add "Open Waz in new window" to directory context menu
...
; Add "Open Waz in new window" to directory background context menu
...
[Tasks]
Name: addToPath; Description: "Add Waz to PATH"
...
{ Returns true when the installer was launched by Waz's auto-update code.
  The auto-update path passes /update=1 on the command line and /NOCLOSEAPPLICATIONS
  so that the installer does not forcibly kill the running Waz process. Instead we
  wait for Waz to exit naturally by polling the app mutex below. }
...
{ For background updates, return an empty mutex name so that Inno Setup skips its
  ...
  installs, return the real mutex name so the user is prompted to close Waz first. }
...
var
  ...
  ResultCode: Integer;
begin
  { Background update: the installer was launched while Waz was still running.
    We passed /NOCLOSEAPPLICATIONS so Inno won't kill it. Wait here - before any
    files are touched - for Waz to release its single-instance mutex, which
    happens as part of normal process exit. }
  if CurStep = ssInstall then
  begin
    if IsBackgroundUpdate() then
    begin
      Log('Background update: waiting for Waz to exit (mutex: {#AppMutexName})...');
      WaitCounter := 0;
      ...
      begin
        Log('Waz mutex still held after timeout; force-killing remaining processes.');
        { Kill by image name. {#MyAppExeName} (e.g. warp.exe, dev.exe) is unique
          ...
          rely on Waz's Job Object (JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE) to
        ...
        Log('Waz has exited; proceeding with file installation.');
    end;
  end;

  { After a successful install, write a helper script for running the Waz CLI. }
  { We use this to add a "warp-" prefix (e.g. "warp-preview.cmd" vs. "preview.exe") }
  if CurStep = ssPostInstall then begin
    { Add Waz to PATH if requested }
