...
Param (
    ...
    # Builds only the Waz binary, skips the installer.
    [Switch]$SKIP_BUILD_INSTALLER = $False,
    # Builds only the installer, skips the Waz binary. Use this if the Waz
...
if ("$CHANNEL" -eq 'local') {
    ...
    $BINARY_NAME = 'waz.exe'
    $APP_NAME = 'Waz'
    # OSS channel uses local crash reporting; does not enable the default release feature set.
    # Autoupdate uses GitHub Releases (codeitlikemiley/waz); downloads to Downloads without invoking Inno Setup.
    $FEATURES = 'release_bundle,gui,nld_improvements,autoupdate'
}

$BINARY_PATH = "$CARGO_TARGET_OUTPUT_DIR\$BINARY_NAME"
# AUMID (Windows AppUserModel ID) must exactly match what the process-side
# `ChannelState::app_id()` generates, otherwise Windows ToastNotificationManager
# silently drops toasts when the Start Menu shortcut / process AUMID do not match.
# OSS (Waz) uses `dev.goldcoders.waz` in `app/src/bin/oss.rs`;
# other official channels use `dev.warp.<Name>`.
if ("$CHANNEL" -eq 'oss') {
    $AUMID = "dev.goldcoders.$APP_NAME"
} else {
...
if ($CHECK_ONLY) {
    ...
    if (-Not $?) {
        Write-Error "Failed to verify Waz $WARP_BIN compilation with profile $CARGO_PROFILE"
        exit 1
    }
    exit 0
}
...
if (-Not $SKIP_BUILD_BINARY) {
    Write-Output "Building Waz for channel $CHANNEL and bundle id $BUNDLE_ID"
    ...
    if (-Not $?) {
        Write-Error "Failed to build Waz $WARP_BIN binary with profile $CARGO_PROFILE"
        exit 1
    }
...
Write-Output 'Building Waz installer'
# Inno Setup `AppId` determines the registry Uninstall entry and upgrade tracking key.
# For OSS it is fixed to `waz`, avoiding the default `warp-terminal-oss`.
# Other channels use the default `warp-terminal-{ReleaseChannel}` from the .iss file.
if ("$CHANNEL" -eq 'oss') {
    $INNO_APP_ID = 'waz'
} else {
