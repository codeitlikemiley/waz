...
/// Whether the model supports 1M context beta(`anthropic-beta: context-1m-2025-08-07`).
///
/// Criteria (alignment opencode `packages/console/.../anthropic.ts`):
/// - The model name contains `sonnet` (all series from Claude Sonnet 4 support 1M)
/// - or for Opus 4.6 and above (`claude-opus-4-6`, `claude-opus-4-7`, `claude-opus-5-x` ...)
///
/// Used in BYOP / direct connection / transit scenarios: without this header, some transits (such as anyrouter) will
/// Directly 400 reject requests with model names such as `claude-opus-4-7`, see zerx-lab/warp #21.
/// After Anthropic officially accepts this header, the regular price will still be used when prompt < 200K, so it is safe to bring it by default.
...
impl Adapter for AnthropicAdapter {
	...
	fn to_web_request_data(
		...
		// -- 1M context beta header (included by default for models that support 1M)
		// If not, some transit gateways (anyrouter, etc.) will directly reject it with 400.
		// Anthropic officially still charges regular prices for prompts < 200K, and it is safe to bring them by default.
		// See zerx-lab/warp issue #21 for details.
		...
		// -- Merge the extra_headers added by the user in ChatOptions (the last write overwrites the previous write)
		// Users can explicitly plug headers with the same name (such as combining multiple betas: `context-1m-..., files-api-...`)
		// to override the adapter default value.
...
mod tests {
	...
	/// Auxiliary: Get the value from Headers by name (case-sensitive).
	...
	/// Regression protection (zerx-lab/warp #21): Models that support 1M context must have
	/// `anthropic-beta: context-1m-2025-08-07`, otherwise anyrouter and other transit gateways 400.
	#[test]
	...
	fn test_extra_headers_can_override_beta() {
		// Users insert custom anthropic-beta in ChatOptions (such as combining multiple beta features)
		// Should override the adapter's default single value of 1m.
