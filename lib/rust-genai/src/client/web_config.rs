...
///
/// **Note (Waz fork)**: gzip default is **false**. Upstream genai defaults
...
pub struct WebConfig {
	...
	/// Enable gzip response decompression. **Default: false** (Waz fork
...
impl Default for WebConfig {
	fn default() -> Self {
		Self {
			...
			proxy: None,
			// Waz: gzip off by default — see struct-level docs above.
			gzip: false,
			tcp_nodelay: true,
		}
	}
