...
Thanks for helping improve Waz! This guide explains how to open issues, propose changes, and get your work reviewed.
...
## How Contributing to Waz Works

Waz's contribution model is shaped by Oz, an agent that automates parts of triage, spec writing, implementation, and review. Compared with a typical open-source repository, a few things work differently here:
...
- **Review is largely automated.** When you open a PR, Oz is auto-assigned and produces an initial review. Once Oz approves, it automatically requests a follow-up review from a Waz team subject-matter expert — you do not need to assign human reviewers yourself.

### Readiness labels

The Waz team applies one of the following labels when an issue is ready for contribution:
...
- **`needs-mocks`** — Design mocks are required before implementation can begin. Wait for the Waz team to land them.
...
Steps owned by you (the contributor) are shown in yellow; steps owned by the Waz team or Oz are shown in blue.

```mermaid
flowchart TD
    A[File an issue] --> B{Waz team triages}
...
If you're already running Waz, the fastest way to file is the `/feedback` command — it opens a public GitHub issue with relevant context (logs, environment details) automatically attached.
...
- Waz version and OS (see `Settings → About`).
...
You **do not need to manually request reviewers**. Oz is auto-assigned to PRs that target a ready issue and produces an initial review. After Oz approves, it automatically requests a follow-up review from the appropriate Waz team subject-matter expert.
...
You can use **any coding agent** to implement a contribution — for example, Waz's built-in agent, Claude Code, Codex, Gemini CLI, or others — or no agent at all. This repository ships agent-readable context (skills under [`.agents/skills/`](.agents/skills/), specs under [`specs/`](specs/), and [`WARP.md`](WARP.md)) that any harness supporting these formats can pick up.
